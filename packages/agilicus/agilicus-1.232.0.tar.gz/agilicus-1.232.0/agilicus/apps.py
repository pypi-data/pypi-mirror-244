...
def update_application_configs(
    ...
    **kwargs,
    ...
    if security_http_cors_allow_origins is not None:
        cors["allow_origins"] = security_http_cors_allow_origins

    cors.setdefault("allow_methods", [])
    if security_http_cors_allow_methods is not None:
        cors["allow_methods"] = security_http_cors_allow_methods

    cors.setdefault("allow_headers", [])
    if security_http_cors_allow_headers is not None:
        cors["allow_headers"] = security_http_cors_allow_headers

    cors.setdefault("enabled", False)
    if security_http_cors_enabled is not None:
        cors["enabled"] = security_http_cors_enabled

    cors.setdefault("origin_matching", "me")
    if security_http_cors_origin_matching is not None:
        cors["origin_matching"] = security_http_cors_origin_matching

    cors.setdefault("mode", "overwrite")
    if security_http_cors_mode is not None:
        cors["mode"] = security_http_cors_mode

    cors.setdefault("expose_headers", [])
    if security_http_cors_expose_headers is not None:
        cors["expose_headers"] = security_http_cors_expose_headers

    cors.setdefault("max_age_seconds", 3600)
    if security_http_cors_max_age_seconds is not None:
        cors["max_age_seconds"] = security_http_cors_max_age_seconds

    cors.setdefault("allow_credentials", False)
    if security_http_cors_allow_credentials is not None:
        cors["allow_credentials"] = security_http_cors_allow_credentials

    if oidc_config_auth_enabled is not None:
        auth["auth_enabled"] = oidc_config_auth_enabled

    if oidc_config_auth_issuer is not None:
        auth["issuer"] = oidc_config_auth_issuer

    if oidc_config_auth_redirect_after_signin_path is not None:
        auth["redirect_after_signin_path"] = oidc_config_auth_redirect_after_signin_path

    if oidc_config_auth_redirect_subpath is not None:
        auth["redirect_subpath"] = oidc_config_auth_redirect_subpath
