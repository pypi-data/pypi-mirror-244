...
class BaseClientSettings:
    ...
    SKY_LOG_ALERTING_TELEGRAM_ALERT_NAME: str = config(
        "DEFAULT_SKY_LOG_ALERTING_TELEGRAM_ALERT_NAME"
    )
    SKY_LOG_ALERTING_PHONE_CALL_ALERT_NAME: str = config(
        "DEFAULT_SKY_LOG_ALERTING_PHONE_CALL_ALERT_NAME"
    )
    SKY_LOG_ALERTING_SMS_ALERT_NAME: str = config(
        "DEFAULT_SKY_LOG_ALERTING_SMS_ALERT_NAME"
    )
...
class BaseClient:
    ...
    def request(
        ...
        req = Request(
            method, url, params=params, data=data, auth=auth, json=json, headers=headers
        )
