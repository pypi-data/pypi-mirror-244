...
class Tk(tkinter.Tk):
    ...
    def wm_geometry(self, newGeometry=None):  # type: (str | None) -> str | None
        ...
        if newGeometry:
            width, height, _width, _height, *_ = (newGeometry + '+0+0').replace('+', 'x').split('x')
            if width != '':
                width = int(width)
                height = int(height)
            if _width != '':
        ...
        width, height, _width, _height, *_ = map(int, (geometry + '+0+0').replace('+', 'x').split('x'))
...
class Toplevel(tkinter.Toplevel, Tk):
    ...
    def __init__(
        ...
        * `**kw`: 与 tkinter.Toplevel 类的参数相同
        ...
        Tk.__init__(self, title, width, height, x, y, shutdown=shutdown, alpha=alpha, toolwindow=toolwindow, topmost=topmost, transparentcolor=transparentcolor, **kw)
...
class Canvas(tkinter.Canvas):
    ...
    def __init__(
        ...
        * `**kw`: 与 tkinter.Canvas 类的参数相同
        ...
        tkinter.Canvas.__init__(self, master, width=width, height=height, highlightthickness=0, **kw)
    ...
    def _zoom(self, rate_x=None, rate_y=None):  # type: (float | None, float | None) -> None
        ...
        * `rate_y`: 纵向缩放比率，默认值同上
        ...
        for item in self.find_all():  # item 位置缩放
            self.coords(item, *[c * (rate_x, rate_y)[i & 1] for i, c in enumerate(self.coords(item))])
        ...
        for item in self._image:  # 图像大小缩放（采用相对的绝对缩放）
            if self._image[item][0] and self._image[item][0].extension != 'gif':
                self._image[item][1] = self._image[item][0].zoom(temp_x * rate_x, temp_y * rate_y, precision=1.2)
    ...
    def itemconfigure(self, tagOrId, **kw):  # type: (str | int, ...) -> dict[str, tuple[str, str, str, str, str]] | None
...
class BaseWidget:
    ...
    def __init__(
        ...
        * `tick`: 复选框控件的标记符号，默认为一个“对号”
        ...
        self.command_ex = {'normal': None, 'touch': None, 'click': None, 'disabled': None}  # type: dict[str, typing.Callable | None]
        ...
        if radius:
            ...
            self.inside = [  # 虚拟控件内部填充颜色
                canvas.create_rectangle(x, _y, x + width, y + height - radius * canvas.ry, **kw),
                canvas.create_rectangle(_x, y, x + width - radius * canvas.rx, y + height, **kw),
                canvas.create_arc(x, y, x + d * canvas.rx, y + d * canvas.ry, start=90, **kw),
                canvas.create_arc(x + _w, y, x + width, y + d * canvas.ry, start=0, **kw),
                canvas.create_arc(x, y + _h, x + d * canvas.rx, y + height, start=180, **kw),
                canvas.create_arc(x + _w, y + _h, x + width, y + height, start=270, **kw)]

            kw = {'extent': 100, 'style': tkinter.ARC, 'outline': color_outline[0]}
            self.outside = [  # 虚拟控件外框
                canvas.create_line(_x, y, x + width - radius * canvas.rx, y, fill=color_outline[0], width=borderwidth),
                canvas.create_line(_x, y + height, x + width - radius * canvas.rx, y + height, fill=color_outline[0], width=borderwidth),
                canvas.create_line(x, _y, x, y + height - radius * canvas.ry, fill=color_outline[0], width=borderwidth),
                canvas.create_line(x + width, _y, x + width, y + height - radius * canvas.ry, fill=color_outline[0], width=borderwidth),
                canvas.create_arc(x, y, x + d * canvas.rx, y + d * canvas.ry, start=90, width=borderwidth, **kw),
                canvas.create_arc(x + _w, y, x + width, y + d * canvas.ry, start=0, width=borderwidth, **kw),
                canvas.create_arc(x, y + _h, x + d * canvas.rx, y + height, start=180, width=borderwidth, **kw),
                canvas.create_arc(x + _w, y + _h, x + width, y + height, start=270, width=borderwidth, **kw)]
        else:
            self.rect = canvas.create_rectangle(x, y, x + width, y + height, width=borderwidth, outline=color_outline[0], fill=color_fill[0])  # 虚拟控件的外框

        self.image = canvas.create_image(x + width / 2, y + height / 2, image=image)  # 背景图片

        self.text = canvas.create_text(  # 虚拟控件显示的文字
            x + (radius + 2 if justify == 'left' else width - radius - 3 if justify == 'right' else width / 2),
            y + height / 2,
            text=text,
            font=font,
            justify=justify,
    ...
    @typing.overload
    def state(self, mode):  # type: (typing.Literal['normal', 'touch', 'click', 'disabled']) -> None
        ...
    ...
    def state(self, mode=None):  # type: (typing.Literal['normal', 'touch', 'click', 'disabled'] | None) -> str | None
        """
...
class TextWidget(BaseWidget):
    ...
    def __init__(
        ...
        self._value = ['', text, ''] if type(text) == str else ['', *text]  # 隐式值
    ...
    def _cursor_update(self, text=' '):  # type: (str) -> None
        ...
        if isinstance(self, Entry):
            self.master.coords(self._cursor, self.master.bbox(self.text)[2], self.y1 + self.height * self.master.ry / 2)  # BUG: 缩放后光标位置会略微显示错误
        elif isinstance(self, Text):
            _pos = self.master.bbox(self._text)
            self.master.coords(self._cursor, _pos[2], _pos[1])
        self.master.itemconfigure(self._cursor, text='' if not text else self.icursor)
...
class Text(TextWidget):
    ...
    def __init__(
        ...
        *,
        ...
        _x = x + (width - radius - 3 if justify == 'right' else width / 2 if justify == 'center' else radius + 2)
        ...
        self.master.itemconfigure(self.text, text=self._value[1], anchor=_anchor)
    ...
    def _input_return(self):  # type: () -> None
        ...
        if _pos[3] + _pos[3] - _pos[1] < self.y2 - self.radius - 2:  # 还没填满文本框
            ...
            self.master.itemconfigure(self.text, text=text + bool(text) * '\n' + _text)
        else:  # 文本框已经被填满了
            text = text.split('\n', 1)[-1]
    ...
    def _input_backspace(self):  # type: () -> None
        ...
        if _text:  # 正常地删除字符
            ...
            if self.value == self.master.itemcget(self.text, 'text'):
                ...
                temp = self.value[:-len(_)] if self.value.endswith(_) else self.value
                __ = temp[:-('\n' in self.value)]
            else:  # 内容已经超出框框的大小啦
                text = self.master.itemcget(self.text, 'text')
                temp = self.value[:-len(text)] if self.value.endswith(text) else self.value
...
class ProgressBar(BaseWidget):
    ...
    def __init__(
        ...
        *,
        ...
        mode='determinate',  # type: typing.Literal['determinate', 'indeterminate']
    ):  # type: (...) -> None
    ...
    def load(self, percentage):  # type: (float) -> None
        ...
        * `percentage`: 进度条的值，范围 0~1，大于 1 的值将被视为 1
        ...
        self.master.coords(self.bar, self.x1 + x1 * self.master.rx, self.y1, self.x1 + x2 * self.master.rx, self.y2)
...
class Switch(Button):
    ...
    def configure(self, *args, **kw):  # type: (...) -> str | tuple | None
        ...
        self._slider.configure(color_fill=fill_slider, color_outline=outline_slider)
...
class PhotoImage(tkinter.PhotoImage):
    ...
    def parse(self, start=0):  # type: (int) -> typing.Generator[int, None, None]
        """
        ...
        * `start`: 动图解析的起始索引（帧数-1）
        ...
        try:
            while True:
                self.image.append(tkinter.PhotoImage(file=self.file, format=f'gif -index {start}'))
    ...
    def play(self, canvas, item, interval, **kw):  # type: (Canvas, int, int, ...) -> None
        ...
        * `interval`: 每帧动画的间隔时间
        ...
        _ind = kw['_ind'] + 1
        canvas.after(interval, lambda: self.play(canvas, item, interval, _ind=0 if _ind == len(self.image) else _ind))  # 迭代执行函数
    ...
    def zoom(self, rate_x, rate_y, *, precision=None):  # type: (float, float, ..., float | None) -> tkinter.PhotoImage
        """
        ...
        * `precision`: 精度到小数点后的位数（推荐 1.2），越大运算就越慢（默认值代表绝对精确）
        ...
        if precision is not None:
            ...
            rate_x = fractions.Fraction(str(rate_x)).limit_denominator(limit)  # type: fractions.Fraction
            rate_y = fractions.Fraction(str(rate_y)).limit_denominator(limit)  # type: fractions.Fraction
            image = tkinter.PhotoImage.zoom(self, rate_x.numerator, rate_y.numerator)
...
class Animation:
    ...
    def __init__(
        ...
        ms,  # type: int
        *,
        control=CONTROL,  # type: tuple[typing.Callable[[float], float], float, float]
        translation=None,  # type: tuple[float, float] | None
        color=None,  # type: tuple[typing.Callable[[str], None], str, str] | None
        fps=FPS,  # type: int
        ...
        * `loop`: 是否循环播放动画，默认不循环，循环时参数 stop 失效
        """
        self.widget = widget
        self.master = canvas if isinstance(widget, int) else widget if isinstance(widget, tkinter.Widget) else widget.master
        self.start = start
        self.step = step
        self.stop = stop
        self.loop = loop
    ...
    def _parts(self, control, up, down):  # type: (typing.Callable[[float], float], float, float) -> list[float]
        """部分比率"""
    ...
    def _run(self, _ind=0):  # type: (int) -> None
        ...
        if self.translation is not None:
            self._translate(*[value * self.parts[_ind] for value in self.translation])
