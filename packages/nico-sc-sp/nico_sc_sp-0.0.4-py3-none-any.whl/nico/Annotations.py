...
def visualize_spatial_anchored_cell_mapped_to_scRNAseq(input,saveas='pdf',transparent_mode=False,figsize=(12,10)):
    """
    ...
    plt.xlabel('Spatial guided clusters')
...
def find_anchor_cells_between_ref_and_query(refpath='./inputSC/',quepath='./inputSP/',ref_cluster_tag='cluster',
neigh=50,no_of_pc=50,minkowski_order=2,across_spatial_clusters_dispersion_cutoff=0.15,guided_spatial_cluster_resolution_tag='leiden0.5',
number_of_iteration_to_perform_celltype_annotations=3,resolved_tie_issue_with_weighted_nearest_neighbor='No'):


    """
    **This is the primary function called by the user to perform the annotation in the spatial cells using the label transfer from
    scRNAseq data. The scRNAseq must have cell type information in the .obs[cluster] tag.**

    **This script should be run twice. The first run will find all the anchored cells between queried and reference data.
    The second time run will find the cell type annotations based on the anchored cells and majority vote or weighted nearest distance from the spatial cell neighbors.**

    sct_spatial.h5ad file should have a different resolution of Leiden clustering to guide the NiCo annotation.
    It is good to have several Leiden clustering resolutions 0.3, 0.4, 0.5, 0.6, 0.7 and 0.8 that can be
    tagged with 'leiden0.3', 'leiden0.4', 'leiden0.5', 'leiden0.6', 'leiden0.7' and 'leiden0.8' in the adata.obs[] to guide our NiCo annotations.
    ...
    | The guided cluster resolution, i.e., different resolution parameters of the Leiden cluster, should be stored in the queried file.
    | (default) guided_spatial_cluster_resolution_tag='leiden0.5'

    | The cutoff used to remove the noisy anchors if anchored cells belong to any guided spatial cluster with less than 0.15 % of the population
    ...
    | (default) ./inputSP/MNN_based_annotations/
    ...
    df=adata.obs[guided_spatial_cluster_resolution_tag]#.to_csv(spatialclusterFilename,header=True)
    ...
    if flag==0:
        data=np.load(fmnn,allow_pickle=True)
        ...
        input={}
        input['fname_mnn_anchors']=data['anchors']
        ...
        input['KNN']=neigh
        input['k_dist']=data['k_dist']
        input['k_index']=data['k_index']
        ...
        input['across_spatial_clusters_dispersion_cutoff']=across_spatial_clusters_dispersion_cutoff
        input['number_of_iteration_to_perform_celltype_annotations']=number_of_iteration_to_perform_celltype_annotations
        input['resolved_tie_issue_with_weighted_nearest_neighbor']=resolved_tie_issue_with_weighted_nearest_neighbor


        inputt=SimpleNamespace(**input)
        good_anchors,visualize_anchors=find_commnon_MNN(inputt)
        input['visualize_anchors']=visualize_anchors
        inputt=SimpleNamespace(**input)
        nico_based_annotation(inputt,good_anchors)


        #os.remove(outputFolder+'final_sct_sc.h5ad')
        #os.remove(outputFolder+'final_sct_sp.h5ad')
        #os.remove(fmnn)

    return inputt


def nico_based_annotation(input,good_anchors):
    "The script to perform the Nico based annotations. When you run find_anchor_cells_between_ref_and_query a second time, then this function will run."
    chosenKNN=input.KNN
    ...
    for mainkey in confused:
            ...
            if True:
                ...
                for i in range(len(t1)):
                    ...
                    temp=[]
                    #This step checks cell type (highest degree to lowest) from neighbors to see whether they belong to the same guided cluster or not
                    #only if the cell type of the majority of neighbors and current cell (confused or unresolved) are in the same guided cluster will it be assigned to this cell type
...
def visualize_umap_and_cell_coordinates_with_all_celltypes(quepath='./inputSP/',positionFilename='./inputSP/tissue_positions_list.csv',
number_of_iteration_to_perform_celltype_annotations=3,
cmap=plt.cm.get_cmap('jet'),saveas='pdf',transparent_mode=False,figsize=(15,6)):
    ...
    | (default) positionFilename='./inputSP/tissue_positions_list.csv'
    ...
    The annotation figure will be saved in './inputSP/MNN_based_annotations/tissue_and_umap_with_all_celltype_annotations*
...
def visualize_umap_and_cell_coordinates_with_selected_celltypes(quepath='./inputSP/',positionFilename='./inputSP/tissue_positions_list.csv',
number_of_iteration_to_perform_celltype_annotations=3,choose_celltypes=[],msna=0.1,ms=0.5,
cmap=plt.cm.get_cmap('jet'),saveas='pdf',transparent_mode=False,figsize=(8,3.5)):
    ...
    The individual celltype annotation figure will be saved in './inputSP/MNN_based_annotations/fig_individual_annotation'
