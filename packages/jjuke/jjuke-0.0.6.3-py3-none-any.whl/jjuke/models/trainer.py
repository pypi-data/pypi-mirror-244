...
class BaseTrainer(BaseWorker):
    ...
    def fit_loop(self):
        ...
        if self.rankzero and self.args.logging.use_wandb:
            train_losses_reduced = reduce_dict(train_losses_dict)
            ...
            val_losses_reduced = reduce_dict(val_losses_dict)
...
class StepTrainer(BaseTrainer):
    ...
    def stage_eval(self, o_train):
        ...
        if self.rankzero and self.args.logging.use_wandb:
            ...
            self.log_wandb(losses_dict, "valid", step=self.epoch)
        ...
        with tqdm(
            ...
            for self.epoch, batch in enumerate(infinite_dataloader(self.dl_train), 1):
                ...
                if self.rankzero and self.args.logging.use_wandb:
                    loss_reduced = reduce_dict(losses_dict)
                    losses_dict = {k: v.mean().item() for k, v in loss_reduced.items()}
                    self.log_wandb(losses_dict, "train", step=self.epoch)
