...
def calculate_fee(
    ...
    if minimal_nanotez_per_gas_unit is None:
        minimal_nanotez_per_gas_unit = int(MINIMAL_MUMAV_PER_GAS_UNIT * 1000)
    fee = MINIMAL_FEES + MINIMAL_MUMAV_PER_BYTE * size + int(minimal_nanotez_per_gas_unit * consumed_gas / 1000)
    return fee + reserve
...
def default_fee(
    ...
    return calculate_fee(
        ...
        extra_size=32 + 64 + 3 * 3,  # branch, signature, fee:gas_limit:storage_limit mumav values (+3 bytes)
...
def default_gas_limit(
    ...
    values: Dict[str, int] = {
        'reveal': {'mv1': 176, 'mv2': 162, 'mv3': 1101, 'mv4': 1681}[content['source'][:3]],
        'delegation': 1000,
