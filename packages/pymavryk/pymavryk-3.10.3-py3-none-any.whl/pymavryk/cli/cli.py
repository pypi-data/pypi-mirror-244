...
from pymavryk import __version__
from pymavryk import pymavryk
from pymavryk.cli.github import create_deployment
from pymavryk.cli.github import create_deployment_status
from pymavryk.context.mixin import default_network
from pymavryk.logging import DEFAULT_LOGGING_CONFIG
...
kernel_json = {
    "argv": ['pymavryk', 'kernel', 'run', "-file", "{connection_file}"],
...
def get_contract(path):
    ...
    if path:
        ...
        contract = pymavryk.using(shell=network).contract(address)
    return contract
...
def activate(_ctx, path: str, network: str) -> None:
    ptz = pymavryk.using(key=path, shell=network)
    logger.info(
...
def deploy(
    ...
    ptz = pymavryk.using(shell=network, key=key)
