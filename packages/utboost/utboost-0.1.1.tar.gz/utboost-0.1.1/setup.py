...
class CMakeBuild(build_ext.build_ext):
    ...
    def build_extension(self, ext: Extension):
        if isinstance(ext, CMakeExtension):
            ...
            build_cmd = 'mingw32-make.exe' if os.name == 'nt' else 'make'
            try:
                _ = subprocess.run([build_cmd, "--version"], check=True)
            except OSError:
                if os.name == 'nt':
                    self.logger.error('MinGW is not installed. '
                                      'Please install MinGW and make sure mingw/bin is added to the system\'s PATH.')
                else:
                    self.logger.error('make not found. Please run `sudo apt-get install make` first.')
                sys.exit(1)
            ...
            self.logger.info('Configure CMAKE_BUILD_TYPE=%s' % build_type)
            subprocess.check_call(cmake_args, cwd=BUILD_DIR, env=env)
            subprocess.check_call([build_cmd, '-j', str(os.cpu_count())], cwd=BUILD_DIR, env=env)
        else:
