...
class NDExHierarchyUploader(object):
    ...
    def _initialize_ndex_client(self):
        ...
        if self._server is None or self._user is None or self._password is None:
            raise CellmapsGenerateHierarchyError('NDEx server or credentials were not specified. '
    ...
    def upload_hierary_and_parent_netowrk_from_files(self, outdir):
        ...
        if not os.path.exists(hierarchy_path) or not os.path.exists(parent_network_path):
            raise CellmapsGenerateHierarchyError(f'Hierarchy or parent network file does not exist in dir {outdir}.')
