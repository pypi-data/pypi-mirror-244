...
def test_client_init_with_project(refuel_client_with_project):
    assert refuel_client_with_project is not None
    assert refuel_client_with_project._project_id == test_project_id
    return
...
def test_get_items_no_task(refuel_client):
    items = refuel_client.get_items(
        dataset=test_dataset_name, project=test_project_name
    )
    assert len(items) == 20
    return
...
def test_get_items_with_task(refuel_client):
    items = refuel_client.get_items(
        dataset=test_dataset_name, task=test_task_name, project=test_project_name
    )
    assert len(items) == 20
    return
...
# Get Items (order_direction = ASC)
def test_get_items_order_ascend(refuel_client):
    items = refuel_client.get_items(
        dataset=test_dataset_name,
        task=test_task_name,
        project=test_project_name,
        order_by="bowler_name",
        order_direction="ASC",
    )
...
# Get Items (order_direction = DESC)
def test_get_items_order_ascend(refuel_client):
    items = refuel_client.get_items(
        dataset=test_dataset_name,
        task=test_task_name,
...
def test_get_datasets(refuel_client):
    datasets = refuel_client.get_datasets(project=test_project_name)
...
def test_get_tasks(refuel_client):
    tasks = refuel_client.get_tasks(project=test_project_name)
    assert tasks is not None
    return


def test_get_task(refuel_client):
    task = refuel_client.get_task(task=test_task_name, project=test_project_name)
...
def test_get_task_run(refuel_client):
    task_run = refuel_client.get_task_run(
        task=test_task_name, dataset=test_dataset_name, project=test_project_name
    )
