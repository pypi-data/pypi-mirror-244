...
def log():
    ...
    def login():
        ...
        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM users WHERE username=? AND password_hash=?', (username, input_password_hash))
        ...
        if user:
            ...
            return False
            # print("User not found or incorrect password. Would you like to register?")
            # register_choice = input("Enter 'yes' to register, 'no' to exit: ")
