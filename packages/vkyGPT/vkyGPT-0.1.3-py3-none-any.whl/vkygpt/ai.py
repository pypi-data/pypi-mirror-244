...
def open_pdf_in_edge(sentence):
    match_after = re.search(r'pdf\s+(\w+)', sentence, flags=re.IGNORECASE)
    match_before = re.search(r'(\w+)\s+pdf', sentence, flags=re.IGNORECASE)
    match_with_extension = re.search(r'(\w+\.pdf)', sentence, flags=re.IGNORECASE)

    if match_with_extension:
        pdf_file_name = match_with_extension.group(1)
    elif match_after:
        pdf_file_name = match_after.group(1) + ".pdf"
    elif match_before:
        pdf_file_name = match_before.group(1) + ".pdf"
    else:
        print(colored("PDF file name not found in the sentence.", "red"))
        return
    ...
    def find_file(start_path, file_name):
        ...
        for root, dirs, files in os.walk(start_path):
            for current_file in files:
                if current_file.lower() == file_name_lower:
    ...
    try:
        ...
        with yaspin(color="yellow") as spinner:
            pdf_file_path = find_file(root_path, pdf_file_name)

        if pdf_file_path:
            spinner.ok(colored("PDF Found......", "green"))
            subprocess.run([edge_path, pdf_file_path], check=True)
        else:
            spinner.fail(colored("PDF not found!", "red"))
...
async def main():
    ...
    while True:
        try:
            ...
            elif 'open pdf' in a or 'find pdf' in a or "pdf" in a:
                speak("Opening Pdf")
                open_pdf_in_edge(a)
