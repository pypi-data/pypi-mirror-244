...
async def export_to_json(client, message):
    try:
        ...
        export_file_path = f'exported_data_{collection_name}.json'
...
async def import_from_json(client, message):
    try:
        ...
        document_message = await app.listen(message.chat.id)
        ...
        if document_message and document_message.document:
            ...
            await app.download_media(document_message.document.file_id, file_name=file_path)
