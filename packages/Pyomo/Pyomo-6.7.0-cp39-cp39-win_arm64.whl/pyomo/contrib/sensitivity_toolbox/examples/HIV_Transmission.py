...
def create_model():
    ...
    m.ij = Set(
        initialize=[(0, 0), (0, 1), (1, 0), (1, 1), (2, 0), (2, 1), (3, 0), (4, 0)],
        ordered=True,
    )
    ...
    def _yy00(m, t):
        return (
            sum(m.pp[kk] * m.yy[t, kk] for kk in m.ij) * m.dyy[t, (0, 0)]
            == sum(m.pp[kk] * m.yy[t, kk] for kk in m.ij)
            * (
                m.II[(0, 0)]
                - (m.vp[t] + m.dd[(0, 0)]) * m.yy[t, (0, 0)]
                + m.omeg * m.yy[t, (0, 1)]
            )
            - m.pp[(0, 0)]
            * sum(m.bb[kk] * m.eta00[kk] * m.pp[kk] * m.yy[t, kk] for kk in m.ij)
            * m.yy[t, (0, 0)]
        )
    ...
    def _yy01(m, t):
        return (
            sum(m.pp[kk] * m.yy[t, kk] for kk in m.ij) * m.dyy[t, (0, 1)]
            == sum(m.pp[kk] * m.yy[t, kk] for kk in m.ij)
            * (m.vp[t] * m.yy[t, (0, 0)] - (m.dd[(0, 0)] + m.omeg) * m.yy[t, (0, 1)])
            - m.pp[(0, 1)]
            * (1 - m.eps)
            * sum(m.bb[kk] * m.eta01[kk] * m.pp[kk] * m.yy[t, kk] for kk in m.ij)
            * m.yy[t, (0, 1)]
        )
    ...
    def _yy10(m, t):
        return (
            sum(m.pp[kk] * m.yy[t, kk] for kk in m.ij) * m.dyy[t, (1, 0)]
            == sum(m.pp[kk] * m.yy[t, kk] for kk in m.ij)
            * (
                m.II[(1, 0)]
                - ((m.sig * m.xi) + m.vp[t] + m.dd[(1, 0)] + m.dd[(0, 0)])
                * m.yy[t, (1, 0)]
                + m.omeg * m.yy[t, (1, 1)]
            )
            + m.pp[(0, 0)]
    ...
    def _yy11(m, t):
        return (
            sum(m.pp[kk] * m.yy[t, kk] for kk in m.ij) * m.dyy[t, (1, 1)]
            == sum(m.pp[kk] * m.yy[t, kk] for kk in m.ij)
            * (
                m.vp[t] * m.yy[t, (1, 0)]
                - (m.omeg + (m.sig * m.xi) + m.dd[(1, 1)] + m.dd[(0, 0)])
                * m.yy[t, (1, 1)]
            )
            + m.pp[(0, 1)]
            * (1 - m.eps)
    ...
    def _yy20(m, t):
        return (
            m.dyy[t, (2, 0)]
            == m.II[(2, 0)]
            + m.sig * m.xi * (m.yy[t, (1, 0)] + m.yy[t, (1, 1)])
    ...
    def _yy40(m, t):
        return (
            m.dyy[t, (4, 0)]
            == m.dd[(3, 0)] * m.yy[t, (3, 0)]
            - (m.dd[(4, 0)] + m.dd[(0, 0)]) * m.yy[t, (4, 0)]
        )

    m.yy40DiffCon = Constraint(m.t, rule=_yy40)

    def _L(m, t):
        return m.dL[t] == exp(-m.rr * t) * (
            m.aa
            * (
                m.kp * m.vp[t] * (m.yy[t, (0, 0)] + m.yy[t, (1, 0)])
                + (m.kt * m.vt[t] * m.yy[t, (2, 0)])
                + sum(m.cc[kk] * m.yy[t, kk] for kk in m.ij)
            )
...
if __name__ == '__main__':
    ...
    m_sipopt = sensitivity_calculation(
        'sipopt', m, [m.eps, m.qq, m.aa], [m.epsDelta, m.qqDelta, m.aaDelta], tee=True
    )
