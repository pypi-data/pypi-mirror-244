...
class SystemCallSolver(OptSolver):
        ...
        if not validate:
            ...
            if exe is None:
                raise ValueError(
                    ...
                    "call set_executable with validate=False." % (self.name, name)
                )
    ...
    def executable(self):
        ...
        """
        return (
            self._user_executable
            if (self._user_executable is not None)
    ...
    def _presolve(self, *args, **kwds):
        ...
        #
        # The pre-cleanup is probably unnecessary, but also not harmful.
        #
    ...
    def process_output(self, rc):
        ...
        if self._report_timing is True:
            print(
                "      %6.2f seconds required to read logfile "
        ...
        if self._results_reader is None:
            ...
            if self._report_timing is True:
                print(
                    "      %6.2f seconds required to read solution file "
            ...
            if len(results.solution) == 1:
                ...
                results = self._results_reader(
                    self._results_file, res=results, suffixes=self._suffixes
                )
