...
class Test_suffix(unittest.TestCase):
    ...
    def test_pickle(self):
        s = suffix(direction=suffix.EXPORT, datatype=suffix.FLOAT)
    ...
    def test_export_suffix_generator(self):
        ...
        # default
        self.assertEqual(
            [id(c_) for c_ in export_suffix_generator(m)],
            [
                id(m.s1),
                id(m.s1i),
                id(m.s3),
        ...
        # descend_into=False
        self.assertEqual(
            [id(c_) for c_ in export_suffix_generator(m, descend_into=False)],
            [id(m.s1), id(m.s1i), id(m.s3), id(m.s3i)],
        )
        # datatype=INT
        self.assertEqual(
            [id(c_) for c_ in export_suffix_generator(m, datatype=suffix.INT)],
            [id(m.s1i), id(m.s3i), id(m.b.s1i), id(m.b.s3i)],
        )
        # active=True
        m.s1.deactivate()
        m.b.deactivate()
        self.assertEqual(
            [id(c_) for c_ in export_suffix_generator(m, active=True)],
    ...
    def test_import_suffix_generator(self):
        ...
        # default
        self.assertEqual(
            [id(c_) for c_ in import_suffix_generator(m)],
            [
                id(m.s1),
                id(m.s1i),
                id(m.s2),
        ...
        # descend_into=False
        self.assertEqual(
            [id(c_) for c_ in import_suffix_generator(m, descend_into=False)],
            [id(m.s1), id(m.s1i), id(m.s2), id(m.s2i)],
        )
        # datatype=INT
        self.assertEqual(
            [id(c_) for c_ in import_suffix_generator(m, datatype=suffix.INT)],
            [id(m.s1i), id(m.s2i), id(m.b.s1i), id(m.b.s2i)],
        )
        # active=True
        m.s1.deactivate()
        m.b.deactivate()
        self.assertEqual(
            [id(c_) for c_ in import_suffix_generator(m, active=True)],
    ...
    def test_local_suffix_generator(self):
        ...
        # default
        self.assertEqual(
            [id(c_) for c_ in local_suffix_generator(m)],
            [id(m.s0), id(m.s0i), id(m.b.s0), id(m.b.s0i)],
        )
        # descend_into=False
        self.assertEqual(
            [id(c_) for c_ in local_suffix_generator(m, descend_into=False)],
            [id(m.s0), id(m.s0i)],
        )
        # datatype=INT
        self.assertEqual(
            [id(c_) for c_ in local_suffix_generator(m, datatype=suffix.INT)],
            [id(m.s0i), id(m.b.s0i)],
        )
        # active=True
        m.s0.deactivate()
        m.b.deactivate()
        self.assertEqual(
            [id(c_) for c_ in local_suffix_generator(m, active=True)], [id(m.s0i)]
        )
    ...
    def test_suffix_generator(self):
        ...
        # default
        self.assertEqual(
            [id(c_) for c_ in suffix_generator(m)],
            [
                id(m.s0),
                id(m.s0i),
                id(m.s1),
        ...
        # descend_into=False
        self.assertEqual(
            [id(c_) for c_ in suffix_generator(m, descend_into=False)],
            [
                id(m.s0),
                id(m.s0i),
                id(m.s1),
        ...
        # datatype=INT
        self.assertEqual(
            [id(c_) for c_ in suffix_generator(m, datatype=suffix.INT)],
            [
                id(m.s0i),
                id(m.s1i),
                id(m.s2i),
        ...
        m.b.deactivate()
        self.assertEqual(
            [id(c_) for c_ in suffix_generator(m, active=True)],
