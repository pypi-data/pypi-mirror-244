...
class ProblemWriter_cpxlp(AbstractProblemWriter):
    ...
    def __call__(self, model, output_filename, solver_capability, io_options):
        ...
        if symbolic_solver_labels and (labeler is not None):
            raise ValueError(
                "ProblemWriter_cpxlp: Using both the "
    ...
    def _print_expr_canonical(
        ...
        if len(x.linear_vars) > 0:
            ...
            if column_order is None:
                ...
                #
                for i, var in sorted(
                    enumerate(x.linear_vars), key=lambda x: column_order[x[1]]
                ):
        ...
        if len(x.quadratic_vars) > 0:
            ...
            if column_order is None:
                    ...
                    if col1 < col2:
                        cols.append(
                            (
                                (
                                    (col1, col2),
                                    variable_symbol_dictionary[id(var1)],
                                    variable_symbol_dictionary[id(var2)],
                                )
                            )
                        )
                    elif col1 > col2:
                        ...
                        quad.add(i)
                        cols.append(
                            ((col1, col1), variable_symbol_dictionary[id(var1)])
                        )
                    i += 1
                for i, cols_ in sorted(enumerate(cols), key=lambda x: x[1][0]):
                    ...
                    if is_objective:
                        output.append(
                            quad_coef_string_template % 2 * x.quadratic_coefs[i]
                        )
                    else:
                        output.append(quad_coef_string_template % x.quadratic_coefs[i])
                    if i in quad:
            ...
            try:
                ...
                if _var is not None:
                    logger.error(
                        ...
                        "block on the submodel being written" % (obj.name, _var.name)
                    )
                raise
            ...
            for objective_data in block.component_data_objects(
                ...
                if gen_obj_repn == False:
                    ...
                    repn = generate_standard_repn(
                        objective_data.expr, quadratic=supports_quadratic_objective
                    )
                    if gen_obj_repn:
        ...
        for vardata in variable_list:
            ...
            if vardata.is_binary():
                ...
                raise TypeError(
                    ...
                    "Variable is not continuous, integer, or binary." % (vardata.name)
                )
