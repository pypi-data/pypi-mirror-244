...
class TestComponentUID(unittest.TestCase):
    ...
    def test_genFromComponent_nested(self):
        ...
        self.assertEqual(cuid._cids, (('b', (1, '2')), ('c', tuple()), ('a', (3,))))
    ...
    def test_parseFromString_wildcard_1(self):
        ...
        self.assertEqual(
            cuid._cids, (('b', (Ellipsis,)), ('c', tuple()), ('a', (_star,)))
        )

    def test_parseFromString_wildcard_2(self):
        cuid = ComponentUID('b[*,*].c.a[*]')
        self.assertEqual(
            cuid._cids, (('b', (_star, _star)), ('c', tuple()), ('a', (_star,)))
        )
    ...
    def test_parseFromRepr2_lexError(self):
        ...
        with self.assertRaisesRegex(IOError, "ERROR: Token ':' Line 1 Column 1"):
            list(cuid._parse_cuid_v2(':'))
        with self.assertRaisesRegex(IOError, "ERROR: Token '\n].b:' Line 1 Column 3"):
    ...
    def test_find_component_deprecated(self):
        ...
        self.assertIn(
            'ComponentUID.find_component() is deprecated.', DEP_OUT.getvalue()
        )
    ...
    def test_generate_cuid_string_map(self):
        ...
        cuids = (
            ComponentUID.generate_cuid_string_map(
                model, descend_into=False, repr_version=1
            ),
        ...
        cuids = (
            ComponentUID.generate_cuid_string_map(model, ctype=Var, repr_version=1),
        ...
        cuids = (
            ...
            ComponentUID.generate_cuid_string_map(model, ctype=Var, descend_into=False),
        )
    ...
    def test_deprecated_ComponentUID_location(self):
        ...
        warning = (
            "DEPRECATED: the 'ComponentUID' class has been moved to "
    ...
    def assertListSameComponents(self, m, cuid1, cuid2):
        ...
        self.assertEqual(
            len(list(cuid1.list_components(m))), len(list(cuid2.list_components(m)))
        )
    ...
    def test_cuid_from_slice_with_call(self):
        ...
        _slice = IndexedComponent_slice(
            m.b[:].fix, (IndexedComponent_slice.call, ('fix',), {})
        )
        with self.assertRaisesRegex(
            ValueError,
        ...
        _slice = IndexedComponent_slice(
            m.b[:].component('v'), (IndexedComponent_slice.call, ('fix',), {})
        )
        with self.assertRaisesRegex(
            ValueError,
    ...
    def test_cuid_from_slice_errors(self):
        ...
        _slice = IndexedComponent_slice(
            m.b[:].component('v'), (IndexedComponent_slice.del_attribute, ('foo',))
        )
        with self.assertRaisesRegex(
            ValueError,
