...
def instance2dat(instance, output_filename):
    ...
    for set_name, set_object in instance.component_map(Set, active=True).items():
        if (set_object.initialize is not None) and (
            type(set_object.initialize) is types.FunctionType
        ):
            continue
    ...
    for param_name, param_object in instance.component_map(Param, active=True).items():
        if (param_object._initialize is not None) and (
            type(param_object._initialize) is types.FunctionType
        ):
            continue
        ...
        if None in param_object:
            output_file.write(
                "param " + param_name + " := " + str(value(param_object[None])) + " ;\n"
            )
            output_file.write("\n")
        else:
