...
class PressureDropModel(ExternalInputOutputModel):
    ...
    def evaluate_derivatives(self):
        jac = [
            [1, -self._F**2, 0, -2 * self._c1 * self._F],
...
class TestExternalInputOutputModel(unittest.TestCase):
    ...
    def test_pyomo_external_model(self):
        ...
        cyipopt_problem = PyomoExternalCyIpoptProblem(
            m, PressureDropModel(), [m.Pin, m.c1, m.c2, m.F], [m.P1, m.P2]
        )
        ...
        # solve the problem
        solver = CyIpoptSolver(
            cyipopt_problem, {'hessian_approximation': 'limited-memory'}
        )
    ...
    def test_pyomo_external_model_dummy_var_initialization(self):
        ...
        self.assertAlmostEqual(
            pyo.value(m._dummy_constraint_CyIpoptPyomoExNLP.body),
            pyo.value(m._dummy_constraint_CyIpoptPyomoExNLP.lower),
        )
        self.assertAlmostEqual(
            pyo.value(m._dummy_constraint_CyIpoptPyomoExNLP.body),
            pyo.value(m._dummy_constraint_CyIpoptPyomoExNLP.upper),
        )
