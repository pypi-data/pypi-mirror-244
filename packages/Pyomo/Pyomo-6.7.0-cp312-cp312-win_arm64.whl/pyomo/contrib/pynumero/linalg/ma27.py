...
class MA27Interface(object):
    ...
    def do_symbolic_factorization(self, dim, irn, icn):
        ...
        self.lib.do_symbolic_factorization(self._ma27, dim, ne, irn, icn)
    ...
    def do_numeric_factorization(self, irn, icn, dim, entries):
        ...
        if self.a_factor is not None:
            ...
            self.lib.alloc_a(self._ma27, int(self.a_factor * min_size))
        if self.iw_factor is not None:
            min_size = self.get_info(6)
            self.lib.alloc_iw_b(self._ma27, int(self.iw_factor * min_size))

        self.lib.do_numeric_factorization(self._ma27, dim, ne, irn, icn, ent)
        return self.get_info(1)
