...
np, numpy_available = attempt_import(
    'numpy', 'inverse_reduced_hessian numpy', minimum_version='1.13.0'
)
...
class TestInverseReducedHessian(unittest.TestCase):
    ...
    def test_invrh_zavala_thesis(self):
        ...
        m.x = pyo.Var([1, 2, 3])
        m.obj = pyo.Objective(
            expr=(m.x[1] - 1) ** 2 + (m.x[2] - 2) ** 2 + (m.x[3] - 3) ** 2
        )
        m.c1 = pyo.Constraint(expr=m.x[1] + 2 * m.x[2] + 3 * m.x[3] == 0)

        status, invrh = inv_reduced_hessian_barrier(m, [m.x[2], m.x[3]])
        expected_invrh = np.asarray(
            [[0.35714286, -0.21428571], [-0.21428571, 0.17857143]]
        )
    ...
    def _simple_model(self, add_constraint=False):
        ...
        model.response_function = pyo.Expression(
            data.tofu, data.chard, rule=response_rule
        )

        def SSE_rule(m):
            return sum(
                (data.y[i] - m.response_function[data.tofu[i], data.chard[i]]) ** 2
                for i in data.index
            )
    ...
    def test_3x3_using_linear_regression(self):
        ...
        tstar = [
            pyo.value(model.b0),
            pyo.value(model.b['tofu']),
            pyo.value(model.b['chard']),
        ]
