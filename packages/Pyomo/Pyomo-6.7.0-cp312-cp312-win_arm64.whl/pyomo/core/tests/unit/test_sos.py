...
class TestSimple(unittest.TestCase):
    ...
    def test_get_variables(self):
        ...
        self.M.c = SOSConstraint(var=self.M.x, sos=1)
        self.assertEqual(
            set(id(v) for v in self.M.c.get_variables()),
...
class TestExamples(unittest.TestCase):
    def test1(self):
        ...
        self.assertEqual(
            set((v.name, w) for v, w in M.c.get_items()),
    ...
    def test10(self):
        ...
        self.assertEqual(
            set((v.name, w) for v, w in M.c[0].get_items()),
            set((M.x[i].name, i) for i in [1, 2]),
        )
        self.assertEqual(
            set((v.name, w) for v, w in M.c[1].get_items()),
    ...
    def test11(self):
        ...
        M.x = Var([1, 2, 3], dense=True)
        M.c = SOSConstraint(
            [0, 1], var=M.x, weights=w, sos=1, index={0: [1, 2], 1: [2, 3]}
        )
        self.assertEqual(
            set((v.name, w) for v, w in M.c[0].get_items()),
