...
class BaronTest(unittest.TestCase):
    ...
    def test_BARON_option_warnings(self):
        ...
        with LoggingIntercept(os, 'pyomo.solvers'):
            ...
            with SolverFactory("baron") as opt:
                results = opt.solve(
                    m, options={'ResName': 'results.lst', 'TimName': 'results.tim'}
                )
        ...
        self.assertIn(
            'Ignoring user-specified option "ResName=results.lst"', os.getvalue()
        )
        self.assertIn(
            'Ignoring user-specified option "TimName=results.tim"', os.getvalue()
        )
