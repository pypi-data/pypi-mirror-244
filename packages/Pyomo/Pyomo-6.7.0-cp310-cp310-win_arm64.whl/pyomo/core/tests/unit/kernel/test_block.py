...
class TestHeterogeneousContainer(unittest.TestCase):
    ...
    def test_preorder_traversal(self):
        ...
        order = list(
            str(obj)
            for obj in pmo.preorder_traversal(
                model, descend=lambda x: (x is not model.k) and (x is not model.K)
            )
        )
        self.assertEqual(
            order,
    ...
    def test_getname(self):
        ...
        self.assertEqual(
            model.J[1][0].b.v.getname(fully_qualified=True, relative_to=model.J[1][0]),
            'b.v',
        )
        self.assertEqual(
            model.J[1][0].b.v.getname(fully_qualified=True, relative_to=model.J[1]),
            '[0].b.v',
        )
        self.assertEqual(
            model.J[1][0].b.v.getname(fully_qualified=True, relative_to=model.J),
            '[1][0].b.v',
        )
        self.assertEqual(
            model.J[1][0].b.v.getname(fully_qualified=True, relative_to=model),
            'J[1][0].b.v',
        )
    ...
    def test_heterogeneous_containers(self):
        ...
        order1 = list(
            str(obj) for obj in heterogeneous_containers(self.model, descend_into=f)
        )
        order2 = list(
            str(obj)
            for obj in heterogeneous_containers(
                self.model, descend_into=lambda x: True if (x is self.model) else False
            )
        )
        self.assertEqual(order1, order2)
        self.assertEqual(
            order1,
            ['<block>', 'b', 'B[0]', 'B[1][0]', 'k', 'K[0]', 'j', 'J[0]', 'J[1][0]'],
        )
        order = list(
            str(obj) for obj in heterogeneous_containers(self.model, ctype=IBlock)
        )
        self.assertEqual(
            order,
            [
        ...
        order = list(
            str(obj) for obj in heterogeneous_containers(self.model, ctype=IJunk)
        )
        self.assertEqual(
            order, ['K[0].j', 'K[0].J[0]', 'K[0].J[1][0]', 'j', 'J[0]', 'J[1][0]']
        )
        order = list(
            str(obj) for obj in heterogeneous_containers(self.model.K, ctype=IJunk)
        )
        self.assertEqual(order, ['K[0].j', 'K[0].J[0]', 'K[0].J[1][0]'])
        order = list(
            str(obj) for obj in heterogeneous_containers(self.model.K[0], ctype=IJunk)
        )
        self.assertEqual(order, ['K[0].j', 'K[0].J[0]', 'K[0].J[1][0]'])
        order = list(
            str(obj) for obj in heterogeneous_containers(self.model.K[0].j, ctype=IJunk)
        )
        self.assertEqual(order, ['K[0].j'])
        order = list(
            str(obj)
            for obj in heterogeneous_containers(self.model.K[0].j, ctype=IBlock)
        )
        self.assertEqual(order, [])
...
class TestMisc(unittest.TestCase):
    ...
    def test_traversal(self):
        ...
        self.assertEqual(
            [
                obj.name
                for obj in pmo.preorder_traversal(b, active=None, descend=descend)
            ],
            [
                None,
                'v',
        ...
        self.assertEqual(
            [obj.name for obj in pmo.preorder_traversal(b, descend=descend)],
            [None, 'v', 'c2', 'c2[0]', 'B'],
        )
        self.assertEqual(
            [
                obj.name
                for obj in pmo.preorder_traversal(b, active=True, descend=descend)
            ],
            [None, 'v', 'c2', 'c2[0]', 'B'],
        )
        ...
        self.assertEqual(
            [
                obj.name
                for obj in pmo.preorder_traversal(
                    b, active=None, ctype=IConstraint, descend=descend
                )
            ],
            [
                None,
        ...
        self.assertEqual(
            [
                obj.name
                for obj in pmo.preorder_traversal(b, ctype=IConstraint, descend=descend)
            ],
            [None, 'c2', 'c2[0]', 'B'],
        )
        self.assertEqual(
            [
                obj.name
                for obj in pmo.preorder_traversal(
                    b, ctype=IConstraint, active=True, descend=descend
                )
            ],
            [None, 'c2', 'c2[0]', 'B'],
        )
        ...
        self.assertEqual(
            [obj.name for obj in pmo.preorder_traversal(m, ctype=IVariable)],
            [None, 'B', 'B[0]', 'B[0].v'],
        )
        self.assertEqual(
            [obj.name for obj in pmo.preorder_traversal(m.B, ctype=IVariable)],
            ['B', 'B[0]', 'B[0].v'],
        )
        self.assertEqual(
            [obj.name for obj in pmo.preorder_traversal(m.B[0], ctype=IVariable)],
            ['B[0]', 'B[0].v'],
        )
        self.assertEqual(
            [obj.name for obj in pmo.preorder_traversal(m.B[0].v, ctype=IVariable)],
            ['B[0].v'],
        )
    ...
    def test_clone1(self):
        ...
        self.assertIs(
            list(_collect_expr_components(b.bdict[0].e.expr).values())[0], b.v
        )
        ...
        self.assertEqual(
            sorted(
                list(
                    id(v_)
                    for v_ in _collect_expr_components(b.blist[0].e.expr).values()
                )
            ),
        ...
        self.assertEqual(
            sorted(
                list(
                    id(v_) for v_ in _collect_expr_components(b_blist0.e.expr).values()
                )
            ),
            sorted(list(id(v_) for v_ in [b.v, b.b.v])),
        )
    ...
    def test_clone2(self):
        ...
        b.clist = constraint_list(
            constraint((0, v_, i)) for i, v_ in enumerate(b.vlist)
        )
        b.p = parameter()
        b.pdict = parameter_dict(((i, parameter(i)) for i in b.vdict))
        b.plist = parameter_list(parameter(i) for i in range(len(b.vlist)))
        b.e = expression(b.v * b.p + 1)
        b.edict = expression_dict(
            ((i, expression(b.vdict[i] * b.pdict[i] + 1)) for i in b.vdict)
        )
        b.elist = expression_list(
            expression(v_ * b.plist[i] + 1) for i, v_ in enumerate(b.vlist)
        )
        ...
        for c1, c2 in zip(b.components(), bc.components()):
            ...
            if hasattr(c1, 'expr'):
                ...
                self.assertEqual(
                    len(_collect_expr_components(c1.expr)),
                    len(_collect_expr_components(c2.expr)),
                )
                for subc1, subc2 in zip(
                    _collect_expr_components(c1.expr).values(),
        ...
        self.assertEqual(
            len(list(bc_init.components())), len(list(sub_bc.components()))
        )
...
class _Test_block_base(object):
    ...
    def test_pickle(self):
        ...
        self.assertEqual(
            len(list(pmo.preorder_traversal(b, active=None))), len(self._names) + 1
        )
    ...
    def test_preorder_traversal(self):
        ...
        self.assertEqual(
            [str(obj) for obj in pmo.preorder_traversal(self._block, active=None)],
            [str(obj) for obj in self._preorder],
        )
        self.assertEqual(
            [id(obj) for obj in pmo.preorder_traversal(self._block, active=None)],
        ...
        self.assertEqual(
            [
                str(obj)
                for obj in pmo.preorder_traversal(
                    self._block, active=None, ctype=IVariable
                )
            ],
            [str(obj) for obj in self._preorder if obj.ctype in (IBlock, IVariable)],
        )
        self.assertEqual(
            [
                id(obj)
            ...
            [id(obj) for obj in self._preorder if obj.ctype in (IBlock, IVariable)],
        )
    ...
    def test_preorder_traversal_descend_check(self):
        ...
        order = list(
            pmo.preorder_traversal(
                self._block, active=None, ctype=IVariable, descend=descend
            )
        )
        # this first test makes failures a
        # little easier to debug
        ...
        self.assertEqual(
            ...
            [
                str(obj)
                for obj in self._preorder
                if (obj.parent is None) or (obj.parent is self._block)
            ],
        )
        self.assertEqual(
            [id(obj) for obj in order],
    ...
    def test_children(self):
        for obj in self._children:
            ...
            if isinstance(obj, IBlock):
                ...
                # little easier to debug
                self.assertEqual(
                    sorted(str(child) for child in obj.children()),
                    sorted(str(child) for child in self._children[obj]),
                )
                self.assertEqual(
                ...
                # little easier to debug
                self.assertEqual(
                    sorted(str(child) for child in obj.children(ctype=IBlock)),
                    sorted(
                        str(child)
                ...
                # little easier to debug
                self.assertEqual(
                    sorted(str(child) for child in obj.children(ctype=IVariable)),
                    sorted(
                        str(child)
                        for child in self._children[obj]
    ...
    def test_components_no_descend_active_None(self):
        for obj in self._components_no_descend:
            ...
            self.assertEqual(
                sorted(
                    str(_b)
                    for _b in obj.components(
                        ctype=IBlock, active=None, descend_into=False
                    )
                ),
                sorted(str(_b) for _b in self._components_no_descend[obj][IBlock]),
            )
            self.assertEqual(
                set(
                    id(_b)
                ...
                set(id(_b) for _b in self._components_no_descend[obj][IBlock]),
            )
            # test ctype=IVariable
            self.assertEqual(
                sorted(
                    str(_v)
                    for _v in obj.components(
                        ctype=IVariable, active=None, descend_into=False
                    )
                ),
            ...
            self.assertEqual(
                sorted(
                    str(_c) for _c in obj.components(active=None, descend_into=False)
                ),
                sorted(
                    str(_c)
            ...
            self.assertEqual(
                set(id(_c) for _c in obj.components(active=None, descend_into=False)),
                set(
                    id(_c)
                    for ctype in self._components_no_descend[obj]
                    for _c in self._components_no_descend[obj][ctype]
        ...
        for obj in self._components:
            ...
            self.assertEqual(
                sorted(
                    str(_b)
                    for _b in obj.components(
                        ctype=IBlock, active=None, descend_into=True
                    )
                ),
                sorted(str(_b) for _b in self._components[obj][IBlock]),
            )
            self.assertEqual(
                set(
                    id(_b)
                ...
                set(id(_b) for _b in self._components[obj][IBlock]),
            )
            # test ctype=IVariable
            self.assertEqual(
                sorted(
                    str(_v)
                    for _v in obj.components(
                        ctype=IVariable, active=None, descend_into=True
                    )
                ),
                sorted(str(_v) for _v in self._components[obj][IVariable]),
            )
            self.assertEqual(
                set(
                    id(_v)
                ...
                set(id(_v) for _v in self._components[obj][IVariable]),
            )
            # test no ctype
            self.assertEqual(
                sorted(
                    str(_c) for _c in obj.components(active=None, descend_into=True)
                ),
                sorted(
                    str(_c)
            ...
            self.assertEqual(
                set(id(_c) for _c in obj.components(active=None, descend_into=True)),
                set(
                    id(_c)
                    for ctype in self._components[obj]
                    for _c in self._components[obj][ctype]
...
class _Test_block(_Test_block_base):
    ...
    def setUpClass(cls):
        ...
        for obj in cls._components_no_descend:
            cls._blocks_no_descend[obj] = [obj] + cls._components_no_descend[obj][
                IBlock
            ]
    ...
    def test_collect_ctypes_small_block_storage(self):
        ...
        B.b = b
        self.assertEqual(
            B.collect_ctypes(descend_into=False, active=None), set([IBlock])
        )
        self.assertEqual(B.collect_ctypes(descend_into=False), set([IBlock]))
        self.assertEqual(
            B.collect_ctypes(descend_into=False, active=True), set([IBlock])
        )
        self.assertEqual(
            B.collect_ctypes(active=None), set([IBlock, IVariable, IConstraint])
        )
        ...
        B.x = variable()
        self.assertEqual(
            B.collect_ctypes(descend_into=False, active=None), set([IBlock, IVariable])
        )
...
class _Test_small_block(_Test_block_base):
    ...
    def test_collect_ctypes(self):
        self.assertEqual(
            self._block.collect_ctypes(active=None), set([IBlock, IVariable])
        )
        self.assertEqual(self._block.collect_ctypes(), set([IBlock, IVariable]))
        self.assertEqual(
            self._block.collect_ctypes(active=True), set([IBlock, IVariable])
        )
        self.assertEqual(
            self._block.collect_ctypes(descend_into=False), set([IBlock, IVariable])
        )
        self.assertEqual(
            self._block.collect_ctypes(active=True, descend_into=False),
        ...
        try:
            ...
            self.assertEqual(
                self._block.collect_ctypes(active=None, descend_into=False),
                set([IBlock, IVariable]),
            )
            self.assertEqual(
    ...
    def test_inactive_behavior(self):
        ...
        self.assertNotEqual(
            len(list(pmo.preorder_traversal(b, active=None, descend=descend))), 0
        )
        self.assertEqual(len(list(pmo.preorder_traversal(b, descend=descend))), 0)
        self.assertEqual(
            len(list(pmo.preorder_traversal(b, active=True, descend=descend))), 0
        )
        ...
        descend.seen = []
        self.assertEqual(
            len(list(pmo.preorder_traversal(b, active=None, descend=descend))), 1
        )
...
class Test_block_tuple(_TestActiveTupleContainerBase, unittest.TestCase):
    _container_type = block_tuple
    _ctype_factory = lambda self: block()


class Test_block_list(_TestActiveListContainerBase, unittest.TestCase):
