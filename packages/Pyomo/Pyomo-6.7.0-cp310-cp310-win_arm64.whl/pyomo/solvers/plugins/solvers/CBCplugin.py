...
class CBCSHELL(SystemCallSolver):
    ...
    def process_logfile(self):
        ...
        for line in output.split("\n"):
            ...
            if n_tokens > 1:
                ...
                # Partial search - best objective %g (best possible %g), took %d iterations and %d nodes
                    ...
                    n_tokens > 15
                    and tokens[1:3] == ('Partial', 'search')
                    and tokens[4:6] == ('best', 'objective')
                    and tokens[7:9] == ('(best', 'possible')
                    and tokens[12] == 'iterations'
                    and tokens[15] == 'nodes'
                ):
                    ...
                    and tokens[1] == 'After'
                    and tokens[3] == 'nodes,'
                    and tokens[8:10] == ('best', 'solution,')
                    and tokens[10:12] == ('best', 'possible')
                ):
                    nodes = int(tokens[2])
                ...
                # https://projects.coin-or.org/Cbc/browser/trunk/Cbc/src/CbcSolver.cpp?rev=2497#L10477
                    ...
                    if n_tokens > 4 and tokens[2] == "objective" and tokens[4] != "and":
                        ...
                        n_tokens > 5
                        and tokens[1] == 'objective'
                        and tokens[5] == 'iterations'
                    ):
                        soln.status = SolutionStatus.optimal
                        optim_value = _float(tokens[2])
                        results.solver.statistics.black_box.number_of_iterations = int(
                    ...
                    n_tokens == 10
                    and "Presolve" in tokens
                    and "iterations" in tokens
                    and tokens[0] == "Optimal"
                    and "objective" == tokens[1]
                ):
                    soln.status = SolutionStatus.optimal
                    optim_value = _float(tokens[2])
        ...
        if soln.status is SolutionStatus.optimal:
            results.solver.termination_message = (
                "Model was solved to optimality (subject to tolerances), and an "
            ...
            if (
                ...
                results.solver.termination_message = (
                    "Optimization terminated because the time expended "
    ...
    def process_soln_file(self, results):
        ...
        for suffix in self._suffixes:
            ...
            if not flag:
                raise RuntimeError(
                    "***CBC solver plugin cannot extract solution suffix=" + suffix
                )
        ...
        processing_constraints = (
            None  # None means header, True means constraints, False means variables.
        )
        ...
        for line in INPUT:
            ...
            if not header_processed:
                if tokens[0] == 'Optimal':
                    ...
                    or (
                        n_tokens > 2
                        and tokens[0] == 'Problem'
                        and tokens[2] == 'unbounded'
                    )
                    or (n_tokens > 1 and tokens[0:2] == ('Dual', 'infeasible'))
                ):
                    results.solver.termination_message = (
                    ...
                    if tokens[2] == 'time':
                        ...
                        if results.solver.termination_condition not in [
                            ...
                            results.solver.termination_message = (
                                "Optimization terminated because a limit was hit"
                            )
                            results.solver.termination_condition = (
                                TerminationCondition.maxIterations
                            )
            ...
            if (processing_constraints is True) and (extract_duals is True):
                ...
                if n_tokens == 4:
                    ...
                    raise RuntimeError(
                        "Unexpected line format encountered "
                ...
                raise RuntimeError(
                    "CBC plugin encountered unexpected "
                    "line=("
                    + line.strip()
                    + ") in solution file="
                    + self._soln_file
