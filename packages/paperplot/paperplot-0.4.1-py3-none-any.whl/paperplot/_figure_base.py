...
class FigureBase:
    def __init__(self, figure=None, figsize=(8, 6)):
        # initialization
        if figure:
    ...
    def set_xaxis(self, label=None, lim=None, ticks=None, labelpad=None):
    ...
    def set_yaxis(self, label=None, lim=None, ticks=None, labelpad=None):
        if label is not None:
            if not isinstance(label, str):
                raise TypeError("The label type is not 'str'.")
            else:
