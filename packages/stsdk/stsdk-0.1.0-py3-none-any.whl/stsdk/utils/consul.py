...
class ConsulClient:
    ...
    def register_service(self, service_name, service_address, service_port):
        ...
        if response.status_code == 200:
            print(f"Successfully registered {service_name} at {service_address}:{service_port}")
        else:
    ...
    def discover_service(self, service_name):
        ...
        if response.status_code == 200:
            ...
            if len(services) > 0:
                print(f"Discovered {service_name} at {services[0]['ServiceAddress']}:{services[0]['ServicePort']}")
            ...
            print(f"Failed to discover {service_name}: {response.text}")
