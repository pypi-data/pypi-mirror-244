...
def _sudo(ds: st.DataSpec) -> st.DataSpec:
    return transformed(
        external(
            id="std.SUDO", ds_args_pos=[0], ds_types={0: str(pd.DataFrame)}
        ),
        ds,
        dataspec_type=sp.type_name(ds.prototype()),
    )
...
def eval(
    ...
    if target_epsilon == 'unlimited':
        # whitelist x for demo ll
        from sarus.pandas.dataframe import DataFrame

        whitelisted = _sudo(x.dataspec())
        return DataFrame.from_dataspec(whitelisted).__sarus_eval__(
            None, verbose
        )
