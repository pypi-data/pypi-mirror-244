...
class FileServices:
    ...
    def download_file(src_path: str, dest_path: str, overwrite: bool = False):
        ...
        if not src_path:
            raise ValueError(f"src_path not specified")

        if not dest_path:
            raise ValueError(f"dest_path not specified")
        ...
        try:
            ...
            with open(temp_file_path, 'wb') as f:
                ...
                logger.debug(f"download file results retrieved")
            ...
            if file_content_hash == message_hash:
                logger.debug(f"download file hash check verified")
                ...
                raise Exception(f"Cannot verify the hash of the downloaded file. File download not completed.")
