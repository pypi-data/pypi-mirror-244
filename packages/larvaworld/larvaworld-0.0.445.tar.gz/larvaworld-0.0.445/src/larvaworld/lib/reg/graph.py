...
class GraphRegistry:
    ...
    def build_graphgroups(self):
        d = aux.AttrDict({
            ...
            'chemo': [
                # autotime(['sv', 'fov', 'b', 'a']),
                self.entry('timeplots', 'chemosensation', ks=['c_odor1', 'dc_odor1', 'A_olf', 'A_T', 'I_T'],
                           individuals=True),
            ...
            'thermo': [
                self.entry('trajectories'),
                self.entry('timeplots', 'thermosensation', ks=['temp_W', 'dtemp_W', 'temp_C', 'dtemp_C', 'A_therm'],
                           show_first=True)
            ],
            'puff': [
            ...
            'deb': [
                *[self.entry('deb', name=f'DEB.{m} (hours)', sim_only=False, mode=m, save_as=f"{m}_in_hours.pdf") for m
                  in ['energy', 'growth', 'full']],
                *[self.entry('deb', name=f'FEED.{m} (hours)', sim_only=True, mode=m, save_as=f"{m}_in_hours.pdf") for m
                  in
            ...
            'endpoint': [
                ...
                self.entry('endpoint box',
                           ks=['l', 'str_N', 'dsp_0_40_max', 'run_tr', 'fv', 'ffov', 'v_mu', 'sv_mu', 'tor5_mu',
                               'tor5_std',
                               'tor20_mu', 'tor20_std']),
