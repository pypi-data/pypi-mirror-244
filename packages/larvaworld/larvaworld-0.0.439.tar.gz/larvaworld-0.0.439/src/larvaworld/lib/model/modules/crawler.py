...
class StrideOscillator(Crawler, StepOscillator):
    ...
    stride_dst_mean = PositiveNumber(0.23, softmax=1.0, step=0.01, label='stride distance mean',
                                     doc='The mean displacement achieved in a single peristaltic stride as a fraction of the body length.')
    stride_dst_std = PositiveNumber(0.04, softmax=1.0, step=0.001, label='stride distance std',
...
class GaussOscillator(StrideOscillator):
    ...
    std = PositiveNumber(0.6, softmax=1.0, step=0.01, label='gaussian stride cycle std',
...
class PhaseOscillator(StrideOscillator):
    ...
    max_vel_phase = Phase(3.49, label='max velocity phase',
                          doc='The phase of the crawling oscillation cycle where forward velocity is maximum.')
    max_scaled_vel = PositiveNumber(0.51, softmax=1.5, step=0.01, label='maximum scaled velocity',
