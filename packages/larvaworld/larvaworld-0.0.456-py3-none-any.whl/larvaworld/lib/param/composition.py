...
class Substrate(NestedConf):
    ...
    def get_w_X(self, compounds=None):
        ...
        if d_X > 0:
            return sum([self.composition[c] * compound_dict[c].ww for c in compounds]) / d_X
        else:
            return 0.0
...
substrate_dict = aux.AttrDict(
    {
        ...
        'cornmeal2': Substrate(dextrose=450 / 6400, yeast=90 / 6400, cornmeal=420 / 6400, agar=42 / 6400),
...
class Life(NestedConf):
    ...
    @classmethod
    def prestarved(cls, age=0.0, h_starved=0.0, rearing_quality=1.0, starvation_quality=0.0,final_quality=None,
                   substrate_type='standard', reach_pupation=False):
