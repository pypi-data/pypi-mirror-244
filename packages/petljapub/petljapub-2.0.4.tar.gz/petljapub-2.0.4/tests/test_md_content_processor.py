import unittest
from src.petljapub.md_content_processor import OutputOrganizerSingleDir, OutputOrganizerHierarchy, LinkProcessorRaw, LinkProcessorPublication, LinkProcessorNoLinks, LinkProcessorTex, LinkProcessorHTML, ImageProcessor, MDContentProcessor
...
class TestLinkFormater(unittest.TestCase):
    ...
    def test_no_links(self):
        md_original = "[](trening)"
        formater = LinkProcessorNoLinks(self.yaml_specification, self.task_repo)
        self.assertEqual(formater.process("", "ponoc-st.md", md_original),
                         "*Тренинг*")
    ...
    def test_tex(self):
        md_original = "[](trening)"
        formater = LinkProcessorTex(self.yaml_specification, self.task_repo)
        self.assertEqual(formater.process("", "ponoc-st.md", md_original),
                         "[Тренинг](#trening)")
        self.assertEqual(formater.process("01 Aritmetika/01 Formule/02 celobrojno_deljenje", "ponoc-st.md", md_original),
                         "[Тренинг](#trening)")
    ...
    def test_html(self):
        ...
        formater = LinkProcessorHTML(self.yaml_specification, self.task_repo, organizer)
        self.assertEqual(formater.process("", "ponoc-st.md", md_original),
...
class TestLinkProcessor(unittest.TestCase):
    def test_exclude_not_in_publication(self):
        ...
        formater = LinkProcessorPublication(yaml_specification, task_repo)
        md = """
<!--- span:link --->
...
"""
        ...
        processor = MDContentProcessor(None, image_processor)
