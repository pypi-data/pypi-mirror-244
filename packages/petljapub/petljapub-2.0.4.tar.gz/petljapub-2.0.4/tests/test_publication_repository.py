...
class TestPublicationRepository(unittest.TestCase):
    ...
    def test_index_md(self):
        ...
        title, _ = self.repo.read_index("geometrija")
        self.assertEqual(title, "Геометријске формуле")

    def test_read_md_file(self):
        self.assertEqual(self.repo.title("01 Formule/formule.md"), "Списак формула")
        title, content = self.repo.read_md_file("01 Formule/formule.md")
        self.assertEqual(title, "Списак формула")
