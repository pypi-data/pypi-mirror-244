    ...
    def transpile(
        ...
        if type(circuits) is QuantumCircuit:
            try:
                ...
                qasm = qasm3.dumps(circuits).replace("\n", " ")
        elif type(circuits) is List[QuantumCircuit]:
            qasm = []
            for circuit in circuits:
                try:
                    qasm_temp = circuits.qasm().replace("\n", " ")
                except QASM2ExportError:
                    qasm_temp = qasm3.dumps(circuits).replace("\n", " ")
                qasm.append(qasm_temp)
