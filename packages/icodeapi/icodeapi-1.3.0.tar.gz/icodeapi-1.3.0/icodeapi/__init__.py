...
Documents: [https://xbzstudio.github.io/_book](https://xbzstudio.github.io/_book).
...
import urllib3, urllib.parse, httpx, warnings, asyncio
...
class IcodeAPI():
    ...
    def submitWork(self, workCode : str = '',
                   workType : str | ScratchWork | PythonWork = 'Scratch', 
                   publish : int = 1,
                   save : int = False,
        ...
        if workDetail:
            ...
            return result
        if not workId:
            if workType in ['Scratch', 'scratch', ScratchWork]:
                ...
                if workType in ['Python', 'python', PythonWork]:
                    if save:
                        body = {
                            'code' : workCode,
            ...
            if workType in ['Scratch', 'scratch', ScratchWork]:
                ...
                headers['Content-Type'] = fields[1]
                response = self.client.post('https://icode.youdao.com/api/work/submit', data = body, headers = headers)
            elif workType in ['Python', 'python', PythonWork]:
                if save:
                    body = {
                        'code' : workCode,
                        'description' : description,
    ...
    def reply(self, content : str, replyType : Comment | Reply, Id : int) -> dict:
        '''
        Reply a comment.
        ...
        if replyType == Comment: 
            body = {'commentId': Id,
                    'content': content}
        elif replyType == Reply:
            body = {'replyId': Id,
                    'content': content}
        else:
            raise ValueError('replyType must be a Comment or Reply class')
        headers = headers
        headers['Content-Type'] = 'application/json'
    ...
    def deleteComment(self, deleteType : Comment | Reply , Id : int) -> dict:
        '''
        Delete a comment.
        '''
        if not self.__loginStatus:
        ...
        match deleteType:
            case temp(Comment):
                response = self.client.post(f'https://icodeshequ.youdao.com/api/works/comment/delete?commentId={Id}', data = 'IcodeAPI delete comment'.encode('utf-8'), headers = headers)
            case temp(Reply):
                response = self.client.post(f'https://icodeshequ.youdao.com/api/works/reply/delete?replyId={Id}', data = 'IcodeAPI delete reply'.encode('utf-8'), headers = headers)
            case _:
                raise ValueError('deleteType must be a Comment or Reply class')
        result = response.json()
        return result
    ...
    def praiseComment(self, praiseType : Comment | Reply, Id : int, mode : int = 1) -> dict:
        '''
        ...
        if praiseType == Comment:
            url += f'commentId={Id}'
        elif praiseType == Reply:
            url = url.replace('comment', 'reply')
            url += f'replyId={Id}'
        else:
            raise ValueError('praiseType must be Comment or Reply class')
...
class AsyncIcodeAPI(IcodeAPI):
    ...
    async def reply(self, content : str, replyType : Comment | Reply, Id : int) -> dict:
        '''
        Reply a comment.
    ...
    async def deleteComment(self, deleteType : Comment | Reply , Id : int) -> dict:
        '''
        Delete a comment.
        '''
        if not self.__loginStatus:
        ...
        match deleteType:
            case temp(Comment):
                response = await self.client.post(f'https://icodeshequ.youdao.com/api/works/comment/delete?commentId={Id}', data = 'IcodeAPI delete comment'.encode('utf-8'), headers = headers)
            case temp(Reply):
                response = await self.client.post(f'https://icodeshequ.youdao.com/api/works/reply/delete?replyId={Id}', data = 'IcodeAPI delete reply'.encode('utf-8'), headers = headers)
            case _:
                raise ValueError('deleteType must be a Comment or Reply class')
        result = response.json()
        return result
    ...
    async def praiseComment(self, praiseType : Comment | Reply, Id : int, mode : int = 1) -> dict:
        '''
        Praise a comment or reply.
        '''
        if not self.__loginStatus:
...
def WorkInfoTupleGenerator(api : IcodeAPI, workId : str) -> tuple:
    '''
    Generate work info tuple.
    '''
    detail = api.getWorkDetail(workId)
    submitInfo = api.getWorkSubmitInfo(workId)
    tup = (detail, submitInfo)
    return tup

async def AsyncWorkInfoTupleGenerator(api : AsyncIcodeAPI, workId : str) -> tuple:
    '''
    Async Generate work info tuple.
    '''
    task1, task2 = asyncio.create_task(api.getWorkDetail(workId)), asyncio.create_task(api.getWorkSubmitInfo(workId))
    detail, submitInfo = await task1, await task2
    tup = (detail, submitInfo)
    return tup
