...
async def ResubmitAllWorks(api : AsyncIcodeAPI) -> list:
    works = await api.getMyWorks(1, INFINITY)
    if not works:
        return
    coros1 = [api.getWorkDetail(i.get('id')) for i in works]
    coros2 = [api.getWorkSubmitInfo(i.get('id')) for i in works]
    details = await asyncio.gather(*coros1)
    submitInfos = await asyncio.gather(*coros2)
    workDetails = [(i, j) for i, j in zip(details, submitInfos)]
    while True:
        coros = [api.submitWork(workDetail = i, workId = j.get('id')) for i, j in zip(workDetails, works)]
