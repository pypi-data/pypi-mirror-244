...
from .decorators.concrete_node_decorators import GenericDecorator, Stmt
...
class CFGBlock:
        ...
        for e in self._preds.begin():
            if e.getReachableBlock() is not None:
                output += colored(
                    "Preds "
                    + "("
                    + str(self._preds.size())
                    + ")"
                    + ": "
                    + "B"
                    + str(e.getReachableBlock().getBlockID())
                    + "\n",
                    "cyan",
                )
    ...
    def addSuccessor(self, succ):
        ...
        if B:
            B._preds.push_back(self.AdjacentBlock(self, isReachable=succ.isReachable()))
