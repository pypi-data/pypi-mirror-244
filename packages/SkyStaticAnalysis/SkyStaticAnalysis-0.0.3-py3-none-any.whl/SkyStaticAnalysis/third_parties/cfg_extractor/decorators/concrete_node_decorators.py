...
from .ast_node_decorator import AstNodeDecorator
...
class GenericDecorator(AstNodeDecorator):
    ...
    def determineExpr(self, c, lista, elementIndex):
        ...
        elif len(lista) > 0 and (
            ((len(lista) - elementIndex) == 1) or (len(lista) - elementIndex) == 2
        ):
            o = OpaqueValueExpr(c)
            return o
    ...
    def obtainStructure(self):
        ...
        for i in tokens:
            ...
            if index == 1 or index == 2:
                if str(i.spelling) == ";":
                    if (
                        str(nextToken.spelling) != ";"
                        and str(nextToken.spelling) != ")"
                    ):
                        l[index] = True
                        index += 1
                    else:
...
class SwitchStmt(Stmt):
    ...
    def isAllEnumCasesCovered(self):
        ...
        if (
            cond.kind() == type_stmt.IMPL_CAST_EXPR
            or cond.kind() == type_stmt.DECL_REF_EXPR
        ):
                        ...
                        if i.referenced.kind == CursorKind._kinds[7]:
                            ...
                            for i in it:
                                ...
                                if i.kind == CursorKind._kinds[101]:
                                    if i.referenced.kind == CursorKind._kinds[7]:
                                        enum = (
                                            i.referenced.semantic_parent.get_children()
                                        )
                                        for e in enum:
                                            l.append(str(e.spelling))
...
class IndirectGoToStmt(Stmt):
    ...
    def __init__(self, cursor):
        super(IndirectGoToStmt, self).__init__(cursor, type_stmt.INDIRECT_GOTO_STMT)
...
class VarDecl(Decl):
    ...
    def getSpecificKind(self):
        ...
        if k == TypeKind._kinds[112] or k == TypeKind._kinds[114]:  # Constant Array
            ...
            k == TypeKind._kinds[16]
            or k == TypeKind._kinds[17]
            or k == TypeKind._kinds[18]
            or k == TypeKind._kinds[21]
            or k == TypeKind._kinds[13]
            or k == TypeKind._kinds[22]
            or k == TypeKind._kinds[23]
            or k == TypeKind._kinds[19]
            or k == TypeKind._kinds[9]
            or k == TypeKind._kinds[10]
            or k == TypeKind._kinds[11]
        ):
...
class EnumConstantDecl(Decl):
    ...
    def __init__(self, cursor):
        super(EnumConstantDecl, self).__init__(cursor, type_stmt.ENUM_CONSTANT_DECL)
