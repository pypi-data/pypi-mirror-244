...
from ..utils import beautified_print_ast
...
class CFGBuilder:
        ...
        while True:
            if rhs.kind() is type_stmt.BINARY_OPERATOR:
                ...
                if b_rhs.isLogicalOp():
                    pair = self.visitLogicalOperator(b_rhs, stmt, trueBlock, falseBlock)
                    break
    ...
    def visitForStmt(self, F):
        ...
        while True:
            ...
            if cond.kind() is type_stmt.BINARY_OPERATOR:
                if cond.isLogicalOp():
                    tie = self.visitLogicalOperator(cond, F, bodyBlock, loopSuccessor)
    ...
    def visitWhileStmt(self, W):
        ...
        while True:
            ...
            if cond.kind() is type_stmt.BINARY_OPERATOR:
                if cond.isLogicalOp():
                    tie = self.visitLogicalOperator(cond, W, bodyBlock, loopSuccessor)
                    entryConditionBlock = tie[0]
                    exitCOnditionBlock = tie[1]
                    break
    ...
    def visitSwitchStmt(self, terminator):
        ...
        self.addSucessor(
            self._switchTerminatedBlock,
            self._defaultCaseBlock,
            not (switchAlwaysHasSuccessor),
        )
    ...
    def visitCaseStmt(self, CS):
        ...
        if sub:
            ...
            while sub.kind() is type_stmt.CASE_STMT:
                ...
                if self.shouldAddCase(
                    self._switchExclusivelyCovered, self._switchCond, CS
                ):
