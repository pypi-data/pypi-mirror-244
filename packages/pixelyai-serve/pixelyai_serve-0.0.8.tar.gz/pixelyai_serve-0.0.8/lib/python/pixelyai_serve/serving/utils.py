...
class BaseClassAgent:
    ...
    def run(self, url="https://api.pixelyai.com/api/gradio/"):
    ...
    def __call__(
        ...
        if contexts is not None:
            for context in contexts:
                ...
                gprs_func = functools.partial(
                    self.client.predict,
                    self.instruct_check.format(question=prompt, context=context),
                ...
                if is_found:
                    grf_func = functools.partial(
                        ...
                        "",
                        (
                            f"Here's Extra information:\n{context}{END_OF_MESSAGE_TURN_HUMAN}Great, "
                            f"thank you for providing me with this "
            ...
            if not is_found:
                ...
                generated_response = gr_response(fn_index=0)[-1]
