...
from deltastore.protocols import (
    AddFile,
    CdfOptions,
    FileAction,
    Metadata,
...
class DeltaSharingRestClient:
        ...
        self._session.headers.update(
            {
                ...
                "User-Agent": DeltaSharingRestClient.USER_AGENT,
            }
        )
    ...
    def list_shares(
        ...
        with self._get_internal("/shares", data) as lines:
            ...
            return ListSharesResponse(
                shares=[
                    Share.from_json(share_json)
                    for share_json in shares_json.get("items", [])
                ],
        ...
        with self._get_internal(f"/shares/{share.name}/schemas", data) as lines:
            ...
            return ListSchemasResponse(
                schemas=[
                    ...
                    for schema_json in schemas_json.get("items", [])
                ],
    ...
    def query_table_version(
        ...
        query_str = (
            f"/shares/{table.share}/schemas/{table.schema}/tables/{table.name}/version"
        )
        ...
        *,
        ...
        with self._post_internal(
            ...
            return ListFilesInTableResponse(
                ...
                metadata=Metadata.from_json(metadata_json["metaData"]),
                add_files=[
                    AddFile.from_json(json.loads(file)["file"]) for file in lines
                ],
            )
    ...
    def list_table_changes(
        ...
        query_str = (
            f"/shares/{table.share}/schemas/{table.schema}/tables/{table.name}/changes?"
        )
