...
import loligram
from loligram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~loligram.Client.start`, :meth:`~loligram.idle` and :meth:`~loligram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that Loligram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`loligram.compose`.
