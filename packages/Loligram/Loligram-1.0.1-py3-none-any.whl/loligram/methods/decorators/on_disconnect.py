...
class OnDisconnect:
    def on_disconnect(self=None) -> Callable:
        ...
        :obj:`~loligram.handlers.DisconnectHandler`.
        """

        def decorator(func: Callable) -> Callable:
            if isinstance(self, loligram.Client):
                self.add_handler(loligram.handlers.DisconnectHandler(func))
            else:
                if not hasattr(func, "handlers"):
                    func.handlers = []

                func.handlers.append((loligram.handlers.DisconnectHandler(func), 0))
