...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~loligram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~loligram.Client.sign_in` or :meth:`~loligram.Client.sign_up`).

        next_type (:obj:`~loligram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~loligram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~loligram.Client.resend_code`.
    """
