...
async def compose(
    clients: List["loligram.Client"],
    sequential: bool = False
):
    """Run multiple clients at once.

    This method can be used to run multiple clients at once and can be found directly in the ``loligram`` package.

    If you want to run a single client, you can use Client's bound method :meth:`~loligram.Client.run`.

    Parameters:
        clients (List of :obj:`~loligram.Client`):
    ...
    Example:
        .. code-block:: python
            ...
            import asyncio
            from loligram import Client, compose
