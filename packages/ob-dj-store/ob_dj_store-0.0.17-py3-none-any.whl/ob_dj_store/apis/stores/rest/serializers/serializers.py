...
class ArabicFieldsMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.arabic_field_suffixes = ["arabic", "ar"]  # Add other suffixes as needed

    def should_include_arabic_fields(self, request):
        """
        Check if the request header includes the Arabic language.
        """
        language_header = request.headers.get("language", "").upper()
    ...
    def get_fields(self):
        ...
        if request and self.should_include_arabic_fields(request):
            for field_name, field in list(fields.items()):
                # Check if the field has an Arabic version based on suffixes
                for suffix in self.arabic_field_suffixes:
                    if field_name.endswith(suffix) or field_name.startswith(suffix):
                        # Include Arabic field and exclude the original field
                        arabic_field_name = field_name
                        original_field_name = field_name.rsplit(suffix, 1)[0]
                        fields[arabic_field_name] = field
                        fields.pop(original_field_name, None)
