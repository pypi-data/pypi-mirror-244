...
class ProjectQuota:
    ...
    def resource_quotas(self) -> list[ResourceQuota]:
        ...
        return [
            ResourceQuota(data)
            for data in self._data["resourceQuotas"]
        ]
...
class Project:
    ...
    def match(self, attribute: property, key: str) -> bool:
        ...
        * Project.url
        ...
        """
        if (
            (attribute is Project.id and self.id == key) or
            (attribute is Project.pid and self.pid == key) or
            (attribute is Project.name and self.name == key) or
            (attribute is Project.url and self.url == key) or (
                (attribute is Project.display_name) and
    ...
    def resource(
        self,
        key: str,
        attribute: property = Resource.display_name
    ) -> Resource:
        """
        ...
        results = list(filter(
            lambda resource: resource.match(attribute, key),
            resources
        ))
        if len(results) > 1:
