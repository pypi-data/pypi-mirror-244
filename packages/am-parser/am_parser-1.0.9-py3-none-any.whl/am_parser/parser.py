...
class Node:
	...
	@property
	def length(self):return len(self.child_nodes)
	def add(B,node):
		A=node
		if A.parent:A.parent.remove(A)
		A.parent=B;B.child_nodes.append(A)
	def remove(A,node):
		B=A.child_nodes.index(node)
		if B!=-1:del A.child_nodes[B]
		node.parent=None
	def __iter__(A):return A
	def __next__(A):
		A._index+=1
		if A._index<len(A.child_nodes):return A.child_nodes[A._index]
		A._index=-1;raise StopIteration
class Sequence(Node):
	def __init__(A,nodes):
		super().__init__()
		for B in nodes:A.add(B)
	def to_string(A):return' '.join([str(A)for A in A.child_nodes])
	def __str__(A):return A.to_string()
	def __eq__(A,other):B=other;return isinstance(B,type(A))and B.length==A.length and all(C==B.child_nodes[A]for(A,C)in enumerate(A.child_nodes))
class Paren(Node):
	...
	@property
	def lparen(self):return self.child_nodes[0]
	@property
	def expression(self):return self.child_nodes[1]
	@property
	def rparen(self):return self.child_nodes[2]
...
class SubSup(Node):
	...
	@property
	def base(self):return self.child_nodes[0]
	@property
	def sub(self):
		A=self.child_nodes[1]
		if isinstance(A,Empty):return
		return A
	@property
	def sup(self):
		A=self.child_nodes[2]
		if isinstance(A,Empty):return
		return A
	def to_string(B):
		A=B.base;C=B.sub
...
class Unary(Node):
	...
	@property
	def operator(self):return self.child_nodes[0]
	@property
	def operand(self):return self.child_nodes[1]
...
class Binary(Node):
	...
	def operator(self):return self.child_nodes[0]
	@property
	def operand1(self):return self.child_nodes[1]
	@property
	def operand2(self):return self.child_nodes[2]
...
class Asciimath:
	def __init__(A):A._ds={OO00OOOOOO0O000OO.__name__:OO00OOOOOO0O000OO for OO00OOOOOO0O000OO in[Sequence,Paren,Group,SubSup,Sub,Sup,Unary,UnaryFunc,Binary,BinaryInfix,Literal,TextValue,NumberValue,Symbol,Identifier,Empty]}
	def parse(A,input):return A._p_e(Tzr(input))
	def set(A,exd):
		for O000OOOOO0O0O000O in exd:
			for O0OO00OO00OOOOOO0 in A._ds:
				if A._i_d_s(O000OOOOO0O0O000O,A._ds[O0OO00OO00OOOOOO0]):A._ds.update({O0OO00OO00OOOOOO0:O000OOOOO0O0O000O});break
