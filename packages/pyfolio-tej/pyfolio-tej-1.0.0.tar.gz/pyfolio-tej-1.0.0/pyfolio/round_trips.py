...
def extract_round_trips(transactions, portfolio_value:None, show_progress=False):
        ...
        for dt, t in trans_sym.iterrows():
            ...
            if ((len(price_stack) == 0) or (copysign(1, price_stack[-1]) == copysign(1, t.amount))) and len(split_info) != 0:
                ...
                if split_ratio > 1: #20231030股票股利、股票分割 Terry
                    ...
                    capital_reducion = pd.Series(price_stack).value_counts().apply(lambda x: int(x * split_ratio))

                    capital_dt = pd.Series(dt_stack).value_counts().apply(lambda x: int(x * split_ratio))

                    if int(len(price_stack)*split_ratio) != capital_reducion.sum():

                        adjust = capital_reducion.sum() - int(len(price_stack)*split_ratio)

                        capital_reducion[capital_reducion.idxmax()] -= adjust #減去經調整後買入成本數量最多的股票，對減資後影響更為平均

                        capital_dt[capital_dt.idxmax()] -= adjust

                    capital_reducion = capital_reducion.reindex(pd.Series(price_stack).unique()) #由於採用先進先出，因此將順序調成price_stack內的順序

                    capital_dt = capital_dt.reindex(pd.Series(dt_stack).unique())

                    price_stack = deque(float(key) for key, value in capital_reducion.items() for _ in range(value)) #展開成price_stack的形式

                    dt_stack = deque(key for key, value in capital_dt.items() for _ in range(value))
                ...
                if (copysign(1, price_stack[-1]) != copysign(1, t.amount)) and len(split_info) != 0:
                    ...
                    if split_ratio > 1: #20231030股票股利、股票分割 Terry
                        ...
                        if int(len(price_stack)*split_ratio) != capital_reducion.sum():
                            ...
                            dt_adjust = capital_dt.sum() - int(len(dt_stack)*split_ratio)

                            capital_reducion[capital_reducion.idxmax()] -= adjust #減去經調整後買入成本數量最多的股票，對減資後影響更為平均

                            capital_dt[capital_dt.idxmax()] -= dt_adjust
