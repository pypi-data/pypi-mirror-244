...
class RoundTripTestCase(TestCase):
    ...
    dates_intraday = date_range(start="2015-01-01", freq="2BH", periods=8)
    ...
    def test_groupby_consecutive(self, transactions, expected):
        ...
        assert_frame_equal(
            grouped_txn.sort_index(axis="columns"),
    ...
    @parameterized.expand(
        [
            ...
            (
                DataFrame(
                    data=[[2, 10.0, "A"], [2, 15.0, "A"], [-9, 10.0, "A"]],
                    columns=["amount", "price", "symbol"],
                    index=dates[:3],
                ),
                DataFrame(
            ...
            (
                DataFrame(
                    data=[[2, 10.0, "A"], [-4, 15.0, "A"], [3, 20.0, "A"]],
                    columns=["amount", "price", "symbol"],
                    index=dates[:3],
                ),
                DataFrame(
            ...
            (
                DataFrame(
                    data=[[4, 10.0, "A"], [-2, 15.0, "A"], [2, 20.0, "A"]],
                    columns=["amount", "price", "symbol"],
                    index=dates[:3],
                ),
                DataFrame(
            ...
            (
                ...
                Series([100.0, 100.0, 100.0], index=dates[:3]),
            ),
        ]
    )
    ...
    def test_txn_pnl_matches_round_trip_pnl(self):
        ...
        transactions_closed["txn_dollars"] = (
            transactions_closed.amount * -1.0 * transactions_closed.price
        )
        round_trips = extract_round_trips(transactions_closed)

        self.assertAlmostEqual(
            round_trips.pnl.sum(), transactions_closed.txn_dollars.sum()
        )
