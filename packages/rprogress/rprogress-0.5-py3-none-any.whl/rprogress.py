...
if __name__ == "__main__":
    ...
    prog = Progress([])
    with prog.progress:
        prog.tasks.append(prog.progress.add_task("taski", total=10))
        for i in range(10):
            task = prog.progress.add_task("taskj", total=10)
            for j in range(10):
                process(f"{i}_{j}")
                prog.progress.update(task, advance=1)
            prog.progress.update(prog.tasks[0], advance=1)
