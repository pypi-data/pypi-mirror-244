...
class Listing:
    def __init__(
        ...
        if data_storage.partial_id is None:
            raise ValueError("data_storage.partial_id cannot be None in Listing")
    ...
    @staticmethod
    async def _insert_dir(data_storage, parent_id, name, time, parent):
    ...
    async def insert_file(self, parent_id, name, time, parent):
        node = Node(
            0,
            "",
            DirType.FILE,
