...
from .model import ScheduleMode, EquationProduct, DeviceMode, DevicePreset
...
class EquationDevice:
    ...
    def update_data(
        ...
        self.preset = self.get_current_schedule_preset() if self.mode == DeviceMode.AUTO else data["status"]
    ...
    def get_current_schedule_preset(self) -> ScheduleMode:
        ...
        if curr_mode == ScheduleMode.COMFORT:
            return DevicePreset.COMFORT
        elif curr_mode == ScheduleMode.ECO:
            return DevicePreset.ECO
        
        return DevicePreset.OFF
