...
def send_invoice(invoice_payload, token):
    ...
    response = requests.post(url, json=json.loads(invoice_payload), headers=headers)
    return response


def save_invoice_log(inv_obj, ser_invoice, response):
    log_entry = InvoiceLog.objects.create(
        invoice=inv_obj,
        request_body=ser_invoice.model_dump_json(exclude_none=True),
        uuid=response.json()["uuid"] if response.ok else None,
