...
from copernicus_marine_client.core_functions.models import SubsetMethod
...
from copernicus_marine_client.download_functions.utils import FileFormat
...
def subset_function(
    ...
    if all(
        ...
        logger.info(
            ...
            f"copernicus-marine get --dataset-id {dataset_id}"
        )
        sys.exit(1)
    ...
    dataset_service, dataset_id, _ = get_dataset_service_and_suffix_path(
        catalogue,
    ...
    if dataset_service.service_type != CopernicusMarineDatasetServiceType.MOTU:
        check_dataset_subset_bounds(
            username=username,
            password=password,
            dataset_url=subset_request.dataset_url,
            service_type=dataset_service.service_type,
