...
from say import ASSISTANT_PATH
...
class TTS:
    ...
    def tts(self, text: str, language, speaker_idx, style_wav):
        ...
        try:
            wavs = self.synthesizer.tts(text, language_name=language, speaker_idx=speaker_idx, speaker_wav=style_wav)#, speaker_name="edwin", voice_dir=f"{ASSISTANT_PATH}/data/TTS/voices/") #, use_cuda=True)
            out = io.BytesIO()
