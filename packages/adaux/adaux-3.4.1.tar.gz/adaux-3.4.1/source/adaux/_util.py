...
__all__ = ["LazyVersionStr", "EnvironmentVariables", "canonical_arch"]
...
def canonical_arch() -> str:
    arch = platform.machine()
    remap = {
        "aarch64": "arm64",
        "x86_64": "amd64",
    }
    res = remap.get(arch, arch)
    valid_archs = ["arm64", "amd64"]
    if res not in valid_archs:
        raise RuntimeError(f"{res} is not in valid {valid_archs}")
    return res
