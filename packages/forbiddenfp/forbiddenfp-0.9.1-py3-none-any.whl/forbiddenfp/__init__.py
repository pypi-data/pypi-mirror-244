...
@chain_and_make_func
def filter_unpack(self: Iterable[Iterable[_P]],
...
@chain_and_make_func
def each_also_unpack(self: Sequence[Sequence[_P]], func: Callable[[_P.args], _R]) -> Sequence[Sequence[_P]]:
    for x in self:
        func(*x)
    return self
...
@chain_and_make_func
def each_unpack(self: Iterable[Iterable[_P]], func: Callable[[_P.args], _R]) -> None:
    for x in self:
        func(*x)
...
def take_unpack(
    ...
    take_unpack(filter)(lambda x, y: x > y, [(1, 2), (3, 2)])
    """
...
chain_as("take_unpack")(lambda self, func: lambda f: take_unpack(func)(f, self))
