...
scraper = Nitter(log_level=1, skip_instance_check=False, skip_initial_check=False)
```
...
The `skip_initial_check` parameter is used to skip the health check of the Nitter instances at the initialization of the library. If you use your own instance or trust the instance you are relying on, then you can skip set it to 'True' to speed up the initialization, otherwise it's better to leave it to false.

The `skip_instance_check` parameter is used to skip the check of the Nitter instances altogether during the execution of the script. Like before, if you use your own instance or trust the instance you are relying on, then you can skip set it to 'True', otherwise it's better to leave it to false.
