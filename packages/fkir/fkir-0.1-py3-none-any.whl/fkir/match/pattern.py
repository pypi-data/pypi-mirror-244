...
class FkAnyPatternNode(FkNode):
  def __init__(self, attr_eq = dict(), attr_has = tuple(), attr_nhas = tuple()) -> None:
    super().__init__()
...
class FkPattern:
  ...
  def extract(self, graph: FkGraph):
    ...
    for subgraph in GM.subgraph_isomorphisms_iter():
      subgraphs.append(FkPatternRet({g2.nodes[v]['obj']: g1.nodes[k]['obj'] for k, v in subgraph.items()}, graph))
    return subgraphs
