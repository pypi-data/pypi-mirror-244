...
class ReactorAbstract:
    ...
    def set_solver(self, t0, y0):
        self.solver = integrate.LSODA(fun=self.derivatives, t0=t0, y0=y0, t_bound=self._ubound,
...
class Inlet:
    ...
    def soot(self):
        if self._soot_inlet_type == SOOT_VALUE_TYPE.NO_SOOT:
            ...
            raise NotImplementedError("Custom inlet soot is not implemented")
...
class PerfectlyStirredReactor(ReactorAbstract, CPSRSoot):
    ...
    def set_mdot(self):
        self.mdot_in = self.soot_gas.rho * self.reactor_volume / self.phy_restime;
    ...
    def soot_carbon_flux(self) -> float:
        return (
            self.soot_wrapper.particle_dynamics_model.C_tot() 
            * MW_carbon 
            * self.mdot_out
        );
    ...
    def total_hydrogen_flux(self) -> float:
        hydrogen_mass = self.soot_gas.elemental_mass_fraction('H') + self.soot_wrapper.particle_dynamics_model.hydrogen_mass();
        return hydrogen_mass * self.mdot_out;
    
    @property
    def gas_hydrogen_flux(self) -> float:
        return self.soot_gas.elemental_mass_fraction('H') * self.mdot_out;
