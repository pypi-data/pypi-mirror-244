...
class Hamiltonian:
    ...
    def __init__(self, 
        ...
        model, str: the name of the model to encode.
        statistics, str: specified whether the model is for bosons, fermions, anyons, or spins.
        sparse, bool: flag that encodes whether the Hamiltonian is saved as a sparse array or not.
        ...
        self.sites = sites
        self.PBC = PBC
        if stats=="bosons" or stats=="spinless-fermions" or stats=="anyons" or stats=="spin-0.5":
            self.Hilbert_dim = 2**self.sites
        elif stats=="spinful-fermions":
            self.Hilbert_dim = 4**self.sites
        else:
            spin = int(stats[5:])
            self.Hilbert_dim = inp.spin_dim(spin, sites)
        self.sparse = sparse
        self.get_parameters()
        ...
        if self.sparse==True:
            self.rows, self.cols, self.mat_els = self.load_Hamiltonian_sparse()
        else:
    ...
    def input_checks(self):
        ...
        if inp.n_lowest_eigenvalues > self.Hilbert_dim:
            raise ArithmeticError(f"The number of lowest eigenvalues {inp.n_lowest_eigenvalues} should be less (or equal) than the Hilbert space dimension {self.Hilbert_dim}!")
        
        if self.model=="staggered-Heisenberg" and self.pars["sz"] is not None and inp.plot_evecs == True:
