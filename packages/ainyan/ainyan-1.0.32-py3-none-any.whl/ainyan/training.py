...
def main():
    ...
    if args.phase == "train" or args.phase == "":
        ...
        upload(config, newmodel_finalname, training)
...
def train(dataset_name, model, model_name, new_model_name, tokenizer, training):
    ...
    if training["empty_cache"]:
        torch.cuda.empty_cache()

    post_query = training["post_query"]
    if post_query:
        prompt = pipeline(task="text-generation", model=model, tokenizer=tokenizer, max_length=150)
        print("Post Query:"+post_query+"\n")
        print(prompt(post_query)[0]['generated_text'])

def upload(config, newmodel_finalname, training):
    s3_config = config["s3"]
