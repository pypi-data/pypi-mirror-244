...
def split_string(input_string):
    ...
    has_japanese = bool(re.search(r'[\u3040-\u30FF\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF]', input_string))

    # Check if the string contains English characters
    has_english = bool(re.search(r'[a-zA-Z]', input_string))

    if has_japanese and has_english:
        # If both English and Japanese characters are present, split the string
        # at the boundary between them
        split_index = re.search(r'(?<=[a-zA-Z])(?=[\u3040-\u30FF\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF])',
                                input_string)

        if split_index:
            index = split_index.start()
            part1 = input_string[:index]
            part2 = input_string[index:]
            return [part1, part2]

    # If no split is needed or the characters are in a different arrangement, return the original string
    return [input_string]
        ...
        for shape in slide.shapes:
            if hasattr(shape, "text"):
                ...
                for text_ in texts:
                    ...
                    if len(text) >= 40:
                        print("PPT:"+text_)
                        formatted_list.append({"label": pptx_file, "index": idx, "text": text_})
    ...
    if not os.path.isdir(directory_path):
        ...
        for root, dirs, files in os.walk(directory_path):
            for file in files:
                ...
                should_include = (not include and not exclude) or (ext in include) or (ext not in exclude)
                if should_include:
