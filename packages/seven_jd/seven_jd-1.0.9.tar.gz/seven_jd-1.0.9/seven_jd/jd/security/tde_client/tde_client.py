# -*- coding:utf-8 -*-
from seven_jd.jd.security.tde_client.http_report_client import HttpReportClient
from threading import RLock
from seven_jd.jd.api.base import RestApi
from seven_jd.jd import appinfo
from seven_jd.jd.security.tde import constants
from seven_jd.jd.security.tde.util import tde_status
from seven_jd.jd.security.tde.util import base64_util
from seven_jd.jd.security.tde.util import key_encryption
import rsa
import json
import base64
import time
...
class TdeClient(object):
    ...
    def init_client(self):
        try:
            ...
            self.hrlc.insert_err_report(tde_status.SDK_THROW_JDK_EXCEPTION.code, ie.args[0], traceback.format_exc(), self.hrlc.level['error'])
            raise ie
        except InvalidTokenException as ite:
            self.hrlc.insert_err_report(tde_status.SDK_USE_INVALID_TOKEN.code, ite.args[0], traceback.format_exc(), self.hrlc.level['severe'])
            raise ite
        except MalformedException as me:
            self.hrlc.insert_err_report(tde_status.SDK_THROW_JDK_EXCEPTION.code, me.args[0], traceback.format_exc(), self.hrlc.level['error'])
            raise me
            ...
            self.hrlc.insert_err_report(tde_status.SDK_THROW_JDK_EXCEPTION.code, re.args[0], traceback.format_exc(), self.hrlc.level['error'])
            raise RuntimeError(re.args[0])
        except Exception as e:
            self.hrlc.insert_err_report(tde_status.SDK_INTERNAL_ERROR.code, e.args[0], traceback.format_exc(), self.hrlc.level['error'])
    ...
    def decrypt_string(self, base64str):
        if TdeClient.is_encrypt_data(base64str):
            ...
            if st['status'] == 'UnDecryptable':
                self.hrlc.insert_err_report(tde_status.SDK_HAS_NO_CORRESPONDING_DEC_KEYS.code, tde_status.SDK_HAS_NO_CORRESPONDING_DEC_KEYS.message, '', self.hrlc.level['severe'])
                ...
                self.hrlc.insert_event_report(tde_status.SDK_TRIGGER_ROTATED_KEY_FETCH.code, tde_status.SDK_TRIGGER_ROTATED_KEY_FETCH.message)
                self.fetch_master_keys()
                if self.cache_ks.has_future_key_id(st['keyid']):
                    self.hrlc.insert_err_report(tde_status.SDK_FAILS_TO_FETCH_UPDATED_KEYS.code, tde_status.SDK_FAILS_TO_FETCH_UPDATED_KEYS.message, '', self.hrlc.level['severe'])
                    self.hrlc.statistic[3] += 1
                    raise NoValidKeyException(tde_status.SDK_FAILS_TO_FETCH_UPDATED_KEYS.message)
            elif st['status'] == 'Malformed':
                self.hrlc.insert_err_report(tde_status.SDK_HAS_CORRUPTED_CIPHER.code, tde_status.SDK_HAS_CORRUPTED_CIPHER.message, '', self.hrlc.level['severe'])
                self.hrlc.statistic[3] += 1
    ...
    def __check_key_status_for_encryption(self, key):
        if key is None:
            self.hrlc.insert_err_report(tde_status.SDK_HAS_NO_AVAILABLE_ENC_KEYS.code, tde_status.SDK_HAS_NO_AVAILABLE_ENC_KEYS.message, '', self.hrlc.level['severe'])
            self.hrlc.statistic[2] += 1
            raise NoValidKeyException(tde_status.SDK_HAS_NO_AVAILABLE_ENC_KEYS.message)
        if key.key_status != constants.KeyStatuses.active():
            self.hrlc.insert_err_report(tde_status.SDK_OPERATE_WITH_INACTIVE_KEYS.code, tde_status.SDK_OPERATE_WITH_INACTIVE_KEYS.message, '', self.hrlc.level['error'])
            self.hrlc.statistic[2] += 1
        ...
        if key.expired < round(time.time() * 1000):
            self.hrlc.insert_err_report(tde_status.SDK_OPERATE_WITH_EXPIRED_KEYS.code, tde_status.SDK_OPERATE_WITH_EXPIRED_KEYS.message, '', self.hrlc.level['warn'])

    def __validate_token(self):
        if (self.token['start_ts'] - minutesBuffer) > round(time.time() * 1000):
            self.hrlc.insert_err_report(tde_status.SDK_USE_INEFFECTIVE_TOKEN.code, tde_status.SDK_USE_INEFFECTIVE_TOKEN.message, '', self.hrlc.level['severe'])
        ...
        if token_state == TokenState.EXPIRED:
            self.hrlc.insert_err_report(tde_status.SDK_USE_HARD_EXPIRED_TOKEN.code, tde_status.SDK_USE_HARD_EXPIRED_TOKEN.message, '', self.hrlc.level['severe'])
            raise InvalidTokenException(tde_status.SDK_USE_HARD_EXPIRED_TOKEN.message)
        elif token_state == TokenState.EXPIREWARNING:
            self.hrlc.insert_err_report(tde_status.SDK_USE_SOFT_EXPIRED_TOKEN.code, tde_status.SDK_USE_SOFT_EXPIRED_TOKEN.message, '', self.hrlc.level['warn'])
    ...
    def request_voucher_token(self):
        ...
        self.token = {'label': data.get('act'), 'start_ts': data.get('effective'), 'end_ts': data.get('expired'), 'id': data.get('id'), 'key': base64.b64decode(data.get('key')), 'service': service, 'stype': data.get('stype'), 'zone': zone}
    ...
    def fetch_master_keys(self):
        try:
            ...
            if response_code == 0:
                ...
                if response_code in [tde_status.TMS_REQUEST_VERIFY_FAILED.code, tde_status.TMS_TOKEN_EXPIRE.code, tde_status.TMS_NO_AVAILABLE_GRANTS_FOR_SERVICE.code, tde_status.TMS_TOKEN_IS_FROZEN.code, tde_status.TMS_TOKEN_IS_REVOKE.code, tde_status.TMS_DB_DATA_NOTFOUND_ERROR.code]:
                    self.hrlc.insert_err_report(response_code, response.get('status_message'), '', self.hrlc.level['severe'])
                    self.cache_ks.remove_all_m_keys()
                    self.key_chain_is_ready = False
                else:
                    self.hrlc.insert_err_report(response_code, response.get('status_message'), '', self.hrlc.level['error'])
                raise ServiceErrorException(response.get('status_message'))
        except Exception as e:
            dummy(e)
            self.hrlc.insert_err_report(tde_status.SDK_CANNOT_REACH_KMS.code, tde_status.SDK_CANNOT_REACH_KMS.message, traceback.format_exc(), self.hrlc.level['severe'])

    def __import_mkeys(self, response):
        if response.get('enc_service') != self.token['service']:
            self.hrlc.insert_err_report(tde_status.SDK_RECEIVED_WRONG_KEYRESPONSE1.code, tde_status.SDK_RECEIVED_WRONG_KEYRESPONSE1.message, '', self.hrlc.level['error'])
            raise RuntimeError(tde_status.SDK_RECEIVED_WRONG_KEYRESPONSE1.message)
        if response.get('tid') != self.token['id']:
            self.hrlc.insert_err_report(tde_status.SDK_RECEIVED_WRONG_KEYRESPONSE2.code, tde_status.SDK_RECEIVED_WRONG_KEYRESPONSE2.message, '', self.hrlc.level['error'])
        ...
        for service_key in service_key_list:
            ...
            for m_data in m_keys:
                m_key = Mkey(service_key.get('service'), base64.b64decode(m_data.get('id')), base64.b64decode(m_data.get('key_string')), m_data.get('key_digest'), m_data.get('version'), m_data.get('key_effective'), m_data.get('key_exp'), m_data.get('key_type'), service_key.get('grant_usage'), m_data.get('key_status'))
                if m_key.is_valid:
            ...
            self.hrlc.insert_key_update_event_report(tde_status.SDK_REPORT_CUR_KEYVER.code, tde_status.SDK_REPORT_CUR_KEYVER.message + str(self.major_key_ver), self.major_key_ver, self.available_key_list)
    ...
    def __send_corrupt_report(self):
        if len(self.corrupt_key_list) > 0:
            ...
            self.hrlc.insert_err_report(tde_status.SDK_HAS_CORRUPTED_KEYS.code, tde_status.SDK_HAS_CORRUPTED_KEYS.message, key_ids, self.hrlc.level['error'])
    ...
    def __check_valid_key_chain(self):
        ...
        if total_count == 0:
            self.hrlc.insert_err_report(tde_status.SDK_HAS_NO_AVAILABLE_KEYS.code, tde_status.SDK_HAS_NO_AVAILABLE_KEYS.message, '', self.hrlc.level['severe'])
...
class CacheKeyStore(object):
    ...
    def update_future_key_ids(self, service, max_ver):
        ...
        for idx in range(s_index, s_index + CacheKeyStore.threshold_value):
            self.future_key_ids.add(base64_util.encode_to_string(md5((service + str(idx)).encode(encoding='utf-8')).digest()))
...
class Mkey(object):
    ...
    def encrypt(self, text):
        ...
        result = constants.CipherTypes.weak().id.to_bytes(length=1, byteorder='big', signed=False) + constants.AlgoTypes.aes_cbc_128().id.to_bytes(length=1, byteorder='big', signed=False) + self.id + cipher
        return result
