...
def gridded_divergence(datasets, wind, date, latgrid, longrid, latgrid_km, longrid_km,
    ...
    for i, file in enumerate(datasets.read_date(date)):
        ...
        if species == 'CO2':
            ...
            data = file.NO2.data.flatten()
            units = file.NO2.attrs['units']
        ...
        if species == 'CO2':
            ...
            # Change CO2 units from ppm to molec/m^2
            data = ucat.convert_columns(data, units, 'm-2', molar_mass='CO2',
                                        p=psurf)

        elif species == 'NO2':
            # Change NO2 units to molec/m^2
            data = ucat.convert_columns(data, units, 'm-2')
        ...
        if species == 'CO2':
            div_map = 44.01 / 6.0221409e23 * divergence([species_Fx_flux,species_Fy_flux],
                                                        [longrid_km*1e3,latgrid_km*1e3])
        elif species == 'NO2':
            # Coefficient to convert NO2 to NOx, normally above 1.0
            coef = 0.9571
            div_map = 46.0055 / 6.0221409e23 * divergence([coef*species_Fx_flux,coef*species_Fy_flux],
                                                        [longrid_km*1e3,latgrid_km*1e3])
    ...
    if len(images) != 0:
        grid_xr = xr.DataArray(species_grids,
                               ...
                               attrs={'long name':f'gridded {species}',
                                      'units':'molec m-2'})
        div_xr = xr.DataArray(div_grids,
                              dims=['n_images','lat','lon'],
                              attrs={'long name':f'Divergence of {species}',
                                     'units':'g m-2 s-1'})
    else:
    ...
    data_vars = {f'{species}_grid' : grid_xr,
    ...
    attrs = {'Description': f'Gridded {species} data values and fluxes'}
...
def estimate_emissions(datasets, wind_folder, sources,
    ...
    # Validate supported trace gases
    for gas in trace_gases:
        if gas not in ['CO2','NO2']:
            raise ValueError(f'Trace gas {gas} not supported')
