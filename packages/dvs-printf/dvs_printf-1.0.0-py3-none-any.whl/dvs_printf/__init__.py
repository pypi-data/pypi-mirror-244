...
def printf(*values, styl: str | None='typing', speed: int | None = 3, intervel: int | None = 2,  stay=False):
    ''' 
        ...
        for x in values:
            ...
            for i in range(len_x):
                ...
                for j in range(len_x-i):
                    print(short+" "*(len_x-j-2-len(short))+index+(" "*j)+f"  <==[{next_let}]=|",end="\r")#*(len(x)-j)
                    sleep(speed)
                sleep(speed)
                short = short + x[i]
            if stay==True:
...
stay --> after styl animetion whether you want the stream OR Not
        defult stay is False, can be True OR False 
                
        but some styles take action on stay 
        whether it is True OR False 
        Ex. ( typing , f2b,  b2f, metrix, metrix2, firing )\n\n""")
        for i in values:
            print(i)
