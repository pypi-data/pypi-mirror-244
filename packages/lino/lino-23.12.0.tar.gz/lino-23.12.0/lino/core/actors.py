...
class Actor(actions.Parametrizable, Permittable, metaclass=ActorMetaClass):
    ...
    def request_from(cls, ar, *args, **kwargs):
        ...
        """
        raise Exception("Deprecated (Actor.request_from). Rather call cls.request(parent=ar)")
    ...
    def slave_as_html(cls, master, ar):
        ...
        """
        ar = cls.request(
            master, request=ar.request,
            param_values=ar.param_values if getattr(cls, "use_detail_params_value", False) else {},
    ...
    def get_table_as_list(cls, obj, ar):
        ...
        for i, obj in enumerate(sar.data_iterator):
            ...
            par = sar.add_detail_link(obj, par)
            par = "<p>{}</p>".format(par)
            # url = sar.obj2url(obj)
            # if url is not None:
            #     url = html.escape(url)
