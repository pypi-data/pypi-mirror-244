...
@dataclass
class JiraHealthCheckResult:
    """
    Representing the result of a Jira ingestion healthcheck report.
    """
    successful: bool
...
@dataclass
class GitHealthCheckResult:
    """
    Representing the result of a Git ingestion healthcheck report.
    """
...
class IngestionHealthCheckResult:
    ...
    def __post_init__(self):
        """
        Sets the fully_successful
        """
        jira_successful = self.jira_healthcheck_result.successful if self.jira_healthcheck_result else False
        git_successful = self.git_healthcheck_result.successful if self.git_healthcheck_result else False
    ...
    def from_json(cls, body: str):
        ...
        # dataclass (without third-party packages), so we intercept the dict result and convert it into a dataclass.
        healthcheck_dict['git_healthcheck_result'] = GitHealthCheckResult.from_dict(healthcheck_dict['git_healthcheck_result']) if healthcheck_dict['git_healthcheck_result'] else None
        healthcheck_dict['jira_healthcheck_result'] = JiraHealthCheckResult.from_dict(healthcheck_dict['jira_healthcheck_result']) if healthcheck_dict['jira_healthcheck_result'] else None
...
def _attempt_anode_id_header_check(jira_connection: JIRA) -> bool:
    ...
    try:
        ...
        if 'X-ANODEID' in headers:
            logger.info('Response headers contains X-ANODEID! Customer is running Jira Data Center.')

            return True
        else:
            logger.info('Response headers does not contain X-ANODEID! Customer is NOT running Jira Data Center.')
...
def get_jira_permissions(jira_connection: JIRA) -> list[str]:
    ...
    permission_set = {"ADMINISTER_PROJECTS", "BROWSE_PROJECTS", "VIEW_READONLY_WORKFLOW", "ASSIGNABLE_USER",
                      "ASSIGN_ISSUES", "CLOSE_ISSUES", "CREATE_ISSUES", "DELETE_ISSUES", "EDIT_ISSUES",
                      "LINK_ISSUES", "MODIFY_REPORTER", "MOVE_ISSUES", "RESOLVE_ISSUES", "SCHEDULE_ISSUES",
                      "SET_ISSUE_SECURITY", "TRANSITION_ISSUES", "ADD_COMMENTS", "DELETE_ALL_COMMENTS",
                      "DELETE_OWN_COMMENTS", "EDIT_ALL_COMMENTS", "EDIT_OWN_COMMENTS", "DELETE_ALL_WORKLOGS",
                      "DELETE_OWN_WORKLOGS", "EDIT_ALL_WORKLOGS", "EDIT_OWN_WORKLOGS", "WORK_ON_ISSUES"}

    params = {
        'permissions': ', '.join(list(permission_set))
    }
...
def validate_jira(config: JiraIngestionConfig) -> JiraHealthCheckResult:
    """
    ...
    healthcheck_result = JiraHealthCheckResult(successful=True)
