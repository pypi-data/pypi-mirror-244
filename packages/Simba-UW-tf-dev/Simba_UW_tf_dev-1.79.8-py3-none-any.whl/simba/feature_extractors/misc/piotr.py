...
class PiotrFeatureExtractor(ConfigReader, FeatureExtractionMixin, GeometryMixin):
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            white_animal_polygons = GeometryMixin().multiframe_bodyparts_to_polygon(data=white_animal_df_arr, pixels_per_mm=pixels_per_mm, parallel_offset=20, verbose=True, video_name=video_name, animal_name='white', core_cnt=-1)
            black_animal_polygons = GeometryMixin().multiframe_bodyparts_to_polygon(data=black_animal_df_arr, pixels_per_mm=pixels_per_mm, parallel_offset=20, verbose=True, video_name=video_name, animal_name='black', core_cnt=-1)
            white_animal_polygons = GeometryMixin().multiframe_minimum_rotated_rectangle(shapes=white_animal_polygons, video_name=video_name, animal_name='white', verbose=True)
            black_animal_polygons = GeometryMixin().multiframe_minimum_rotated_rectangle(shapes=black_animal_polygons, video_name=video_name, animal_name='black', verbose=True)
            results['polygon_pct_overlap'] = GeometryMixin().multiframe_compute_pct_shape_overlap(shape_1=white_animal_polygons, shape_2=black_animal_polygons, animal_names='black_white', video_name=video_name, verbose=True)
            combined_list = [list(pair) for pair in list(zip(white_animal_polygons, black_animal_polygons))]
            difference = GeometryMixin().multiframe_difference(shapes=combined_list, verbose=True, animal_names='white_black', video_name=video_name)
            results['difference_area'] = GeometryMixin().multiframe_area(shapes=difference, pixels_per_mm=pixels_per_mm, verbose=True, video_name=video_name)
            write_df(df=results, file_type=self.file_type, save_path=save_path)
...
def main():
    parser = argparse.ArgumentParser(description='RHD to EDF converter')
    parser.add_argument('--config_path', type=str, help='SimBA project config path')
    args = parser.parse_args()
    PiotrFeatureExtractor(config_path=args.config_path)
