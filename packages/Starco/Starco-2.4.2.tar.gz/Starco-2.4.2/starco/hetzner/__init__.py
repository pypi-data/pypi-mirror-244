...
class Hetzner:
    ...
    def change_server_ip(self,server_id):
        self.power_off(server_id)
        sleep(10)
        server_info = self.client.servers.get_by_id(server_id)
        sleep(2)
        ...
        res = self.client.primary_ips.create('ipv4',datacenter=server_info.datacenter,name=new_ip_name)
        sleep(5)
        self.client.primary_ips.assign(res.primary_ip,server_id)
        sleep(5)
        self.power_on(server_id)
        return True
