...
from dql.data_storage.schema import DATASET_CORE_COLUMN_NAMES, PARTITION_COLUMN_ID
...
class UDF(Step, ABC):
    ...
    def custom_columns_created(self) -> Dict[str, SQLType]:
        return {
            col_name: col_type
            for (col_name, col_type) in self.udf.output.items()  # type: ignore [union-attr]
            if col_name not in DATASET_CORE_COLUMN_NAMES
        }

    @staticmethod
    def adjust_outputs(data_storage, rows: List[Dict[str, Any]], udf) -> None:
        """
        ...
        for row in rows:
            for col_name, col_type in udf.output.items():
                ...
                try:
                    ...
                    col_type = (
                        col_type()
                        if isinstance(col_type, TraversibleType)
                        else col_type
                    )
    ...
    def populate_udf_table(self, udf_table, query, udf) -> None:  # noqa: C901, PLR0912
        ...
        if (
            not workers
            and os.environ.get("DQL_DISTRIBUTED")
            and os.environ.get("DQL_SETTINGS_WORKERS")
        ):
        ...
        try:
            if workers:
                ...
                for udf_output in udf_results:
                    ...
                    self.adjust_outputs(self.catalog.data_storage, new_rows, udf)
