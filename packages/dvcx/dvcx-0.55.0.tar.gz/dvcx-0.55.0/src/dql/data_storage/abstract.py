...
class AbstractDataStorage(ABC):
    ...
    def __init__(self, uri: str = "", partial_id: Optional[int] = None):
        ...
        self.dataset_fields = [
            c.name for c in self.datasets_columns() if c.name  # type: ignore[attr-defined]
        ]
        self.dataset_version_fields = [
            c.name for c in self.datasets_versions_columns() if c.name  # type: ignore[attr-defined]
        ]
    ...
    def convert_type(self, val: Any, col_type: SQLType) -> Any:
        """
    ...
    def create_dataset_version(
        ...
        custom_column_types: Dict[str, str] = None,
    ) -> "DatasetRecord":
    ...
    @staticmethod
    def python_type(col_type: Union["TypeEngine", "SQLType"]) -> Any:
    ...
    def select_node_fields_by_parent_id(
        ...
        return self.ddb.execute(
            ...
            .where((n.c.parent_id == parent_id) & (n.c.is_latest == true()))
            .order_by(n.c.source, n.c.parent, n.c.name, n.c.version, n.c.etag)
        )
