...
"""
import re
from sdk.utils import util_file, util_folder, util_json,util_encrtpt,util_bos


class IsSolution(object):
    """
    ...
    def get_points(self, points: dict, mode=0, format=False):
        ...
        if mode == 0:
            if format:
                return [[int(i["x"]), int(i["y"])] for i in points]
            else:
                return [[i["x"], i["y"]] for i in points]
        elif mode == 1:
            lis = []
            if format:
                for li in [[int(i["x"]), int(i["y"])] for i in points]:
                    lis.append(li)
            else:
                for li in [[i["x"], i["y"]] for i in points]:
                    lis.append(li)
            return lis
        else:
            lis = []
    ...
    def make_out_path(self, base_path: str,
                      path_list: list = ["result"]) -> str:
        """
        生成输出路径
        :param base_path:根路径
        :param name:文件名
        ...
        _path_list.insert(0, base_path)
        self.folder.create_folder(_path_list[0])
        _save_path = self.folder.merge_path(_path_list)
