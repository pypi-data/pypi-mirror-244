...
class Mark(MarkProcess):
    ...
    def _init_mark_normal_process(self, file, answer, save_file):
        """
        常规标注处理流程
        ...
        try:
            # print("answer",answer)
            map = self.make_map(date=answer)
            img = self.read_image(file)
            img = self.mark(img, map)
    ...
    def make_map(self, file=None, date=None) -> dict:
        ...
        if file:
            for args in self.file.read_yield(file):
                ...
                num = args["num"]
                insert_args = {
                    "opt": [[int(i["x"]), int(i["y"])] if len(date[headers.index("points")]) != 1 else [int(i["x"]), int(i["y"]), 1] for i in date[headers.index("points")]],
                    "text": date[headers.index("text")],
                }
                map[str(num)] = insert_args
        if date:
            json_data = self.json.loads(date)
            if json_data.get("result"):
                for result in json_data["result"]:
                    for index, element in enumerate(result["elements"]):
                        insert_args = {
                            "opt": [[int(i["x"]), int(i["y"])] if len(element["points"]) != 1 else [int(i["x"]), int(i["y"]), 1] for i in element["points"]],
                            "text": element["text"]
                        }
                        num = index + 1
                        map[str(num)] = insert_args
