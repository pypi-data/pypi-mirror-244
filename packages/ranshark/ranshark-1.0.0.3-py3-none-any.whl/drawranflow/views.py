        ...
        if item:
            ...
            if os.path.exists(file_path):
                ...
                with ThreadPoolExecutor() as executor:
                    executor.submit(async_process_file, f, csv, item_id)
...
def async_process_file(filename, csv, item_id):
    ...
    ph(input_pcap=filename, output_csv=csv, item_id=item_id).capture_packets_and_save_to_csv()
...
def streaming_table_view(request):
    ...
    identifiers_table_data = Identifiers.objects.filter(uploadedFiles_id=id).order_by('id').values()
...
def fetch_associated_data(request, main_id):
    try:
        messages = Message.objects.filter(identifiers=main_id).order_by('FrameTime')
...
def fetch_packet_data(request):
    ...
    frame_numbers = [message.FrameNumber for message in Message.objects.filter(
...
def get_updated_table_data(request):
    ...
    data = UploadedFile.objects.values('id', 'filename', 'upload_date', 'proces_date', 'processed', 'completed',
                                       'complete_at').order_by('id')
    ...
    try:
        ...
        for cumulative_stat in cumulative_stats:
            cumulative_stat_list.append({
                ...
                'cucp_ip': cumulative_stat.cucp_f1c_ip
            })
        ...
        print(unique_cucp_ips, cumulative_stat_list)
        return render(request, 'show_stats.html', context)
    except Exception as e:
