...
def TestFunc(file_name):
    AIP_AUTH_STR = "313332373733262626756645426772664b2f55324561667955626f79654a42484763565259343178692626263333323737332626263257785156694e35484a68476657564d334831346874764b436f7836324b7364"
    AUTH_SPLIT_STR = "&&&"
    decode_str = binascii.a2b_hex(AIP_AUTH_STR).decode("utf-8")
    key_list = decode_str.split(AUTH_SPLIT_STR)
    # 识别类型
    pred_type = "30400"
    api = FateadmApi(key_list[2], key_list[3], key_list[0], key_list[1])
    # 查询余额
    balance = api.QueryBalcExtend()  # 直接返余额
    # api.QueryBalc()

    # 通过文件形式识别：
    # file_name = "E:\\desktop\\response.jpeg"
...
def getVerifyCodeFromImage(image_data):
    AIP_AUTH_STR = "313332373733262626756645426772664b2f55324561667955626f79654a42484763565259343178692626263333323737332626263257785156694e35484a68476657564d334831346874764b436f7836324b7364"
    AUTH_SPLIT_STR = "&&&"
    decode_str = binascii.a2b_hex(AIP_AUTH_STR).decode("utf-8")
    key_list = decode_str.split(AUTH_SPLIT_STR)
    ...
    rsp = api.Predict(pred_type, image_data)  # 返回详细的识别结果
    return rsp
...
if __name__ == "__main__":
    # rsp = TestFunc()
    headers = {
        'User-Agent': 'User-Agent:Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:78.0) Gecko/20100101 Firefox/78.0',
        'Connection': 'keep-alive'}
    image_url = "https://data.stats.gov.cn/waf_captcha/"
    # image_data = requests.get(image_url, headers=headers, verify=False)
    ...
    with open('cap.jpg', 'wb') as f:
        f.write(image_data)
    rsp = getVerifyCodeFromImage(image_data)
    print("识别结果1为：" + rsp.pred_rsp.value)
    rsp = TestFunc("cap.jpg")
    print("识别结果2为：" + rsp.pred_rsp.value)
    if rsp.ret_code == 0:
        print("识别结果为：" + rsp.pred_rsp.value)
    else:
        print("识别失败：" + rsp.err_msg)
