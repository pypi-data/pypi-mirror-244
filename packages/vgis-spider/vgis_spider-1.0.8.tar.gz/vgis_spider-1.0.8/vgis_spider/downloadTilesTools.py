...
class downloadTilesHelper:
    ...
    def loop_download_tiles(self, xmin, xmax, ymin, ymax):
        ...
        if self.image_service_type == "wmts":
            ...
            # 同时计算左上角瓦片的地图单位
            # 瓦片原点在左下角
            if "geoserver" in image_service_info["service_type"]:
                pass
    ...
    def vconcat_tileImage(self, tile_stride_dir):
        ...
        elif self.image_service_type == "wmts" or self.image_service_type == "xyz" :
...
if __name__ == '__main__':
    ...
    try:
        ...
        log_file_path = os.path.join(log_path,
                                     "{}_download_tiles_log.log".format(datetime.now().strftime('%Y%m%d%H%M%S')))
        ...
        # # WMS服务参数设置
        # # wms服务 每像素对应的地图单位值(分辨率），值越小，精度越高，合成图片越大
        # # 大概相当于geoserver瓦片的15级
        # resolution = 0.000019705111
        # # 下载瓦片像素大小
        # tile_size = 256
        # # 瓦片后缀jpeg,png
        # tile_pic_suffix = "png"
        # # 支持geoserver_wms服务
        # image_service_info["service_type"] = "geoserver_wms"
        # image_service_info["service_url"] = "http://192.168.31.79:8085/geoserver/ttxm/wms?"
        # image_service_info["layer_name"] = "ttxm:TW_2021_DDZD1_0.51645445328767"
        # image_service_info["epsg"] = 4326
        # image_service_type = "wms"
        # # 下载瓦片的地图范围
        # xmin = 120.45251369475
        # ymin = 22.66166696535
        # xmax = 120.47436262675
        # ymax = 22.676074893150002
        #
        # # 卫星中心_wms服务
        # image_service_info["service_type"] = "satellite_center_wms"
        # image_service_info["service_url"] = "http://117.141.148.176:14002"
        # # 2米图 OneMap{year},还有亚米图等
        # image_service_info["layer_name"] = "OneMap2019"
        # image_service_info["epsg"]=4326
        # image_service_type = "wms"
        # #
        # # 下载瓦片的地图范围
        ...
        # # WMTS服务参数设置
        # # geoserver wmts服务
        # image_service_info["service_type"] = "geoserver_wmts"
        # image_service_info["service_url"] = "http://192.168.31.79:8085/geoserver/gwc/service/wmts?"
        # image_service_info["geoserver_username"] = "admin"
        # image_service_info["geoserver_password"] = "geoserver"
        # image_service_info["layer_name"] = "xyk:20230407_EQHP"
        # image_service_info["gridset_name"] = "EPSG:4326"
        # image_service_info["zoom_level"] = 18
        # image_service_info["epsg"] = 4326
        # image_service_type = "wmts"
        # # 瓦片后缀jpeg,png
        # tile_pic_suffix = "png"
        # # 读取瓦片服务的瓦片大小
        # # 根据 zoom_level获取分辨率大小
        # # 再带入公式，计算包含的行列号，对每个瓦片进行获取
        # resolution, tile_size = None, None
        # # 下载瓦片的地图范围
        # xmin = 109.816145
        # ymin = 22.701422
        # xmax = 109.818952
        # ymax = 22.703963
        # # 上述的分辨率等参数需要重新计算，根据当前的瓦片级别

        # # XYZ服务参数设置
        # # google xyz服务
        # image_service_info["service_type"] = "google_xyz"
        # image_service_info["service_url"] = "https://gac-geo.googlecnapps.cn/maps/vt?"
        # image_service_info["layer_name"] = "s"
        # # 18级大概0.5米分辨率
        # image_service_info["zoom_level"] = 18
        # image_service_info["epsg"] = 3857
        # image_service_type = "xyz"
        # # 瓦片后缀jpeg,png
        # tile_pic_suffix = "jpg"
        # # 读取瓦片服务的瓦片大小
        # # 根据 zoom_level,计算包含的行列号，对每个瓦片进行获取
        # resolution, tile_size = None, None
        # # 下载瓦片的地图范围（需要将地图的3857转为4326）
        # xmin = 109.816145
        # ymin = 22.701422
        # xmax = 109.818952
        # ymax = 22.703963
        ...
        # # XYZ服务参数设置
        # # arcgisserver xyz服务，有些地方18级没有覆盖
        # image_service_info["service_type"] = "arcgis_xyz"
        # image_service_info["service_url"] = "https://server.arcgisonline.com/ArcGIS/rest/services/"
        # image_service_info["layer_name"] = "World_Imagery"
        # # 18级大概0.5米分辨率
        # image_service_info["zoom_level"] = 18
        # image_service_info["epsg"] = 3857
        # image_service_type = "xyz"
        ...
        # # 读取瓦片服务的瓦片大小
        # # 根据 zoom_level,计算包含的行列号，对每个瓦片进行获取
        # resolution, tile_size = None, 256
        # # 下载瓦片的地图范围（需要将地图的3857转为4326）
        # xmin = 129.6508076041478
        # ymin = 44.61979806877633
        # xmax = 129.65667802893515
        # ymax = 44.62330248690819

        # XYZ服务参数设置
        # tdt xyz服务，有些地方18级没有覆盖
        image_service_info["service_type"] = "tdt_xyz"
        image_service_info["service_url"] = "http://t0.tianditu.com/DataServer?"
        image_service_info["layer_name"] = "img_w"
        # 18级大概0.5米分辨率
        image_service_info["zoom_level"] = 18
        image_service_info["epsg"] = 3857
        image_service_type = "xyz"
        ...
        # 合成图片名称
        merge_pic_name = "merged_{}_tiles_{}.{}".format(image_service_info["service_type"], uuid.uuid4(),
                                                        tile_pic_suffix)
        # 瓦片合成大图存放路径
        out_image_path = os.path.join(os.getcwd(), ".cache", "mergePic")
        # 初始化瓦片下载器
        downloadOperator = downloadTilesHelper(tile_pic_suffix, merge_pic_name, resolution, tile_size,
                                               image_service_info,
                                               image_service_type, None, None,
                                               out_image_path, is_multi_thread, logger)
        logger.info("正在下载瓦片数据")

        downloadOperator.fetch_tiles(xmin, xmax, ymin, ymax)
        logger.info("瓦片拼接大图名称：{}".format(merge_pic_name))

    except Exception as exp:
        logger.error(exp)
        logger.error(exp.__traceback__.tb_frame.f_globals["__file__"])  # 发生异常所在的文件
        logger.error(exp.__traceback__.tb_lineno)  # 发生异常所在的行数
        logger.error(u"下载瓦片数据失败:{}".format(exp))
