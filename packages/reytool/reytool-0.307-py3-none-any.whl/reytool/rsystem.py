...
def search_process(
    ...
    ports: Optional[Union[str, int, Sequence[Union[str, int]]]] = None,
) -> List[Process]:
    """
    ...
    Parameters
    ----------
    ids : A value or sequence of process ID.
    names : A value or sequence of process name.
    ports : A value or sequence of process port.

    Returns
    -------
    List of process instances found.
    """

    # Handle parameter.
    if ids is None:
        ids = []
    elif ids.__class__ == int:
        ids = [ids]
    if names is None:
        names = []
    elif names.__class__ == str:
        names = [names]
    if ports is None:
        ports = []
    elif ports.__class__ in (str, int):
        ports = [ports]
    ports = [
        int(port)
        for port in ports
...
def kill_process(
    ...
    # Search.
    processes = search_process(ids, names, ports)
