...
class BaseModel(db.Model):
    ...
    def filter(
        ...
        for join in bridge_joins:
            ...
            # Remove bridge rows that are deleted - and if not present ignores
            q = q.filter(
                or_(
                    join['bridge_model'].is_deleted == False,
                    join['bridge_model'].id_ == None,
                )
            )
        ...
        for result in results:
            ...
            if len(joins) > 0 or len(bridge_joins) > 0:
            ...
            if primary_object.id_ not in primary_objects_map:
                ...
                details = primary_objects_map[primary_object.id_]
