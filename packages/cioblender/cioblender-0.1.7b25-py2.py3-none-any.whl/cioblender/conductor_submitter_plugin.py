...
class SubmitOperator(Operator):
    ...
    def execute(self, context):
        ...
        # Check if the current Blender file has unsaved changes
        ...
        print("Saving blender render file to: ", render_filepath)
        if render_filepath:
            bpy.ops.wm.save_as_mainfile(filepath=render_filepath)
        time.sleep(0.5)
...
def register():
    ...
    bpy.types.Scene.instance_type = bpy.props.EnumProperty(
        name="Instance Type",
        description="Choose between machines with or without graphics cards. If you select Eevee render software, you must choose a GPU instance type.",
...
def unregister():
    try:
        ...
        props_to_remove = [
            ...
            'render_software', 'chunk_size', 'use_custom_range',
