...
def create_plugin_at_blender_folders(platform):
    ...
    try:
            ...
            for version_folder in os.listdir(blender_versions_folder):
                ...
                if os.path.exists(existing_plugin_path):
                    try:
                        ...
                        print(f"Unable to remove existing plugin at {existing_plugin_path}, error: {e}")
        ...
        print(f"Unable to copy plugin {ADDOON_FILE} to folder {addon_destination}, error: {e}")
...
def main():
    ...
    if copied_folders:
        sys.stdout.write("Created Conductor Blender plugin at the following folders:\n")
