...
class Etatherm:
    ...
    async def get_current_temperatures(self)-> (dict[int,int] | None):
        ...
        (data, )=await self.__send_read_request(0,1, b'\x00\x60', 16)
        if self._params is None:
            await self.__read_params()
    ...
    async def set_mode(self, pos:int, auto:bool) ->bool:
        ...
        (data, )=await self.__send_read_request(0,1,addr.to_bytes(2,'big'),6)
        if data is None:
            return False
        if auto:
        ...
        (success, )=await self.__send_write_request(0,1, addr.to_bytes(2,'big'), data)
    ...
    async def set_temporary_temperature(self, pos:int, temperature: int, duration: int=120)-> bool:
        ...
        (data, )=await self.__send_read_request(0,1,addr.to_bytes(2,'big'),1)
        if data is None:
            return False
        now = datetime.now()
    ...
    async def get_required_temperatures(self) -> (dict[int,dict[str,any]] | None):
        ...
        (data, )=await self.__send_read_request(0,1, b'\x00\x70', 16)
        if self._params is None:
            await self.__read_params()
        if data is None or len(data)!=16 or self._params is None:
            return None
