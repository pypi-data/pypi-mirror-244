...
from flowi.components.preprocessing.transformers import ColumnFilter, filter_columns
...
class PreprocessingDataframe(ComponentBase):
    ...
    def drop_columns(self,
        ...
        self._logger.debug(f"exclude_columns: {exclude_columns}")

        def drop_columns_func(X, columns_: List[str] = None, exclude_columns_: List[str] = None):
            columns_to_use = filter_columns(df=X, columns=columns_, exclude_columns=exclude_columns_)
            return X[columns_to_use]

        transformer = FunctionTransformer(lambda X: drop_columns_func(X, columns, exclude_columns), validate=False)
