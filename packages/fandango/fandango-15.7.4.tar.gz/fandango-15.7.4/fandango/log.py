...
TRACE_LEVEL = os.getenv('TRACE_LEVEL') or 2
...
def tracer(msg='', obj=None, insight=True, level=None):
    level = notNone(level, TRACE_LEVEL)
    ...
    def getLogLevel(self, alias=None):
        ...
        if alias is None:
            ...
            if not isString(alias):
                try:
                    return next(
                        (k for k, v in self.__levelAliases.items() if v == alias)
                    )
                except:
                    return None
