...
class AtomicGrid:
    def __init__(self, at, nspheres=32, nangpts='110', lmax=8):
        """
        ...
        self.__set_bragg_slater_radius(self.__aidx)
        self.__build_chebychev_grid(nspheres)
        self.__lebedev_coeff = self.__load_lebedev_coefficients(str(nangpts))
    ...
    def __load_lebedev_coefficients(self, order):
        """
    ...
    def __build_chebychev_grid(self, nspheres):
        """
        Build Chebychev grid of concentric spheres
        
        nspheres: number of concentric spheres
        """
        N = nspheres
        f = np.pi / (N+1)
    ...
    def __set_bragg_slater_radius(self, atidx):
        """
        Set the Bragg-Slater radius for the atom
        """
        self.__rm = bragg_slater.BSRADII[atidx]
