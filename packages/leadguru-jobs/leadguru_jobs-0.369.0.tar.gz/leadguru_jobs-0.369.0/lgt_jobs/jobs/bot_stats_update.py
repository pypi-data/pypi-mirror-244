    ...
    def exec(self, data: BotStatsUpdateJobData):
        ...
        if bot.source.source_type == SourceType.DISCORD:
            ...
            for server in discord_servers:
                discord_server: Server = next(filter(lambda x: x.id == server.id, bot.servers), None)
                if discord_server:
                    server.active = discord_server.active
                discord_channels = [Channel.from_dic(channel) for channel in client.get_channels(server.id)]
                for channel in discord_channels:
                    discord_channel: Channel = next(filter(lambda x: x.id == channel.id, discord_server.channels), None)
                    if discord_channel:
