...
class BotStatsUpdateJobData( BaseModel):
    bot_id: Optional[str]


class BotStatsUpdateJob( ABC):
    @property
    ...
    def exec(self, data: BotStatsUpdateJobData):
        ...
        if bot.source.source_type == SourceType.DISCORD:
            ...
            for server in bot.servers:
                discord_server: Server = next(filter(lambda x: x.id == server.id, discord_servers), None)
                if discord_server:
            ...
            for server in bot.servers:
                ...
                for channel in discord_channels:
                    discord_channel: Channel = next(filter(lambda x: x.id == channel.id, server.channels), None)
                    if discord_channel:
                        channel.active = discord_channel.active
                server.channels = discord_channels
                server.icon = f'https://cdn.discordapp.com/icons/{server.id}/{server.icon}.png'
                for active_server in bot.active_servers:
                    if active_server.id == server.id:
                        updated_active_servers.append(server)
                        break

            bot.active_servers = updated_active_servers
...
BotStatsUpdateJob().exec(BotStatsUpdateJobData(bot_id='65535563a6ecebe85ffee277'))