...
class DivGallery(BaseDiv):
    ...
    scrollbar: typing.Optional[typing.Union[Expr, DivGalleryScrollbar]] = Field(
        description=(
            "Scrollbar behaviour. Hidden by default. On different "
            "platforms, the scrollbar mayoccupy a different space, since "
            "this is a user setting. Consider this in sizes.`none` — "
            "scrollbar is hidden.`auto` — scrollbar is shown if there is "
            "not enoughspace and is necessary out the current platform."
        ),
    )
