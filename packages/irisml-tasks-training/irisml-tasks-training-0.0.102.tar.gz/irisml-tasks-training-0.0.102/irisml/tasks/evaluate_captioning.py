...
class Task(irisml.core.TaskBase):
    ...
    def execute(self, inputs):
        ...
        total_target_ngrams = collections.Counter()
        ...
        for pred, targets in zip(inputs.predictions, inputs.targets):
            tokenized_pred = pred.lower().split()
            tokenized_targets = [t.lower().split() for t in targets]
            ...
            total_target_ngrams |= total_target_ngrams_for_image
        ...
        for pred_ngrams, target_ngrams in zip(all_pred_ngrams, all_target_ngrams):
            pred_vector = self._get_cider_vec(pred_ngrams, total_target_ngrams, log_num_targets)

            scores = torch.zeros((4,))
            for t in target_ngrams:
                target_vector = self._get_cider_vec(t, total_target_ngrams, log_num_targets)
                scores += self._calculate_cider_similarity(pred_vector, target_vector)
        ...
        return self.Outputs(bleu1=bleu_scores[0], bleu2=bleu_scores[1], bleu3=bleu_scores[2], bleu4=bleu_scores[3], cider=cider_score)
    ...
    def _calculate_cider_similarity(self, pred_vec, target_vec):
        scores = torch.zeros((4,))
        pred_n1_length = sum(pred_vec[0].values())
        target_n1_length = sum(target_vec[0].values())
        delta = pred_n1_length - target_n1_length
