                ...
                if not self.stop_flag:
                    ...
                    try:
                        ...
                        try:
                            # notification
                            send_notification(
                                self.storage, 'algo_sched', message)
                            # smartcall
                            smartcall(self.storage, 'algo_sched',
                                      f"算法代码执行错误为 {str(e)} ", 60)
                        except Exception as e:
                            errmsg = traceback.format_exc()
                            logging.error(
                                f"failed send sched failure, {e}: {errmsg}")
        ...
        while True:
            ...
            try:
                ...
                if confirms:
                    ...
                    ml = min(len(uc), 30)
                    logging.info(f"user confirmation stored: {uc[-ml:]}")
                # else:
                #     uc = [{"_none_": -1,
                ...
                ml = min(len(confirms), 30)
                logging.info(
                    f"end of data clean and confirm process: {confirms[-ml:]}")
