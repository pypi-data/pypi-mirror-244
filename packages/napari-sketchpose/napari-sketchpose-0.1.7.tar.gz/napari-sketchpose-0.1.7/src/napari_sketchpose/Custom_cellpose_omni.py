...
class CustomCellposeModel(CellposeModel):
    ...
    def sparse_omni_loss(self, lbl, y, S_t, D_t, S1_t, S2_t):
        ...
        if nt == 2:  # semantic segmentation
            ...
            """if self.iepoch % 10 == 0:
                ...
                plt.show()
                plt.imshow((dt > 0).detach().cpu().numpy().astype("uint8")[0] - (dist > 0).detach().cpu().numpy().astype("uint8")[0])
                plt.colorbar()
                plt.title("diff cartes de distance")
                plt.show()
                plt.imshow(w.tolist()[0])
                plt.colorbar()
                plt.title("W")
                plt.show()"""
            ...
            print('flow:%1.2f -- bd:%1.2f -- flow2:%1.2f -- dist:%1.2f -- flow3:%1.2f -- total:%1.2f' % (loss1, loss4, loss5, loss6, loss9,
                                                                                                         loss1 + loss4 + loss5 + loss6 + loss9))
            # print('loss1',loss1,loss1.type())
            # print('loss4',loss4,loss4.type())
            # print('loss5',loss5,loss5.type())
            # print('loss6',loss6,loss6.type())
    ...
    def loss_fn(self, lbl, y, S_t, D_t, S1_t, S2_t):
        """
        ...
        if self.omni and OMNI_INSTALLED:  # loss function for omnipose fields
            loss = self.sparse_omni_loss(lbl, y, S_t=S_t, D_t=D_t, S1_t=S1_t, S2_t=S2_t)
        else:  # original loss function
            loss = self.sparse_cp_loss(lbl, y, S_t=S_t, D_t=D_t, S1_t=S1_t, S2_t=S2_t)
        return loss
    ...
    def train(self, train_data, train_labels, train_D_t, train_S_t, train_S1_t, train_S2_t,
              train_links=None, train_files=None, test_data=None, test_labels=None, test_links=None, test_files=None,
        ...
        if multi_threading is False:
            model_path = self._train_net(train_data, train_labels, train_D_t, train_S_t, train_S1_t, train_S2_t, train_links,
            ...
            for iepoch in range(self.n_epochs):
                ...
                for ibatch in range(0, nimg_per_epoch, batch_size):
                    ...
                    self.iepoch = iepoch
                    imgi, lbl, D_ti, S_ti, S1_ti, S2_ti, scale = random_rotate_and_resize([train_data[i] for i in inds],
                                                                                              [train_labels[i] for i in
                                                                                               inds],
                                                                                              [train_D_t[i] for i in inds],
                                                                                              [train_S_t[i] for i in inds],
                                                                                              [train_S1_t[i] for i in inds],
                                                                                              [train_S2_t[i] for i in inds],
                    ...
                    S1_ti = torch.asarray(S1_ti).to("cuda")
                    S2_ti = torch.asarray(S2_ti).to("cuda")
                    D_ti = torch.asarray(D_ti).to("cuda")

                    if self.unet and lbl.shape[1] > 1 and rescale:
                        lbl[:, 1] /= diam_batch[:, np.newaxis, np.newaxis] ** 2
                    train_loss = self._train_step(imgi, lbl, D_t=D_ti, S_t=S_ti, S1_t=S1_ti, S2_t=S2_ti)
                    lavg += train_loss
                    nsum += len(imgi)
    ...
    def _train_step(self, x, lbl, D_t, S_t, S1_t, S2_t):
        """plt.imshow(x[0, 0, :, :])
        plt.title("Patch")
        plt.show()
        plt.imshow(lbl[0, 0, :])
        ...
        if self.torch:
            ...
            if self.autocast:
                with autocast():
                    ...
                    del X
                    loss = self.loss_fn(lbl, y, D_t=D_t, S_t=S_t, S1_t=S1_t, S2_t=S2_t)
    ...
    def _train_net(self, train_data, train_labels, train_D_t, train_S_t, train_S1_t, train_S2_t,
                   train_links, test_data=None, test_labels=None,
        ...
        for iepoch in range(self.n_epochs):
            ...
            for ibatch in range(0, nimg_per_epoch, batch_size):
                ...
                if self.unet and lbl.shape[1] > 1 and rescale:
                    lbl[:, 1] /= diam_batch[:, np.newaxis, np.newaxis] ** 2
                train_loss = self._train_step(imgi, lbl, D_ti, S_ti, S1_ti, S2_ti)
                lavg += train_loss
                nsum += len(imgi)
