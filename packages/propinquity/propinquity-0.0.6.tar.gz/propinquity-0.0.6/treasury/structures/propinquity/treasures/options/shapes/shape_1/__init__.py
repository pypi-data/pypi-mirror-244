...
def assertions (shape_1):
	...
	assert (len (shape_1) >= 1, True), len (shape_1)
	...
	for expiration in expirations:
		...
		assert (
			len (expiration ["calls"]["strikes"]) >= 1, True
		), len (expiration ["calls"]["strikes"])
	
		assert ("puts" in expiration)
		assert ("strikes" in expiration ["puts"])
		assert (
			len (expiration ["puts"]["strikes"]) >= 1, True
		), len (expiration ["puts"]["strikes"])
