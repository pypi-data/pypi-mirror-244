...
from syamaguc.gyazo import Image, ImageList
...
class TestImageList:
    def test_from_list(self):
        tmp = ImageList.from_list([image_1_dict, image_2_dict, image_2_dict_empty_str])
        assert tmp.images == [image_1, image_2, image_2]
    ...
    def test_add(self):
        ...
        tmp = l1 + l2
        assert tmp.total_count == 3
        assert len(tmp.images) == 3
        assert tmp.current_page is None
        assert tmp.per_page is None
        assert tmp.user_type is None
