...
class Api:
    ...
    def __init__(
        ...
        upload_url: str = "https://upload.gyazo.com",
        db_path: str = "gdb",
    ) -> None:
        """Constructor.
        ...
        :param db_path: (optional) KVS directory path
                           (default: './gdb')
        """
    ...
    def get_oembed(self, url: str) -> Dict[str, Any]:
        ...
        _, result = self._parse_and_check(response)  # type: Tuple[Any, Dict[str, Any]]
        return result
    ...
    ### My Inplementation ###
    def sync(self):
        """Sync images from Gyazo API to local KVS."""
        page = 0
        while True:
            page += 1
            images = self.get_image_list(page=page, per_page=100)
            if len(images) == 0:
                break
            with ProcessPoolExecutor(os.cpu_count()) as executor:
                executor.map(self.flash, images)
            if images.has_next_page is False:
                break

    def flash(self, image):
        value = gzip.compress(dill.dumps(image))
        with Path(self.db_path, image.image_id).open(mode="wb") as f:
            f.write(value)

    def is_exists(self, key):
        if Path(self.db_path, key).exists():
            return True
        else:
            return False

    def keys(self):
        return [key.name for key in Path(self.db_path).glob("*")]

    def read(self, key):
        if self.is_exists(key) is False:
            return None
        with Path(self.db_path, key).open(mode="rb") as f:
            value = f.read()
        image = dill.loads(gzip.decompress(value))
        return image
