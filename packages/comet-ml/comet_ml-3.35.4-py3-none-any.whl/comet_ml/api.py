...
from . import logging_messages, messages
    ...
    def log_parameters(
        ...
        if nested_support and exclusions.can_be_flattened(
            ...
            if flatten_op_result.max_depth_limit_reached:
                LOGGER.warning(
                    logging_messages.LOG_PARAMS_MAX_DEPTH_REACHED, param_dict
                )
...
class API(object):
    ...
    * API.get_experiments(WORKSPACE, PROJECT_NAME, PATTERN): get all APIExperiments in WORKSPACE/PROJECT/PATTERN
    ...
    def get_panel_options(self):
        ...
        """
        # type: () -> Dict[str, Any]
        options_str = os.environ.get("COMET_PANEL_OPTIONS", "{}")
    ...
    def get_panel_experiments(self):
        ...
        experiment_keys = json.loads(os.environ.get("COMET_EXPERIMENT_KEYS", "[]"))
        if experiment_keys:
            return [
            ...
            if workspace and project_name:
                return self.get_experiments(
                    get_config("comet.workspace"), get_config("comet.project_name")
                )
