...
class MakeModel(object):
    ...
    def model_main(self,feature_list=[]):     
        ...
        self.LR = sm.Logit(y, X).fit()
        summary = self.LR.summary()
        pvals = self.LR.pvalues
        pvals = pvals.to_dict()
        self.train_data['predicted'] = self.LR.predict(X.astype(float))
        # 处理原始测试集
        self.val_data=self.deal_raw_data(self.val_data)
        X_val = self.val_data[self.feature_list]
        X_val['intercept'] = [1]*X_val.shape[0]
        self.val_data['predicted']= self.LR.predict(X_val.astype(float))
    ...
    def model_predict(self,input_data):    
        ...
        obj = dir(self)
        if 'LR' in obj:
            input_data=self.deal_raw_data(input_data)
            input_data = input_data[self.feature_list]
            input_data['intercept'] = [1]*input_data.shape[0]
            input_data['predicted']= self.LR.predict(input_data.astype(float)) 
            self.output_data=input_data                       
        else:
            print('需要先执行model_main函数，产生预测模型文件')      
if __name__ == '__main__':
    # data = pd.read_csv(r"F:\论文-风控\train.csv")  #读取训练集
    # m=MakeModel(data,"id","isDefault",0.02,5,drop_value=["issueDate","earliesCreditLine"])
    # m.model_main()
    # print(m)
    df = pd.read_csv(r'G:\cc_approvals.data', sep=",", header=None)
    df=df.reset_index(drop=False)
    
    m=MakeModel(df,"index","Class",0.02,5,drop_value=[])
    m.model_main()
    m.model_predict(df)   
