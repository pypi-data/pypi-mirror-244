...
def test_inner_import(base_dag, tmp_out_dir, fp_op_with_defaults):
    ...
    assert (
        ...
        in (
            ...
            / "2023-01-29"
            / "simple_test_dag__first_task__manual__2023-01-29T00:00:00+00:00"
            / "simple_test_dag__first_task__manual__2023-01-29T00:00:00+00:00__inner_nb.ipynb"
        ).read_text()
    )
...
def test_inter_dagrun_communication(base_dag, tmp_out_dir, tmp_shared_data_dir, fp_op_with_defaults):
    fp_op_with_defaults(
        dag=base_dag, task_id="inter_dag_comms_task", notebook_name="inter_dagrun_nb.ipynb"
    )
    base_dag.test(execution_date=EXECUTION_DATE)
    ...
    out_path = (
            ...
            / "simple_test_dag__inter_dag_comms_task__manual__2023-02-28T00:00:00+00:00__inter_dagrun_nb.ipynb"
    )
    assert "DEADBEEF" in out_path.read_text()
