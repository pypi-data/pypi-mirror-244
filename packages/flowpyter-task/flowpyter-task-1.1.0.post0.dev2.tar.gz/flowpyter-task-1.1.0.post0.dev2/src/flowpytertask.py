...
class PapermillOperator(DockerOperator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._reserved_param_check(nb_params, host_dagrun_data_dir, host_shared_data_dir, read_only_mounts)
    ...
    @staticmethod
    def _reserved_param_check(nb_params, dagrun_data_dir, shared_data_dir, read_only_mounts):
        reserved_params = set(read_only_mounts.keys())
        if dagrun_data_dir:
            if "dagrun_data_dir" in reserved_params:
                raise ReservedMountError(
                    "dagrun_data_dir is already mounted as a writeable mount - please"
                    " use a different mount name"
                )
            reserved_params = reserved_params.union({"dagrun_data_dir"})
        if shared_data_dir:
            if "shared_data_dir" in reserved_params:
                raise ReservedMountError(
                    "shared_data_dir is already mounted as a writeable mount - please"
                    " use a different mount name"
                )
            reserved_params = reserved_params.union({"shared_data_dir"})
    ...
    def _setup_mounts(self):
        ...
        if self.host_dagrun_data_dir:
            ...
            self.nb_params["dagrun_data_dir"] = self.CONTAINER_DAGRUN_DATA_DIR
        ...
        if self.host_shared_data_dir:
            ...
            self.nb_params["shared_data_dir"] = self.CONTAINER_SHARED_DATA_DIR
    ...
    def _setup_notebook_paths(
        ...
        self.container_notebook_out_name = (
            "{{dag.dag_id}}__{{task_instance.task_id}}__{{run_id}}__" f"{Path(notebook_name).name}"
        )
    ...
    def _setup_dagrun_data_dir(self, host_dagrun_data_dir):
        ...
        self.host_dagrun_data_dir = f"{self.host_data_root}/{self.host_run_dagrun_data_dir}"
