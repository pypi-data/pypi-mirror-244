from typing import Any, Dict, Type, TypeVar, Union

import attr

from ..types import UNSET, Unset

T = TypeVar("T", bound="CostCentre")

@attr.s(auto_attribs=True)
class CostCentre:
    """
    Attributes:
        code (str): The unique code for this Cost Centre
        title (str): The name of this Cost Centre
        color (Union[Unset, None, str]): A color to used to represent this Cost Centre, in hex format. ie 'ff0000'
        accounting_code (Union[Unset, None, str]):
        employee_count (Union[Unset, int]): [readonly] The number of employees with this set as their primary cost
            centre
    """

    code: str
    title: str
    color: Union[Unset, None, str] = UNSET
    accounting_code: Union[Unset, None, str] = UNSET
    employee_count: Union[Unset, int] = UNSET


    def to_dict(self) -> Dict[str, Any]:
        code = self.code
        title = self.title
        color = self.color
        accounting_code = self.accounting_code
        employee_count = self.employee_count

        field_dict: Dict[str, Any] = {}
        field_dict.update({
            "code": code,
            "title": title,
        })
        if color is not UNSET:
            field_dict["color"] = color
        if accounting_code is not UNSET:
            field_dict["accountingCode"] = accounting_code
        if employee_count is not UNSET:
            field_dict["employeeCount"] = employee_count

        return field_dict



    @classmethod
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        d = src_dict.copy()
        code = d.pop("code")

        title = d.pop("title")

        color = d.pop("color", UNSET)

        accounting_code = d.pop("accountingCode", UNSET)

        employee_count = d.pop("employeeCount", UNSET)

        cost_centre = cls(
            code=code,
            title=title,
            color=color,
            accounting_code=accounting_code,
            employee_count=employee_count,
        )

        return cost_centre

