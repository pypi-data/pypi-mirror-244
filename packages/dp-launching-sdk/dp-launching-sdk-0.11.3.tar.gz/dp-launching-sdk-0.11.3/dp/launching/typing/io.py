...
class OutputCommandDirectory(str):
    def get_value(self):
        return str(self)

    @classmethod
    def __modify_schema__(cls, field_schema: Dict[str, Any]) -> None:
        field_schema.update(format="output_command_directory", scope="io")
    ...
    @classmethod
    def validate(cls, value: Any) -> "OutputCommandDirectory":
        if isinstance(value, OutputCommandDirectory):
            return value
        elif isinstance(value, str):
            return OutputCommandDirectory(value)
        elif isinstance(value, (bytes, bytearray, memoryview)):
            return OutputCommandDirectory(base64.b64encode(value).decode())
        else:
            raise Exception(f"OutputCommandDirectory Wrong type: {type(value)}")


class OutputCommandScript(str):
    def get_value(self):
        return str(self)

    @classmethod
    def __modify_schema__(cls, field_schema: Dict[str, Any]) -> None:
        field_schema.update(format="output_command_script", scope="io")
    ...
    @classmethod
    def validate(cls, value: Any) -> "OutputCommandScript":
        if isinstance(value, OutputCommandScript):
            return value
        elif isinstance(value, str):
            return OutputCommandScript(value)
        elif isinstance(value, (bytes, bytearray, memoryview)):
            return OutputCommandScript(base64.b64encode(value).decode())
        else:
            raise Exception(f"OutputCommandScript Wrong type: {type(value)}")
