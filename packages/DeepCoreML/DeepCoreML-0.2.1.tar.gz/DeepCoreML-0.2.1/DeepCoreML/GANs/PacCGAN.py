...
class PackedDiscriminator(nn.Module):
    ...
    def __init__(self, architecture=(128, 128), input_dim=2, pac=10, p=0.2, negative_slope=0.2):
        """
        ...
        for lay in architecture:
            seq += [nn.Linear(dim, lay), nn.Dropout(p=p), nn.LeakyReLU(negative_slope=negative_slope)]
            dim = lay
...
class Generator(nn.Module):
    ...
    """
    def __init__(self, architecture=(128, 128), input_dim=2, output_dim=2, normalize=False, negative_slope=0.2):
        """
        ...
        def residual_block(in_dim, out_dim, norm, slope):
            ...
            if norm:
                # Applying Batch Normalization before the activation function is harmful
                layers.append(nn.BatchNorm1d(out_dim, eps=1e-5, momentum=0.1))

            layers.append(nn.LeakyReLU(negative_slope=slope))

            # Applying Batch Normalization after the activation function is also harmful
            # layers.append(nn.BatchNorm1d(output_dim, eps=1e-5, momentum=0.1))
        ...
        dim = input_dim
        for lay in architecture:
            # Batch normalization is harmful; LeakyReLU is better than ReLU
            # seq += [nn.Linear(dim, l),  nn.BatchNorm1d(l), nn.LeakyReLU() ]
            seq += [*residual_block(dim, lay, normalize, negative_slope)]
            dim = lay
...
class PacCGAN(BaseGenerator):
    ...
    A Packed Conditional GAN (PacCGAN) conditionally generates data from a specific class and also accepts "packed"
    input samples. Therefore, PacCGAN uses a Packed Discriminator to prevent the model from mode collapsing.
    """

    def __init__(self, discriminator=(128, 128), generator=(256, 256), pac=10, random_state=0):
        """
        ...
        """
        super().__init__(discriminator, generator, random_state)
    ...
    def train(self, x_train, y_train, epochs=1000, batch_size=64, lr=0.001, loss_function=nn.BCELoss()):
        """
        ...
        self.G_ = Generator(self.G_Arch_, input_dim=self.dim_ + self.n_classes_, output_dim=self.dim_,
                            normalize=False).to(self.device_)
    ...
    def adaptive_train(self, x_train, y_train, clf, epochs=1000, batch_size=64, lr=0.001, loss_function=nn.BCELoss(),
        ...
        """
        Adaptive training by evaluating the quality of generated data during each epoch. The quality is quantified by:

        * measuring the accuracy of a classifier `clf` and/or
        * the KL divergence between the original and generated data from the original data.
        ...
        generated_data = [[None for i in range(self.n_classes_)] for j in range(epochs)]
        ...
        for epoch in range(epochs):
            ...
            for n, real_data in enumerate(train_dataloader):
                temp_disc_loss, temp_gen_loss = self.train_batch(real_data, loss_function)
                if temp_gen_loss > 0:
