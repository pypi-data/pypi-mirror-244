...
class DataHandler(object):
    ...
    def predict_with_single_epoch(self, eeg, acc, clf, cached_feature_arr):
        ...
        features = RSCFeature(meta_feature, raw_eeg=eeg, raw_acc=acc,
        ...
        cached_feature = None
        if cached_feature_arr is None or len(cached_feature_arr) % 60 != 0:
            cached_feature = features
        else:
            cached_feature = pd.DataFrame(columns=features.columns)
            cached_feature_count = (len(cached_feature_arr) // 60)

            for i in range(cached_feature_count):
                new_df = pd.DataFrame([cached_feature_arr[i*raw_feature_count:(i+1)*raw_feature_count]], columns=features.columns)
        ...
        if len(cached_feature) > 8:
            cached_feature = cached_feature.iloc[1:]

        rollp = cached_feature.rolling(window=8, min_periods=1).mean()
    ...
    def plot_sw_stim_sham(self, savefig=None):
        ...
        plot_stim_sham_sw(self.device_type, eeg_copy, self.raw_sti, self.sf_eeg, savefig)
