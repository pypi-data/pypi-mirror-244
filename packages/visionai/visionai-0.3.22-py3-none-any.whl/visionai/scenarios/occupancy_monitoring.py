...
class OccupancyMonitoring(Scenario):
    ...
    def draw_boxes(self, img, bbox, id, count, total_time, offset=(0, 0)):
        for i, box in enumerate(bbox):
            ...
            label = f"ID: {str(id)} - Duration: {str(int(total_time))}"
            ...
            [255, 255, 255], 1)
            cv2.putText(img, "Count: "+str(count), (30, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.6, 
            [255,191,0], 2)
    ...
    def start(self, camera_name=0, test = False):
        ...
        sort_min_hits = 2
        sort_iou_thresh = 0.2
        sort_tracker = Track(max_age=sort_max_age,min_hits=sort_min_hits,iou_threshold=sort_iou_thresh)
                ...
                if len(tracked_dets)>0:
                    ...
                    for i in range(len(identities)):
                        ...
                        if id in info_dic:
                            ...
                            self.draw_boxes(frame, [bbox], info_dic[id]['id'], count, info_dic[id]['duration'])  
                            ...
                            info_dic[id] = {"id": int(id), "frames": 1, "duration": 0, "bbox": list(bbox)}
                            self.draw_boxes(frame, [bbox], info_dic[id]['id'], count, info_dic[id]['duration'])                 
