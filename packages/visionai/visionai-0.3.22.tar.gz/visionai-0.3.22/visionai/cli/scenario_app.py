...
def scenario_download(
    ...
    if name.lower() == 'world':
        ...
        for scen in all_scenarios:
            scen_name = scen['name']
            if 'model_url'  in scen['models']['latest']:
                scen_url = scen['models']['latest']['model_url']
                if scen_url is None:
                    continue
...
def scenario_test(
    ...
    for model_item in model_list:
        print(model_name)
        if model_name == 'yolov5s-ergonomics-detection':
            if model_item['name'] in model_name:
                model_to_test_available = True
                break
        elif model_name == 'yolov5-ppe-detection':
            if model_item['name'] in model_name:
                model_to_test_available = True
                break
        elif model_name == 'yolov5s-smoking-detection':
            if model_item['name'] in model_name:
                model_to_test_available = True
                break
        else:
    ...
    if str(camera) == '0':
        ...
        for cam in config_data['cameras']:
            if cam['name'] == camera:
                camera_exist = True
                if '-i' in cam['videoConfig']['source']:
                    rtsp = cam['videoConfig']['source'].split('-i')[1].strip()
                break
                
        if camera_exist is False:
            print(f'ERROR: Camera {camera} not found.')
            raise typer.Exit()   
        else: 
            inference_engine.start(rtsp)
