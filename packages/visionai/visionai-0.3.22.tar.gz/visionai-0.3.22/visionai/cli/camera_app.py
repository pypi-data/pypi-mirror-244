...
import datetime
from visionai.apiutils.api import Api
from visionai.util.camera_util import common_option
api = Api()
...
def camera_add(
    ...
    camera = {
        ...
        "motionTimeout": 15,
        "recordOnMovement": None,
        "prebuffering": None,
        "videoConfig": {
            "source": f"-i {uri}",
            "stillImageSource": f"-i {uri}",
            "stimeout": 10,
            "audio": None,
            "debug": None,
            "subSource": f"-i {uri}",
    ...
    config_data['cameras'].append(camera)
