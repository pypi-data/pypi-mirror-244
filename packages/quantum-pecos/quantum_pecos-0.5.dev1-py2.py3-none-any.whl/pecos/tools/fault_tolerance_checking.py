...
def find_pauli_fault(qcirc: QuantumCircuit,
                     wt: int,
                     fail_func: Callable,
                     num_qubits: Optional[int]=None,
                     simulator: str = 'stabilizer',
                     verbose: bool = True,
                     failure_break: bool = True) -> List[Union[dict, Tuple[dict, dict]]]:
    """
...
def get_all_spacetime(qcirc: QuantumCircuit,
                      initial_qubits: Optional[Sequence[int]] = None):
    ...
    for gates, tick, _ in qcirc.iter_ticks():
        ...
        for sym, locations, metadata in gates.items():
            ...
            for loc in locations:
                ...
                yield {'tick': tick, 'location': loc, 'before': sym.startswith('meas'), 'symbol': sym,
                       'metadata': metadata}


def get_wt_paulis(circ: QuantumCircuit,
                  wt: int,
                  intial_qubits: Optional[Sequence[int]] = None,
                  make_qc: bool = True):
    """
    ...
    for gate_data in permutations(get_all_spacetime(circ, intial_qubits), wt):
        ...
        for pauli_errs in product(*iter_list):
            ...
            for errs, tick, locs, before, cond in zip(pauli_errs, tick_list, loc_list, before_list, cond_list):
                # print(errs, tick, locs, after)

                if before:
                    tick_dict = tick_dict_before
                else:
                    tick_dict = tick_dict_after
