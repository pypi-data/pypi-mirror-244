...
function test_files_are_the_same {
    # This is the source (if copy) or the target (if symlink):
    primary_path="${1}"
    # This is the target (if copy) or the link (if symlink):
    secondary_path="${2}"

    # Makes sure (whatever links used in paths) both point to the same "physical" file (inode).
    test "${primary_path}" -ef "${secondary_path}"
}
...
function detect_file_deployment_command {
    ...
    then
        ...
        # Do not verify copied files - they are normally under version control by target project:
        verify_deployment_command="echo"
    else
        # TODO_82_25_90_46: Fix: this does not override existing links.
        file_deployment_command="ln -snf"
        verify_deployment_command="test_files_are_the_same"
    fi

    eval "${file_deployment_command}" "${primary_path}" "${secondary_path}"
    eval "${verify_deployment_command}" "${primary_path}" "${secondary_path}"
}
...
function deploy_files_procedure {
    ...
    case "${deployment_mode}" in
        "symlink")
            # TODO_82_25_90_46: Fix: this does not override existing links. What was the reason why "ln -snf" did not work?
...
python_module_path_EOF
            ...
            then
                ...
                eval "${verify_deployment_command}" "${config_file_path}" "${target_dir}/${file_name}"
            fi
        fi
    done
}
