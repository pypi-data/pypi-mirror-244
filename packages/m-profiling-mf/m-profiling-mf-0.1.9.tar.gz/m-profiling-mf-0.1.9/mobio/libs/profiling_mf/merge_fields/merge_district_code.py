...
class MergeDistrictCode(BaseMerge):
    def serialize_data(
        self,
        suggest_data,
        profile_data,
    ...
    def merge_data(self, target_data, source_data, is_master_data=False):
        if source_data:
            ...
            #     lst_district = df_get_district_data()
            #     district_code = next(
            #         (x for x in lst_district if x.get("id") == value), None
            #     )
            #     if district_code:
            #         target_data[self.field_key] = ProfileEmbedded.create_simple_data_type(
            #             _id=district_code.get("id"), _name=district_code.get("name")
            #         )
