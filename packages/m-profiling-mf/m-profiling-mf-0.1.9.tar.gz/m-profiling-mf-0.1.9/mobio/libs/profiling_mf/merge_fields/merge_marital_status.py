...
from mobio.libs.profiling_mf.profiling_data.marital_status_data import df_get_marital_status_data


class MergeMaritalStatus(BaseMerge):
    def serialize_data(
        self,
        suggest_data,
        profile_data,
    ...
    def merge_data(self, target_data, source_data, is_master_data=False):
        if source_data:
            ...
            if suggest and value is not None:
                try:
                    marital_status_code = int(value)
                except Exception as ex:
                    print(
                        "merge_data: marital_status_code ERROR: {}".format(ex)
                    )
                    marital_status_code = -1
                lst_marital_status = df_get_marital_status_data()
                if lst_marital_status:
                    marital_status = next(
                        (
                            CommonHelper.create_simple_data_type(
                                _id=x["id"], _name=x["name"]
                            )
                            for x in lst_marital_status
                            if x["id"] == marital_status_code
                        ),
                        None,
                    )
                    if marital_status:
