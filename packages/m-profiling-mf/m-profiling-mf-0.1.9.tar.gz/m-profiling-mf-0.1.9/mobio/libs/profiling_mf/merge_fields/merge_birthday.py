...
class MergeBirthday(BaseMerge):
    def serialize_data(
        ...
        field_value = self.__build_value__(value=profile_data, suggest=suggest_birthday, predict=predict)
    ...
    def merge_data(self, target_data, source_data, is_master_data=False):
        if source_data:
            ...
            if suggest and value is not None:
                birthday = value if type(value) == datetime else CommonHelper.parse_time(value)
                if (
                        type(birthday) == datetime
                        and birthday <= datetime.utcnow()
                        and birthday.year > 1900
                ):
                    target_data[self.field_key] = birthday
