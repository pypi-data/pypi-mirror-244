...
class MergeProfileId(BaseMerge):
    def serialize_data(
        self,
        suggest_data,
        profile_data,
        ...
        field_value = self.__build_value__(value=str(profile_data), suggest=suggest)
    ...
    def serialize_origin_data(
        ...
        if suggest:
            set_suggest_fields.add(self.field_key)
        field_value = self.__build_value__(value=str(origin_data), suggest=suggest)
        suggest_data[self.field_key] = self.build_merge_data(
            translate_key=translate_key,
            field_property=field_property,
            display_type=display_type,
