...
def collect_information(bin_precision,bin_width,percent,pre_result,result,cut,keep_best_feature,keep_best_group):
	result_folder='mass_align_all_information'
	if not os.path.exists(result_folder):
		os.mkdir(result_folder)

	total_number=len(list(result.keys()))
	n=0
	for mass_name in list(result.keys()):
		n=n+1
		window=mass_name.split('.csv')[0]
		print('step_6: first combining',n,window,total_number)
		file_df=result[mass_name]
		if n==1:
	...
	for mass_name,group in grouped:
		...
		mass_name_df=pre_result[mass_name]
		all_index=[]
		group_list={}
		group_sample_list={}
		score_list={}
