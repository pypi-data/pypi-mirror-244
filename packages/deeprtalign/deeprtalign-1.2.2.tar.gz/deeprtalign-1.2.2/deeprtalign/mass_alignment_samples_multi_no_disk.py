...
def mass_alignment(mass_name,mass_df,total_fraction_number,total_sample_number,max_mz,max_time,max_log_intensity,max_sample_number,percent):
...
def run_alignment(processing_number,max_mz,max_time,max_log_intensity,percent,total_fraction_number,total_sample_number,max_sample_number,pre_result):
	pd.set_option('mode.chained_assignment', None)
	...
	for mass_name in pre_result.keys():
		...
		file_arg.append(max_sample_number)
	...
	n=1000
	m=0
	total_pool_number=len(pool_arg)
	while len(pool_arg)>n:
		...
		pool.close()
		pool.join()
		print('step_5:',str(m*n),'/',total_pool_number,'finish')
