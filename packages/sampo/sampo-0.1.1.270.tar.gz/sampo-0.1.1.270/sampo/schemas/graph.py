...
class WorkGraph(JSONSerializable['WorkGraph']):
    ...
    def to_frame(self) -> pandas.DataFrame:
        ...
        for node in self.nodes[1:]:
            ...
            graph_df_structure['predecessor_ids'].append(
                ','.join([str(predecessor_info[0]) for predecessor_info in node_info_dict['parent_edges']]))
            graph_df_structure['lags'].append(
                ','.join([str(predecessor_info[1]) for predecessor_info in node_info_dict['parent_edges']]))
            graph_df_structure['connection_types'].append(
                ','.join([str(predecessor_info[2]) for predecessor_info in node_info_dict['parent_edges']]))

        return pandas.DataFrame.from_dict(graph_df_structure)
