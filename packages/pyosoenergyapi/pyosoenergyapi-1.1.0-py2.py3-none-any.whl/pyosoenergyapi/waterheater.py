...
class OSOWaterHeater:
    ...
    def get_heater_state(self, device: OSOEnergyWaterHeaterData):
    ...
    def turn_on(self, device: OSOEnergyWaterHeaterData, full_utilization: bool):
        """Turn device on.
    ...
    def turn_off(self, device: OSOEnergyWaterHeaterData, full_utilization: bool):
        """Turn device off.
    ...
    def set_v40_min(self, device: OSOEnergyWaterHeaterData, v40min: float):
    ...
    def set_optimization_mode(self, device: OSOEnergyWaterHeaterData, option: Number, sub_option: Number):
    ...
    def set_profile(self, device: OSOEnergyWaterHeaterData, profile: array):
        """Set heater profile.
...
class WaterHeater(OSOWaterHeater):
    ...
    def get_water_heater(self, device: OSOEnergyWaterHeaterData) -> OSOEnergyWaterHeaterData:
        ...
        """
        device.online = self.session.attr.online_offline(device.device_id)
        if(device.online):
            ...
            dev_data.current_operation = self.get_heater_state(device)

            attributes = self.session.attr.state_attributes(device.device_id)
