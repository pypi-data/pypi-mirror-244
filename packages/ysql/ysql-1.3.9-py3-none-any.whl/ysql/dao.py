...
FETCH_FLAG = 'limit 1'  # 在sql语句中包含此字符，则直接返回Record，不再返回列表形式。
...
def Dao(entity):
    ...
    def decorator(cls):
        ...
        # 新增批量插入方法
        setattr(cls, "insert_many", insert_many)
        return cls
...
def Sql(static_sql_statement: str):
    ...
    def decorator(func):  # noqa
        ...
        def wrapper(self, *args, **kwargs):
            ...
            log.debug(f"生成的执行sql:{sql_statement}")

            self.cursor.execute(sql_statement, (*args, *kwargs.values()))
            # 确定返回类型
            if FETCH_FLAG not in sql_statement.lower():
...
def InsertMany(func):  # noqa
    """相较于Insert装饰器，提供了批量插入数据的功能。

    Returns:
        None, 相较于Insert装饰器，不再返回刚插入记录的自增主键。
    ...
    Example:
        ...
        dao.insert_many(entities=students)  # 将整条记录以数据类的形式插入数据库，避免了同时使用多个参数的麻烦。

    """

    def wrapper(self, entities):
        if not isinstance(entities, Iterable):
            raise TypeError(
                f"InsertMany的被装饰函数传入的参数必须是可迭代的，'iterable[your_entity]'"
                f"但得到的是 {type(entities).__name__}")
...
@InsertMany
def insert_many(self, entities):  # noqa
    """批量插入"""
    pass
