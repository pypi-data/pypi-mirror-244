...
class NegtiveInf(object):
    ...
    def __eq__(self, other):
        return other is self or isinstance(other, NegtiveInf)
...
class PostiveInf(object):
    ...
    def __eq__(self, other):
        return self is other or isinstance(other, PostiveInf)
...
class IntervalSet(object):
    ...
    def __init__(self, interval_arrays: List[Union[list, Any]] = None, /, *, merge: bool = True):
        """
    ...
    def inf_tvs(cls) -> 'IntervalSet':
        ...
        :return:
        """
        return cls([[NEG_INF, POS_INF, False, False, 0]], merge=False)
    ...
    def _calc_merge(cls, merged_intervals: List[list], new: list, ):
        ...
        if new[0] == cu[0] and not cu[2] and new[2]:
            ...
            """
            if len(merged_intervals) > 1 and merged_intervals[-2][1] == cu[0]:
                ...
                cu[0] = merged_intervals[-2][0]
                cu[2] = merged_intervals[-2][2]
    ...
    def union(self, *tvs_lis: 'IntervalSet', ) -> 'IntervalSet':
        """Return union of several interval sets.
    ...
    def _calc_union(cls, interval_arrays_1: List[list], interval_arrays_2: List[list], ) \
        ...
        i, j = 0, 0
        arrays_unionised = []
        cu, new1, new2 = [NEG_INF, NEG_INF, False, False], None, None
        while True:
            ...
            if cu[0] == new[0]:
                ...
                cu[2] = cu[2] or new[2]
                if cu[2] and arrays_unionised and arrays_unionised[-1][1] == cu[0]:
                    # 触发连接
                    cu[0] = arrays_unionised[-1][0]
                    cu[2] = arrays_unionised[-1][2]
                    del arrays_unionised[-1]
        ...
        if remain and cu[3] and cu[1] == remain[0]:
            # cu 和 remain[0] 合并
            cu[1] = remain[1]
            cu[3] = remain[3]
            del remain[0]
