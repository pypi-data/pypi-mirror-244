...
from azureml.data._dataset_rest_helper import _dto_to_dataset, _dataset_to_dto, _dataset_to_saved_dataset_dto, \
    _saved_dataset_dto_to_dataset, _restclient, _dto_to_registration, _custom_headers, \
    _make_request, _fetch_v2_data_asset
...
class AbstractDataset(object):
    ...
    def _get_partition_key_values(self, get_common_path=False, partition_keys=None):
        ...
        if not get_common_path:
            ...
            dataflow = dataflow._remove_data_read_steps_and_drop_columns(True)
    ...
    def _get_by_name(workspace, name, version):
        ...
        def get_by_name_v2_request():
            return _fetch_v2_data_asset(workspace=workspace, name=name, version=version)
        ...
        if not success:
            # check to see if this is a v2 data asset
            v2_success, v2_result = _make_request(get_by_name_v2_request, handle_error)
    ...
    def __repr__(self):
        ...
        if is_dataprep_installed():
            try:
                ...
                encoder = None
                # this means dataflow has no steps, fall back to it's own __repr__ logic
                content['definition'] = self._dataflow.__repr__()
        else:
            encoder = None
    ...
    def partition_keys(self):
        ...
        partition_format = self._dataflow._get_partition_format()
        if partition_format:
            parsed_result = parse_partition_format(partition_format)
