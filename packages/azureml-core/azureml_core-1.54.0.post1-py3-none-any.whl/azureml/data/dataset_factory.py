...
class TabularDatasetFactory:
    ...
    @staticmethod
    @track(_get_logger, custom_dimensions={'app_name': 'TabularDataset'}, activity_type=_PUBLIC_API)
    def register_pandas_dataframe(dataframe,
                                  target,
                                  name,
                                  description=None,
                                  tags=None,
        ...
        :param row_group_size: Optional. Max size of row group to use when writing parquet file. Defaults to None.
        ...
        sanitized_df.to_parquet(file_path, row_group_size=row_group_size, engine='pyarrow')
        FileDatasetFactory._upload_directory(
            src_dir=temp_dir,
            target=(datastore, relative_path_with_guid),
    ...
    def register_dask_dataframe(dataframe, target, name, description=None, tags=None, show_progress=True):
        ...
        FileDatasetFactory._upload_directory(
            tempfolder,
            (datastore, relative_path_with_guid),
            None,
            True,
            show_progress)
...
class FileDatasetFactory:
    ...
    @staticmethod
    def _upload_directory(src_dir: str, target, pattern=None, overwrite=False, show_progress=True):
        ...
        src_dir = Path(src_dir).absolute().as_posix()
        console = get_progress_logger(show_progress)
        console("Validating arguments.")
        ...
        console("'overwrite' is set to True. Any file already present in the target will be overwritten." if overwrite
                else "'overwrite' is set to False. Any file already present in the target will be skipped.'")

        console("Uploading files from '{}' to '{}'".format(src_dir, relative_path))

        ensure_rslex_environment()

        destination_uri = "azureml://subscriptions/{}/resourcegroups/{}/workspaces/{}/datastores/{}/paths/{}".format(
        ...
        # if pattern is provided do pattern matching on src_dir in python and then upload matched files
        if pattern is not None:
            ...
            if len(stream_infos) == 0:
                console("No files found with pattern {}".format(pattern))
                return

            copier.copy_stream_infos(stream_infos, "", show_progress, False)
        else:
            volume_info = PyLocationInfo('Local', "{}/".format(src_dir.rstrip('/')), {})

            copier.copy_volume(volume_info, "", show_progress, False)
...
def _sanitize_pandas(df):
    ...
    # dropping index, as only data columns would be written out to parquet files.
