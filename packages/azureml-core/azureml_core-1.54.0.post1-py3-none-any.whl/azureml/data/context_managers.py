...
def _safe_mkdirs(dir_path):
    try:
        ...
        if not os.path.exists(dir_path):
            raise
        _get_logger().warning('directory to make already exists. skipped making it.')
...
def _upload_to_datastore(src_path, destination_uri, glob_patterns, overwrite, is_volume=False):
    from azureml.dataprep.rslex import Copier, PyLocationInfo, PyIfDestinationExists, StreamInfo
    from azureml.dataprep.api._rslex_executor import ensure_rslex_environment
    import os
    if os.environ.get('DATASET_RSLEX_UPLOAD', 'true').lower() == 'false':
        _get_logger().warning('Configuration is set to use CLEX upload, but it is not supported anymore, ignoring.')

        ensure_rslex_environment()

        destination_info = PyLocationInfo.from_uri(destination_uri)
        copier = Copier(
            destination_info,
            src_path,
        ...
        if is_volume:
            if glob_patterns and len(glob_patterns) > 0:
                _log_and_print(
                    f'Starting upload from: {src_path}, with glob patterns: {glob_patterns}, \
                    overwrite set to: {overwrite}')
                stream_infos = []
                from dataset_factory import _get_all_files_from_directory
                import fnmatch
                from pathlib import Path
                count = 0
                for file in _get_all_files_from_directory(src_path):
                    count += 1
                    resource_id = Path(file).absolute().as_posix()
                    for pattern in glob_patterns:
                        if fnmatch.fnmatch(resource_id, pattern):
                            stream_infos.append(StreamInfo('Local', resource_id, {}))
                            break
                if len(stream_infos) == 0:
                    _log_and_print(f'No files found under: {src_path} using glob patterns: {glob_patterns}')
                    return
                _log_and_print(
                    f'Filtered {count} files to {len(stream_infos)} files using glob patterns: {glob_patterns}')
                copier.copy_stream_infos(stream_infos, "", False, True)
            else:
                _log_and_print(f'Starting upload of all files under: {src_path}, overwrite set to: {overwrite}')
                volume_info = PyLocationInfo('Local', src_path, {})
                copier.copy_volume(volume_info, None)
        else:
            _log_and_print(f'Starting upload of a single file: {src_path}, overwrite set to: {overwrite}')
            source_stream_info = StreamInfo('Local', src_path, {})
            copier.copy_stream_info(source_stream_info, None)
...
class DatasetContextManager(_CommonContextManager):
        ...
        if asset_type and is_eval_mode:
            ...
            if self._is_download(data_configuration):
                ...
                try:
                    ...
                    exception_str = str(e)
                    if "InvalidUriScheme" in exception_str or "StreamError(NotFound)" in exception_str \
                            or "InvalidInput" in exception_str:
                        raise UserErrorException(exception_str)
                    else:
                        raise AzureMLException(exception_str)
                action = "Downloaded"
    ...
    def _upload(self, name, config):
        ...
        single_file_name = self.__class__._get_file_name(relative_path) if is_single_file else None

        # single file output handling
        upload_destination_uri = self._construct_datastore_uri(
            datastore,
            os.path.dirname(relative_path) if is_single_file else relative_path)
        ...
        try:
            ...
            if asset_type is None:
                _upload_to_datastore(src_path, upload_destination_uri, globs, overwrite, not is_single_file)
    ...
    def _is_single_file_no_transform(dataset):
        ...
        original_path = dataset._dataflow._get_path_from_step(first_step.step_type, argument)
                    ...
                    if self._is_datastore_adlsgen1(ds):
                        ...
                        destination_uri = \
                            "azureml://subscriptions/{}/resourcegroups/{}/workspaces/{}/datastores/{}/paths/{}".format(
                                ...
                                self._workspace.resource_group,
                                self._workspace.name,
                                df_config.data_store_name,
                                df_config.path_on_data_store.lstrip("/"))
