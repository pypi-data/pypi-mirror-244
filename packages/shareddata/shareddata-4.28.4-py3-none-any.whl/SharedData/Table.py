...
class Table:
    ...
    def subscribe(self, host, port, lookbacklines=1000, lookbackdate=None):
        if self.subscription_thread is None:
            self.subscription_thread = threading.Thread(
                target=ClientSocket.table_subscribe_thread,
                args=(self, host, port, lookbacklines, lookbackdate),
            )
            self.subscription_thread.start()
            Logger.log.info('Subscription to %s:%s -> %s started...' % (host,str(port),self.relpath))
        else:
