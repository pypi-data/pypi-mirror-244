    ...
    def _get_ims_token(self, new_token=False, store_token=False):
        ...
        if store_token:
            ...
            if not self.auth_token:
                try:
                    auth_token_kvp = st2_client.keys.get_by_name(name='ipass_auth_token', decrypt=True)
                    if auth_token_kvp:
        ...
        if new_token or not self.auth_token:
            ...
            if "access_token" in ims_response:
                ...
                if store_token:
                    from st2client.models import KeyValuePair
                    st2_client.keys.update(KeyValuePair(name='ipass_auth_token', value=self.auth_token, secret=True))
            else:
