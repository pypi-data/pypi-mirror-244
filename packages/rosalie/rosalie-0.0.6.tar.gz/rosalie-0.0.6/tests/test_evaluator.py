...
def evaluator_instance(df_sample):
    return Evaluator(df_sample, baseline_evaluator="wls")
...
def test_preprocessor_names(evaluator_instance):
    assert set(evaluator_instance._preprocessor_names()) == {"_empty_preprocessor"}


def test_empty_preprocessor(evaluator_instance, df_sample):
    assert evaluator_instance._empty_preprocessor(df_sample).equals(df_sample)
...
def test_add_treatment_effect(evaluator_instance, df_sample):
    ...
    df_effect = evaluator_instance._add_treatment_effect(df_assigned, "y", 10)
...
def test_missing_timeframe_column():
    ...
    with pytest.raises(AttributeError):
        Evaluator(df, sample_timestamps=True)
