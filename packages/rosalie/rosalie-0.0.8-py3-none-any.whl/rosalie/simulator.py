...
from .utils.evaluators import welch_t_test, wls
...
class Simulator:
    ...
    evaluators : list of callables (optional)
        List of evaluators to use, default is None. If no evaluators are specified, then a baseline evaluator must be specified.
    preprocessors : list of callables (optional)
        List of preprocessors to use, default is None, in which case passed data is not processed before being passed to evaluators.
    baseline_evaluator : string (optional)
        Name of baseline evaluator to use, default is None. Select one of ['welch', 'wls'].
    ...
    def __init__(
        ...
        baseline_evaluator: Optional[str] = None,
        ...
        num_runs: Optional[int] = 20,
        random_seed: Optional[int] = 2312,
        mdes: Optional[List[float]] = [0.02],
    ...
    def _check_input(self):
        ...
        evaluator_supplied(self.evaluators, self.baseline_evaluator)
    ...
    def _set_defaults(self):
        ...
        if self.baseline_evaluator is not None:
            self._add_baseline_evaluator()

    def _add_baseline_evaluator(self):
        """Add baseline evaluator if specified."""
        evaluators = {
            "welch": welch_t_test,
            "wls": wls,
        }
