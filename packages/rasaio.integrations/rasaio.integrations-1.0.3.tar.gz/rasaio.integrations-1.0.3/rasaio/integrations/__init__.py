...
class IntegrationBase(abc.ABC):
  ...
  def __get_persons_to_create_or_update(self):
    ...
    for mapped_person in mapped_persons:
      ...
      if not rasa_person.is_subscribed and rasa_person.unsubscribe_reason == ONE_CLICK_UNSUBSCRIBE and self.rasa_api_attrs.sync_direction == SyncDirection.TO_RASA:
