...
import pandas as pd
try:
    from IPython.display import display, update_display, Javascript, HTML, clear_output
...
def create_connection_context(original_connection_context: ConnectionContext) -> ConnectionContext:
    ...
    try:
        if original_connection_context.userkey:
            ...
            conn = ConnectionContext(url, port, user, password="", sslKeyStore=original_connection_context.sslKeyStore)
        else:
            conn = ConnectionContext(url, port, user, password)
    except:
        if original_connection_context.userkey:
            ...
            conn = ConnectionContext(url, port, user, password="", sslKeyStore=original_connection_context.sslKeyStore, encrypt='true', sslValidateCertificate='false')
        else:
...
class AutomaticObjProxy(object):
    ...
    def create_fetch_table_task(self, task_manager, fetch_table_interval):
        return FetchProgressStatusFromSystemTableTask(task_manager, fetch_table_interval, self.connection_context)
...
class TaskManager(threading.Thread):
    def __init__(self, automatic_obj_proxy: AutomaticObjProxy, update_ui_interval, fetch_table_interval, runtime_platform):
        ...
        self.should_print_special_log = False
    ...
    def run(self):
        ...
        if self.should_print_special_log:
            print('task.start: {}: {}'.format(self.update_to_ui_task.tempdir_path + os.sep, self.update_to_ui_task.frame_id))
        ...
        while True:
            ...
            if self.is_cancelling():
                ...
                self.set_cancelled()
                if self.should_print_special_log:
                    print('task.cancel: {}'.format(self.update_to_ui_task.frame_id))
                break
...
class UpdateProgressStatusToUITask(object):
    ...
    def __init__(self, manager: TaskManager, update_ui_interval, runtime_platform):
        ...
        self.tempdir_path = get_tempdir_path()
        self.frame_file_path = self.tempdir_path + os.sep + self.frame_id

        # if self.is_jupyter_platform is False:
        #     print('In order to cancel the monitor task on the BAS or VSCode platform, you must import the VSCode extension package manually.')
        #     extension_path = self.tempdir_path.replace('templates' + os.sep + 'temp', 'templates' + os.sep + 'hanamlapi-monitor-1.0.0.vsix')
...
class UpdateProgressStatusToConsoleTask(object):
    def __init__(self, manager: TaskManager, update_ui_interval):
        ...
        self.progress_id = self.manager.automatic_obj_proxy.progress_indicator_id
...
class PipelineProgressStatusMonitor(object):
    ...
    runtime_platform : {'jupyter', 'sap_bas', 'vscode', 'console'}, optional
    ...
    def __init__(self, connection_context: ConnectionContext, automatic_obj, fetch_table_interval=1, runtime_platform='jupyter'):
        if runtime_platform not in ['jupyter', 'sap_bas', 'vscode', 'console']:
            raise ValueError("The value of runtime platform parameter can only be taken as 'jupyter', sap_bas', 'vscode' or 'console'")
