...
class PolynomialRegression(PALBase):
    ...
    def __init__(self,
        ...
        if self.search_strategy is not None:
            ...
            if degree_set_count == 0:
                msg = ("One of the following parameters must be set: " +
                       "`degree`, `degree_values`, `degree_range`.")
                logger.error(msg)
                raise ValueError(msg)
            if self.degree_values is not None:
    ...
    @trace_sql
    def predict(self, data, key=None, features=None, model_format=None, thread_ratio=0.0):
        r"""
        ...
        model_format : int or str, optional(deprecated)
        ...
        if getattr(self, 'pmml_', None) is not None:
            model, model_format = self.pmml_, 1
...
class GLM(PALBase):
    ...
    output_fitted : bool, optional
        ...
        Defaults to False.
    alpha : float, optional(deprecated)
    ...
    lamb : float, optional(deprecated)
    ...
    enet_alpha : float, optional
        Elastic-net regularization mixing parameter.
        Only accepted when using coordinate descent(i.e. when ``solver`` is 'cd').
    ...
    __cv_params = ['link', 'alpha', 'lamb', 'enet_alpha', 'enet_lambda']
    ...
    def __init__(self,#pylint:disable=too-many-statements
        ...
        self.enet_alpha = enet_alpha if enet_alpha is not None else alpha
        self.enet_lambda = self._arg('enet_lambda', enet_lambda, float)
        self.enet_lambda = enet_lambda if enet_lambda is not None else lamb
            ...
            if self.param_values is not None:
                for x in self.param_values:#pylint:disable=invalid-name
                    ...
                    if x[0] in set_param_list:
                        ...
                        logger.error(msg)
                        raise ValueError(msg)
                    if x[0] in ['lamb', 'alpha', 'enet_alpha', 'enet_lambda'] and self.solver != 'cd':
            ...
            if self.param_range is not None:
                ...
                for x in self.param_range:#pylint:disable=invalid-name
                    ...
                    if not (isinstance(x[1], list) and len(x[1]) in rsz and all(isinstance(t, (int, float)) for t in x[1])):#pylint:disable=line-too-long
                        ...
                        raise TypeError(msg)
                    set_param_list.append(x[0])
            if any([x in set_param_list] for x in ['lamb', 'alpha', 'enet_lambda', 'enet_alpha']):
                self.solver = 'cd'
    ...
    def fit(self,# pylint: disable=too-many-arguments
        ...
        categorical_variable : list of str or str, optional
            INTEGER columns specified in this list will be treated as categorical
            data. Other INTEGER columns will be treated as continuous.
...
class BiVariateGeometricRegression(PALBase):
    ...
    >>> er.predict(data=df2, key='ID').collect()
        ID               VALUE
    0    0                   1
    1    1  3.9723699817481437
    2    2   8.901666037549536
    3    3   5.779723271893747
    4    4   24.60086108408644
    """
...
class BiVariateNaturalLogarithmicRegression(PALBase):
    ...
    >>> er.predict(data=df2, key='ID').collect()
       ID                VALUE
    0   0          14.86160299
    1   1     82.9935329364932
    2   2    122.8481570569525
    3   3    151.1254628829864
    5   4   173.05904529166017
    """
...
class CoxProportionalHazardModel(PALBase):
    ...
    tie_method : {'breslow', 'efron'}, optional
        ...
        Defaults to 'efron'.
    status_col : bool, optional(deprecated)
        If a status column is defined for right-censored data:

          - False : No status column. All response times are failure/death.
          - True : There is a status column, of which 0 indicates right-censored data,
            and 1 indicates failure/death.
    ...
    output_fitted : bool, optional
        ...
        Defaults to False.
    type_kind : str, optional(deprecated)
        The prediction type:
    ...
    >>> cox.fit(data=df1, key='ID',
                features=['STATUS', 'X1', 'X2'], label='TIME')
    ...
    @trace_sql
    def fit(self, data, key=None, features=None, label=None, status_col=None): #pylint:disable=too-many-locals, too-many-arguments
        r"""
        Fit regression model based on training data.
        ...
        label : str, optional
            Name of the dependent variable(indicating the time before a failure/death
            event occurs or data is right censored.)
        ...
        status_col : bool, optional
          ...
          **True** : There is a status column in ``data``, of which 0 indicates
          right-censored data and 1 indicates failure/death.
          The column should correspond to:
        ...
        features = self._arg('features', features, ListOfStrings)
        label = self._arg('label', label, str)
        status_col = self._arg('status_col', status_col, bool)
        cols = data.columns
        ...
        param_rows = [
            ...
            ('MAX_ITERATION', self.max_iter, None, None),
            ('CONVERGENCE_CRITERION', None, self.convergence_criterion,
             None),
            ('SIGNIFICANCE_LEVEL', None, self.significance_level, None),
    ...
    @trace_sql
    def predict(self, data, key=None, features=None, thread_ratio=None,#pylint:disable=unused-argument
                pred_type=None, significance_level=None):
        r"""
        Predict dependent variable values based on fitted model.
        ...
        thread_ratio : float, optional(deprecated)
            Controls the proportion of available threads to use for prediction.
        ...
        features = self._arg('features', features, ListOfStrings)
        #thread_ratio = self._arg('thread_ratio', thread_ratio, float)
        pred_type = self._arg('pred_type', pred_type, {x:x for x in ['risk', 'lp']})
        significance_level = self._arg('significance_level', significance_level,
                                       float)
        if features is None:
            cols = data.columns
            cols.remove(key)
        ...
        param_rows = [
            ('TYPE', None, None, pred_type if pred_type else self.type_kind),
            ('SIGNIFICANCE_LEVEL', None, significance_level, None)]
