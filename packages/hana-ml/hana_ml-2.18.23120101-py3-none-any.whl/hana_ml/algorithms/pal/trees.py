...
class RDTClassifier(_RDTBase):#pylint: disable=too-many-instance-attributes
    ...
    Perform score() on given dataframe:
...
class DecisionTreeClassifier(_DecisionTreeBase):
    ...
    Perform predict() on given dataframe df2:
...
#pylint: disable=too-many-instance-attributes
    ...
    Create an instance of Hybrid Gradient Boosting Classifier:
    ...
    >>> hgbc.fit(data=df, features=['ATT1', 'ATT2', 'ATT3', 'ATT4'], label='LABEL')
    >>> hgbc.stats_.collect()
             STAT_NAME STAT_VALUE
    ...
    >>> result = hgbc.fit(data=df_predict, key='ID', verbose=False)
    >>> result.collect()
       ID SCORE  CONFIDENCE
    0   1     A    0.852674
    1   2     A    0.852674
...
class HybridGradientBoostingRegressor(_HybridGradientBoostingBase):
    ...
    Create an instance of Hybrid Gradient Boosting Regressor and training the model:
    ...
    >>> hgbr.fit(data=df, features=['ATT1','ATT2','ATT3', 'ATT4'], label='TARGET')
    ...
    >>> result = hgbr.predict(data=df_predict, key='ID', verbose=False)
    >>> result.collect()
       ID               SCORE CONFIDENCE
