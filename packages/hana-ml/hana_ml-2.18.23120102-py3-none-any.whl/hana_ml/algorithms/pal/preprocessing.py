...
class Imputer(PALBase):
    ...
    >>> result = impute.fit_transform(data=df,
                                      categorical_variable=['V1'],
...
class MDS(PALBase):
    ...
    Create a MDS instance and invoke fit_transform():
...
class ImputeTS(PALBase):
    ...
    >>> res = imp.fit_transform(data=data, key='ID')
    >>> res.collect()
       ID    V     X
    0   0  0.1     A
    1   1  0.3     A
...
class PowerTransform(PALBase):
    ...
    >>> res = pt.fit_transform(data=data, key='ID')
    ...
    def fit(self, data, key=None, features=None, feature_interval=None, feature_interval_hint=None):
        ...
        ['ID', 'X1', 'X2', 'X3']
        >>> pt = PowerTransform(method='yeojohnson')
        >>> pt.fit(data=data, key='ID', features=['X1', 'X2', 'X3'],
    ...
    def transform(self, data, key=None, features=None,
        ...
        ['ID', 'X1', 'X2', 'X3']
        >>> pt = PowerTransform(method='yeojohnson')
        >>> pt.fit(data=data, key='ID')
        <hana_ml.algorithms.pal.preprocessing.PowerTransform at xxxxxxxxxx>
        >>> res = pt.transform(data=test_data, key='ID')
        >>> res.collect()
            ID  TRANSFORMED_X1  TRANSFORMED_X2  TRANSFORMED_X3
        .   ..             ...             ...             ...
        """
