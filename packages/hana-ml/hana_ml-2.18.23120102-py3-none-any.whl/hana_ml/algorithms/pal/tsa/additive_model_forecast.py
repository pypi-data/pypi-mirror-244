...
class AdditiveModelForecast(_AdditiveModelForecastBase):
    r"""
    Additive model time series analysis uses an additive model to forecast time series data. It handles data with strong seasonal effects and is robust to shift in the historical trend.

    Additive model time series analysis – also called as Prophet – uses a decomposable time series model with three main components: trend, seasonality, and holidays or events.
