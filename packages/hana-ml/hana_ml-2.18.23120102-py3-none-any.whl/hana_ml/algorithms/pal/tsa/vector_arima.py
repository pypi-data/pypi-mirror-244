...
The following class are available:
    ...
    * :class:`VectorARIMA`
"""
#pylint: disable=too-many-lines, line-too-long, too-many-locals, too-many-statements, too-many-branches,  bare-except
...
class VectorARIMA(PALBase):#pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    def fit(self, data, key=None, endog=None, exog=None):#pylint: disable=too-many-arguments,too-many-branches, too-many-statements
        """
        Generates a VectorARIMA model with given parameters.
        ...
        key : str, optional
            ...
            The timestamp column of data. The type of key column should be INTEGER, TIMESTAMP,
            DATE, or SECONDDATE.

            Defaults to the first column of data if the index column of data is not provided.
            Otherwise, defaults to the index of data.

        endog : list of str, optional

            The endogenous variables, i.e. time series. The type of endog column can be
            INTEGER, DOUBLE or DECIMAL(p,s).

            Defaults to all non-key and non-exog columns of data if not provided.

        exog : str of a list of str, optional

            An optional array of exogenous variables. The type of exog column can be INTEGER,
    ...
    @trace_sql
    def predict(self, data=None, key=None, forecast_length=None, allow_new_index=False):
        r"""
        Makes time series forecast based on the given VectorARIMA model.
        ...
        data : DataFrame, optional
            ...
            Index and exogenous variables for forecast. The structure is as follows:
              ...
              - Other columns : exogenous variables, with type INTEGER, DOUBLE or
                DECIMAL(p,s).
        ...
        # for data is None, we need a value in the return
        #if key is None:
        #    key = 'TIMESTAMP'
        ...
        try:
            if data_ is None:
                data_ = conn.sql("SELECT TOP 0 * FROM (SELECT NULL TIMESTAMP FROM DUMMY) dt;").cast({"TIMESTAMP": "INTEGER"})
        ...
        if self.endog is not None:
            for col in self.endog:
                result_dict[col] = result.filter("COLNAME='{}'".format(col)).deselect("COLNAME")
