...
class _KNNBase(PALBase):
    ...
    """
    algorithm_map = {'brute_force':0, 'kd_tree':1, 'brute-force':0, 'kd-tree':1}
    ...
    resampling_map = {'classification': ['cv', 'cv_sha', 'cv_hyperband',
                      ...
                      'regression': ['cv', 'cv_sha', 'cv_hyperband', 'bootstrap', 'bootstrap_sha',
                                     'bootstrap_hyperband']}
    ...
    def __init__(self,
        ...
        self.resampling_method = self._arg('resampling_method', resampling_method,
                                           {x:x for x in self.resampling_map[functionality]})
            ...
            if not self._disable_hana_execution:
                ...
                with conn.connection.cursor() as cur:
                    ...
                    execute_logged(cur,
                                   ...
                                   conn.sql_tracer,
                                   conn)
                param_rows.extend([(row[0],
                                    None if row[1] is None else (None if not(row[1] > 0 or row[1] <= 0) else int(row[1])),
                                    None if row[2] is None else (None if not(row[2] > 0 or row[2] <= 0) else row[2]),
                                    row[3]) for _, row in self.optim_param_.collect().iterrows()])#pylint:disable=line-too-long
    ...
    def _predict(self, data, key=None, features=None,
        ...
        param_rows = [] if interpret is False else [('SAMPLESIZE', sample_size, None, None),
...
class KNNClassifier(_KNNBase):
    ...
    param_values : dict or ListOfTuples, optional
        ...
        ``metric``, ``minkowski_power``, ``category_weights``,
    ...
    Perform predict() on given predict data df_class_predict:
    ...
    def __init__(self, n_neighbors=None, thread_ratio=None, stat_info=None,
                 ...
                 min_resource_rate=None, reduction_rate=None,
                 aggressive_elimination=None):
        setattr(self, 'hanaml_parameters', pal_param_register())
        super(KNNClassifier, self).__init__(functionality='classification',
                                            ...
                                            min_resource_rate=min_resource_rate,
                                            reduction_rate=reduction_rate,
                                            aggressive_elimination=aggressive_elimination)

    def fit(self, data, key=None, features=None, label=None,
            categorical_variable=None, string_variable=None,
    ...
    def predict(self, data, key=None, features=None,
        ...
        DataFrame
            ...
            KNN prediction statistics information. Structured as follows:
              ...
              - DISTANCE: Distance.
        """
        return super(KNNClassifier, self)._predict(data=data, key=key,
                                                   features=features,
                                                   interpret=interpret,
...
class KNNRegressor(_KNNBase):
    ...
    Examples
    --------
    Input dataframe df_regr_train for regression training:
    ...
    Create a KNNRegressor instance:
    ...
    >>> knn.fit(data=df_regr_train, key='ID',
                categorical_variable='X1', label='VALUE')

    Input dataframe df_reg_predict:

    >>> df_reg_predict.collect()
       ID  X1       X2 X3
    0   0   2      1.0  A
    1   1   1     10.0  C
    2   2   1     11.0  B
    ...
    >>> res, stats = knn.predict(data=df_reg_predict,
                                 key='ID', categorical_variable='X1')
    ...
    def __init__(self, n_neighbors=None, thread_ratio=None, stat_info=None, aggregate_type=None,
                 ...
                 min_resource_rate=None, reduction_rate=None, aggressive_elimination=None):
        setattr(self, 'hanaml_parameters', pal_param_register())
...
class KNN(PALBase):
    ...
    Create a KNN instance and call fit():

    >>> knn = KNN(n_neighbors=3, voting_type='majority',
    ...           thread_ratio=0.1, stat_info=False)
    >>> knn.fit(data=df, 'ID', features=['X1', 'X2'], label='TYPE')
