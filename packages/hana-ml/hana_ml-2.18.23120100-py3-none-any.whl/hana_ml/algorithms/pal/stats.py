...
def chi_squared_goodness_of_fit(data, key, observed_data=None, expected_freq=None):
    ...
    >>> res, stat = chi_squared_goodness_of_fit(data=df, key='ID')
    >>> res.collect()
...
def chi_squared_independence(data, key, observed_data=None, correction=False):
    ...
    >>> res, stats = chi_squared_independence(data=df, key='ID')
    >>> res.collect()
...
def iqr(data, key, col=None, multiplier=None):
    ...
    >>> res, stat = iqr(data=df, key='ID',
                        col='VAL', multiplier=1.5)
    >>> res.collect()
         ID  IS_OUT_OF_RANGE
    0    P1                0
    1    P2                0
...
def factor_analysis(data, key, factor_num, col=None,
    ...
    >>> res = factor_analysis(data=df, key='ID',
                              factor_num=2,
                              rotation='promax',
                              matrix='correlation')
    >>> res[0].collect()
...
def kaplan_meier_survival_analysis(data, event_indicator=None, conf_level=None):
    ...
    >>> survival_estimates, res, stats = kaplan_meier_survival_analysis(data=df)
    >>> res.collect()
...
def cdf(data, distr_info, col=None, complementary=False):
    ...
    >>> res = cdf(data=df, distri=df_distri)
    >>> res.collect()
       DATACOL  PROBABILITY
    0     37.4     0.014160
    1    277.9     0.632688
...
def quantile(data, distr_info, col=None, complementary=False):
    ...
    >>> res = quantile(data=df, distr=df_distr)
    >>> res.collect()
       DATACOL      QUANTILE
    0     0.3     170.755854
    1     0.5     233.608506
...
def interval_quality(data, significance_level, score_type=None, ave_abs_error=None,
                     percent=None, check_consistency=None, thread_ratio=None):
    r"""
    ...
    while the :math:`\alpha` represents the significance level for the prediction interval(i.e. 1-:math:`\alpha` is the
    nominial coverage of the prediction interval), and :math:`1_{*}` is simply the indicator function for condition \*.
    ...
    significance_level : float
        Specifies the significance level of prediction intervals, i.e.
        nominial probability that the true value falling outside the prediction intervals.
    ...
    ave_abs_error : float, optional
        Specifies the factor to be divided by the classical interval score to obtain
        the mean-scaled interval score. So this parameter is valid only when
        ``score_type`` is specified as 'msis'.
    ...
    DataFrame 1
        Interval score result, structured as follows:

        - 1st column : Same name and dtype as the 1st column of ``data``.
        - 2nd column : SCORE, type DOUBLE, the interval score.
        - 3rd column : REASON, type NVARCHAR, explaining the interval score by dispersion, upper_score,
          and lower_score components, in json format.

    DataFrame 2
        Statistics for interval score, structured as follows:

        - 1st column : STAT_NAME, type NVARCHAR
        - 2nd column : STAT_VALUE, type NVARCHAR
    ...
    if (significance_level <= 0 or significance_level >= 1):
        msg = "The value of 'significance_level' must be greater than 0 but less than 1."
        logger.error(msg)
        raise ValueError(msg)
    score_type = arg('score_type', score_type, {'classical': 0, 'msis': 1})
    ave_abs_error = arg('ave_abs_error', ave_abs_error, float)
    percent = arg('percent', percent, bool)
    check_consistency = arg('check_consistency', check_consistency, bool)
    thread_ratio = arg('thread_ratio', thread_ratio, float)
    param_rows = [('SIGNIFICANCE_LEVEL', None, significance_level, None),
                  ('SCORE_TYPE', score_type, None, None),
                  ('AVE_ABS_ERROR', None, ave_abs_error, None),
                  ('PERCENT', percent, None, None),
                  ('CHECK_CONSISTENCY', check_consistency, None, None),
                  ('THREAD_RATIO', None, thread_ratio, None)]
    unique_id = str(uuid.uuid1()).replace('-', '_').upper()
    tables = ["#INTERVAL_QUALITY_{}_TBL_{}".format(tb, unique_id) for tb in ['RESULT', 'STATS']]
    res_tbl, stats_tbl = tables
    if check_pal_function_exist(conn,
                                '%INTERVAL_QUALITY%',
                                like=True) or data.connection_context.disable_hana_execution:
        try:
            call_pal_auto_with_hint(conn,
                                    None,
                                    'PAL_INTERVAL_QUALITY',
                                    data,
                                    ParameterTable().with_data(param_rows),
                                    *tables)
        except dbapi.Error as db_err:
        ...
        msg = 'The version of your SAP HANA does not support Interval Quality!'
        logger.error(msg)
        raise ValueError(msg)
    return conn.table(res_tbl), conn.table(stats_tbl)
