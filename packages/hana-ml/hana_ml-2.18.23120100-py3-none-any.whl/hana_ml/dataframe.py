...
def _univariate_analysis(data, cols=None):
    ...
    execute_sql = "DO\nBEGIN\nDECLARE param_name VARCHAR(5000) ARRAY;\nDECLARE int_value INTEGER ARRAY;\nDECLARE double_value DOUBLE ARRAY;\nDECLARE string_value VARCHAR(5000) ARRAY;\nparam_name[1] := N'HAS_ID';\nint_value[1] := 0;\ndouble_value[1] := NULL;\nstring_value[1] := NULL;\nparams = UNNEST(:param_name, :int_value, :double_value, :string_value);\nin_0 = {0};\nCALL _SYS_AFL.PAL_UNIVARIATE_ANALYSIS(:in_0, :params, out_0, out_1);\nCREATE LOCAL TEMPORARY COLUMN TABLE {1} AS (SELECT * FROM :out_0);\nCREATE LOCAL TEMPORARY COLUMN TABLE {2} AS (SELECT * FROM :out_1);\nEND;".format(data.select_statement, tables[0], tables[1])
    with data.connection_context.connection.cursor() as cur:
        cur.execute(execute_sql)
    return data.connection_context.table(tables[0]), data.connection_context.table(tables[1])
    ...
    def diff(self, index, periods=1, diff_datetime="days"):
        ...
        def _check_type(_type):
            if "INT" in _type.upper():
                return "numeric"
            elif "DOUBLE" in _type.upper():
                return "numeric"
            elif "DECIMAL" in _type.upper():
                return "numeric"
            elif "FLOAT" in _type.upper():
                return "numeric"
            elif "BOOL" in _type.upper():
                return "numeric"
            elif "NUMBER" in _type.upper():
                return "numeric"
            elif "TIME" in _type.upper():
                return "datetimes"
            elif "DATE" in _type.upper():
                return "datetimes"
            return "unsupported"
