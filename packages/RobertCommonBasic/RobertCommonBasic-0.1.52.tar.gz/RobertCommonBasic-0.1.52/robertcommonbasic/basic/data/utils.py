...
def replace_value(value: str, replaces: dict = {'true': 1, 'active': 1, 'false': 0, 'inactive': 0}):
    """将相应值转换为0,1"""
    try:
...
# 添加自定义函数
g_s.functions.update(convert=convert_value, max=max, min=min, sum=sum, _zero=_zero, bit=bit, _in=_in, signed=signed, inverse_long=inverse_long, inverse_float=inverse_float, inverse_double=inverse_double, _and=_and, _or=_or)
