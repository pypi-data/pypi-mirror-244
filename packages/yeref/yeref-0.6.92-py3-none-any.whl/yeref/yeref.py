...
trg_utms = [
    '360', 'ad', 'advert', 'aesthetic', 'ai', 'air', 'alpha', 'beta', 'big', 'blog', 'boost', 'bot', 'box',
    ...
    'fast', 'fest', 'festival', 'field', 'file', 'first', 'fit', 'fix', 'flow', 'fly', 'focus', 'for', 'forest',
    'free', 'fresh', 'fun', 'future', 'get', 'gift', 'giga', 'git', 'go', 'grade', 'grape', 'guide', 'hackathon',
    'have', 'here', 'high', 'hill', 'hot', 'hub', 'ig', 'infinite', 'info', 'intensive', 'landing', 'last', 'lead',
    'league', 'learn', 'lesson', 'level', 'life', 'like', 'lime', 'limit', 'link', 'linkedin', 'load', 'look', 'map',
    'marathon', 'master', 'mastermind', 'max', 'medium', 'meet', 'method', 'mind', 'mix', 'mobile', 'more', 'msg',
    'neuro', 'new', 'newsletter', 'night', 'nike', 'node', 'note', 'nova', 'number', 'ocean', 'omega', 'one', 'orange',
    'paint', 'pinterest', 'plane', 'platform', 'play', 'podcast', 'portal', 'post', 'present', 'press', 'pro', 'promo',
    'puzzle', 'quick', 'ran', 'rate', 'read', 'ready', 'real', 'reality', 'realm', 'red', 'reddit', 'refer',
    'referral', 'review', 'round', 'run', 'safe', 'salute', 'save', 'scale', 'school', 'sea', 'seminar', 'seo',
    'service', 'session', 'set', 'share', 'shop', 'sky', 'smart', 'snow', 'social', 'soundcloud', 'special', 'spotify',
    'spring', 'star', 'stat', 'status', 'stellar', 'stone', 'subscribe', 'summer', 'sun', 'symposium', 'tap',
    'taplink', 'target', 'team', 'tech', 'technology', 'tele', 'telegram', 'telegraph', 'telescope', 'ten',
    'testdrive', 'text', 'this', 'tiktok', 'time', 'top', 'training', 'trend', 'trigger', 'true', 'try', 'turbo',
    'unity', 'university', 'unsubscribe', 'up', 'utm', 'vimeo', 'vip', 'wait', 'wave', 'web', 'webinar', 'webmaster',
    'website', 'week', 'win', 'winter', 'wordpress', 'work', 'workbook', 'workshop', 'wow', 'x', 'yes', 'you', 'you2b',
    'youtube', 'yt', 'zip'
]
        ...
        if file_name and os.path.exists(file_name) and os.path.getsize(file_name) < 5242880 and ext in ['.jpg', '.jpeg',
            ...
            while cnt >= 0:
                try:
                    ...
                    await asyncio.sleep(round(random.uniform(6, 12), 2))
                    cnt -= 1
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
...
async def show_offers_admin(bot, FsmOffer, chat_id, lz, state, has_restricted, BASE_D, bot_un, post_id=1,
    ...
    try:
        ...
        OFFER_TEXT = await correct_tag(
            f"{l_post_text[0:(len(OFFER_TEXT) + moment)]}") if moment <= 0 else OFFER_TEXT
        ...
        buttons = [
            ...
            types.InlineKeyboardButton(
                text=f"✅ {l_silence[lz]}" if OFFER_ISSILENCE else f"☑️ {l_silence[lz]}",
        ...
        buttons = [
            types.InlineKeyboardButton(
                text=f"✅ {l_gallery[lz]}" if OFFER_ISGALLERY else f"☑️ {l_gallery[lz]}",
            ...
            types.InlineKeyboardButton(
                text=f"✅ {l_spoiler[lz]}" if OFFER_ISSPOILER else f"☑️ {l_spoiler[lz]}",
                callback_data=f'ofr_isspoiler_{OFFER_ID}_{post_id}'),
        ]
        reply_markup.row(*buttons)

        buttons = [
            types.InlineKeyboardButton(text=l_post_new[lz],
                                       callback_data=f'ofr_new_{OFFER_ID}_{post_id}'),
            types.InlineKeyboardButton(text=l_post_delete[lz],
                                       callback_data=f'ofr_del_{OFFER_ID}_{post_id}'),
            types.InlineKeyboardButton(text=l_post_change[lz],
                                       callback_data=f'ofr_edit_{OFFER_ID}_{post_id}'),
        ]
        reply_markup.row(*buttons)

        reply_markup.row(types.InlineKeyboardButton(text=l_post_publish[lz],
...
async def generate_calendar_admin(bot, state, lz, chat_id, message_id=None, is_new=True):
    try:
        ...
        buttons_ = [
            types.InlineKeyboardButton(text=l_weekday_1[lz], callback_data='cb_99'),
...
async def callbacks_ofr_admin(bot, FsmOffer, call, state, BASE_D, bot_un):
    try:
        ...
        if cmd == 'new':
            ...
            reply_markup = InlineKeyboardBuilder()
            buttons = [
                types.InlineKeyboardButton(text=l_me[lz], callback_data=f"publication_me_{offer_id}"),
...
async def format_link_md(txt):
    ...
    try:
        ...
        for tlg_link in arr_tlg_links:
            ...
            result = result.replace(arr_tlg_links_item, f"[{link_name}]({arr_tlg_links_item.strip()})")
