...
cdef class BipartiteSemisupervisedCriterion(GMO):
    ...
    cdef int init(
        ...
        # FIXME: when children criteria calls update_supervision_() in the next
        # node, it will use this modified value we set below as the
        # "current_supervision" argument, but it seems that it should use the
        # original instead.
        (<SSCompositeCriterion>self.criterion_rows)._curr_supervision = \
