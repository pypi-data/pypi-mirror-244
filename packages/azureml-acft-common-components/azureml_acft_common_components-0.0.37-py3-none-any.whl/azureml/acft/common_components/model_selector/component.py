...
class ModelSelector:
    ...
    def convert_mlflow_model_to_pytorch_model(
        ...
        try:
            ...
            for root, dirs, files in os.walk((os.path.normpath(mlflow_model_path)), topdown=False):
                for name in files:
                    Sourcepath = os.path.join(root, name)
                    shutil.copy2(Sourcepath, download_dir)
            ...
            raise ACFTValidationException._with_error(
                AzureMLError.create(
                    InvalidMlflowModelFormat, str(e)
                )
            )
    ...
    def _model_name_download_from_registry(self) -> bool:
        ...
        is_model_name_prefixed_with_mmd = self._add_mmd_model_prefix()
        ...
        if model_info_response.status_code == 200:
            ...
            if model_format == ModelSelectorAPIConstants.CUSTOM_MODEL:
                ...
                self._remove_mmd_model_prefix(is_model_name_prefixed_with_mmd)
    ...
    def _add_mmd_model_prefix(self) -> bool:
        """ Add mmd-3x prefix to model name for MMDetection Models if not present.
        :return: True if prefix is explicitly added to existing registry_model_name, False otherwise
        :rtype: bool
        """
    ...
    def _remove_mmd_model_prefix(self, is_model_name_prefixed_with_mmd: bool) -> None:
        """ Remove mmd-3x prefix from model name if present
        :param is_model_name_prefixed_with_mmd: whether model name is prefixed with mmd-3x
        :type is_model_name_prefixed_with_mmd: bool
        """
        if is_model_name_prefixed_with_mmd:
            # Only remove MMD model prefix from the start of the model_name
