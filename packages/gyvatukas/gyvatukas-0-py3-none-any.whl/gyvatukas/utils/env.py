...
def get_env(
    name: str, type: SUPPORTED_ENV_TYPES,
    ...
    if value is None or value == "":
        if required:
            raise ValueError(f"Value `{value}` is required but not found in environment!")
        return default

    if type == bool:
        if value.lower() not in truthy + falsy:
            raise ValueError(f"Invalid value `{value}` for variable `{name}`. Cannot be parsed as bool...")
    ...
    if type == dict:
        try:
            ...
            raise ValueError(f"Invalid value `{value}` for variable `{name}`. Cannot be parsed as dict...")
