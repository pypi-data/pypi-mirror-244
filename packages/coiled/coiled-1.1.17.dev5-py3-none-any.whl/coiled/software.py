...
from coiled.utils import BAD_PYTHON_VERSIONS
...
async def create_env_spec(
    ...
    python_pkg = next((p for p in spec["packages"] if p["name"] == "python" and p["source"] == "conda"), None)
    if python_pkg and any(python_pkg["specifier"] == f"=={ver}" for ver in BAD_PYTHON_VERSIONS):
        logger.warning(
            f"Python version {python_pkg['specifier'][2:]} is not compatible with dask, using 3.9.18 instead"
        )
        python_pkg["specifier"] = "==3.9.18"
        if spec["raw_conda"] and "dependencies" in spec["raw_conda"]:
            spec["raw_conda"]["dependencies"] = [
                "python==3.9.18" if isinstance(raw_dep, str) and raw_dep.startswith("python==") else raw_dep
                for raw_dep in spec["raw_conda"]["dependencies"]
            ]
