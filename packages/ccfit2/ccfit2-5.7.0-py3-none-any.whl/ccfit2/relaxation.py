...
class Dataset(ABC):
    '''
    Contains experimental rates, rate bounds (+-) and dependent variable
    on which the rate depends
    ...
    dep_vars: array_like
        Dependent variable of which the rate is a function\n
    ...
    @property
    @abstractmethod
    def DEP_VAR_NAMES() -> list[str]:
        'string names of dependent variables with which rate varies e.g. Field'
        raise NotImplementedError

    @property
    @abstractmethod
    def DEP_VAR_LABELS() -> list[str]:
        'string label of dependent variables with which rate varies e.g. H'
        raise NotImplementedError

    @property
    @abstractmethod
    def DEP_VAR_UNITS() -> list[str]:
        'string units of dependent variables with which rate varies e.g. Oe'
        raise NotImplementedError

    def __init__(self, rates: npt.ArrayLike, dep_vars: npt.ArrayLike,
                 lograte_pm: npt.ArrayLike = []):

        self.rates = np.asarray(rates)
        self.dep_vars = np.asarray(dep_vars)
    ...
    @classmethod
    def from_raw(cls, dep_vars: npt.ArrayLike, lntaus: npt.ArrayLike,
                 lntau_stdevs: npt.ArrayLike = [],
                 lntau_fus: npt.ArrayLike = [],
                 lntau_fls: npt.ArrayLike = []) -> 'Dataset':
        '''
    ...
    def _from_ccfit2_files(cls, file_names: str | list[str]) -> 'Dataset':
        ...
        -------
        Dataset
            Single Dataset, rate vs dependent variable

        Raises
        ------
        ValueError if either dependent variable or lntau values\n
        ...
        if cls.DEP_VAR_NAMES[0] == 'Field':
            headers['fields'] = ['H', 'H (Oe)']
        elif cls.DEP_VAR_NAMES[0] == 'Temperature':
    ...
    def from_ccfit2_csv(cls, file_names: str | list[str]) -> 'Dataset':
        ...
        Raises
        ------
        ValueError if either dependent variable (T, H) or lntau values\n
        cannot be found in file
        '''
    ...
    def extract_ac_dc_model(models: list[ac.Model | dc.Model],
        ...
        dep_var_name: str {'field', 'temperature'}
            Dependent variable to extract, either field or temperature
        ...
        ------
        ValueError
            If dependent variable name is unknown
        '''
        ...
        if dep_var_name == 'field':
            ...
            raise ValueError('Unknown Dependent Variable')

        # Sort by dependent variable, low to high
        order = sorted(range(len(dep_vars)), key=lambda k: dep_vars[k])
...
class HDataset(Dataset):
    ...
    @fields.setter
    def fields(self, value: npt.ArrayLike):
        self._fields = value
    ...
    @dep_vars.setter
    def dep_vars(self, value: npt.ArrayLike):
        self._dep_vars = value
        self._fields = value
        return
...
class TDataset(Dataset):
    ...
    @temperatures.setter
    def temperatures(self, value: npt.ArrayLike):
...
class HTDataset(Dataset):
    ...
    rate_pm: ndarray of floats
        ...
        not! 10**(lograte_pm)
    '''
    DEP_VAR_NAMES = ['Field', 'Temperature']
    DEP_VAR_UNITS = ['Oe', 'K']
    DEP_VAR_LABELS = ['H', 'T']

    def __init__(self, rates: npt.ArrayLike, fields: npt.ArrayLike,
                 temperatures: npt.ArrayLike, lograte_pm: npt.ArrayLike = []):
    ...
    @classmethod
    def from_raw(cls, fields: npt.ArrayLike, temperatures: npt.ArrayLike,
                 lntaus: npt.ArrayLike, lntau_stdevs: npt.ArrayLike = [],
                 lntau_fus: npt.ArrayLike = [], lntau_fls: npt.ArrayLike = []
                 ) -> 'HTDataset':
        '''
...
class LogTauModel(ABC):
    ...
    def model(parameters: dict[str, float],
              dep_vars: npt.ArrayLike) -> npt.NDArray:
        '''
        ...
        dep_vars: array_like
            Dependent variable values at which model function is evaluated
    ...
    @classmethod
    def residuals(cls, params: dict[str, float], dep_vars: npt.ArrayLike,
    ...
    def fit_to(self, dataset: Dataset, verbose: bool = True) -> None:
        ...
        ----------
        dataset: HDataset
            Dataset to which a model of rate vs dependent variable will\n
            be fitted
...
class LogTauHTModel(LogTauModel):
    ...
    @staticmethod
    @abstractmethod
    def model(parameters: dict[str, float], fields: npt.ArrayLike,
...
class MultiLogTauModel(ABC):
    ...
    @abstractmethod
    def DEP_VAR_NAMES() -> list[str]:
        'string name of dependent variable with which rate varies e.g. Field'
        raise NotImplementedError
    ...
    @abstractmethod
    def DEP_VAR_UNITS() -> list[str]:
        'string unit of dependent variable with which rate varies e.g. Oe'
        raise NotImplementedError
    ...
    def fit_to(self, dataset: HDataset | TDataset,
        ...
        dataset: HDataset | TDataset
            Dataset to which a model of rate vs dependent variable
            will be fitted
        verbose: bool, default True
            If True, prints information to terminal
...
class LogFDQTMModel(LogTauHModel):
    '''
    Field depdendent QTM Model of log10(Relaxation rate)
...
class LogBVVConstantModel(LogTauHModel):
    ...
    def model(parameters: dict[str, float],
              fields: npt.ArrayLike) -> npt.NDArray:
        '''
        Evaluates field depdendent Brons-van Vleck * constant model of\n
        log10(relaxation rate) using provided parameter and field values.
...
class LogRamanQTMPBModel(LogTauTModel):
    ...
    def model(parameters: dict[str, float],
        ...
        timespinlat = 1 / (10**r * np.asarray(temperatures)**n + 10**-q)

        timelatbath = 10**b * np.asarray(temperatures)**(-m)
...
class LogDirectPBModel(LogTauTModel):
    ...
    def model(parameters: dict[str, float],
        ...
        timespinlat = 1 / (10**d * np.asarray(temperatures)**-1)
...
class LogRamanPBModel(LogTauTModel):
    ...
    def model(parameters: dict[str, float],
        ...
        timespinlat = 1 / (10**r * np.asarray(temperatures)**n)
...
class LogFTDRamanIIDirectModel(LogTauHTModel):
    ...
    def model(parameters: dict[str, float],
        ...
        lograte = np.log10(10**g * np.asarray(fields)**x * np.asarray(temperatures)**y) # noqa
...
class LogFTDBVVRamanIModel(LogTauHTModel):
    ...
    def model(parameters: dict[str, float],
        ...
        lograte = np.log10(
            10**r * np.asarray(temperatures)**n * (1 + 10**e * np.asarray(fields)**2) / (1 + 10**f * np.asarray(fields)**2) # noqa
        )
...
def plot_fitted_times(dataset: TDataset | HDataset,
    ...
    if save:
        ...
        if verbose:
            ut.cprint(
                f'\n Fitted ln(τ) vs 1/{dataset.DEP_VAR_LABELS[0]} plot saved to \n {save_name}\n', # noqa
                'cyan'
            )
...
def plot_times(dataset: TDataset | HDataset, show: bool = True,
    ...
    if save:
        ...
        if verbose:
            ut.cprint(
                f'\n Fitted ln(tau) vs 1/{dataset.DEP_VAR_LABELS[0]} plot saved to \n {save_name}\n', # noqa
                'cyan'
            )
...
def _plot_fitted_times(dataset: TDataset | HDataset,
    ...
    if dataset.DEP_VAR_NAMES[0] == 'Field':
        ax.legend(
...
def plot_fitted_rates(dataset: TDataset | HDataset,
    ...
    if save:
        ...
        if verbose:
            ut.cprint(
                f'\n Fitted τ⁻¹ vs {dataset.DEP_VAR_LABELS[0]} plot saved to \n {save_name}\n', # noqa
                'cyan'
            )
...
def _plot_fitted_rates(dataset: TDataset | HDataset,
    ...
    gui.format_rate_x_y_axes(
        ...
        x_type=dataset.DEP_VAR_NAMES[0].lower()
    )
...
def write_model_data(dataset: TDataset | HDataset,
    ...
    header = [
        f'{dataset.DEP_VAR_LABELS[0]} ({dataset.DEP_VAR_UNITS[0]})'
    ]
