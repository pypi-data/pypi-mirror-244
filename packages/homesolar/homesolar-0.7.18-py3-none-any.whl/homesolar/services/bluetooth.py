...
class BluetoothClient:
    ...
    def run(self, task_queue: Queue):
        try:
            if self.read_size == 0:
                raise Exception("read_size is 0, please make sure to setup the device correctly")
            if self.params is None:
                raise Exception("parameters are not set, please make sure to setup the device correctly")
            if self.request_code == "":
                raise Exception("request_code is empty, please make sure to setup the device correctly")
