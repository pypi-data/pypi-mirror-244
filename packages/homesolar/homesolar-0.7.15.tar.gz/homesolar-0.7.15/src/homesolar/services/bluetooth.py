                    ...
                    try:
                        if self.request_code != "":
                            try:
                                ...
                                logger.debug(f"Failed to write, retrying [{e}]")
                                self.connection.close()
                                self.connection.open()
                                time.sleep(1)
                                self.connection.write(unhexlify(self.request_code))
                        ...
                        if verified:
                            ...
                            logger.warning("Data is not verified, might want to check it out")
                            logger.debug(f"Received data: {json.dumps(fields)}")
...
def initialize(main_task_queue: Queue):
    ...
    ant_bms = BluetoothClient(config.homesolar_config['BLUETOOTH']['address'])
