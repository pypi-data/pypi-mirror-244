...
class AHEventFilter:
    def __init__(self, respiratory_events, od_events, arousal_events=None):
    ...
    def get_AH_with_OD(self, row, od_eps=45):
        if row['Type'] == 'Hypopnea':
            ...
            condition = (hypopnea_start <= self.od_events['Start']) & (hypopnea_end + od_eps > self.od_events['Start'])
            rows_to_drop = self.od_events[condition].index
            self.od_events.drop(rows_to_drop, inplace=True)
    ...
    def get_AH_with_Arousal(self, row, aro_eps=6):
        if row['Type'] == 'Hypopnea':
            ...
            self.arousal_events.drop(rows_to_drop, inplace=True)
            return any(condition)
        else:
            return True
    ...
    def get_filtered_AH_events(self, type='AHI3', od_eps=45, aro_eps=6):
        if type == 'AHI4':
            ...
            ah_with_od = self.respiratory_events[
                self.respiratory_events.apply(self.get_AH_with_OD, od_eps=od_eps, axis=1)]
            ah_with_arousal = self.respiratory_events[
                self.respiratory_events.apply(self.get_AH_with_Arousal, aro_eps=aro_eps, axis=1)]
            return merge_drop_duplicates_sort(ah_with_od, ah_with_arousal)
...
class AHI:
    ...
    def _assign_sleep_stage_label_to_respiratory_events(self):
        if not self.AH_events.empty:
            for idx, row in self.sleep_stages.iterrows():
                start_time = row['Start']
                end_time = row['End']
                self.AH_events.loc[
                    (self.AH_events['Start'] >= start_time) & (self.AH_events['End'] <= end_time),
                    'SleepStage'] = row['Type']
            sleep_stage_idx_cache = 0
            for resp_idx, resp_row in self.AH_events[self.AH_events['SleepStage'].isna()].iterrows():
                for sleep_stage_idx, sleep_stage_row in self.sleep_stages.loc[sleep_stage_idx_cache:].iterrows():
                    if resp_row['End'] > sleep_stage_row['End'] and resp_row['Start'] < sleep_stage_row['End']:
                        ...
                        stage_set = list(set([
                            sleep_stage_row['Type'],
                            self.sleep_stages.loc[sleep_stage_idx + 1, 'Type']
                        ]))
                        if len(stage_set) != 1 and 'Wake' in stage_set:
                        ...
                        self.AH_events.loc[resp_idx, 'SleepStage'] = cur_event_stage
                        sleep_stage_idx_cache = sleep_stage_idx
                        break
            assert self.AH_events['SleepStage'].isna().sum() == 0, "存在未标记的呼吸事件"
