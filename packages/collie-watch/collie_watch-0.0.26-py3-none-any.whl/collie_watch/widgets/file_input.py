...
class FileInput(Widget):
    def __init__(self,id="",flex="",file_types=[],label="<button>Upload File</button>"):
        """
    ...
    def render(self):
        return f"""
    <label for="{self.id}">{self.label if not isinstance(self.label,Widget) else self.label.render()}</label>
    <input style="display: none;" type="file" id="{self.id}" accept="{' '.join(self.file_types)}">
"""
