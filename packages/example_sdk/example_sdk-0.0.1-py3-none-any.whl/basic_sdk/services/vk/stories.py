...
class VKStory(BaseModel):
    ...
    def parse_obj(cls, obj):
        result = super(VKStory, cls).parse_obj(obj)
        result.raw = obj
        return result


class VKNewStory(BaseModel):
    name: str
    description: str
    price: int
...
class VKStories:
    ...
    def list(self):
        ...
        if result:
            return [VKStory.parse_obj(item) for item in result["items"]]
    ...
    def add(self, product: VKNewStory):
        assert isinstance(
            product, VKNewStory
        ), f"Продукт передан не того типа: {product.__class__} != VKNewStory"
