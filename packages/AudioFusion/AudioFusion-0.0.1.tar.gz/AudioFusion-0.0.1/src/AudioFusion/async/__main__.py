...
class Fusion:
    ...
    async def loadSound(cls, inputFile: str):
        ...
        if os.path.isfile(inputFile) and inputFile.lower().endswith(('.mp3', '.wav', '.aac', '.ogg', '.flac', '.m4a')):
            return AudioSegment.from_file(inputFile, format=inputFile.split(".")[-1])
        else:
    ...
    async def effectReverb(
        ...
        wetLevel: float = 0.3,
        dryLevel: float= 0.8,
        outputFile: str = "tempWavFileForReverb.wav"
    ):
        """
    ...
    @classmethod
    async def saveSound(cls, sound, outputFile: str, ifReverb: bool = False, tempFile: str = "tempWavFileForReverb.wav"):
        """
        ...
        with open(outputFile + ".wav", "wb") as outf:
            sound.export(outf, format="wav")
        sound, sampleRate = soundfile.read(outputFile + ".wav")
