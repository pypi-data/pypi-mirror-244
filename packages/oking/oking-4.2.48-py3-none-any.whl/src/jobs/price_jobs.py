            ...
            if prices is not None and len(prices) > 0:
                ...
                for price in prices:
                    if api_prices.__len__() < 50:
                        ...
                        elif not src.client_data['operacao'].lower().__contains__('mplace'):
                ...
                if api_prices.__len__() > 0:
                    send_log(job_config_dict.get('job_name'), False, False, f'Enviando Pacote: {api_prices.__len__()}',
                             'info',
                             'PRECO', '')
                    if src.client_data['operacao'].lower().__contains__('okvendas'):
                        api_okVendas.post_prices(api_prices)
                    elif not src.client_data['operacao'].lower().__contains__('mplace'):
                        response = api_okHUB.post_prices(api_prices)
