...
class PxWallet:
    ...
    def create_blockcypher_hd_wallet(self):
        ...
        try:
                ...
                elif( str(blockcyper_wallet_response).lower().__contains__('unexpected') ):
                   
                    raise Exception((f"[blockcyper wallet request response] %s" %{str(blockcyper_wallet_response)}))
