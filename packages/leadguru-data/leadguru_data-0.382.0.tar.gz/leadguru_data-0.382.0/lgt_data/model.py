...
class DedicatedBotModel(BaseBotModel):
    def __init__(self):
        ...
        self.paused_servers: List[Server] = []
        self.state = 0
        self.source: Source | None = None
    ...
    def to_dic(self):
        ...
        result['active_servers'] = [Server.to_dic(server) for server in self.active_servers]
        result['servers'] = [Server.to_dic(server) for server in self.servers]
        result['paused_servers'] = [Server.to_dic(server) for server in self.paused_servers]
        return result
    ...
    def from_dic(cls, dic: dict):
        ...
        model.source = Source.from_dic(dic.get("source"))
        model.active_servers = [Server.from_dic(server) for server in dic.get("active_servers", [])]
        model.servers = [Server.from_dic(server) for server in dic.get("servers", [])]
        model.paused_servers = [Server.from_dic(server) for server in dic.get("paused_servers", [])]
        return model
