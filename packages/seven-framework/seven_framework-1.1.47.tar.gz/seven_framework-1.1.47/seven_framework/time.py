...
class TimeHelper:
    ...
    def get_last_day_of_the_month(self):
        ...
        """
        now_datetime = self.get_now_datetime()
        return datetime.datetime(now_datetime.year, now_datetime.month, calendar.monthrange(now_datetime.year, now_datetime.month)[1])
    ...
    def is_this_month(self, date, format='%Y-%m-%d %H:%M:%S'):
        ...
        if type(date) == int:
            ...
            compare_datetime = self.format_time_to_datetime(date, format)
        if compare_datetime.date() >= self.get_first_day_of_the_month(
        ).date() and compare_datetime.date() <= self.get_last_day_of_the_month().date():
            return True
        return False
    ...
    def is_this_year(self, date, format='%Y-%m-%d %H:%M:%S'):
        ...
        if type(date) == int:
            ...
            compare_datetime = self.format_time_to_datetime(date, format)
        if compare_datetime.date() >= self.get_first_day_of_the_year(
        ).date() and compare_datetime.date() <= self.get_last_day_of_the_year().date():
            return True
        return False