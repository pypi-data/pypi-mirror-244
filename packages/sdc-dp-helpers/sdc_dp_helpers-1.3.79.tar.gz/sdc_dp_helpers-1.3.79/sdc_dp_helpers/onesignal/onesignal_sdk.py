...
class ViewNotificationsHandler(OneSignalHandler):
    ...
    def check_limit_reached(
        ...
        for result in results:
            queued_at_date = int(self.format_date(result["queued_at"]))
            if queued_at_date > enddate:
                continue
            if queued_at_date < startdate:
                exit_condition.append(True)
            else:
                result["completed_at_date"] = self.format_date(
                    result.get("completed_at")
                )
                result["queued_at_date"] = str(queued_at_date)
                result["app_id"] = self.creds["app_id"]
                partition_dataset.setdefault(queued_at_date, []).append(result)

        if any(exit_condition):
            more_records = False
    ...
    def fetch_data(self):
        ...
        enddate: int = int(
            date_string_handler(self.configs.get("start_date", "today")).strftime(
                "%Y%m%d"
            )
        )
        partition_dataset: Dict[Any, Any] = {}
