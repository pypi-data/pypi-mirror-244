...
class ViewNotificationsHandler(OneSignalHandler):
    ...
    def check_limit_reached(
        ...
        for result in results:
            queued_at_date_str = self.format_date(result["queued_at"])
            queued_at_date_int = int(queued_at_date_str)
            if queued_at_date_int > enddate:
                continue
            if queued_at_date_int < startdate:
