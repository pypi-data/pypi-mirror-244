...
class Address:
    ...
    def set_addr(self, addr: str or bytes or InputBuffer):
        ...
        if isinstance(addr, str):
            ...
            raise ValueError(f"Invalid Address: cannot initialize from type {type(addr)}")
        if not addr_bin:
            return None
        ...
        if self._size == kWALLET_ADDR_SIZE or self._size == kNODE_ADDR_SIZE:
            ...
            raise RuntimeError(f"Invalid Address: prefix != num bytes given ({prefix_size} != {self._size})")
