...
class Transfer:
    ...
    def __init__(self, xfer: dict or bytes or InputBuffer or None = None):
        ...
        if isinstance(xfer, InputBuffer):
            ...
            raise ValueError(f"Invalid Transfer: could not initialize from {type(xfer)}")
    ...
    def set_canonical(self):
        ...
        if len(self._canonical) != self._size + 1:
            raise RuntimeError(f"Invalid Transfer: canonical size invalid ({len(self._canonical)} != {self._size + 1})")
