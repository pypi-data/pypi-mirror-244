...
class Signature:
    ...
    def set_sig(self, sig: str or bytes or InputBuffer):
        ...
        if isinstance(sig, str):
            ...
            raise ValueError(f"Invalid Signature: cannot initialize from type {type(sig)}")
        if not sig_bin:
            return None
        ...
        if self._size == kWALLET_SIG_SIZE or self._size == kNODE_SIG_SIZE:
            ...
            raise RuntimeError(f"Invalid Signature: prefix != num bytes given ({prefix_size} != {self._size})")
