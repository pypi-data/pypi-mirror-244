...
class DevvLogger(logging.Logger):
    ...
    def __init__(self, name, level=logging.NOTSET):
        ...
        output_loglevel = self._parse_severity(os.getenv('DEVV_OUTPUT_LOGLEVEL', 'info'))
        ...
        base_format = "%(timestamp)s %(levelname)-5s %(job_hash)s %(thread)x %(name)s [%(filename)s:%(lineno)d]" \
