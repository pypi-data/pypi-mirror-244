...
class DeltaPositionMonitor:
    ...
    def update_prices_and_greeks(self, iv_adjustment: float = 1.0) -> None:
        # Fetching prices
        underlying_ltp = self.underlying.fetch_ltp()
        call_ltp, put_ltp = self.strangle.fetch_ltp()
        tte = time_to_expiry(self.strangle.expiry)
        call_iv = self.strangle.call_option.fetch_iv(
            spot=underlying_ltp, price=call_ltp, t=tte
        )
        put_iv = self.strangle.put_option.fetch_iv(
            spot=underlying_ltp, price=put_ltp, t=tte
        )
        call_iv = call_iv * iv_adjustment
        put_iv = put_iv * iv_adjustment

        call_greeks = greeks(
            underlying_ltp,
            self.strangle.call_strike,
            tte,
            0.06,
            call_iv,
            "c",
    ...
    def adjust_strangle(self, target_strangle: Strangle):
        ...
        if target_strangle.call_option != self.strangle.call_option:
...
def calculate_strangle_deltas(
    ...
    option_prices = efficient_ltp_for_strangles(strangles)
    option_greeks = {
        option: option.fetch_greeks(spot=underlying_ltp, price=option_prices[option])
