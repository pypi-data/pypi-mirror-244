...
class Greeks:
    ...
    _delta: float = field(default=np.nan, repr=False)
    _gamma: float = field(default=np.nan, repr=False)
    _theta: float = field(default=np.nan, repr=False)
    _vega: float = field(default=np.nan, repr=False)
...
def error_handled_iv(opt_price, spot, strike, tte, opt_type):
    try:
        return implied_volatility(opt_price, spot, strike, tte, 0.06, opt_type)
...
def calculate_strangle_iv(
    ...
    call_iv = error_handled_iv(call_price, spot, call_strike, time_left, "c")
    put_iv = error_handled_iv(put_price, spot, put_strike, time_left, "p")
