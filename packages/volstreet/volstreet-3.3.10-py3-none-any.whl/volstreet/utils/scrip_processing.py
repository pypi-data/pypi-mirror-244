...
def get_expiry_dates(expiry_frequency):
    if expiry_frequency == "monthly":
        ...
        nearest_three_weekly = weekly_expiry_dates[:3]
        nearest_monthly = monthly_expiry_dates[:1]

        # Special case for bank-nifty
        if nearest_monthly[0] - current_time() < timedelta(
            days=7
        ):  # If we are in the week of monthly expiry
            if nearest_monthly[0] - current_time() < timedelta(
                days=1
            ):  # Special handling
                ...
                nearest_three_weekly[0] = nearest_monthly[0]
