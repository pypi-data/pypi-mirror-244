...
def delta_hedged_strangle(
    ...
    max_exposure: int,
    delta_range: tuple[float, float] = (0.35, 0.62),
    augmenting_multiple: int | float = 3.5,
    delta_interval_minutes: int = 1,
    iv_adjustment: float = 1.0,
    ...
    while current_time().time() < time(*scan_end_time):
        ...
        if (
            (target_strangle is not None)
            and (minimum_unevenness > augmenting_multiple * 0.8)
            or target_strangle is None
        ):
            message = (
                f"{underlying.name} {strategy_tag} unable to find strangle "
                f"with unevenness less than {augmenting_multiple}. "
                f"Minimum unevenness: {minimum_unevenness} on {target_strangle}"
            )
            logger.info(message)
            notifier(message, notification_url, "INFO")
            continue
        else:
            strangle = target_strangle

        notifier(
            f"{underlying.name} Deploying delta hedged strangle with {strangle}.",
            notification_url,
            "INFO",
        )
        ...
        while not any(position_monitor.exit_triggers.values()):
            ...
            position_monitor.update_prices_and_greeks(iv_adjustment=iv_adjustment)
            ...
            if abs(position_monitor.position_greeks.delta) > delta_threshold:
                ...
                if breach:
                    ...
                    if current_time().time() > time(*scan_end_time):
                        ...
                        success = position_monitor.process_breach(
                            augmenting_multiple, delta_range=delta_range
                        )
                        if not success:
                            position_monitor.exit_triggers["qty_breach_exit"] = True
                            message = f"{underlying.name} max qty breached and strike not eligible."
                            logger.info(message)
                            notifier(message, notification_url, "INFO")
                            break
                        else:
        ...
        logger.info(message)
        notifier(message, notification_url, "INFO")
        position_monitor.record_position_status(for_exit=True)
...
def intraday_strangle(
    ...
    if not call_sl and not put_sl:  # Both stop losses not hit
        ...
        exit_option_type: OptionType = OptionType.PUT if call_sl else OptionType.CALL
        ...
        if place_sl_orders or place_orders_on_sl:
            cancel_pending_orders(
                shared_info_dict[f"{exit_option_type.value}_stop_loss_order_ids"]
            )
        shared_info_dict[f"{exit_option_type.value}_exit_price"] = non_sl_exit_price
