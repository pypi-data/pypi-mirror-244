...
class PriceFeed(SmartWebSocketV2):
    ...
    def subscribe_tokens(self, tokens=None, mode=None, exchange_type=None):
        if tokens is None:
            tokens = ["99926000", "99926009", "99926037", "99926074"]
        if mode is None:
            mode = 1
    ...
    def add_options(self, *underlyings, range_of_strikes=10, expiries=None, mode=1):
        ...
        If expiries is None, then the current and next expiry are added.
...
class Option:
    ...
    def fetch_iv(
        ...
        return bs.error_handled_iv(
            price, spot, self.strike, t, opt_type=self.option_type.value
        )
    ...
    def fetch_greeks(
        ...
        iv = self.fetch_iv(spot=spot, t=t, effective_iv=effective_iv, price=price)
        return bs.greeks(
            spot,
            self.strike,
            t,
...
class Strangle:
    ...
    def fetch_greeks(
        ...
        call_greeks = bs.greeks(
            ...
            t,
            0.06,
            self.call_option.fetch_iv(spot=spot, price=call_price, t=t),
            "c",
        )
        put_greeks = bs.greeks(
            ...
            t,
            0.06,
            self.put_option.fetch_iv(spot=spot, price=put_price, t=t),
            "p",
        )
