...
class DeltaBackTest(IntradayBackTest):
    ...
    def moving_interest_rates(self, daily_prices: pd.DataFrame) -> pd.DataFrame:
        daily_prices = self._build_option_chain_skeleton(
        ...
        with_option_prices = self.merge_with_option_prices(daily_prices)
        with_option_prices["synthetic_rate"] = (
            with_option_prices["call_strike"]
            + with_option_prices["call_price"]
            - with_option_prices["put_price"]
        )
    ...
    def prepare_segment_for_processing(
        ...
        entry_snapshot = self.snapshot_at_entry(segment_prices.iloc[0], num_strikes=4)
    ...
    def run_day(
        ...
        moving_interest_rates = self.moving_interest_rates(intraday_prices)
