...
class TrafficSignInterpretionSimulator:
    ...
    def step(self, action="", user_type=''):
        ...
        if user_type == "school-zone":
            ...
            if right_lane_available and self.user_previous_action != "keep-right":
                self.required_action = "keep-right"
            else:
                self.required_action = ""
        elif user_type == "keep-left":
            if left_lane_available and self.user_previous_action != "keep-left":
                self.required_action = "keep-left"
            else:
                self.required_action = ""
        elif user_type == "stop":
        ...
        if self.running:
            ...
            # get the self.perception from host vehicle
            self.collided_with_road_block = self.host_vehicle.traffic_sign_sensing(self.sub_scenario_objects)
                    ...
                    if self.show_blank:
                        ...
                        while selected_type == self.previous_type:
                            ...
                            if len(self.traffic_sign_data[selected_type].images) == 0:
                                ...
                                if self.host_vehicle.get_speed() == 0 and (selected_type == 'keep-right' or selected_type == 'keep-left'):
