...
def open_storage_session_from_dir(storage_dir: Path, *, ensure_dir: bool = False) -> "FileSystemStorageSession":
    ...
    return FileSystemStorageSession.from_uri(uri)


def open_storage_session_from_uri_or_dir(
    storage_uri_or_dir: StorageUri | Path, *, ensure_dir: bool = False
) -> "StorageSession":
    if isinstance(storage_uri_or_dir, StorageUri):
        return StorageSession.from_uri(storage_uri_or_dir)

    if isinstance(storage_uri_or_dir, Path):
        return open_storage_session_from_dir(storage_uri_or_dir, ensure_dir=ensure_dir)
