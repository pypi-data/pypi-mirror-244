...
def iter_child_files_or_directories(base_dir: Path, *, ignore_hidden: bool = True) -> Iterator[Path]:
...
def merge_directories(source_dir: Path, target_dir: Path) -> None:
    shutil.copytree(source_dir, target_dir, dirs_exist_ok=True)
    shutil.rmtree(source_dir)


def move_children(source_dir: Path, target_dir: Path, *, overwrite: bool = False) -> None:
    for child in iter_child_files_or_directories(source_dir, ignore_hidden=True):
        if overwrite:
