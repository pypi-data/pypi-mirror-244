...
class JsonLinesDatasetDirectoryManager(BaseDatasetDirectoryManager[JsonLinesDatasetJson]):
    ...
    def iter_series(
        ...
        *,
        ...
        @cache
        def load_dimensions_codes_order() -> list[str] | None:
            try:
                dataset_json = self.load_dataset_json()
            except (DatasetJsonLoadError, DatasetJsonNotFound) as exc:
                raise DatasetSomeSeriesLoadError(dataset_id=self.dataset_id, revision_id=self.revision_id) from exc

            return dataset_json.dimensions_codes_order

        series_codes_list = None if series_codes is None else list(series_codes)

        def _iter() -> Iterator[Series]:
            dimensions_codes_order = None
            ...
            for json_lines_series_item in self.iter_json_lines_series_items(
                ...
                if isinstance(json_lines_series_item.dimensions, list):
                    dimensions_codes_order = load_dimensions_codes_order()

                try:
                    yield json_lines_series_item.to_domain_model(dimensions_codes_order=dimensions_codes_order)
