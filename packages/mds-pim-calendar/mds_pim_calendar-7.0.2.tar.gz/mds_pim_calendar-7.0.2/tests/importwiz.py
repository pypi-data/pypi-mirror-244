...
class ImportWizardTestCase(object):
    ...
    def prep_importwizard_calendar(self):
        ...
        with Transaction().set_user(usr1.id):
            create_calendar('Holiday', Calendar.default_owner())
    ...
    def test_importwizard_check_default(self):
        ...
        # no valid model
        with Transaction().set_context(active_model='pim_calendar.event'):
        ...
        with Transaction().set_context(
                active_model='pim_calendar.calendar', active_ids=[]):
            (sess_id, start_state, end_state) = ImportWizard.create()
            w_obj = ImportWizard(sess_id)
            self.assertTrue(w_obj)
            ...
            self.assertRaisesRegex(
                UserError,
                r"Please select \(only\) a calendar.",
                ImportWizard.execute,
                sess_id, {}, start_state)
    ...
    def test2_importwizard_store_icsfile(self):
        ...
        with Transaction().set_context(
            ...
            ev2 = Event.search([
                ('calendar', '=', cal1.id)],
                order=[('name', 'ASC')])  # select events in cal1
