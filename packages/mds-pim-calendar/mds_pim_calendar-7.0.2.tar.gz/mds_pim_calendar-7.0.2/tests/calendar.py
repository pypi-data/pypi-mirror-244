...
class CalendarTestCase(object):
    ...
    def test_calendar_add_visitors(self):
        ...
        self.prep_calendar_newuser('Frida', 'PIM Calendar - User')
        self.prep_calendar_newuser('Diego', 'PIM Calendar - User')
    ...
    def test_calendar_ical_read_ics(self):
        ...
        self.assertRaisesRegex(
            UserError,
            r"Error during import \(Content line could not be parsed " +
            r"into parts: 'nope': Invalid content line\)",
    ...
    def test_calendar_ical_addevent(self):
        ...
        usr1 = self.prep_calendar_newuser('Frida', 'PIM Calendar - User')
        create_calendar('Cal1', usr1)
        calendar, = Calendar.search([('name', '=', 'Cal1')])
        ...
        self.assertEqual(
            str(event1['DTSTART'].dt), '2008-06-21 09:00:00+02:00')
        self.assertEqual(
            str(event1['DTEND'].dt), '2008-06-21 09:15:00+02:00')
        self.assertEqual(event1['SUMMARY'], 'Ventile bewegen')
        event1a = copy.deepcopy(event1)
        del event1a['RRULE']

        Event.ical_add_events(calendar, [event1a])
        ...
        self.assertEqual(
            event2['DESCRIPTION'], 'Alle Termine auf www.schulferien.org')
        ...
        Event.ical_add_events(calendar, [event2])
