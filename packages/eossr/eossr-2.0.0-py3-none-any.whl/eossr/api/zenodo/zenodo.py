...
class ZenodoAPI:
    ...
    def answer_to_pending_request(self, community, record_id, action):
        """
        ...
        action : str
            The action to perform. Valid values are 'accept', 'decline', and 'cancel'.
        ...
        # get url in links
        if action not in pending_record['links']['actions'].keys():
            raise KeyError(
                f"Action {action} is not available for record {record_id}")
        else:
            url = pending_record['links']['actions'][action]
        # send request
        response = requests.post(
            url, params=self.parameters, timeout=_default_timeout)
        http_status.ZenodoHTTPStatus(response)
        return response
