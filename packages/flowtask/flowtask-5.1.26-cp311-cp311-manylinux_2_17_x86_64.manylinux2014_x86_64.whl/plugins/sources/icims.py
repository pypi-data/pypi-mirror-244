...
from querysource.exceptions import DataNotFound, ConfigError, OutputError
...
class icims(restSource):
    ...
    async def people(self):
        ...
        """
        self.url = self.original_url + 'customers/{customer_id}/search/people'
        self.type = 'people'
        self._legacy_call = True
        self.method = 'get'
    ...
    async def person(self):
        ...
        """
        self.url = self.original_url + 'customers/{customer_id}/people/{person_id}'
        self.type = 'person'
        self._legacy_call = True
        self.method = 'get'
    ...
    async def forms_list(self):
        ...
        Get a list of Forms.
        """
        self.url = self.original_url + 'customers/{customer_id}/forms/list'
    ...
    async def form_data(self):
        ...
        Get a list of Forms.
        """
        self.url = self.url + 'customers/{customer_id}/forms/{form_data_id}'
        self.type = 'form_data'
        self._legacy_call = True
        self.method = 'get'
        try:
        ...
        try:
            self._args['form_data_id'] = self._conditions['form_data_id']
            del self._conditions['form_data_id']
        except (KeyError, AttributeError) as exc:
            raise ValueError(
                "ICIMS: Missing Form Data ID"
            ) from exc
    ...
    async def query(self, url: str = None, params: dict = {}):
        ...
        try:
            ...
            if self.type == 'stream_data':
                ...
                self._result = result
            elif self.type == 'form_data':
                if 'errors' in result:
                    errors = []
                    for error in result['errors']:
                        message = f"{error['errorMessage']}, code: {error['errorCode']}"
                        raise OutputError(
                            message
                        )
                        return
                try:
                    owner = result['owner']
                    result['associate_id'] = str(owner['id'])
