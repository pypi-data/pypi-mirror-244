    ...
    async def run(self):
                ...
                if sql := mdl.model(dialect='sql'):
                    ...
                    async with await self.get_connection() as conn:
                        if _drop is True:
                            ...
                            self._logger.debug(
                                f'DROP Table: {result}, {error}'
                            )
                        result, error = await conn.execute(
                            sentence=sql
                        )
                        self._logger.debug(
                            f'Create Table: {result!s}'
                        )
                        if error:
                            raise ComponentError(
        ...
        if isinstance(self.data, pd.DataFrame):
            ...
            if hasattr(self, 'use_chunks') and self.use_chunks is True:
                ...
                chunks = (
                    self.data.loc[self.data.index[i:i + chunk_size]] for i in range(0, self.data.shape[0], chunk_size)
                )
                count = 0
                numrows = 0
                for chunk in chunks:
