...
class HTTPClient(DownloadFromBase):
    ...
    def __init__(
        ...
        # Executor:
        self._executor = ThreadPoolExecutor(max_workers=HTTPCLIENT_MAX_WORKERS)
        self._semaphore = asyncio.Semaphore(HTTPCLIENT_MAX_SEMAPHORE)  # Adjust the number as neededd
    ...
    async def async_request(self, url, method: str = 'GET', data: dict = None, use_json: bool = False):
        """
    ...
    async def process_response(self, response, url: str) -> tuple:
        ...
        if response.status > 399:
            if hasattr(self, 'download') and self.download is True:
            ...
            elif hasattr(self, 'download') and self.download is False:
    ...
    async def request(self, url, method: str = 'GET', data: dict = None) -> tuple:
        """
    ...
    def create_dataframe(self, result: Union[List, Dict]):
        """
        Converts the result of an HTTP request into a pandas DataFrame.
            ...
            for url in self.url:
                ...
                try:
                    if self.use_async is True:
                        result, err = await self.async_request(
                            uri, self.method
                        )
                    else:
