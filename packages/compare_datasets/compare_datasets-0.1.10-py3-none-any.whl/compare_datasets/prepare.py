...
class PrepareForComparison:
    ...
    def __init__( self, tested: pl.DataFrame, expected: pl.DataFrame, key=None, tolerance: float = 10e-6, verbose: bool = False, progress_bar=None, low_memory=False, case_sensitive_columns=False, strict_schema=False) -> None:
        ...
        if not self.case_sensitive_columns:
            if (self.key is not None):
                if self.verbose:
    ...
    def testColumnNames(self):
        ...
        if not self.case_sensitive_columns:
            self.verbose and logger.info("Converting column names to uppercase")
    ...
    def matchRowCounts(self):
        ...
        self.verbose and logger.info("====================MATCHING ROW COUNTS====================")
            ...
            if (not unique_keys_match) or self.low_memory:
                ...
                if not unique_keys_match:                    
                    ...
                    if self.expected.shape[0] == self.tested.shape[0]:
                        print("The keys provided do not UNIQUELY identify the rows in the provided dataframes. The comparison will be performed by matching the rows after sorting the dataframes on the provided key.\nIt is highly recommended to provide a column or a set of columns as key that uniquely identifies the rows in the dataframes for a robust comparison.")  
                    if self.verbose:
