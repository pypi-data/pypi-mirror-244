...
class Resolver(ResolverProtocol[P, T], abc.ABC):
    ...
    # Only used by OnlineResolver for now
    deferred_registry: "ClassVar[List[Callable[..., OnlineResolver]]]" = []

    hook: Callable[[], None] | None = None
    data_sources: Optional[List[BaseSQLSource]] = None
    ...
    def add_to_registry(self, unique_by_shortname: bool = False):
        ...
        :param deferred_resolver: If specified, add to the deferred registry instead of the main registry.
        """
    ...
    @classmethod
    def add_to_deferred_registry(cls, deferred_resolver: Callable[..., "OnlineResolver"]):
        cls.deferred_registry.append(deferred_resolver)

    @classmethod
    def get_resolvers_from_registry(cls):
        cls.registry.extend([r() for r in cls.deferred_registry])
        cls.deferred_registry[:] = []
        return cls.registry
...
class SinkResolver(Resolver[P, T]):
    ...
    deferred_registry: "list[Callable[..., SinkResolver]]" = []
    ...
    def __init__(
        ...
        data_sources: Optional[List[BaseSQLSource]] = None,
        lsp_builder: ResolverErrorBuilder | None = None,
    ):
        self.lsp_builder = lsp_builder or get_resolver_error_builder(fn)
        self.owner = owner
...
class OnlineResolver(Resolver[P, T]):
    def __init__(
        ...
        source_line: int | None = None,
        data_sources: list[BaseSQLSource] | None = None,
        lsp_builder: ResolverErrorBuilder | None = None,
    ):
        self.function_definition = function_definition
        self.fqn = fqn
    ...
    if not isinstance(default, state_typ):
        error_builder.add_diagnostic(
            message=(
                ...
                f"but default '{default.__name__}' does not match."
            ),
            code="119",
    ...
    *,
    ...
    def decorator(fn: Callable[P, T]):
        ...
        if (
            ...
            error_builder.add_diagnostic(
                message=f"Duplicate resolver '{parsed.fqn}'. Multiple resolvers cannot have the same name.",
                code="71",
                label="duplicate name",
                range=error_builder.function_name(),
                raise_error=TypeError,
            )
...
class StreamResolver(Resolver[P, T]):
    ...
    deferred_registry: "list[Callable[..., StreamResolver]]" = []
...
def parse_and_register_stream_resolver(
    *,
    ...
    for resolver in StreamResolver.get_resolvers_from_registry():
