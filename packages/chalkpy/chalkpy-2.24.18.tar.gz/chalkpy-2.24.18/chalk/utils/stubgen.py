...
from chalk.features import Feature, Features, FeatureSetBase, Vector
...
class StubGenerator:
    ...
    def _add_import(self, item: Union[str, type, typing._SpecialForm], import_as: Optional[str] = None) -> str:
    ...
    def _register_imports_for_annotation(self, annotation: type) -> Tuple[str, str]:
        ...
        if origin is None:
            assert args == ()
            if isinstance(annotation, type) and issubclass(annotation, Vector):
            ...
            if f.typ.as_dataframe() is not None:
                ...
                if include_in_proto_cls:
                    ...
                    annotations[f.attribute_name] = ParsedAnnotation(
                        ...
                        protocol_annotation="Any" if (f.typ.as_features_cls() is not None) else proto_annotation,
                    )
