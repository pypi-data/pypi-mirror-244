...
from chalk import OnlineResolver, Resolver
...
def _validate_offline_resolver_has_one(
    resolver: UpsertResolverGQL, builder: _LogBuilder, lsp_builder: ResolverErrorBuilder
):
    if resolver.kind != "offline":
        return

    for i, inp in enumerate(resolver.inputs or []):
        if inp.path is not None and len(inp.path) > 1:
            full_path = [inp.path[0].parent.fqn]
            for p in inp.path[1:]:
                full_path.append(p.child.namespace)
            full_path.append(inp.path[-1].parentToChildAttributeName)
            pathname = ".".join(full_path)
            lsp_builder.add_diagnostic(
                label="invalid offline resolver input",
                message=(
...
def validate_graph(request: UpsertGraphGQL) -> List[UpdateGraphError]:
    ...
    garbage_resolver_builder = ResolverErrorBuilder(uri="garbage.py", node=None)
    for resolver in request.resolvers or []:
        try:
            lsp_builder = garbage_resolver_builder
            if LSPErrorBuilder.lsp:
                for b in Resolver.registry:
                    if b.fqn == resolver.fqn:
                        lsp_builder = b.lsp_builder
                        break
            _validate_offline_resolver_has_one(resolver, builder, lsp_builder)
