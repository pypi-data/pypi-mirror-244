...
from chalk.features.resolver import Resolver, ResolverProtocol, StreamResolver
...
class SeriesBase:
    ...
    def __init__(
        ...
        self._all_valid_resolver_strings = set()
        for resolver in Resolver.get_resolvers_from_registry():
            self._all_valid_resolver_strings.add(resolver.fqn)
            self._all_valid_resolver_strings.add(resolver.name)
    ...
    def _feature_name_filter(self: TSeries, *features: Tuple[Any], equals: bool) -> TSeries:
        ...
        if len(features) == 1 or not equals:
            ...
            [self._validate_feature(value) for value in values]
