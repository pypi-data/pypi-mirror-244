...
class Vector:
    """Construct a Vector feature

    The Vector class can be used type annotation to denote a Vector feature -- for example:
    ...
    Instances of this class will be provided when working with raw vectors inside of resolvers. Generally, you do not need to construct
    instances of this class directly, as Chalk will automatically convert list-like features into Vector instances when working
    with a Vector annotation.

    Parameters
    ----------
    data: PyArrow Scalar, Numpy Array, or List[Float]
        The vector values
    ...
    def is_near(
        ...
        """Define a nearest neighbor relationship for performing Vector similiarity search
