...
class _TimeUnitCode(str, enum.Enum):
    SECOND = "s"
    MILLISECONDS = "ms"
    MICROSECONDS = "us"
    NANOSECONDS = "ns"
...
class _BaseSerializedDType(BaseModel):
...
class _SingletonDType(_BaseSerializedDType):
    type_code: Literal[
        _DTypeCode.BOOL,
        _DTypeCode.INT8,
        _DTypeCode.INT16,
...
class _TimeUnitDType(_BaseSerializedDType):
...
class _TimestampDType(_BaseSerializedDType):
...
class _BinaryDType(_BaseSerializedDType):
...
class _ListDType(_BaseSerializedDType):
...
class _LargeListDType(_BaseSerializedDType):
...
class _StructDType(_BaseSerializedDType):
    type_code: Literal[_DTypeCode.STRUCT]
    fields: List[_StructField]
...
_SerializedDType = Annotated[
    Union[_SingletonDType, _TimeUnitDType, _TimestampDType, _BinaryDType, _ListDType, _LargeListDType, _StructDType],
    Field(discriminator="type_code"),
]

_StructField.update_forward_refs()
...
def serialize_pyarrow_dtype(pa_dtype: pa.DataType) -> _SerializedDType:
    """Deprecated -- use `serialize_dtype` instead. For backwards compatibility with Engine"""
    return _serialize_pyarrow_dtype(pa_dtype)
...
def _serialize_pyarrow_dtype(pa_dtype: pa.DataType) -> _SerializedDType:
    ...
    if pa.types.is_time32(pa_dtype):
        ...
        return _TimeUnitDType(type_code=_DTypeCode.TIME32, time_unit=_TimeUnitCode(pa_dtype.unit))

    if pa.types.is_time64(pa_dtype):
        assert isinstance(pa_dtype, pa.Time64Type)
        return _TimeUnitDType(type_code=_DTypeCode.TIME64, time_unit=_TimeUnitCode(pa_dtype.unit))
    ...
    if pa.types.is_timestamp(pa_dtype):
        ...
        return _TimestampDType(
            ...
            time_unit=_TimeUnitCode(pa_dtype.unit),
    ...
    #     return _TimeUnitDType(type_code=_DTypeCode.DURATION, time_unit=_TimeUnitCode(pa_dtype.unit))
    ...
    if pa.types.is_struct(pa_dtype):
        ...
        return _StructDType(
            ...
            fields=[
                _StructField(name=name, dtype=_serialize_pyarrow_dtype(dtype))
