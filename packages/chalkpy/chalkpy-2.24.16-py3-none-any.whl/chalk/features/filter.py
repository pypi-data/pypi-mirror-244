...
from typing import TYPE_CHECKING, Any, Hashable, Set, Type, Union

if TYPE_CHECKING:
    from chalk.features.feature_field import Feature

_DISABLE_FILTER_BOOL: contextvars.ContextVar[bool] = contextvars.ContextVar("DISABLE_FILTER_BOOL", default=False)


@contextlib.contextmanager
def disable_filter_bool(value: bool = True):
    token = _DISABLE_FILTER_BOOL.set(value)
    try:
        yield
    finally:
        _DISABLE_FILTER_BOOL.reset(token)


class FilterBoolNotAllowedError(ValueError):
    """Exception that is raised if Filter.__bool__ is called when DISABLE_FILTER_BOOL is True"""
...
class Filter:
    ...
    def __bool__(self):
        if _DISABLE_FILTER_BOOL.get():
            raise FilterBoolNotAllowedError(
                "Joining filters together with `and` or `or` is not allowed in the current context"
            )
        else:
            # By default custom class instances are always "True"
            return True
