...
def get_sql_file_resolver(sources: Iterable[BaseSQLSource], sql_string_result: SQLStringResult) -> ResolverResult:
    ...
    if len(outputs) == 0:
        errors.append(
            ResolverError(
                display=(
                    "No outputs detected. Make sure that all select outputs are aliased to features "
...
def _parse_glot(glot_result: GlotResult, path: str, sources: Iterable[BaseSQLSource]) -> ParseResult:
    ...
    if not source:
        errors.append(
            ResolverError(
                display=f"Source '{source_name}' not found. "
                f"This error may occur when the the code describing your data source fails to register due to a thrown exception",
                path=path,
                parameter=source_name,
            )
        )
