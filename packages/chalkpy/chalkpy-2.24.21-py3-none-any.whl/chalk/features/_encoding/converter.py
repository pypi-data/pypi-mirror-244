...
def _recursively_unwrap_numpy(x: Any) -> Any:
    if isinstance(x, list):
        return [_recursively_unwrap_numpy(y) for y in x]
    if isinstance(x, dict):
        return {k: _recursively_unwrap_numpy(v) for (k, v) in x.items()}
...
class FeatureConverter(Generic[_TPrim, _TRich]):
    ...
    def from_pyarrow_to_primitive(self, values: Union[pa.Array, pa.ChunkedArray]) -> Sequence[_TPrim]:
        return [_recursively_unwrap_numpy(x) for x in values.to_pylist()]
    ...
    def from_primitive_to_pyarrow(self, value: Sequence[_TPrim]) -> Union[pa.Array, pa.ChunkedArray]:
        ...
        if pa.types.is_fixed_size_list(self._pyarrow_dtype):
            ...
            if pa.types.is_float16(self._pyarrow_dtype.value_type):
                empty = [None] * self._pyarrow_dtype.list_size
                # NOTE: Numpy converts both nan and null to nan for individual array elements.
                # Because pa.compute.if_else doesn't work for float16 arrays, it is difficult to flip the null bits from the python api
                ...
                ans = pa.FixedSizeListArray.from_arrays(
                    ...
                    self._pyarrow_dtype.list_size,
                )
                mask = [x is not ... and x is not None for x in value]
                ans = pa.compute.if_else(pa.array(mask, pa.bool_()), ans, pa.scalar(None, self._pyarrow_dtype))
                return ans
    ...
    def from_primitive_to_json(
        ...
        if options.encode_structs_as_objects:
    ...
    def is_rich_valid(self, value: _TRich):
