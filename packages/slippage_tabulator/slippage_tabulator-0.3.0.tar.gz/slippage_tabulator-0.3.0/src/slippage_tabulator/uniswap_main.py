...
For multi path routes (MPR), I am making the assumption that each of the paths have identical in/out tokens.
After analyzing multiple paths in hybrid routes, I've found they all have the same in/out across routes.
This means the main purpose of hybrid routes is not to extend a path so that it can pass from V2 to V3, but instead to reduce fees (v3) and potential slippage.

With this assumption in mind, I will do a weighted average of slippages, weighted on the proportion of the entire swap of the exact asset.
I will create something to validate if this assumption always holds true.


Function Uniformity Assumption

I will assume that in a multi path transaction, 
agnostic to version, all functions are either exact out 
or exact in, but never a mix of both.
...
class Uniswap:
    ...
    def process_bridge_routes(self,web3,data,bridge_routes):
        ...
        for route in bridge_routes:
            ...
            returnable_list.append({'slippage':slippage,'amount_out':slippage_dict['amount_out'],'amount_in':amount_in0,'in_token':in_token})
