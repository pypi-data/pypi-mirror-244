...
class Object:
    ...
    def calculate(self):
        ...
        if self.T_out_target>self.T_in:
            ...
            self.ho=air_humide.Enthalpie(self.T_out_target,self.wi)
            # self.ho=air_humide_NB.Air2_Hs(self.T_out_target, air_humide_NB.Air4_RH(self.T_out_target,self.Outlet.P, self.wi/1000), self.Outlet.P) 
          #  print("ho=",self.ho)
            self.F_dry=(self.F)/(1+self.wi/1000) # [kg air sec/s]
           ...
           # print("self.Q_th=",self.Q_th)
            self.RH_out=air_humide.HR(air_humide.func_Pv_sat(self.T_out_target),self.wi,self.Outlet.P) #parametrer la pression
