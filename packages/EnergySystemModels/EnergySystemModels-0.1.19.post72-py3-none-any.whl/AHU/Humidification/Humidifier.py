...
class Object:
    ...
    def calculate(self):
        ...
        if self.w_out_target>self.w_in:
            # print("self.w_out_target>self.w_in",self.w_out_target,self.w_in)
            ...
            if self.HumidType=="adiabatique" :
                ...
                def syst(var): # définition du système
                    ...
                    eq1 =self.Pv_sat_out-air_humide.func_Pv_sat(self.T_out)
                    eq2 =self.w_out_target-air_humide.HA(self.Pv_sat_out,self.RH_out,101325)
                    eq3 =self.h_in-air_humide.Enthalpie(self.T_out, self.w_out_target)
                    res = [eq1, eq2, eq3]
                    return res
                #self.h_out=self.h_in
               ...
               # print(x)
                ...
                self.F_water=self.F_dry*(self.w_out_target-self.w_in)/1000 #débit d'eau en kgH2O/s
               # print("self.F",self.F,"\n","self.F_water",self.F_water,"\n","self.T_out",self.T_out,"\n","self.w_out_target",self.w_out_target,"\n","self.RH_out",self.RH_out,"\n")
            
            if self.HumidType=="vapeur" :
              #  print("hvap=",self.h_vap)
                def syst(var): # définition du système
                    self.Pv_sat_out,self.T_out, self.RH_out,self.h_out = var[0], var[1], var[2], var[3] # définition des variables
                    eq1 =self.Pv_sat_out-air_humide.func_Pv_sat(self.T_out)
                    eq2 =self.w_out_target-air_humide.HA(self.Pv_sat_out,self.RH_out,101325)
                    eq3 =self.h_out-air_humide.Enthalpie(self.T_out, self.w_out_target)
                    eq4 =self.h_out-self.h_in-(self.w_out_target-self.w_in)/1000*self.h_vap
                    res = [eq1, eq2, eq3, eq4]
                    return res
                #self.h_out=self.h_in
               ...
               # print(x)
                ...
                self.F_water=self.F_dry*(self.w_out_target-self.w_in)/1000 #débit d'eau en kgH2O/s = [kg_air_sec/s] * kg_H20 /kg_air_sec
            ...
            self.Outlet.w=self.w_out_target
           ...
           # self.F_water=self.F_dry*(self.w_out_target-self.w_in)/1000
