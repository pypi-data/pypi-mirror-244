...
def add_parser(subparsers: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    ...
    parser.add_argument("-c", "--config", type=lambda p: Path(p).absolute(), help="Configuration file (as JSON/YAML)")
    parser.add_argument(
        "-a",
        "--ansible-version",
        type=str,
        ...
        metavar="[2.0, 2.16]",
        help="Target Ansible Core version to scan against (e.g., 2.16). If not specified, Spotter will try to discover "
...
def scan(
    ...
    with progress:
        ...
        if export_payload:
            ...
            api_client = ApiClient(api_endpoint, storage, api_token, username, password, debug)
            ...
            if project_id:
                ...
                response_scan_async = api_client.post(
                    ...
                    payload=to_jsonable_python(scan_payload),
            ...
            try:
                ...
                while scan_progress.progress_status.value not in ending_statuses:
                    response_scan = api_client.get(
                        f"/v3/scans/{scan_start_async.uuid}", timeout=timeout if timeout else 120
                    )
            ...
            try:
                formatted_output = scan_result.format_output(output_format, no_color, no_docs_url, rewrite)
                if output_path:
