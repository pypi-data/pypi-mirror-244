...
def Test_DrawTree():
	...
	#Line="我们大家今天下午在操场集合"
	gpf=GPF()
	#Line=gpf.RunService(Line,Structure="Tree")
	gpf.AddGridJS(Line)
	print(Line)
	DrawTree(gpf)
...
def BCC_DrawTree():
	Line="我们大家今天下午在操场集合"
	gpf=GPF()
	Line=gpf.Parse(Line,Structure="Tree")
	gpf.AddGridJS(Line)
	DrawTree(gpf)
...
def Test():
    gpf = GPF()
    Files=["D:/Xunendong/GPF/Src/pysetup/gpflib/Examples/BuildBCC/treebankEx.txt"]
...
def Test3():
    ...
    Num=0
    for S in In:
        Ret=gpf.Parse(S.strip(),Structure="Chunk")
        if Num%500 == 0:
            print(Num,Ret)
        Num+=1 
    In.close()
...
def Test6():
    gpf = GPF()   
    Ret=gpf.BCC("(a)的",Command="Freq",Target="$1",WinSize=30,Speedup=10)
    print(Ret)

def Test7():
    gpf = GPF()   
    Ret=gpf.BCC("(a)的",Command="Freq",Target="$1",WinSize=30,ContextNum=5)
    print(Ret)

def Parse1():
    Line="我们大家今天下午在邵逸夫操场集合"
    gpf=GPF()
    Ret=gpf.Parse(Line,Structure="POS")
    print(Ret)
...
def Parse2():
    ...
    gpf=GPF()
    gpf.SetGridText(Line)
    gpf.AddGridKV("A","B V,O;U")
    Value=gpf.GetGridKV("A")
    for V in Value:
        print(V)
