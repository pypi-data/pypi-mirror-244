...
class bezierTrajectory:
    ...
    def trackArray(self, start, end, numberList, le=4, deviation=5, bias=0.5, type=3, cbb=0, yhh=0):
        '''
        :param start:开始点的坐标 如 start = [0, 0]
        :param end:结束点的坐标 如 end = [100, 100]
    ...
    def combinationTrack(self, x, y, start, end):
        rx = random.randint(x[0], y[0])
        ry = random.randint(x[1], y[1])
        number = random.randrange(15, 20)
        le = random.randrange(2, 4)
        t = random.randrange(1, 3)
        track1 = self.trackArray(start, [rx, ry], number, le=le, type=t)['trackArray']
        # print('track1 -> ', track1)

        track2 = self.trackArray([rx, ry], end, number, le=le, type=t)['trackArray']
        # print('track2 -> ', track2)
        return np.concatenate((self.sort(start, [rx, ry], track1), self.sort([rx, ry], end, track2)), axis=0)

    def sort(self, start, end, track):
        if start[0] > end[0]:
            return track[track[:, 0].argsort()][::-1]
        else:
            return track[track[:, 0].argsort()]

    def distance(self, p1, p2):  # x1, y1, x2, y2
