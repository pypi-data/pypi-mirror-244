...
class CpModelSolver(IModel):
    ...
    def addVar(self, lb: Real = 0, ub: Real = INFINITY,
        ...
        if vtype == Vtype.CONTINUOUS or vtype == Vtype.INTEGER:
            var1: CpVar = CpVar(tempModel, Domain(lb, ub), name, self.__solver)
        elif vtype == Vtype.BINARY:
            var1: CpVar = CpVar(tempModel, Domain(0, 1), name, self.__solver)
        self.__all_vars.append(var1)
        return var1

    def addVars(self, *indices, lb: Real = 0, ub: Real = INFINITY, vtype: Vtype = Vtype.CONTINUOUS,
                name: str = "") -> tupledict:
        li = []
        for ind in indices:
