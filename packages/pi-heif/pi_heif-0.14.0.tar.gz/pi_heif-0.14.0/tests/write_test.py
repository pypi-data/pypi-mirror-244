...
if parse_version(pi_heif.libheif_version()) < parse_version("1.17.3"):
...
def test_save_format(save_format):
    ...
    mime = pi_heif.get_file_mimetype(buf.getbuffer().tobytes())
...
def test_premultiplied_alpha(img):
    heif_file = pi_heif.open_heif(img, convert_hdr_to_8bit=False)
    ...
    out_heif_file = pi_heif.open_heif(out_heic, convert_hdr_to_8bit=False)
...
def test_outputs():
    heif_file = pi_heif.open_heif(helpers.create_heif((31, 64)))
    output = BytesIO()
...
def test_heif_save_one_all():
    im = pi_heif.open_heif(helpers.create_heif((61, 64), n_images=2))
    out_heif = BytesIO()
    im.save(out_heif, format="HEIF")
    assert len(pi_heif.open_heif(out_heif)) == 2
    im.save(out_heif, format="HEIF", save_all=False)
    assert len(pi_heif.open_heif(out_heif)) == 1
...
def test_heif_no_encoder():
    with mock.patch.dict("pi_heif.heif._pi_heif.lib_info", {"libheif": "1.14.2", "HEIF": "", "AVIF": ""}):
        im_heif = pi_heif.from_pillow(Image.new("L", (64, 64)))
        out_buffer = BytesIO()
...
def test_heif_save_zero(size: tuple):
    out_heif = BytesIO()
    im = pi_heif.from_bytes("L", size, b"")
    with pytest.raises(ValueError):
        im.save(out_heif, format="HEIF")


def test_heif_save_empty():
    heic_file = pi_heif.open_heif(helpers.create_heif())
    del heic_file[0]
    with pytest.raises(ValueError):
...
def test_save_empty_with_append():
    out_buffer = BytesIO()
    empty_heif_file = pi_heif.HeifFile()
    heif_file = pi_heif.open_heif(helpers.create_heif())
    empty_heif_file.save(out_buffer, append_images=[heif_file[0]])
    helpers.compare_heif_files_fields(heif_file, pi_heif.open_heif(out_buffer))
    empty_heif_file.save(out_buffer, append_images=[heif_file[0]], save_all=False)
    heif_file = pi_heif.open_heif(out_buffer)
    assert len(heif_file) == 1


def test_hif_file():
    hif_path = Path("images/heif_other/cat.hif")
    heif_file1 = pi_heif.open_heif(hif_path)
    assert heif_file1.info["bit_depth"] == 10
    out_buf = BytesIO()
    heif_file1.save(out_buf, quality=80)
    heif_file2 = pi_heif.open_heif(out_buf)
...
def test_hdr_save(im_path, save_format):
    ...
    heif_file = pi_heif.open_heif(im_path, convert_hdr_to_8bit=False)
    out_buf = BytesIO()
    heif_file.save(out_buf, quality=-1, format=save_format, chroma=444)
    heif_file_out = pi_heif.open_heif(out_buf, convert_hdr_to_8bit=False)
...
def test_encoder_parameters():
    ...
    im_heif = pi_heif.open_heif(heif_buf)
...
def test_heif_YCbCr_color_mode():  # noqa
    ...
    im_heif = pi_heif.from_pillow(im)
    assert im_heif.mode == "RGB"
    out_heif = BytesIO()
...
def test_I_color_modes_to_10_12_bit(enc_bits, save_format):  # noqa
    try:
        pi_heif.options.SAVE_HDR_TO_12_BIT = bool(enc_bits == 12)
        ...
        for mode in ("I", "I;16", "I;16L"):
            ...
            assert pi_heif.open_heif(out_heic, convert_hdr_to_8bit=False).info["bit_depth"] == enc_bits
            heic_pillow = Image.open(out_heic)
            helpers.compare_hashes([src_pillow, heic_pillow], hash_size=8)
    finally:
        pi_heif.options.SAVE_HDR_TO_12_BIT = False
...
def test_gif():
    ...
    helpers.compare_hashes([gif_pillow, out_heic])
    # convert all frames of gif(pi_heif does not skip identical frames and saves all frames like in source)
...
def test_add_from():
    heif_file1 = pi_heif.open_heif("images/heif/L_8__29x100.heif")
    heif_file2 = pi_heif.open_heif("images/heif/RGB_8__128x128.heif")
    im_pil3 = Image.open("images/non_heif/RGBA_8__29x100.png")
    ...
    out_heif = pi_heif.open_heif(out_buf)
...
def test_remove():
    ...
    _ = pi_heif.open_heif(out_buffer)
    assert len(_) == 2
...
def test_heif_save_multi_frame():
    heif_file = pi_heif.open_heif(Path("images/heif_other/nokia/alpha.heic"), convert_hdr_to_8bit=False)
    heif_buf = BytesIO()
    heif_file.save(heif_buf, quality=-1)
    out_heif_file = pi_heif.open_heif(heif_buf, convert_hdr_to_8bit=False)
    for i in range(3):
...
def test_encode_function(mode, size: tuple):
    ...
    im = im.convert(mode)
    buf = BytesIO()
    pi_heif.encode(im.mode, im.size, im.tobytes(), buf, quality=-1)
...
def test_encode_function_with_stride(mode):
    ...
    im = im.convert(mode)
    buf = BytesIO()
    pi_heif.encode(im.mode, (257, im.size[1]), im.tobytes(), buf, quality=-1, stride=512 * len(mode))
...
def test_encode_function_not_enough_data(mode):
    buf = BytesIO()
    with pytest.raises(ValueError):
        pi_heif.encode(mode, (128, 128), b"123456", buf)
...
def test_invalid_ispe_stride(image_path):
    im = pi_heif.read_heif(image_path)
    buf = BytesIO()
    pi_heif.encode(im.mode, im.size, im.data, buf)
    im = pi_heif.open_heif(buf)
...
def test_nclx_profile_write():
    ...
    try:
        pi_heif.options.SAVE_NCLX_PROFILE = False
        ...
        pi_heif.options.SAVE_NCLX_PROFILE = True
