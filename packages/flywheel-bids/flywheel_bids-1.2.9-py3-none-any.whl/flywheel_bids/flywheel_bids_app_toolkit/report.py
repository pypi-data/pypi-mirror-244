...
def package_output(
    ...
    if app_context.save_intermediate_output:
        work_zip_file_name = (
            f"{gear_name}_work_{app_context.run_label}_{app_context.destination_id}.zip"
        )
        zip_output(
    ...
    if app_context.save_intermediate_files:
        selected_work_zip_file_name = f"{gear_name}_work_selected_files_{app_context.run_label}_{app_context.destination_id}.zip"
    ...
    if app_context.save_intermediate_folders:
        ...
        for dir in dirs:
            selected_work_zip_file_name = f"{gear_name}_work_{str(Path(dir).name)}_{app_context.run_label}_{app_context.destination_id}.zip"
            zip_output(
                str(Path(dir).parent), str(Path(dir).name), selected_work_zip_file_name
            )
