...
def test_archive(tmp_path):
    ...
    with ZipFile(test_zip_path, "w") as zipf:
        zipf.writestr("file1.txt", "Test file 1")
        zipf.writestr("file2.txt", "Test file 2")
...
def test_unzip_archive_files(test_archive, mock_context):
    ...
    assert (Path(result_dir) / "file1.txt").is_file()
    assert (Path(result_dir) / "file2.txt").is_file()
...
def test_htmls(tmp_path):
    ...
    html_file1 = test_dir / "file1.html"
    html_file1.touch()

    html_file2 = test_dir / "folder" / "file2.html"
...
def test_zip_htmls(test_htmls, tmp_path):
    ...
    zip_file1 = tmp_path / f"file1_{destination_id}.html.zip"
    assert zip_file1.is_file()

    zip_file2 = tmp_path / f"file2_{destination_id}.html.zip"
    assert zip_file2.is_file()

    # Assert that the zip files contain the correct files
    with ZipFile(zip_file1, "r") as zip1:
        assert any("file1.html" in n for n in zip1.namelist())

    with ZipFile(zip_file2, "r") as zip2:
        assert any("folder/file2.html" in n for n in zip2.namelist())
...
def test_data(tmp_path):
    ...
    test_file.write_text("There is something in this file.")
...
def test_zip_it_zip_it_good(test_data, tmp_path):
    test_dir, test_file = test_data

    output_dir = tmp_path
    destination_id = "1234abcd"

    zip_it_zip_it_good(str(output_dir), destination_id, test_file)

    # Assert that the zip file is created in the output directory
    zip_file = tmp_path / f"test_{destination_id}.html.zip"
    assert zip_file.is_file()

    # Assert that the zip file contains the correct file
    with ZipFile(zip_file, "r") as zip_ref:
        assert any("test.html" in n for n in zip_ref.namelist())
        for file_name in zip_ref.namelist():
            # Check if the desired file name is found, regardless of path differences and case sensitivity
            if "test.html" in file_name.lower():
                with zip_ref.open(file_name) as f:
                    extracted_content_bytes = f.read()
                    extracted_content = extracted_content_bytes.decode("utf-8")
                    assert extracted_content == "There is something in this file."
                    break
        else:
            raise ValueError("The file 'test.html' was not found in the zip archive.")
