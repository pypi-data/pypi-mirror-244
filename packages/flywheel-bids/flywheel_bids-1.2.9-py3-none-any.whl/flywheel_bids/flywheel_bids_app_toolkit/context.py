"""Parse includes the parser module to parse gear config.json
   as well as name parsing functions."""
import argparse
import logging
...
def parse_unknown_args(unknown_args: list) -> Dict:
    """
    Generates a dictionary of optional parameters entered to the command-line field on the config page.

    Takes advantage of ArgParse to convert the command-line supplied into known args (intentionally
        undefined and discarded by Flywheel b/c all four positional entries must be overwritten to
        coincide with FW args) and unknown args. This method uses the list of unknowns as input that
        is converted into a dictionary of options and corresponding args to be passed back into the
...
class BIDSAppContext:
    def __init__(
        ...
        self.parse_run_settings(gear_context)
    ...
    def parse_app_settings(
        ...
        self.bids_app_modalities = gear_context.manifest.get("custom").get(
            "bids-app-modalities"
        )
    ...
    def parse_bids_app_commands(
        ...
        # None of the first three, typical (positional) BIDS App args are defined (by design)
        # We need to build up the first four args our way
        # The combo of known and unknown arg calls will give us the rest of the args
        ...
        # As of 2023, only participant can be enabled on FW
    ...
    def parse_performance_settings(self, gear_context: GearToolkitContext):
        ...
        """
        if gear_context.get_input("n_cpus"):
        ...
        if gear_context.get_input("mem_mb"):
            self.bids_app_options["mem"] = int(
                1024 * set_mem_gb((gear_context.config.get("n_cpus") or 0) / 1024)
            )
    ...
    def parse_run_settings(
        ...
        """
        # self.analysis_level: gear_context.manifest.get("custom").get("analysis-level")
