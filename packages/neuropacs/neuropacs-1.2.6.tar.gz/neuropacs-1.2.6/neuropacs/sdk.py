...
class neuropacs:
    ...
    def upload_dataset(self, directory, connection_id, order_id, aes_key):
        ...
        with tqdm(total=total_files, desc="Uploading", unit="file") as prog_bar:
            for dirpath, _, filenames in os.walk(directory):
                for filename in filenames:
                    ...
                    self.upload(file_path, connection_id, order_id, aes_key)
                    prog_bar.update(1)  # Update the outer progress bar for each file
 
        self.disconnect_from_socket()
        return 201   

    def upload(self, data, connection_id, orderID, aes_key):
        ...
        encrypted_orderID = self.encrypt_aes_ctr2(orderID, aes_key, "string")
        ...
        if isinstance(data,bytes):
            ...
            headers = {
            "Content-Type": "application/octet-stream",'connection-id': connection_id, 'client': 'API', 'order-id': encrypted_orderID
            }
            ...
            with open(data, 'rb') as f:
                ...
                max_ack_wait_time = 10   #5 seconds
    ...
    def run_job(self, connectionID, aes_key, productID, order_id ):
        """Run a job
    ...
    def check_status(self, connection_id, aes_key, order_id):
        """Check job status
        ...
        if res.status_code == 200:
            print(res.text)
            # enrcyptedResponse = res.text
            # decryptedResponse = self.decrypt_aes_ctr(enrcyptedResponse,aes_key,"json")
            # return decryptedResponse
            return True
        else:
            raise RuntimeError("Status check failed.")

    def get_results(self, connection_id, aes_key, orderID, format):
        """Get job results
