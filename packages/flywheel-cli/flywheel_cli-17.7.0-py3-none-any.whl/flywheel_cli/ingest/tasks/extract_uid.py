...
class ExtractUIDTask(Task):
        ...
        for filepath in item.files:
            ...
            with self.walker.open(
                ...
                if isinstance(result, T.UIDIn):
                    self.insert_uids.push(result.model_dump(exclude_none=True))
                    uids.append(result)
                    ...
                    self.update_items.push(
                        {
                            "id": item.id,
                            "context": item.context.model_dump(exclude_none=True),
                        }
                    )
                elif isinstance(result, T.Error):
                    self.insert_errors.push(result.model_dump())
                else:
    ...
    def _scan_dicom_file(
        ...
        acq = dcm.get("AcquisitionNumber", None)
        if acq:
            data["acquisition_number"] = str(acq)
