...
class GFF3:
    ...
    You may have additional information to be added within the GFF.
    This class support eggnog files, which have 13 columns
    (see :meth:`GFF3.read_eggnog_annotation`). You can read these files and then
    save the gff.
    ...
    def read_eggnog_annotation(self, filename, col="Label", merge_on="locus_tag"):
        """Read eggnog annotation and update current annotation

        :param col: the column in the eggnog annotation to use as key to merge on
        :param merge_on: the column in original annotation to use as key to merge on

        The eggnog file is a tabulated file with 13 columns that must be labelled
        as follows: Label, Type, Frame, Begin, End, Mutation, Gene, Product,
        Score, OG_id, COGclassID, ClassDescription, COGprocess
        ...
        df.drop(["Type", "Frame", "Mutation", "Gene", "Score"], inplace=True, axis=1)

        # sanity check
        for x in ["Product", "OG_id", "COGclassID", "ClassDescription", "COGprocess"]:
            assert x in df.columns

        df.set_index("Label", inplace=True)
        df["Label"] = df.index
        cols = ["Begin", "End", "Product", "OG_id", "COGclassID", "ClassDescription", "COGprocess", "Label"]
        for index in tqdm(df.index):
            data = []
            for col in cols:
                try:
                    datum = df.loc[index][col].values
                    if len(set(datum)) == 1:
                        datum = datum[0]
                    else:
                        datum = ",".join(datum)
                    ...
                    datum = df.loc[index][col]
                    data.append(datum)

            self.df.loc[self.df.query("locus_tag==@index").index, cols] = data

        # We overwrite the attributes, which is used to save the data in method to_gff()
        for x in self.df.iterrows():
            for col in ["Product", "OG_id", "COGclassID", "ClassDescription", "COGprocess"]:
                self.df.loc[x[0], "attributes"][col] = x[1][col]
