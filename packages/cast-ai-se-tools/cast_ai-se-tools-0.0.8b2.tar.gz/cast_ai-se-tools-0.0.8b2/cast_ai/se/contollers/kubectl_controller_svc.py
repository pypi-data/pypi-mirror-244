...
from cast_ai.se.misc_utils import get_get_deployments_command
from se.services.logging_svc import setup_logging
...
class KubectlController:
    ...
    def scale_deployments(self, replica_count: int) -> ExecutionStatus:
        ...
        try:
            ...
            self._logger.debug(f"Command=[{str(get_deployments_command)}] | Output=[{result.strip()}]")
