...
class TieUi:
    def __init__(self, app_name=None, og_app_name=None, execute_callback=None, isLocal=None):
        ...
        if isLocal: 
            ...
            node_modules_path = os.path.join(npm_path, 'node_modules')
            if not os.path.exists(node_modules_path):
                # Run npm install if node_modules does not exist
                npm_install_command = f'npm install'
                subprocess.run(npm_install_command, shell=True, cwd=npm_path)

            # TODO: somehow cd into local-version and run npm install ONLY IF IT IS NOT INSTALLED BEFORE OR MAYBE BY JUST UPLOADING THE NODE MODULES YOU ARE GOOD?
