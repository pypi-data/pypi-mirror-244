...
class ConversableAgent(Agent):
    ...
    def __init__(
        ...
        # 支持异步方法
        self.register_reply([Agent, None], ConversableAgent.a_generate_oai_reply, is_async=True)
        self.register_reply([Agent, None], ConversableAgent.a_generate_code_execution_reply,
                            is_async=True)
    ...
    def register_reply(
        ...
        if is_async:
            self._a_reply_func_list.insert(
                position,
                {
                    "trigger": trigger,
    ...
    def send(
        ...
        global_chat_messages: Optional[List] = [],
    ...
    def receive(
        ...
        if not silent:
            ...
            global_chat_messages.append({f'{sender_name} to {receiver_name}': message})
    ...
    async def a_receive(
        ...
        logger.debug(f"enter, message={message}")
        self._process_received_message(message, sender, silent)
        if request_reply is False or request_reply is None and self.reply_at_receive[sender] is False:
            return
        ...
        if not isinstance(self, GroupChatManager):
            ...
            receiver = {"user_name": sender.name, "is_self": sender in self.reply_at_receive}
            await run_manager[0].on_text(text=reply, sender=self.name, receiver=receiver, category="autogen")
            global_chat_messages.append({"message": reply, "sender": self.name, "receiver": receiver})
            if self.stop:
                return reply
        else:
            # 群聊过程不会进入，从group返回
        ...
        logger.debug(f"exist, message={message} reply={reply}")
        if reply is not None:
    ...
    def initiate_chat(
        ...
        **context,
        ...
        self.send(self.generate_init_message(**context), recipient, 
                  silent=silent, global_chat_messages=global_chat_messages,
    ...
    async def a_generate_code_execution_reply(
        self,
        messages: Optional[List[Dict]] = None,
        sender: Optional[Agent] = None,
        config: Optional[Any] = None,
        **kwargs,
        ...
        for i in range(min(len(messages), last_n_messages)):
            ...
            # found code blocks, execute code and push "last_n_messages" back
            exitcode, logs = await self.a_execute_code_blocks(code_blocks, run_manager=run_manager)
            code_execution_config["last_n_messages"] = last_n_messages
            exitcode2str = "execution succeeded" if exitcode == 0 else "execution failed"
            return True, f"exitcode: {exitcode} ({exitcode2str})\nCode output: {logs}"
    ...
    async def a_check_termination_and_human_reply(
        ...
        **kwargs,
        ...
        no_human_input_msg = ""
        if self.human_input_mode == "ALWAYS":
            await run_manager.on_text(f"Please give feedback to {sender.name}. Press enter or type 'exit' to stop the conversation: ",
                                type="system", category="autogen")
            # 让当前事件hold住
            await self.event.wait()
            # 外部注入输入，获取输入值
            reply = list(self.input.values())[0] if isinstance(self.input, dict) else self.input
            no_human_input_msg = "NO HUMAN INPUT RECEIVED." if not reply else ""
            # if the human input is empty, and the message is a termination message, then we will terminate the conversation
            reply = reply if reply or not self._is_termination_msg(message) else "exit"
        else:
        ...
        # for history message
        if self.input:
            global_chat_messages = kwargs.get("global_chat_messages")
            msg = {"message": self.input, "sender": self.name, "receiver": {"is_bot": True}}
    ...
    async def a_generate_reply(
        ...
        **kwargs,
        ...
        for reply_func_tuple in self._a_reply_func_list:
            reply_func = reply_func_tuple["reply_func"]
            if exclude and reply_func in exclude:
                continue
            if self._match_trigger(reply_func_tuple["trigger"], sender):
                if asyncio.coroutines.iscoroutinefunction(reply_func):
                    final, reply = await reply_func(
                        self, messages=messages, sender=sender,
                        config=reply_func_tuple["config"],
                        callbacks=callbacks,
    ...
    async def a_execute_code_blocks(self, code_blocks, run_manager):
        """Execute the code blocks and return the result."""
        logs_all = ""
        for i, code_block in enumerate(code_blocks):
            lang, code = code_block
            ...
            await run_manager.on_text(
                text=f"~EXECUTING CODE BLOCK {i} (inferred language is {lang})... \n\n", category="autogen"
            )
            if lang in ["bash", "shell", "sh"]:
                exitcode, logs, image = self.run_code(code, lang=lang, **self._code_execution_config)
            elif lang in ["python", "Python"]:
    ...
    async def a_execute_function(self, func_call, run_manager):
        """Execute a function call and return the result.
        ...
        if func is not None:
            ...
            if arguments is not None:
                ...
                await run_manager.on_text(
                    text=f"EXECUTING FUNCTION {func_name}...\n\n", category="autogen"
                )
                try:
                    content = func(**arguments)
                    is_exec_success = True
