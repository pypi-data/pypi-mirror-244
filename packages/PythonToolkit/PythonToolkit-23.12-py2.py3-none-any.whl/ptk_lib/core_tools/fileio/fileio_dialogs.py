...
class ExportDialog(wx.Dialog):
    ...
    def _InitControls(self):
        ...
        self.root = self.tree.AddRoot('', 0, 0, data=('',''))
    ...
    def AddName(self,address, type_string=None):
        ...
        index = self.list.InsertItem(n, address, icon)
        self.list.SetItem(index, 1,type_string)
    ...
    def GetObjectNames(self):
        ...
        """
        return [name for name in self.names.keys()]

    #---internal methods--------------------------------------------------------
    def _AddChildren(self, item):
        self.tree.DeleteChildren(item)
        address,type_string = self.tree.GetItemData(item)
        #get dir listing
        eng = self.console.get_engine_console(self.engname)
        if eng is None:
            return
        ...
        for name in names:
            ...
            data=(child_address, type_string)
    ...
    def OnAdd(self,event):
        ...
        for treeid in treeids:
            address,type_string = self.tree.GetItemData(treeid)
...
class ObjectsToMultipleFilesPanel(wx.Panel):
    ...
    def _RefreshList(self):
        ...
        for n in range(0, len(self.onames)):
            ...
            icon = self._GetTypeIconIndex(type_string)
            index = self.list.InsertItem(n, name, icon)
            self.list.SetItem(index, 1,filename)
...
class MultipleObjectToFileDialog(wx.Dialog):
    ...
    def OnExtChanged(self,event):
        ...
        if self.exts[n] in self.options:
            self.opt_but.Show()
        else:
            self.opt_but.Hide()
        self.Layout()
