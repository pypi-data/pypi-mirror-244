...
class TrainingMasterClient:
    ...
    def progress_report(self, cur_progress: int, total_progress: int):
        ...
        if percentage % notice_interval != 0 and self.__report_percentage != percentage and percentage != 100:
            return

        self.__report_percentage = percentage
        job_id = self._job_metadata["job_id"]
        # print(f"[INFO] do report progress {percentage}%/{total_progress} to training master")
        try:
