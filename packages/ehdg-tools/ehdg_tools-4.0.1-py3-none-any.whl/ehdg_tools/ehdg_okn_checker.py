...
def detect_with_okn_detector(csv_to_b_detected, odl, odcl):
    start_time = time.time()
    ...
    commandline = f"okndetector -c \"{odcl}\" -i \"{csv_to_b_detected}\" -o \"{out_put_dir}\""

    if not os.path.isdir(odl):
