...
def trial_plot(trial_plot_info_input, signal_dir_input, output_dir_input=None, image_name_input=None, gaze_csv_dir=None,
    ...
    if output_dir_input:
        ...
        result_folder_dir = os.path.join(signal_dir_input, os.pardir)
        trial_folder_dir = os.path.join(result_folder_dir, os.pardir)
...
def get_plot_info(data_dir, plot_info_input, summary_csv_name_input=None):
    ...
    if summary_csv_name_input is not None:
        summary_csv_name_input = str(summary_csv_name_input)
        if summary_csv_name_input.lower().endswith(".csv"):
            summary_csv_name = str(summary_csv_name_input)
        else:
            summary_csv_name = f"{summary_csv_name_input}.csv"
...
def summary_plot(folder_dir_input, summary_plot_info_input, output_dir_input=None, image_name_input=None,
    ...
    if not max_graph_limit:
        ...
        if int(max_graph_in_a_row) <= 0:
            ...
            temp_logmar_info_array = []
            graph_count = 0
            current_logmar_level = None
            for info in plot_data_array:
                current_logmar_level = info["logmar"]
                if graph_count < max_graph_in_a_row:
                    temp_logmar_info_array.append(info)
                    graph_count += 1
                    if graph_count == max_graph_in_a_row:
                        temp_dict = {"logmar_level": current_logmar_level, "info_array": temp_logmar_info_array}
                        final_plot_array.append(temp_dict)
                        temp_logmar_info_array = []
                        graph_count = 0

            # when temp_logmar_info_array is not empty then add to final plot array
            if temp_logmar_info_array:
                temp_dict = {"logmar_level": current_logmar_level, "info_array": temp_logmar_info_array}
                final_plot_array.append(temp_dict)
