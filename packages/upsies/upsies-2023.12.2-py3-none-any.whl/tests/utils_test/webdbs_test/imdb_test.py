...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        (Query('Bron Broen', type=ReleaseType.series, year=2011), ('Sweden', 'Denmark', 'Germany', 'Norway')),
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        (Query('The Wingwall Auditions', type=ReleaseType.movie, year=2000), ''),
        ...
        (Query('My Best Fiend', type=ReleaseType.movie, year=1999),
         ("In the 1950s, an adolescent Werner Herzog was transfixed by a film performance "
          ...
          "in an unabated, forty-eight-hour fit of rage, Kinski completely destroyed the bathroom. "
          "From this chaos, a violent, love-hate, profoundly creative partnership was born. In 1972, "
          "Herzog cast Kinski in Aguirre, the Wrath of God (1972). Four more films would follow. In "
          "this personal documentary, Herzog traces the often violent ups and downs of their "
          "relationship, revisiting the various locations of their films and talking to the people "
          "they worked with.")),
...
@pytest.mark.parametrize(
    argnames=('query', 'unexp_summary'),
    argvalues=(
        (Query('the tick', year=1994), 'TV-Y7'),
    ),
    ids=lambda value: str(value),
)
@pytest.mark.asyncio
async def test_search_result_summary_negated(query, unexp_summary, api, store_response):
    results = await api.search(query)
    assert results
    for r in results:
        summary = await r.summary()
        assert unexp_summary not in summary
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        (Query('Star Wars', year=2008), 'Star Wars: The Clone Wars'),
        (Query('Star Wars', year=2014), 'Star Wars: Rebels'),
        (Query('housebound', year=2014), 'Housebound'),
    ),
    ids=lambda value: str(value),
)
@pytest.mark.asyncio
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        (Query('My Best Fiend', year=1999, type=ReleaseType.movie), ReleaseType.movie),
    ),
    ids=lambda value: str(value),
)
@pytest.mark.asyncio
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        ('tt0048918', ('United Kingdom',)),  # 1984 (movie)
        ...
        ('tt1733785', ('Sweden', 'Denmark', 'Germany', 'Norway')),  # The Bridge (series)
        ('tt0348914', ('United States',)),  # Deadwood (series)
        ('tt9319668', ('Germany', 'United States')),  # 1899 (series)
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        # Blues Brothers (movie)
        ('tt0080455', 'https://m.media-amazon.com/images/M/MV5BYTdlMDExOGUtN2I3MS00MjY5LWE1NTAtYzc3MzIxN2M3OWY1XkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_UY1200_AL_.jpg'),
        # Wind in the Willows (TV movie)
        ('tt0192802', 'https://m.media-amazon.com/images/M/MV5BOGQ3NWM5OTAtOThjZS00MzMyLTkwZjMtOWIyOTdlZjFiZGI1XkEyXkFqcGdeQXVyNzMwOTY2NTI@._V1_UY1200_AL_.jpg'),
        # Bender's Big Score (Video)
        ('tt0471711', 'https://m.media-amazon.com/images/M/MV5BM2U3NmRiNDItMDQwYy00Y2Q0LWJiYzItNjAzNThkZjFlM2RiXkEyXkFqcGdeQXVyNTA4NzY1MzY@._V1_UY1200_AL_.jpg'),
        # Elephant (TV movie)
        ('tt0097270', 'https://m.media-amazon.com/images/M/MV5BMWE0ZTBiOWItY2ZkNS00MzI0LWE5Y2QtYjJmNjM1MGRkZGNmXkEyXkFqcGdeQXVyMzU0MTk1Nzc@._V1_UY1200_AL_.jpg'),
        # Kung Fury (Short)
        ('tt3472226', 'https://m.media-amazon.com/images/M/MV5BMjQwMjU2ODU5NF5BMl5BanBnXkFtZTgwNTU1NjM4NTE@._V1_UY1200_AL_.jpg'),
        # The Forest (mini series)
        ('tt6560040', 'https://m.media-amazon.com/images/M/MV5BMzY5NThkOWItN2I1OC00MzQ2LThlYjktMGExYTk2YzM1ZGRmXkEyXkFqcGdeQXVyODEyMzI2OTE@._V1_UY1200_AL_.jpg'),
        # Deadwood (series)
        ('tt0348914', 'https://m.media-amazon.com/images/M/MV5BMWE2MmU2MmEtNTVhMS00MTBhLWIyZjAtOGY2MGYxZGZlMDRhXkEyXkFqcGdeQXVyNDIzMzcwNjc@._V1_UY1200_AL_.jpg'),
        # Deadwood - S02E04 (episode)
        ('tt0556307', ''),
        (None, ''),
    ),
    ids=lambda value: str(value),
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        ('tt0253248', {}),  # The Wingwall Auditions (short, no runtime found)
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        ('tt0253248', ''),  # The Wingwall Auditions
        # Links in summary
        ('tt0200849', (
            # Mein liebster Feind
            ('In the 1950s, an adolescent Werner Herzog was transfixed by a '
             ...
             'Aguirre, the Wrath of God (1972). Four more films would '
             'follow. In this personal documentary, Herzog traces the often violent ups and '
             'downs of their relationship, revisiting the various locations of their films '
             'and talking to the people they worked with.'),
            ('In the 1950s, an adolescent Werner Herzog was transfixed by a '
             'film performance of the young Klaus Kinski. Years later, they would share '
             'an apartment where, in an unabated, forty-eight-hour fit of rage, Kinski '
             'completely destroyed the bathroom. From this chaos, a violent, love-hate, '
             'profoundly creative partnership was born. In 1972, Herzog cast Kinski in '
             'Aguirre, der Zorn Gottes (1972). Four more films would '
             'follow. In this personal documentary, Herzog traces the often violent ups and '
             'downs of their relationship, revisiting the various locations of their films '
             'and talking to the people they worked with.'),
        )),
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        ('tt1526578', '', 'Ao, le dernier Néandertal'),
        ('tt1530509', '', 'The Human Centipede II (Full Sequence)'),
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        ('tt0048918', ReleaseType.movie),    # 1984 (movie)
...
async def test_type(id, exp_type, api, store_response):
    assert await api.type(id) == exp_type  # noqa: E721: do not compare types
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        ('tt9319668', '2022'),  # 1899 (series)
        ('tt2917940', ''),      # Thale 2 (unreleased movie)
        (None, ''),
    ),
    ids=lambda value: str(value),
)
