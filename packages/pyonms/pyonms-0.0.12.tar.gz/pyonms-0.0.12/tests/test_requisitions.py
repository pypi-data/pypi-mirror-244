...
def test_requisition_all(test_instance: PyONMS):
    requisitions = test_instance.requisitions.get_requisitions(limit=0)
    assert len(requisitions) == 8
    assert requisitions[0].foreign_source == "Example"


@pytest.mark.vcr()
def test_requisition_batch(test_instance: PyONMS):
    requisitions = test_instance.requisitions.get_requisitions(limit=3, batch_size=2)
