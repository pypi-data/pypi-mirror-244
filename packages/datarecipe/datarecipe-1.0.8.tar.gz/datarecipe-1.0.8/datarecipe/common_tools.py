...
def local_to_df(path, partial_file_name, skip_first_row = False, keep_file_name=False, sheet_num=1):
    ...
    for file_name in tqdm(file_list, desc="Processing Files"):
        ...
        if file_extension == "csv":
            if skip_first_row:
                file_data = pd.read_csv(file_name, encoding=encoding, skiprows=1)
            else:
