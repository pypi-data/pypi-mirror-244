...
class YandexMarketManager(Platform, Feed):
    ...
    def get_offer(self, product):
        ...
        appt.currencyId = "RUB"
        appt.price = round(product.price.now + (product.price.now * 0.11))
        if ((product.price.current_discount / product.price.base_price) * 100) > 5:
            appt.oldprice = round(
                product.price.base_price + (product.price.base_price * 0.11)
            )

        appt.purchase_price = product.price.neitral + (product.price.neitral * 0.11)
        appt.cofinance_price = product.price.neitral + (product.price.neitral * 0.11)
