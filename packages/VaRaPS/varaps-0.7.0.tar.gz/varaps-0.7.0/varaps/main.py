...
def main():
    ...
    if args.mode == 3:
        ...
        if args.filter_num is None or args.filter_num < 0:
            args.filter_num = input("Enter number of reads that must contain a mutation to be kept as a mutation (default 0): ")
            if args.filter_num:
                args.filter_num = int(args.filter_num)
    ...
    if args.filter_num is None or not args.filter_num in range(0, 1001):
        args.filter_num = 0
    ...
    if args.mode == 3:
        ...
        temp_output_dir = os.path.join(output_dir, f"temp_{rand_id}")
