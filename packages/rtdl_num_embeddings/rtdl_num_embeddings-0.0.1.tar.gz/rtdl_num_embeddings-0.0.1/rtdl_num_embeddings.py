...
class LinearReLUEmbeddings(nn.Sequential):
    ...
    **Examples**
    ...
    >>> d_embedding = 8
    >>> m = nn.Sequential(
    ...     LinearEmbeddings(n_features, d_hidden),
    ...     nn.ReLU(),
    ...     delu.nn.NLinear(n_features, d_hidden, d_embedding),
    )
    >>> m(x).shape
    torch.Size([2, 3, 8])
    """
    ...
    def __init__(self, n_features: int, d_embedding: int = 32) -> None:
        super().__init__(
            OrderedDict(
                [
                    ('linear', LinearEmbeddings(n_features, d_embedding)),
                    ('activation', nn.ReLU()),
                ]
            )
        )
...
class _Periodic(nn.Module):
    ...
    def __init__(
        ...
        k: int,
        sigma: float,
        frequency_init_distribution: Literal['trunc-normal', 'normal'],
    ) -> None:
        if sigma <= 0.0:
        ...
        super().__init__()
        self._sigma = sigma
        self._frequency_init_distribution = frequency_init_distribution
        self.weight = Parameter(torch.empty(n_features, k))
        self.reset_parameters()

    def reset_parameters(self):
        if self._frequency_init_distribution == 'trunc-normal':
            nn.init.trunc_normal_(self.weight, 0.0, self._sigma)
        elif self._frequency_init_distribution == 'normal':
            nn.init.normal_(self.weight, 0.0, self._sigma)
        else:
            raise ValueError(
                'Unknown frequency_init_distribution='
                f'{self._frequency_init_distribution}'
            )
    ...
    **Examples**
    ...
    def __init__(
        ...
        *,
        ...
        # NOTE[DIFF]
        # In the paper, frequency_init_distribution='normal' is used. Here, we use the
        # truncated normal distribution to avoid frequencies of too large magnitude.
        frequency_init_distribution: Literal['trunc-normal', 'normal'] = 'trunc-normal',
        ...
        Args:
            ...
            frequency_init_distribution: the initialization distribution
                for the frequencies.
        ...
        self.periodic = _Periodic(
            n_features, n_frequencies, frequency_init_scale, frequency_init_distribution
        )
