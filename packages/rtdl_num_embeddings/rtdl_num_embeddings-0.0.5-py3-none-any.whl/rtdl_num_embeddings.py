...
# NOTE
# _LinearEmbeddings is a copy of rtdl_revisiting_models.LinearEmbeddings.
...
class LinearReLUEmbeddings(nn.Sequential):
    ...
    **Examples**
    ...
    >>> d_embedding = 8
    >>> m = nn.Sequential(
    ...     LinearEmbeddings(n_cont_features, d_hidden),
    ...     nn.ReLU(),
    ...     delu.nn.NLinear(n_cont_features, d_hidden, d_embedding),
    ... )
    >>> m(x).shape
    torch.Size([2, 3, 8])
    """
    ...
    def __init__(self, n_features: int, d_embedding: int = 32) -> None:
        super().__init__(
            OrderedDict(
                [
                    ('linear', _LinearEmbeddings(n_features, d_embedding)),
                    ('activation', nn.ReLU()),
                ]
            )
        )
