...
def find_specific_SFT(SFTgpsStart, tSFT, channel, parentDir, mode=None):
    parentDir = os.path.abspath(parentDir)
    SFTstart = gps_to_datetime(SFTgpsStart)
    SFTend = gps_to_datetime(SFTgpsStart+tSFT)
    ...
    for avgDur in avgDurs:
        ...
        for checkfolder in checkfolders:
            pattern = os.path.join(
                ...
                chformat,
                "sfts",
                f"*-{SFTgpsStart}-{tSFT}.sft")
            foundSFTs = glob(pattern)
            if len(foundSFTs) >= 1:
                if len(foundSFTs) > 1:
                    print("Warning: multiple matching SFTs found in a single "
                          f"subdirectory (pattern: {pattern}. Please check "
                          "data in this folder.")
                return foundSFTs[0]
            else:
                pass
