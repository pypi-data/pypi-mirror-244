...
def main():
    ...
    parser.add_argument('-p', '--postproc-path', type=str,
                        default='/home/pulsar/.conda/envs/fscan-py3.10',
                        help='Path to the conda environment for running '
                             'postProcess')
    parser.add_argument('-c', '--analysissummary-path', type=str,
                        default=f'{os.path.dirname(__file__)}',
                        help='Path to the installation of AnalysisSummary')
    ...
    for args in multiConfArgs:
        ...
        for segtype_idx, (segtype, channels) in enumerate(
            ...
            for ep_idx, ep in enumerate(epochs_info):
                ...
                # set no_data variable to False always, assuming we find data
                no_data = False

                # analysis workflow

                # make SFTs (if requested)
                if args.create_sfts:
