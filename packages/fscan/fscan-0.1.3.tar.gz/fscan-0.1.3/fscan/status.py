...
def diagnose_dag(path, detailSuccessful=False):
    ...
    for line in dag:
        if 'VARS MakeSFTs' in line:
            obs = int(re.search('-O (\\d+)', line).group(1))
            kind = re.search('-K ([a-zA-Z]+)', line).group(1)
            rev = int(re.search('-R (\\d+)', line).group(1))
            sftpaths = line.split(' -p ')[-1].split()[0].split(',')
            gpsstart = int(re.search('-s (\\d+)', line).group(1))
            Tsft = int(re.search('-t (\\d+)', line).group(1))
            window = re.search('-w ([a-zA-Z]+)', line).group(1)
            channels = line.split(' -N ')[-1].split()[0].split(',')
            if line.startswith('#'):
                sftsToLink += len(sftpaths)
            else:
                sftsToGenerate += len(sftpaths)
            for idx, sftpath in enumerate(sftpaths):
                sftname = (
                    f"{channels[idx][0]}-1_{channels[idx][:2]}_"
                    f"{Tsft}SFT_O{obs}{kind}+R{rev}+"
                    f"C{channels[idx][3:].replace('-', '').replace('_', '')}+"
