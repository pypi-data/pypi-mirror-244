...
class ThreadReader(ShutdownableThread):
    def __init__(self, ctx: AutoContext):
        super().__init__(command_name=get_current_command_name(), thread_name="pread", daemon=True)
        self._ctx = ctx
...
class ThreadKiller(ShutdownableThread):
    def __init__(self, ctx: AutoContext):
        super().__init__(command_name=get_current_command_name(), thread_name="pkill", daemon=True)
        self._ctx = ctx
        ...
        while True:
            ...
            while len(self._killing_queue):
                ...
                try:
                    ...
                    self.ctx.journal.write_event(f"Still alive", pinfo, important=True)
                except Exception as e:
