...
def test_sequential_dataset(tmp_datasets_dir):
    ...
    for cpp, dp in zip(ds2.tensors[1][0].numpy(aslist=True), ds.seq_labels[0].numpy(aslist=True)):
        assert np.array_equal(cpp, dp)

    for cpp, dp in zip(ds2.tensors[0][0].numpy(aslist=True), ds.seq[0].numpy(aslist=True)):
        assert np.array_equal(cpp, dp)
...
def test_tag_data():
    ...
    assert (
        ds.abc2[0].numpy() == ['tag 0', 'tag 1', 'tag 2', 'tag 3', 'tag 4']
    )
