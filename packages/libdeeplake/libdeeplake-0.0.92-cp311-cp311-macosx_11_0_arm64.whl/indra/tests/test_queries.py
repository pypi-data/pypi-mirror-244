...
def test_vectorstore_crash():
    ...
    view = ds.query("select embedding, metadata, text, id, score from (select *, COSINE_SIMILARITY(embedding, ARRAY[1]) as score order by COSINE_SIMILARITY(embedding, ARRAY[1]) DESC limit 4) LIMIT 4")
