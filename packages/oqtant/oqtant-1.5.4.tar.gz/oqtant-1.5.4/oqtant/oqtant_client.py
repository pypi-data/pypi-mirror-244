...
from oqtant.simulator import QMPotential, Simulator, WaveFunction
...
class OqtantClient:
    ...
    def convert_matter_to_job(self, matter: QuantumMatter) -> OqtantJob:
        ...
        if matter.is_sim:
            it_plot = matter.sim.it_plot

        if matter.is_sim and matter.sim and it_plot is not None:
            output = job_schema.Output(values=job_schema.PlotOutput(it_plot=it_plot))
        else:
            output = None
    ...
    def get_sim_result(self, sim: Simulator) -> Simulator:
    ...
    def submit_sim(self, matter: QuantumMatter) -> Simulator:
        ...
        sim = Simulator(wavefunction, potential)
        return sim

    def submit(
        self, matter: QuantumMatter, track: bool = False, sim: bool = False
    ) -> str:
    ...
    def get_job_without_output(
        ...
        """Method to get an OqtantJob from the Oqtant REST API. This can return all runs within a job or a single run
        based on whether a run value is provided. The OqtantJobs returned will not have any output data, even if they
        are complete. This is useful for taking an existing job and creating a new one based on it's input data

        Args:
            job_id (str): The external_id of the job to fetch
            run (int | None, optional): Specific run to target
            include_notes (bool, optional): Flag to include any notes associated with the job inputs

        Returns:
            oqtant.schemas.job.OqtantJob: An OqtantJob instance of the job
        """
        request_url = f"{self.base_url}/{job_id}"
    ...
    def create_job(
        ...
        """Method to create an instance of OqtantJob. When not providing a dictionary of job data this method will return
        an OqtantJob instance containing predefined input data based on the value of job_type and runs. If a dictionary
        is provided an OqtantJob instance will be created using the data contained within it
    ...
    def submit_job(self, *, job: OqtantJob, write: bool = False) -> dict:
        """Method to submit a single OqtantJob to the Oqtant REST API. Upon successful submission this method will return a dictionary
        containing the external_id of the job and it's position in the queue. Will write the job data to file when the write
        argument is True
    ...
    def cancel_job(self, job_id: str) -> None:
        ...
        response = requests.put(
            url=f"{self.base_url}/{job_id}/cancel",
            headers=self.__get_headers(),
            timeout=(5, 30),
        )
        if response.status_code in [401, 403]:
    ...
    def run_jobs(
        ...
        """Method to submit a list of OqtantJobs to the Oqtant REST API. This method provides some optional functionality
        to alter how it behaves. Providing it with an argument of track_status=True will make it wait and poll
        the Oqtant REST API until all jobs in the list have completed. Providing it with and argument of write=True
        will make it write the results of the jobs to file when they complete (only applies when the track_status
        argument is True)
