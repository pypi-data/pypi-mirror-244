...
class QuantumMatter(BaseModel):
    ...
    def output_values_to_oqtant_output(
        output_values,
    ) -> OqtantPlotOutput | OqtantNonPlotOutput:  # TODO
        """Method to convert a completed job's output values to OqtantOutput

        Args:
            output_values (#TODO): The output values to convert
    ...
    def get_sim_result(self) -> None:
        ...
        self.sim = self.client.get_sim_result(self.sim)

        self.result = self.client.convert_matter_to_job(self)

        self.output = OqtantPlotOutput(it_plot=self.sim.it_plot)
    ...
    def get_sim_potential(
        self, time: float, include_magnetic: bool = True  # TODO not used
    ) -> list[float]:
        ...
        Returns:
            list(float): List of potential energy corresponding to each requested position
        """
        Nr = 150
        Lr = 5
        dr = Lr / Nr
...
class QuantumMatterFactory:
    ...
    def create_barrier(
        ...
        Args:
            position (float, optional): The barrier position
            height (float, optional): The barrier height
            width (float, optional): The barrier width
            birth (float, optional): The barrier birth time
            lifetime (float, optional): The barrier lifetime
