...
def joinToPod(namespace, pod_name, exec_command):
    resp = stream(core_v1_api.CoreV1Api().connect_post_namespaced_pod_exec, pod_name, namespace, command=exec_command, stderr=True, stdin=True, stdout=True, tty=True, _preload_content=False)
