...
const onHandleLegServicing = (flag: boolean, service?: number) => {
  ...
  const handleService = (
    ...
    if (
      ...
      currentLeg?.cob_lbs &&
      prevLeg?.cob_lbs &&
      currentLeg?.cob_lbs !== prevLeg?.cob_lbs
    ) {
      if (serviceIndex < 0) {
        flag &&
    ...
    if (currentLeg?.cob_lbs && prevLeg?.cob_lbs && currentLeg?.cob_lbs !== prevLeg?.cob_lbs) {
      if (serviceIndex < 0) {
        flag &&
          leg.servicing.services.push({
            ...missionLegServiceDefaults(service),
    ...
    // Should we even consider the second leg?
    if (
    ...
    if (
      ...
      nextLeg?.cob_lbs === currentLeg?.cob_lbs
    ) {
    ...
    if (
      leg.servicing.services?.[serviceIndex]?.[oppositeServicingType] !== null &&
      ((prevLeg?.cob_lbs === currentLeg?.cob_lbs && nextLeg?.cob_lbs === null) ||
        prevLeg?.cob_lbs === currentLeg?.cob_lbs)
    ) {
...
watch(
  ...
  (isAmlServiceSelected) => {
    if (isAmlServiceSelected) {
      ...
      console.log('handleAutomaticalServices', handleAutomaticalServices)
    }
  }
)
