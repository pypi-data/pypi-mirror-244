...
const onHandleLegServicing = (flag: boolean, service?: number) => {
  ...
  const handleService = (
    ...
    // prevLeg value is exist and nextLeg value is exist and when currentLeg cargo selected, service state
    if (
      flag &&
    ...
    if (
      !flag &&
      serviceIndex < 0 &&
      (prevLeg?.cob_lbs === null || prevLeg?.cob_lbs === undefined) &&
    ...
    if (
      ...
      nextLeg?.cob_lbs &&
      Number(prevLeg?.cob_lbs) === Number(currentLeg?.cob_lbs) &&
      Number(currentLeg?.cob_lbs !== nextLeg?.cob_lbs)
    ) {
      if (serviceIndex < 0) {
        currentLeg.servicing.services.push({
          ...missionLegServiceDefaults(service),
    ...
    //nextLeg is exist and when current checkbox click
    if (
      flag &&
      currentLeg?.cob_lbs &&
      nextLeg?.cob_lbs &&
