...
number ps (long *v, number *w, int d)
{
	int i;
	number r = 0;
	for (i=0;i<d+1;i++)
...
number norm2_long (long *w, int d)
{
	int i;
	number r = 0;
	for (i=0;i<d+1;i++)
...
void set_rand (number *v, int d)
{
	int i;
	for (i=0;i<d+1;i++)
	{
		v[i] = (number)rand()/((long long)RAND_MAX*RAND_MAX) + (number)rand()/RAND_MAX;
	}
}

void set_rand_long (long *v, int d)
{
	int i;
	for (i=0;i<d+1;i++)
	{
		v[i] = (long)rand() + (long)rand()*RAND_MAX;
	}
}

// compute theta1 and theta2 for a win-lose graph, starting from vertex e
void ComputeTheta2 (Automaton *a, int ei, double *t1, double *t2, int niter)
{
	int d = a->na-1;
	...
	long *v = (long *)malloc(sizeof(long)*(d+1));
	number *w = (number *)malloc(sizeof(number)*(d+1));
	
	long bm = (long)1<<62;
	int nr = log(RAND_MAX)/log(2);
	...
	long r = random();
	int ir = nr; // number of remaining random bits
	int ns = 0; // number of times we shift
	int nbw = 0; // number of shift of bits of w
	
	int e = ei; // current state
	long m;
	int im;
	...
	for (i=0;i<niter;i++)
	{
		...
		if (j > d)
		{
			...
			for (j=0;j<=d;j++)
			{
				if (w[j] > (number).000000000000000001)
					break;
			}
			if (j > d)
			{
			...
			{
				// renormalize in the other direction if needed
				for (j=0;j<=d;j++)
				{
					if (fabsl(w[j]) < (number)100000000000000000)
						break;
				}
				if (j > d)
				{
		...
		// apply the continued fraction algorithm
		m = -1;
		for (j=0;j<=d;j++)
		{
			if (a->e[e].f[j] != -1)
	...
	number nv2 = norm2_long(v, d);
	number nw2 = norm2(w, d);
	
	*t1 = (log(nv) - log(nv2) + ns*log(2))/niter;
	*t2 = (log(nw2)-log(nw) - nbw*log(2))/niter;
...
int equalsLanguages2(Automaton *a1, Automaton *a2, Dict a1toa2, int minimized, int pruned, int verb)
{
	...
	else
	{
		//browse the automata
		//alloc the stack
		int sp = ilog2(a1->n) + 1; //size of allocated stack
...
int findWord2 (Automaton a, Dict *w, int verb)
{
	...
	int sp = ilog2(a.n)+2; //size of stack
...
Automaton Product2(Automaton a1, Automaton a2, Dict d, int verb)
{
	...
	int sp = ilog2(a1.n)+2; //allocated size of the stack
...
int Intersect2 (Automaton a1, Automaton a2, int verb)
{
	...
	for (i=0;i<a1.n*a2.n;i++)
		vu[i] = 0;
	int res = 0;
	int sp = ilog2(a1.n)+2; //allocated size of stack
...
// assume that res is of size a.n
int StronglyConnectedComponents2 (Automaton a, int *res)
{
	...
	int i, j,f; //,c, state;
	int count2;
	int *se = (int*)malloc(sizeof(int)*a.n); // états
	int *sj = (int*)malloc(sizeof(int)*a.n); // arête de l'état
	int *sc = (int*)malloc(sizeof(int)*a.n); // numéro attribué à l'état
	int sp = 0;
	int cp = 0;
	int state = 0, c=0;
	for (i=0;i<a.n;i++)
...
int StronglyConnectedComponents3 (Automaton a, int *res)
{
	int i, e, f, j, k;
	int *m = (int*)malloc(sizeof(int)*a.n); // valeur attribuée aux sommets
	for (i=0;i<a.n;i++)
	{
		res[i] = -1;
		m[i] = -1;
	}
	int *pile = (int*)malloc(sizeof(int)*a.n); // pile des états
	int np = 0; // nombre d'états sur la pile
	int *se = (int*)malloc(sizeof(int)*a.n); // états
	int *sj = (int*)malloc(sizeof(int)*a.n); // arête de l'état
	int *sc = (int*)malloc(sizeof(int)*a.n); // numéro initial attribué à l'état
	int n; // nombre d'éléments dans la pile
	int ncc = 0; // numéro courant de composante fortement connexe
	int c = 0; // numéro courant
	for (i=0;i<a.n;i++)
	{
		...
		if (m[i] == -1)
		{
			...
			while (n > 0)
			{
				...
				*/
				...
				{
					// on a parcouru tous les fils de ce sommet
					if (m[e] == sc[e])
					...
					{
						if (n > 0)
						{ // on transmet l'information de composante au précédent dans la pile
							if (m[se[n-1]] > m[e])
							{
								m[se[n-1]] = m[e];
							}
...
Automaton Lpriv_subgraph (Automaton *a, int *l, int n)
{
	Automaton r = NewAutomaton(a->n, a->na);
	int i,j;
	int b;
	int *indL = (int *)malloc(sizeof(int)*a->na);
	for (i=0;i<a->na;i++)
	{
		indL[i] = 0;
	}
	...
	for (i=0;i<a->n;i++)
	{
		...
		for (j=0;j<n;j++)
		{
			if (a->e[i].f[l[j]] != -1)
			{
				b = 1; // there is an edge labeled in L
				break;
			}
		}
		for (j=0;j<a->na;j++)
...
// Check the Fougeron's criterion for the subset of letters L
int CheckL (Automaton *a, int *l, int n, int verb)
{
	Automaton a2 = Lpriv_subgraph(a, l, n);
	int *cc = (int *)malloc(sizeof(int)*a->n);
	int nc = StronglyConnectedComponents(a2, cc);
	int i, j, f, c;
	int *indL = (int *)malloc(sizeof(int)*a->na);
	int *cs = NULL; // list of critical states
	int ncs = 0;
	int r = 1;
	for (i=0;i<a->na;i++)
	{
	...
	for (i=0;i<a->n;i++)
	{
		...
		for (j=0;j<n;j++)
		{
			...
			if (f != -1)
			{
				c++;
				if (cc[i] != cc[f])
					break; // we can leave from this state
			}
		}
		if (j >= n)
		{
			...
			{
				if (verb)
					printf("state %d is critical\n", i);
				// critical state found !
				if (ncs == 0)
				{
					cs = (int *)malloc(sizeof(int)*a->n);
				}
				cs[ncs] = i;
				ncs++;
			}
	...
	{
		if (verb)
			printf("No critical state !\n");
	}else
	{
		if (verb)
		{
			printf("%d critical states : [ ", ncs);
			for (i=0;i<ncs;i++)
			{
				printf("%d ", cs[i]);
			}
		...
		// find states of a2 from which we can leave in one step
		int *t = (int *)malloc(sizeof(int)*a2.n);
		int *to_see = (int *)malloc(sizeof(int)*a2.n);
		int e;
		int nts = 0;
		for (i=0;i<a2.n;i++)
		{
			...
			for (j=0;j<n;j++)
			{
				...
				if (f != -1)
				{
					...
					if (cc[i] != cc[f])
					{
						t[i] = 1; // there is an edge in L going out of the component
						break;
					}
				}
			}
			if (c == 0)
			{ // all outgoing edges are in the complement of L
				//printf("all edges in complement...\n");
				t[i] = 1;
				for (j=0;j<a2.na;j++)
				{
					f = a2.e[i].f[j];
				...
				if (c == 0)
				{
					fprintf(stderr, "Error : the L-priviledge subgraph is incorrect !\n");
					return 0;
				}
			}
			if (t[i] == 1)
		...
		if (verb)
		{
			printf("states that can leave immediatly : [ ");
			for (i=0;i<a2.n;i++)
			{
				if (t[i] == 1)
				{
		...
		// compute the mirror of a2
		NAutomaton am = Mirror(a2);
		// browse the graph am from every states i such that t[i] = 1
		while (nts > 0)
		{
			if (verb)
			{
			...
			e = to_see[nts];
			// browse from e
			for (j=0;j<am.e[e].n;j++)
			{
				f = am.e[e].a[j].e;
				if (cc[e] != cc[f])
					continue;
				if (indL[am.e[e].a[j].l])
				{
					if (!t[f])
					{
						t[f] = 1;
				...
				}else
				{
					...
					if (t[f] == 1)
					{
						// every outgoing edge of f permits to leave the strongly connected component
						to_see[nts] = f;
						nts++;
					}
				}
		...
		// check that we can leave from every critical state
		for (i=0;i<ncs;i++)
		{
			if (t[cs[i]] != 1)
			{
				if (verb)
				{
					printf("There is no strategy to leave from critical state %d.\n", cs[i]);
				}
				r = 0;
				break;
			}
...
// check that from every state we can reach any label
int CanReachAllLabels (Automaton *a)
{
	int i,j,c,f;
	int *cc = (int *)malloc(sizeof(int)*a->n);
	int nc = StronglyConnectedComponents(*a, cc);
	int *term = (int *)malloc(sizeof(int)*nc);
	int **ls = (int **)malloc(sizeof(int *)*nc); // letters seen in each scc
	for (i=0;i<nc;i++)
	{
		term[i] = 1;
		ls[i] = (int *)malloc(sizeof(int)*a->na);
		for (j=0;j<a->na;j++)
			ls[i][j] = 0;
	}
	for (i=0;i<a->n;i++)
...
// test if the graph satisfies the Fougeron's criterion
int FougeronCriterion (Automaton *a, int verb)
{
	int j, n, r;
	unsigned long long i;
	if (a->na > sizeof(unsigned long long))
	{
		fprintf(stderr, "Error : too much differents letters to compute Fougeron's criterion !!!\n");
		return 0;
	}
	if (!CanReachAllLabels(a))
	{
		if (verb)
		{
			printf("There is a state from which we cannot reach every labels.\n");
		}
		return 0;
	}
	const unsigned long long m = (unsigned long long)1 << a->na;
	int *l = (int *)malloc(sizeof(int)*a->na);
	unsigned long long i2;
	// browse the possible L
	r = 1;
	for (i=1;i<m-1;i++)
	{
		// count the number of elements of L
		n = 0;
		i2 = i;
		for (j=0;j<a->na;j++)
		{
		...
		if (n < 2)
			continue;
		if (!CheckL(a, l, n, verb))
		{
			if (verb)
			{
				i2 = i;
				printf("Criterion not satisfied for L = [ ");
				for (j=0;j<n;j++)
				{
					printf("%d ", l[j]);
				}
...
// convert a win-lose graph to graph with full comparaison written in each edge
// !!!!!!!! assume the graph has arity 2 !!!!!!!!!
Automaton RelabelFougeron2 (Automaton *a)
{
	Automaton r = NewAutomaton(a->n, a->na*a->na);
	int i,j,k;
	int lab[2];
	int ia;
	for (i=0;i<a->n;i++)
	{
		...
		for (j=0;j<a->na;j++)
		{
			...
			if (k != -1)
			{
				...
				{
					printf("error\n");
					fprintf(stderr, "Error : the graph has more than two leaving edges from state %d !\n", i);
					return r;
				}
				lab[ia] = j;
				ia++;
		...
		{
			printf("error\n");
			fprintf(stderr, "Error : the graph has less than two leaving edges from state %d !\n", i);
			return r;
		}
		r.e[i].f[lab[0]+lab[1]*a->na] = a->e[i].f[lab[0]];
		r.e[i].f[lab[1]+lab[0]*a->na] = a->e[i].f[lab[1]];
		r.e[i].final = 1;
	}
	return r;
}

Automaton Fougeron2FromRelabel (Automaton *a)
{
	int d = sqrt(a->na);
	Automaton r = NewAutomaton(a->n, d);
	int i,j,k;
	for (i=0;i<a->n;i++)
	{
		for (j=0;j<a->na;j++)
...
double InvariantDensityApprox(NAutomaton *a, int i, int** A, int* lA, double* v, int na, int n) //, int verb=0)
{
	double **vstack; // stack of vectors
	double *cv; // current vector
	double *nv; // current next vector
	double *t; // buffer vector
	int *istack; // stack of states
	int *kstack; // stack of edges
	int nstack = 1; // current size of the stack
	double sum = 0; // the result
	double s;
	int j,k,l;
	//printf("i = %d, na = %d, n = %d", i, na, n);
	//printf("alloc...\n");
	//fflush(stdout);
	// allocate memory
	vstack = (double**)malloc(sizeof(double*)*n);
	istack = (int*)malloc(sizeof(int)*n);
	kstack = (int*)malloc(sizeof(int)*n);
	for (k=0;k<n;k++)
	{
		vstack[k] = (double*)malloc(sizeof(double)*na);
	}
	t = (double*)malloc(sizeof(double)*na);
	// stack the first element
	istack[0] = i;
	kstack[0] = 0;
	for (k=0;k<na;k++)
	{
	...
	while (nstack > 0)
	{
		...
		{
			cv = vstack[nstack];
			//printf("i=%d, k=%d, cv=[%lf, %lf, %lf], nstack=%d\n", i, k, cv[0], cv[1], cv[2], nstack);
			if (nstack == n-1)
				nv = t;
			else
				nv = vstack[nstack+1];
			...
			{
				// stack the next element
				nstack++;
				istack[nstack] = a->e[i].a[k].e;
				kstack[nstack] = 0;
			}
			nstack++;
		}
...
void TestMemLeak (int size)
{
	int **t = (int **)malloc(sizeof(int*)*size);
	int i,j;
	printf("do a memory of size %ld\n", (long)size*size);
	for (i=0;i<size;i++)
	{
		if (i%2)
		{
