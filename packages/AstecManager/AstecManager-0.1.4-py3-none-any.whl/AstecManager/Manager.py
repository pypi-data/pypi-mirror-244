...
class Manager:
    ...
    def test_fusion(self,parameters,parameter_exploration=False,rerun=False):
        embryo_name = parameters["embryo_name"].replace('"', '').replace("'", "")
        ...
        if rerun or parameter_exploration:
            ...
            self.start_fusion(parameters,run_now=False, keep_temp=False)
            ...
            parameters["acquisition_orientation"] = "'right'"
            self.start_fusion(parameters, keep_temp=False)
    ...
    def start_fusion(self,parameters,run_now=True,keep_temp=False,channel_count=1):
        omero_config_file = get_omero_config(parameters)
        embryo_name = parameters["embryo_name"].replace('"','').replace("'","")
    ...
    def final_fusion(self, parameters):
        ...
        real_parameters = self.start_fusion(parameters,channel_count=number_of_channels)
        try :
                ...
                for surfacepair in surfaces_full[cell_name][t]:
                    ...
                    if down_subcell_surface is None:
                        ...
                        if shift != -1:
                            ...
                            cell_to_cell_shift[cell_name][t].append((subcell_name,shift))
                            if shift > 0:
