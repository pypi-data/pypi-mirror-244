...
class Grid:
    ...
    def _reproject(self, transform, crs, width, height, resampling: Resampling):
        ...
        destination = np.zeros((int(round(height)), int(round(width))), grid.dtype) * np.nan  # type: ignore
        reproject(
            source=grid.data,
        ...
        result = Grid(destination, crs, transform, grid.nodata, grid._cmap)
    ...
    def _resample(
        ...
        width = (ymax - ymin) / abs(self.transform.e) / scaling
        height = (xmax - xmin) / abs(self.transform.a) / scaling
    ...
    def save(self, file: str, nodata: Union[float, int, None] = None):
        with rasterio.open(
            file,
            "w",
...
def read(file: str, band: int = 1):
    with rasterio.open(file) as dataset:
        return Grid(dataset.read(band), dataset.crs, dataset.transform, dataset.nodata)
