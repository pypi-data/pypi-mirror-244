...
class ReduceMixin:
    ...
    def dispatch_extractor(self, action, soup, response, prev_results, results_cache, context):
        extractset = {}
    ...
    def dispatch_parser(self, action, response, extracted, soup, prev_results, results_cache, context):
        results = self.dispatch(
            'parser', action.parser,
            response=response, parsed=extracted, soup=soup,
            prev=prev_results,
            results_set=results_cache, context=context, **context
        )
    ...
    def dispatch_urlfilter(self, action, url, prev_results, context):
        ...
        return self.dispatch(
            ...
            url=url, prev=prev_results, context=context, **context
        )
    ...
    def dispatch_response_filter(self, action, response, prev_results, context):
        ...
        return self.dispatch(
            ...
            response=response, prev=prev_results, context=context, **context
        )
    ...
    def default_parser(self, response, prev, context, **kwargs):
        return prev, context
