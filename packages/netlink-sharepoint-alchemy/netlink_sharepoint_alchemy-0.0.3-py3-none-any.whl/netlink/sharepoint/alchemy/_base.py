...
class _SharepointBase:
    ...
    def bind_to_sharepoint(cls, sharepoint_site):
        ...
        if 'sharepoint_site' in state:
            ...
            state['sharepoint_list'] = List(sharepoint_site, title=cls._sharepoint_list_title, map=cls._get_map(), upper_case=cls._upper_case, required=cls._required)
    ...
    def _get_map(cls) -> dict:
        ...
        if 'map' not in state:
            ...
            state['map'] = {i: getattr(cls, i).__doc__ for i in cls._sa_class_manager.local_attrs if getattr(cls, i).__doc__}
        return state['map']
            ...
            for column in sharepoint_site.get_list_columns(cls._sharepoint_list_title, hidden=True):
                if column.internal_name in cls._get_map().values():
                    if column.type_as_string == "DateTime":
                        state['convert'][column.internal_name] = lambda x: \
                            datetime.datetime.strptime(x, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=datetime.timezone.utc)
