...
class AbstractTestPromptTuning(abc.ABC):
    ...
    def test_15_get_summary_details(self):
        ...
        self.assertEqual(run_summary_details.get('Enhancements')[0][0], run_details['entity'].get('prompt_tuning').get('tuning_type'))
        self.assertEqual(run_summary_details.get('Base model')[0], run_details['entity'].get('prompt_tuning').get('base_model')['model_id'])
        self.assertEqual(run_summary_details.get('Auto store')[0], run_details['entity'].get('auto_update_model'))
        self.assertEqual(run_summary_details.get('Epochs')[0], run_details['entity'].get('prompt_tuning').get('num_epochs'))
    ...
    def test_23_list_repository(self):
        repository_data_frame_list = self.wml_client.repository.list()
    ...
    def test_94_delete_models(self):
        for model in self.space_models_to_delete:
            self.wml_client.repository.delete(model)

        self.wml_client.set.default_project(self.project_id)
        for model in self.project_models_to_delete:
            self.wml_client.repository.delete(model)
