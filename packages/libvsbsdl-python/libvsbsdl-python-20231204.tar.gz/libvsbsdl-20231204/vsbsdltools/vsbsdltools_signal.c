...
void (*vsbsdltools_signal_signal_handler)( vsbsdltools_signal_t ) = NULL;
...
{
	static char *function = "vsbsdltools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( vsbsdltools_signal_signal_handler != NULL )
			{
				vsbsdltools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "vsbsdltools_signal_attach";
	...
	vsbsdltools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) vsbsdltools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	vsbsdltools_signal_initialize_memory_debug();
#endif
...
int vsbsdltools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "vsbsdltools_signal_detach";
	...
	vsbsdltools_signal_signal_handler = NULL;
