/*
 * Common output functions for the vsbsdltools
 *
 ...
 */
int vsbsdltools_output_initialize(
     int stdio_mode,
     libcerror_error_t **error )
{
	static char *function = "vsbsdltools_output_initialize";
 ...
 */
void vsbsdltools_output_copyright_fprint(
      FILE *stream )
{
	if( stream == NULL )
	{
	...
	fprintf(
	 stream,
	 "Copyright (C) 2023, Joachim Metz <%s>.\n"
 ...
 */
void vsbsdltools_output_version_fprint(
      FILE *stream,
      const char *program )
{
	if( stream == NULL )
 ...
 */
void vsbsdltools_output_version_detailed_fprint(
      FILE *stream,
      const char *program )
{
	if( stream == NULL )
