...
class AdwMDI(AdwDrawRoundFrame3):
    ...
    def create_child(self, x=10, y=10, width=150, height=150, closebutton=AdwDrawCircularButton, can_close: bool = True,
                     childframe=AdwDrawRoundFrame3, titlebarframe=AdwDrawRoundFrame3, easydndr: bool = False):
        """
        创建一个子窗口
        ...
        if easydndr:
            from tkadw.designer.designerframe import Dndr
            Dndr(child)
        else:
    ...
    def create_designer_child(self, x=10, y=10, width=150, height=150, closebutton=AdwDrawCircularButton,
                              childframe=AdwDrawRoundFrame3, titlebarframe=AdwDrawRoundFrame3, can_close: bool = True):
        from tkadw.designer.designerframe import AdwDesignerFrame
        child = AdwDesignerFrame(self, childframe)
        child.place(x=x, y=y, width=width, height=height)

        child.titlebar = titlebarframe(child.widget, height=38)
        child.titlebar.frame_border_width = 0
