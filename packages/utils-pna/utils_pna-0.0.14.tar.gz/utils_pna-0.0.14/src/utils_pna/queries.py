...
from utils_pna.gap import compute_gaps
...
class QueryAPI:
    ...
    def query_gap(
        self, 
        dates,
        delta_gap=timedelta(days=3),
        **args
    ):
        if not isinstance(dates, list):
            raise ValueError("Error en fechas. Debe ser una lista con fecha de comienzo y fin")
        
        dates_gap = [dates[0]-delta_gap, dates[1]+delta_gap]
        ...
        gdf = parse_positions(data_list)
        gap_gdf = compute_gaps(gdf)
