...
def call(url: str, workspace_api_key: Optional[str] = None, data: Optional[dict] = None):
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to call {url} with {data} with status code {response.status_code} and response {response.text}")
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.post(url, data=data) as response:
            if response.status != 200:
                raise Exception(
                    f"Failed to call {url} with {data} with status code {response.status} and response {await response.text()}")
