...
class OfflineScanTreeRewriter:
    def __init__(self, options_providers: Dict[str, Iterable[OfflineStoreOptionsProvider]]) -> None:
    ...
    def rewrite(
        ...
        if isinstance(tree_node, nodes.OfflineStoreScanNode):
            ...
            if not fdw.has_delta_offline_store:
                msg = (
                    "Offline retrieval is only supported from Deltalake tables when using Rift, but the "
                    + f'FeatureView {fdw.name} is configured to use Parquet. Add compute_mode="spark" to your method '
                    + "invocation to use spark instead."
                )
