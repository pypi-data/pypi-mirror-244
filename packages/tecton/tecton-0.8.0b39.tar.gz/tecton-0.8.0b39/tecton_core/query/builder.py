...
def build_temporal_time_range_validity_query(
    ...
    qt = _filter_by_time_range(
        dialect,
        compute_mode,
        qt,
        query_time_range,
        start_timestamp_field=query_consts.valid_from(),
        end_timestamp_field=query_consts.valid_to(),
    )
...
def build_aggregated_time_range_validity_query(
    ...
    spine = _build_internal_spine(dialect, compute_mode, fdw, partial_aggs, explode_anchor_time=True)
...
def _build_internal_spine(
    ...
    if explode_anchor_time:
        spine = ExplodeTimestampByTimeWindowsNode(
            dialect=dialect, compute_mode=compute_mode, input_node=spine, timestamp_field=anchor_time(), fdw=fdw
        ).as_ref()
...
def wip_build_compaction_query(
    ...
    if enable_from_source_for_test:
        base_node = build_pipeline_querytree(
            dialect, compute_mode, fdw, for_stream=False, feature_data_time_limits=None
        )
    else:
        base_node = OfflineStoreScanNode(
            ...
            compute_mode=compute_mode,
            feature_definition_wrapper=fdw,
            partition_time_filter=None,  # TODO(samantha): Change time limits
        ).as_ref()
