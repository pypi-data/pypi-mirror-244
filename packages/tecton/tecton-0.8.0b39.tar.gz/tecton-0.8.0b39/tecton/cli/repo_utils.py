...
def get_tecton_objects(
    debug: bool, repo_config_path: Optional[Path]
) -> Tuple[List[base_tecton_object.BaseTectonObject], str, List[Path]]:
    repo_file_handler.ensure_prepare_repo()
    repo_files = repo_file_handler.repo_files()
    repo_root = repo_file_handler.repo_root()

    _maybe_load_repo_config_or_default(repo_config_path)
...
def get_default_repo_config_path() -> Path:
    repo_file_handler.ensure_prepare_repo()
    repo_root = repo_file_handler.repo_root()
    return Path(repo_root) / "repo.yaml"


def _maybe_load_repo_config_or_default(repo_config_path: Optional[Path]) -> None:
    """Load the repo config from the yaml file at repo_config_path. Expects repo_config_path to exist."""
    if repo_config.get_repo_config() is not None:
        # Repo config was already loaded. This is expected when tecton objects are collected multiple times, e.g.
        # during `tecton plan` the objects are collected for tests and then the plan.
        return

    if repo_config_path is None:
        repo_config_path = get_default_repo_config_path()

    load_repo_config(repo_config_path)
...
def load_repo_config(repo_config_path: Path) -> None:
    ...
    if not repo_config_path.exists():
        printer.safe_print(
            Fore.RED
            + f"Repo config required for this command and not found at {repo_config_path}. Run `tecton repo-config init` to create a starter config."
            + Fore.RESET
        )
        sys.exit(1)
    ...
    with open(repo_config_path, "r") as file:
        try:
            repo_config_dict = yaml.safe_load(file)
    ...
    try:
        config = repo_config.RepoConfig(**repo_config_dict if repo_config_dict else {})
    except pydantic.ValidationError as e:
        printer.safe_print(
            Fore.RED
            + f"Invalid Tecton Repo Config schema at {repo_config_path}."
            + f"\nError details:\n{e}"
            + Fore.RESET
        )
        sys.exit(1)

    repo_config.set_repo_config(config)
