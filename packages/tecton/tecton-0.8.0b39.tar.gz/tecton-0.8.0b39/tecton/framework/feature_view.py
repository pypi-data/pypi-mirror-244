...
class FeatureView(base_tecton_object.BaseTectonObject):
    ...
    def print_transformation_schema(self) -> None:
        ...
        print(type_utils.schema_pretty_str(transformation_schema))
...
class MaterializedFeatureView(FeatureView):
    ...
    @sdk_decorators.sdk_public_method(
        ...
        supports_skip_validation=(ComputeMode.SPARK, ComputeMode.RIFT),
    )
    def get_features_in_range(
        ...
        compute_mode = ComputeMode(compute_mode) if compute_mode is not None else get_compute_mode()
    ...
    def run_transformations(
        ...
        .. code-block:: python
            ...
            result = feature_view.run_transformations(
            ...
            result = aggregate_feature_view.run_transformations(
              start_time=datetime(2022, 5, 1),
              end_time=datetime(2022, 5, 2)
            )
        """
        ...
        if any(ds._data_source_type == DataSourceType.PUSH_NO_BATCH for ds in self.sources):
            msg = "The `run_transformations()` method is currently unsupported for Feature Views that are backed by a PushSource without a batch_config"
            raise TectonValidationError(msg)

        if start_time is None or end_time is None:
            msg = "run_transformations() requires start_time and end_time to be set."
            raise TypeError(msg)
        ...
        return querytree_api.run_transformations_batch(
            dialect,
            compute_mode,
            feature_definition,
            start_time,
...
class OnDemandFeatureView(FeatureView):
    ...
    def run_transformations(
        ...
        .. code-block:: python
            ...
            result = fv.run_transformations(input_data=input_data)
