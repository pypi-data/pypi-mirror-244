...
class DeltaWriter(OfflineStoreWriter):
    ...
    def _in_range(self, time_range: pendulum.Period) -> str:
        if self._params.time_column == anchor_time():
            ...
            start_time_str = datetime_to_partition_str(time_range.start, self._partition_size)
            end_time_str = datetime_to_partition_str(time_range.end, self._partition_size)
            time_col_predicate = f"{self._params.time_column} >= to_timestamp('{start_time_str}') AND {self._params.time_column} < to_timestamp('{end_time_str}')"
        ...
        if self._partition_type == PartitionType.EPOCH:
            ...
            partition_predicate = f"{TIME_PARTITION} BETWEEN to_timestamp('{start_partition_str}') AND to_timestamp('{end_partition_str}')"
