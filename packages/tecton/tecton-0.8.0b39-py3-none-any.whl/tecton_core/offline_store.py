...
class OfflineStoreOptionsProvider(ABC):
    @abstractmethod
...
DEFAULT_OPTIONS_PROVIDERS = {"s3": [BotoOfflineStoreOptionsProvider]}
...
class DeltaReader(OfflineStoreReader):
    def __init__(
        ...
        options_providers: Dict[str, Iterable[OfflineStoreOptionsProvider]],
    ) -> None:
        self._params = params
    ...
    def _storage_options(self) -> Dict[str, str]:
        protocol, sep, _ = self._params.delta_table_uri.partition("://")
        # Treat local paths like /foo/bar as if they were URIs with empty protocol
        if sep != "://":
            protocol = ""
        providers = self._options_providers.get(protocol, [])
        fvid = id_helper.IdHelper.from_string(self._fd.id)
        for provider in providers:
            options = provider.get_options(fvid)
