...
def pubs(county=None, serving_guinness=None, closed=None, with_prices=False):
    ...
    for datetime_col in ["creation_date", "last_submission_time"]:
        pub_df[datetime_col] = pd.to_datetime(
            pub_df[datetime_col], format="ISO8601", errors="coerce"
        ).fillna(
            pd.to_datetime(
                pub_df[datetime_col],
...
def pints(county=None):
    ...
    for pub_id in pubs_with_prices_ids:
        ...
        pub_pints_df["starRating"] = pub_pints_df["starRating"].astype(float)

        if len(pub_pints_df) > 0:
    ...
    pints_df["creation_date"] = pd.to_datetime(
        pints_df["creation_date"], format="ISO8601", errors="coerce"
    ).fillna(
        pd.to_datetime(
            pints_df["creation_date"],
            format="%Y-%m-%dT%H:%M:%S%z",
            errors="coerce"
        )
    )
