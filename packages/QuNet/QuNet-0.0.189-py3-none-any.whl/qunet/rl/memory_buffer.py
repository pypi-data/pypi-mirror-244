...
class MemoryBuffer:
    ...
    """
    def __init__(self, capacity, n_states, n_actions, dtype=np.float32):
        """
        ...
        self.memo = np.zeros( (capacity, self.nS*2 + 4), dtype = dtype)
    #------------------------------------------------------------------------------------
    ...
    def add(self, s0, a0, s1, r1, done, rewrite = 0):
        ...
        self.memo[self.index] = np.concatenate(( s0.astype(self.dtype), s1.astype(self.dtype), [a0], [r1], [done], [norm]) )
