...
class Targetcli:
    ...
    def _run(self, *args: Optional[str], **kwargs: Optional[str]) -> CommandResult:
    ...
    def set_(self, *args: Optional[str], **kwargs: Optional[str]) -> CommandResult:
    ...
    def get(self, *args: Optional[str], **kwargs: Optional[str]) -> CommandResult:
    ...
    def create(self, *args: Optional[str], **kwargs: Optional[str]) -> CommandResult:
        return self._run('create', *args, **kwargs)

    def delete(self, *args: Optional[str], **kwargs: Optional[str]) -> CommandResult:
...
def create_basic_iscsi_target(
    ...
    if not check_result(LUNs(target_wwn=target_wwn).create_lun(storage_object=f'{backstore.path}')):
        return False

    if not check_result(ACL(target_wwn=target_wwn, initiator_wwn=initiator_wwn).create_acl()):
        return False

    if userid and not check_result(
