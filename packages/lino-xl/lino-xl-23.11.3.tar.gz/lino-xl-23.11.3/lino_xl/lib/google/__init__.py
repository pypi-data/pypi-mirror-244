...
class Plugin(ad.Plugin):
    ...
    def get_migration_steps(self, other_sources):
        ...
        if 'lino_xl' in other_sources and other_sources['lino_xl'] == '23.9.1' and parse_version(xlcv) > parse_version('23.9.1'):
    ...
    def migrate_from_lino_xl_23_9_1(self, migrator, globals_dict):
        ...
        google_CalendarSubscription = globals_dict['google_CalendarSubscription']
        google_CalendarSyncToken = globals_dict['resolve_model']("google.CalendarSyncToken")
        google_EventSyncToken = globals_dict['resolve_model']("google.EventSyncToken")

        def create_users_user(id, email, language, modified, created, start_date, end_date, password, last_login, username, user_type, initials, first_name, last_name, remarks, partner_id, time_zone, date_format, dashboard_layout, event_type_id, calendar_sync_token, calendar_page_token):
            kw = dict()
            kw.update(id=id)
            ...
            tokens.append((id, calendar_sync_token, calendar_page_token, modified))
            return users_User(**kw)
        globals_dict['create_users_user'] = create_users_user

        e_tokens = []
        def create_google_calendarsubscription(id, modified, user_id, calendar_id, is_hidden, primary, access_role, sync_token, page_token):
            kw = dict()
            kw.update(id=id)
        ...
        globals_dict['create_google_calendarsubscription'] = create_google_calendarsubscription

        def after_load(loader):
            for token in tokens:
                loader.save(google_CalendarSyncToken(user_id=token[0], sync_token=token[1], page_token=token[2], modified=token[3]))
            for token in e_tokens:
                loader.save(google_EventSyncToken(user_id=token[0], subscription_id=token[1], sync_token=token[2], page_token=token[3], modified=token[4]))
