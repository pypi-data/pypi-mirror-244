...
# Polymorphic,
class Page(Hierarchical, Sequenced, Previewable, Commentable, PublishableContent, Interesting):
    ...
    translated_from = dd.ForeignKey('pages.Page',
        verbose_name=_("Translated from"), null=True, blank=True,
    ...
    def as_page(self, ar, display_mode="detail", hlevel=1, home=None):
        ...
        if display_mode == "detail" and hlevel == 1:
            ...
            if len(breadcrumbs) > 1:
                breadcrumbs = [
                    """<a href="{0}">{1}</a>""".format(ar.obj2url(p.mti_child()), p.title)
        ...
        if ar and ar.request and self.language != ar.request.LANGUAGE_CODE:
            ...
            if self.translated_from_id and self.translated_from.language == rqlang:
                ...
                while p is not None:
                    sources.add(p.id)
                    p = p.translated_from
                qs = self.__class__.objects.filter(language=rqlang, translated_from__in=sources)
                obj = qs.first()
