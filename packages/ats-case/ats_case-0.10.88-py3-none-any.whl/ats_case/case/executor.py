...
from ats_case.common import dingding
...
class Executor(object):
    ...
    def _err_msg_notify(self, err: str):
        project = '{}'.format(self._context.case.name)
        subhead = '表位[{}]  步骤[{}]'.format(self._context.meter.pos, self._context.runtime.step)

        if self._context.mode == WorkMode.FORMAL:
            project = '{}'.format(self._context.project)
            subhead = '{}:  表位[{}]  步骤[{}]'.format(self._context.case.name, self._context.meter.pos, self._context.runtime.step)

        dingding.send(Payload.ERROR, project, self._context.test_sn,
                      self._context.start_time, datetime.now(), msg=err, subhead=subhead)
