...
class Conversation:
    ...
    def load_from_file(cls, file_path: str) -> "Conversation":
        with open(os.path.join(file_path, "conversation.json")) as f:

            conversation_json = json.load(f)
            return cls(
                    name=conversation_json.get("name"),
                    system_template=conversation_json.get("system_template"),
                    system_message=conversation_json.get("system_message"),
                    roles=conversation_json.get("roles"),
                    sep_style=SeparatorStyle[conversation_json.get("sep_style")],
                    sep=conversation_json.get("sep"),
                    stop_token_ids=conversation_json.get("stop_token_ids"),  # "<|endoftext|>", "<|im_start|>", "<|im_end|>"
                    stop_str=conversation_json.get("stop_str"),
                )
...
#
#
# def get_conv_template(file: str) -> Conversation:
