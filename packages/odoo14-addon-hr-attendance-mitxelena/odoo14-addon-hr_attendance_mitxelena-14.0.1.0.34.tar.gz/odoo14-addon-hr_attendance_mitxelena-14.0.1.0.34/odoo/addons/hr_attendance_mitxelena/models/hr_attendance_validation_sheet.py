...
class HrAttendanceValidationSheet(models.Model):
    ...
    def _compute_nights_worked(self):
        for record in self:
            ...
            rounded_nights_worked = round(entire_nights_worked + overtime_nights_worked, 2)
        ...
        for record in self:
            ...
            for day in unique_days.copy():
                ...
                if hours < 7.5:
                    ...
                    round_hours = f"{int(hours):02d}:{int(minutes * 60):02d}"
                    msg = f"{day} ({round_hours} hours)"
                    discarded_relevo_days.append(msg)
            ...
            record.relevo_ids = record.attendance_ids.filtered(
                lambda att: att.check_in.date().day in unique_days
            )
            if len(discarded_relevo_days) == 0:
                record.relevo_discarded_days = "-"
            else:
                record.relevo_discarded_days = (
                    "<ul><li>" + "</li><li>".join(discarded_relevo_days) + "</li></ul>"
                )
