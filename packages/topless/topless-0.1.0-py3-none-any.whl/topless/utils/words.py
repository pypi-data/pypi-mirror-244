...
def capitalize(data: str | Dict[str, str]) -> str | Dict[str, str]:
    if isinstance(data, str):
        return stringcase.capitalcase(data)
    elif isinstance(data, dict):
        return {stringcase.capitalcase(key): value for key, value in data.items()}
    else:
        raise ValueError("Input data should be a string or dictionary")
