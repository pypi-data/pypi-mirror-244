...
def validate(data_graph: rdflib.Graph):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        conforms = bool(report_g.subjects(predicate=SH.conforms, object=rdflib.Literal(True)))
        validates = not has_violation or conforms
        print(f"has_violation: {has_violation}, conforms: {conforms}, validates: {validates}")
