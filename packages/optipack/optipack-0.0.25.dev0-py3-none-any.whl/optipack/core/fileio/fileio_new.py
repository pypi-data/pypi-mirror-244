...
class ReaderWriterBase(BaseModel):
    ...
    # local_extension_map: Dict = Field(default=None)
    # gcsfs_extension_map: Dict = Field(default=None)
    ...
    def __init__(
        ...
        from address_separator.utils.logging import get_logger
        ...
        logger = get_logger(name=__name__, level=config.logging_level)
        logger.info(f"{msg}")
        if not fs:
...
class FileReader(ReaderWriterBase):
    def __init__(self, cfg: ReaderWriterConfig, fs: GCSFS = None):
        ...
        # local_extension_map = dict(
        #     csv=self.read_df_local,
        #     xlsx=self.read_df_local,
        #     json=self.read_json_local,
        #     yaml=self.read_yaml_local,
        # )
        mapper = dict(
            csv=self.read_df,
            xlsx=self.read_df,
