...
def rerun_with_detector(main_folder_dir, opmdc, es, uc, di, okndc, okndl, pil, ril, overwritten=False):
    ...
    try:
        with open(opmdc) as opm_config:
            config_info = json.load(opm_config)
    ...
    try:
        with open(pil) as plot_info_config:
            plot_info = json.load(plot_info_config)
            print("<Plot Info>")
            try:
    ...
    try:
        with open(ril) as rule_info_config:
            rule_info = json.load(rule_info_config)
            print("<Rule Info>")
            try:
    ...
    for data_info in data_info_array:
        ...
        rerun_buffer = TinyFillBuffer()
        if not os.path.isdir(trial_folder_dir):
            print(f"Folder directory: {trial_folder_dir} could not be found.")
            return
        rerun_trial_csv_dir = rerun_trial(rerun_detector, rerun_buffer, trial_folder_dir,
                                          event_id, direction)
...
def rerun_trial(detector, buffer, trial_folder_dir, event_id_input, direction_input):
...
def main():
    ...
    parser.add_argument('--version', action='version', version='3.2.0'),
    ...
    if type_input:
        if str(type_input) == "trial_data_lost":
            ...
            if not okn_detector_location_exist:
                ...
                print("")
                return
            if input_file and direction_input and okn_detector_location_exist:
                # Determine whether default extra_string or custom input needs to be used
                if extra_string:
                    extra_string = str(extra_string)
        ...
        elif str(type_input) == "rerun_with_detector":
            ...
            if directory_input is None:
                ...
                print("oknpatch -t rerun_with_detector -d directory_to_be_rerun")
                print("")
                return
            overwritten = args.overwritten
            if overwritten is not None:
                overwritten_indicators = ["yes", "y", "1", "allow"]
                if str(overwritten) in overwritten_indicators:
                    overwritten = True
                else:
                    overwritten = False
            else:
            ...
            if okn_detector_location:
                ...
                if not okn_detector_location_exist:
                    print("")
                    print("The okn detector location input is missing.")
                    print("oknpatch -t rerun_with_detector -d recording_folder_directory -okndl okn_detector_location")
                    print("")
                    return
            else:
                print("")
            ...
            if opm_detector_config:
                ...
                if valid_file_name:
                    ...
                    if okn_detector_config_dir_exist:
                        ...
                        opm_detector_config_location = get_config_location("oknpatch", "okndetector.gaze.config")
            ...
            success, error = rerun_with_detector(directory_input, opm_detector_config_location, extra_string,
                                                 updater_config_location,
                                                 direction_input, okn_detector_config_location, okn_detector_location,
                                                 plot_info_location, rule_info_location, overwritten)
        else:
