    ...
    async def confirm_trend(self):
        try:
            ...
            if self.tracker.bullish and current.close_XA_ema:
                self.tracker.update(snooze=self.entry_time_frame.time, order=OrderType.BUY)
            elif self.tracker.bearish and current.close_XB_ema:
                self.tracker.update(snooze=self.entry_time_frame.time, order=OrderType.SELL)
            else:
                self.tracker.update(snooze=self.entry_time_frame.time, order=None)
        except Exception as exe:
