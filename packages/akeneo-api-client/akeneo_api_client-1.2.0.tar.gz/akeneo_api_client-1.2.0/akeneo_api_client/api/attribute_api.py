...
class AttributeApi:
    ...
    def create(self, data: dict = {}) -> Response:
        return self.resource_client.create_resource(self.ATTRIBUTES_URI, [], DictSerialize(data))

    def upsert(self, code: str, data: dict = {}) -> Response:
        return self.resource_client.upsert_resource(self.ATTRIBUTE_URI, [code], DictSerialize(data))

    def upsert_list(self, data: list[dict]) -> list[dict]:
        batch = LineSerialize()
        batch.add_items(data)

        response = self.resource_client.upsert_list_resource(self.ATTRIBUTES_URI, [], batch)
