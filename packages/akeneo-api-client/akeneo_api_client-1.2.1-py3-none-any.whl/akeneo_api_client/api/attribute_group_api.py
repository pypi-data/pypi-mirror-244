...
class AttributeGroupApi:
    ...
    ATTRIBUTE_GROUPS_URI = "api/rest/v1/attribute-groups"
    ATTRIBUTE_GROUP_URI = "api/rest/v1/attribute-groups/%s"
    ...
    def get(self, code: str) -> dict[str, any]:
        response = self.resource_client.get_resource(self.ATTRIBUTE_GROUP_URI, [code])

        return json.loads(response.content)

    def all(self, page_size: int = 10, query_params: dict = {}) -> Iterable[list]:
        response = self.resource_client.get_resources(self.ATTRIBUTE_GROUPS_URI, [], query_params, page_size)
