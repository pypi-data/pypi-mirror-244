...
def prepare_dataset(dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=None, fine_tune=False):
    huggingface_dataset = {}
    if few_shot_size:
...
def run_instructiona_fine_tuning_strategy(config, params, experiment, validate, logger, sampling_strategy, similarity, fine_tune=True):
    all_test_preds = []
    all_test_labels = []
    cutoff_len = 256
    ...
    #dataset[experiment_type] = dataset[experiment_type].sample(10)
    ...
    for _, record in dataset[experiment_type].iterrows():
        ...
        if sampling_strategy == "similar":
            ...
            few_shots = sample_diverse_examples(experiment, experiment_type, few_shot_size)
...
def run_instructional_fine_tuning(config, params, experiment, validate, logger, sampling_strategy=None, similarity=None, fine_tune=True):

    if sampling_strategy and similarity:
        return run_instructiona_fine_tuning_strategy(config,params,experiment,validate,logger,sampling_strategy,similarity)
    else:
        cutoff_len= 256
        path_template = get_template_path()
        model_name = config["model-name"]
        ...
        tokenizer = LlamaTokenizer.from_pretrained(tokenizer_name)
        dataset = load_splits(experiment, oversample=False)
        dataset["test"] = dataset["test"].sample(16)
        dataset["validation"] = dataset["validation"].sample(16)
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=config["few-shot-size"], fine_tune=fine_tune)
        if validate:
            test_data = val_data
