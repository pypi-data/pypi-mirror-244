...
def prepare_dataset(dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=None, fine_tune=False, diverse=False):
    ...
    if few_shot_size:
        if len(dataset["training"]) > few_shot_size:
            log_message(logger, f"Dataset is large sampling {few_shot_size}")
            dataset["training"] = dataset["training"].sample(few_shot_size)
            log_message(logger, f'Sampled dataset is {dataset["training"].to_string()}')
    mapper = {0:"CON", 1:"PRO"}
...
def run_instructiona_fine_tuning_similar(config, params, experiment, validate, logger, similarity, fine_tune=True):
    all_test_preds = []
    all_test_labels = []
    cutoff_len = 256
...
def run_instructional_fine_tuning(config, params, experiment, validate, logger, sampling_strategy=None, similarity=None, fine_tune=True):
    ...
    if sampling_strategy =="similar":
        return run_instructiona_fine_tuning_similar(config, params, experiment, validate, logger, similarity, fine_tune=fine_tune)
    else:
        cutoff_len= 256
        path_template = get_template_path()
        model_name = config["model-name"]
        ...
        if sampling_strategy == "diverse":
            ...
            if validate:
                ...
                experiment_type = "test"
            few_shots = sample_diverse_examples(experiment, experiment_type, config["few-shot-size"])
            log_message(logger, f"sampling diverse few shots got {len(few_shots)}")
            log_message(logger, f"diverse few shots are {few_shots.to_string()}")
