...
class Dxr_Crypto(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        if platform == "nt":
            # windows系统
            lib_path = current_path + "/lib/windows/x64/tsdkTest.dll"
            pass
        elif platform == "posix":
            # linux系统
        ...
        if os.path.exists('key.txt'):
            ...
            if self.dekey == None:
                ...
                print("从文件中读取密钥成功")
                if type(self.dekey) == str:
                    self.dekey = int(self.dekey)
                if type(self.dekey) == int:
                    self.dekey = struct.pack('Q', self.dekey)
                    need_init = False
        if need_init:
            self.__pre_main()
    ...
    def encrypt(self, inputString="hello world"):
        ...
        ret = self.lib.lu_encrypt(plainBuff, plainLen, ctypes.byref(cipherBuff), ctypes.byref(cipherLen), self.dekey)  # 使用明文密钥进行加密
        if ret == 0:
            # print("加密成功")
    ...
    def decrypt(self, cipherBuff, cipherLen=-1):
        ...
        ret = self.lib.lu_decrypt(cipherBuff, cipherLen, ctypes.byref(plainBuff), ctypes.byref(plainLen), self.dekey)  # 使用明文密钥进行解密
        if ret == 0:
            # print("解密成功")
