...
from .download_image import _validate_gallery
from .download_meta import _download_gallery_meta


DEPRECAT_COMPLETED_FILE = 'DepercatCompleted'


def _depercat_gallery_history(parent_url, parent_gallery_dir, child_url, child_gallery_dir, config, logger):
    """Move depercated images from parent gallery to child gallery"""
    ...
    if not _validate_gallery(parent_url, parent_gallery_dir, config, logger):
        raise RuntimeError(f"Cannot depercate from a invalid gallery {parent_gallery_dir}")
    ...
    for src_image_token, src_imgpath in src.items():
        if src_image_token in dst:
            os.rename(src_imgpath, dst[src_image_token])
