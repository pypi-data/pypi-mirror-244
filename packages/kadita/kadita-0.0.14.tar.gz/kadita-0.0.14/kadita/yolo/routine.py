...
class ImgBuzz(ImgRex):  # 8
    ...
    def load(self, names, weight):
        ...
        try:
            # Attempt to import YOLOv8 module
            from ultralytics.models.yolo import YOLOv8  # Replace with the actual module name for YOLOv8

            # Instantiate YOLOv8 model
            self.model = YOLOv8(weight)

        except ModuleNotFoundError:
            print("YOLOv8 module not found. Please install YOLOv8 and update the import statement.")
...
#                 detected_name = "Nama Tidak Teregister"  # Use "Unknown" as the name if the ID is not registered
#
#             # Save the fingerprint image as a PNG file with a unique format
#             timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
#             filename = f"{detected_name}_{detected_id}_{timestamp}.png"
#             while self.finger.get_image():
#                 pass
#
#             image_saved = self.save_fingerprint_image(filename)
...
#                 print("Failed to save fingerprint image")
#
#     def get_fingerprint_self_zebra(self):
#         # while True:
#         import json
#         import time
#
...
#                 x += 1
#
#         datasets_path = f"213/{filename}"  # Path to the datasets directory
...
#         while True:
#             print("Fingerprint Recognition")
#             print("----------------")
#             print("e) Training Fingerprint")
#             print("f) Deteksi Fingerprint")
#             print("d) Delete User pada Fingerprint")
#             print("s) Save Gambar Fingerprint")
...
#
#             elif c == "z":
#                 if self.get_fingerprint_self_zebra():
#                     detected_id = self.finger.finger_id  # Get the detected fingerprint's ID
#                     if detected_id in self.id_name_map:
#                         detected_name = self.id_name_map[detected_id]  # Get the name associated with the ID
#                     else:
#                         detected_name = "Zebra Name not register"  # Use "Unknown" as the name if the ID is not registered
#
#                     print("Zebra Detected #", detected_id, detected_name, "Zebra with confidence",
#                           self.finger.confidence)
#                     finger_detected = True
#                 else:
#                     print("Zebra Finger not found")
#                     finger_detected = False
#
#             elif c == "d":
