...
class Bingx(CcxtFutureExchange):
    ...
    def get_last_prices(self):
        ...
        datas = self.ccxt_inst.swap_v2_public_get_quote_price()['data']
        for data in datas:
    ...
    def get_contract_sizes(self):
        ...
        for data in datas:
            sizes[data['asset']] = float(data['size'])
    ...
    def post_market_order(self, ticker, side, open_close, amount):
        ...
        symbol = f'{ticker}-USDT'
        if side == 'buy':
            position_side = 'LONG' if open_close == 'open' else 'SHORT'
        else:
            position_side = 'SHORT' if open_close == 'open' else 'LONG'
