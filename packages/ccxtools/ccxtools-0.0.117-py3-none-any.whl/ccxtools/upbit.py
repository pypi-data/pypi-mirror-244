...
class Upbit(CcxtExchange):
    ...
    def __init__(self, who, env_vars):
        ...
        self.withdrawal_decimal_limits = {}
    ...
    def get_withdrawal_decimal_limits(self, tickers):
        ...
        for i, ticker in enumerate(tickers):
            ...
            limits[ticker] = int(response['withdraw_limit']['fixed'])
    ...
    def post_withdraw(self, ticker, amount, destination):
        if ticker not in self.withdrawal_decimal_limits:
            self.withdrawal_decimal_limits.update({
                ticker: self.get_withdrawal_decimal_limits([ticker])[ticker]
            })

        fixed_amount = round(amount, self.withdrawal_decimal_limits[ticker])
