...
from cgn_pytorch.util import mesh_utils
from cgn_pytorch.pointnet2 import FPModule, SAModule, MLP
...
class CGN(nn.Module):
    def __init__(self, config, device):
        ...
        self.set_abstract_msg = self.SAnet_msg(config["model"]["sa"])

        # Final set abstraction model without multi-scale grouping
        self.set_abstract_final = self.SAnet(config["model"]["sa_final"])

        # Feature propagation
        self.feat_prop = self.FPnet(config["model"]["fp"])

        # Final multihead for 2 vectors, confidence, and width
        ...
        self.conf_loss_fn = nn.BCEWithLogitsLoss(reduction="none").to(self.device)

    def forward(
        self,
        input_pcd: torch.Tensor,
        pcd_poses: torch.Tensor,
        batch: torch.Tensor,
        idxs: torch.Tensor,
        width_labels=None,
        ...
        """Maps each point in the input point cloud to a grasp vector and confidence score

        Args:
            input_pcd (torch.Tensor): Full point cloud of cluttered scene.
            pcd_poses (torch.Tensor): Poses of each point in the point cloud.
            batch (torch.Tensor): batch
            idxs (torch.Tensor): Indices to run network on. Typically from Farthest Point Sampling.
            width_labels (_type_, optional):. Defaults to None.
            gripper_depth (float, optional): Depth of the gripper. Defaults to 0.1034.
            gripper_width (float, optional): Width of the gripper. Defaults to 0.08.

        Returns:
            tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
             points, final_grasps, s, width, point_features, collide_pred_list
        """
        # Step 1: PointNet++ Set Aggregation (with multi-scale grouping)
        sample_pts = input_pcd.float()
        input_list = (sample_pts, pcd_poses, batch)
        ...
        for mod_idx, module_list in enumerate(self.set_abstract_msg):
            ...
            for i, module in enumerate(module_list):
                if (
                    (i == 0) and (mod_idx != 0)
                ):  # sample down point list (beginning of every module except very first one in network)
                    feat, pos, batch, idxs = module(*input_list)
                else:
                    feat, pos, batch, idxs = module(*input_list, sample=False, idx=idxs)
        ...
        for net in self.multihead[1:]:
            result = torch.flatten(
                net(point_features).transpose(1, 2), start_dim=0, end_dim=1
            )
            finals.append(result)

        # confidence prediction, grasp vector 1, grasp vector 2, grasp width
        (
            z1,
            z2,
            w,
        ...
        w = torch.clamp(w, min=-gripper_width, max=gripper_width/2)

        if gripper_depth is None:
            gripper_depth = self.config["gripper_depth"]
        final_grasps = self.build_6d_grasps(
            points, z1, z2, w, gripper_depth, width_labels
        )

        # unsqueeze point features and points into batches
        num_batches = torch.max(input_list[2]) + 1
        ...
        s = torch.flatten(
            self.multihead[0](point_features).transpose(1, 2), start_dim=0, end_dim=1
        )
    ...
    def build_6d_grasps(
        self, contact_pts, z1, z2, w, gripper_depth=0.1034, width_labels=None
    ):
        """
        ...
        base_dirs = z1 / (
            torch.unsqueeze(torch.linalg.norm(z1, dim=1), dim=0).transpose(0, 1)
        )  # z1/torch.linalg.norm(z1, dim=1)
        inner = torch.sum((base_dirs * z2), dim=1)
        prod = torch.unsqueeze(inner, -1) * base_dirs
        approach_dirs = (z2 - prod) / (
            torch.unsqueeze(torch.linalg.norm(z2, dim=1), dim=0).transpose(0, 1)
        )
        ...
        for i in range(contact_pts.shape[0]):
            ...
            grasp[:3, 3] = (
                contact_pts[i]
                - gripper_depth * grasp.clone()[:3, 2].to(self.device)
                + (w[i] / 2) * grasp.clone()[:3, 0].to(self.device)
            )
    ...
    def get_key_points(self, grasps_list, include_sym=False):
        pts_list = []
        gripper_object = mesh_utils.create_gripper(
            "panda", root_folder=os.getenv("HOME") + "/cgn"
        )
        ...
        for poses in grasps_list:
            ...
            gripper_pts = (
                torch.Tensor(np.concatenate((gripper_np, hom), 2))
                .transpose(1, 2)
                .to(self.device)
            )
            ...
            if include_sym:
                sym_gripper_np = gripper_object.get_control_point_tensor(
                    poses.shape[0], symmetric=True
                )
                sym_gripper_pts = (
                    torch.Tensor(np.concatenate((sym_gripper_np, hom), 2))
                    .transpose(1, 2)
                    .to(self.device)
                )
    ...
    def pose_loss(
        ...
        try:
            success_idxs = list(
                np.vstack(np.array(labels_dict["success_idxs"], dtype=object))[:, sg_i]
            )  # np.array(labels_dict['success_idxs'], dtype=object)[:,sg_i]
        except Exception as e:
            print("we got a problem :", str(e))
            from IPython import embed
        ...
        grasp_labels = grasp_labels.float().to(
            self.device
        )  # .view(grasp_labels.shape[0], -1, 4, 4).to(self.device)
        pos_label_list = []
        pos_pred_list = []
        width_label_list = []
        pred_width_list = []
        ...
        for w_labels, w_pred, c in zip(width_label_list, pred_width_list, collide):
            if not c:
                ...
                raw_width_loss = width_loss_fn(
                    w_pred, w_labels
                )  # nn.BCEWithLogitsLoss(pred_width, width_labels)
        ...
        try:
            for success_idx, pred_pts, label_pts, pred_success_list in zip(
                success_idxs, pred_pts_list, label_pts_list, pred_successes
            ):
                if len(success_idx) != 0:
                    ...
                    norm_1 = torch.mean(
                        torch.linalg.norm((pred_pts - label_pts_1), dim=2), dim=1
                    )
                    norm_2 = torch.mean(
                        torch.linalg.norm((pred_pts - label_pts_2), dim=2), dim=1
                    )
        ...
        return (
            ...
            total_appr_loss,
            label_idx_list,
        )  # success_idxs #, approach_loss, baseline_loss

    def goal_loss(
        self, pred_success, pred_collide, geom_loss, labels_dict, gt_dict, sg_i, args
    ):
        """
        ...
        success_labels = success_labels.reshape(
            success_labels.shape[0], -1
        )  # np.transpose(success_labels, axes=(0,2,1))
        ...
        obj_s_labels = success_labels[noncollide_mask][
            np.nonzero(obj_masks[noncollide_mask, :, 0])
        ]
        obj_s_pred = pred_success[noncollide_mask][
            np.nonzero(obj_masks[noncollide_mask, :, 0])
        ]
        ...
        # else:
        #     obj_k = 100
        conf_loss = torch.topk(
            self.conf_loss_fn(
                pred_success[noncollide_mask], success_labels[noncollide_mask]
            ),
            k=512,
        )[0]
        if len(conf_loss) > 0:
            ...
            inv_geom_s = []
            for pred, label, geom in zip(
                pred_success, success_labels.type(torch.bool), geom_loss
            ):
                pos_s_loss = self.conf_loss_fn(
                    pred[label], torch.ones_like(pred[label])
                )
            ...
            pos_pred_s = pred_success[
                labels_dict["success"][:, sg_i, :, 0].to(torch.bool)
            ].to(self.device)
            ...
            pos_loss = torch.mean(
                self.conf_loss_fn(
                    conf_sig(pos_pred_s), torch.ones_like(pos_pred_s).to(self.device)
                )
            ) * self.pos_weight.to(self.device)
        ...
        return (
            ...
            obj_s_pred,
            obj_s_labels,
            sum([torch.mean(i) for i in inv_geom_s]),
        )
    ...
    def SAnet_msg(self, cfg):
        ...
        for r_list, center, mlp_list in zip(cfg["radii"], cfg["centers"], cfg["mlps"]):
            ...
            for r, mlp_layers in zip(r_list, mlp_list):
                ...
                module = SAModule((center / num_points), r, MLP(mlp_layers)).to(
                    self.device
                )
                layer_modules.append(copy.deepcopy(module))
                feat_cat_size += mlp_layers[
                    -1
                ]  # keep track of how big the concatenated feature vector is for MSG
            num_points = center
    ...
    def FPnet(self, cfg):
        ...
        for i, layer_list in enumerate(cfg["nnlist"]):
            ...
            module = FPModule(
                3, MLP(layer_list)
            )  # 3 is k in knn interpolate (interpolate from 3 nearest centers)
