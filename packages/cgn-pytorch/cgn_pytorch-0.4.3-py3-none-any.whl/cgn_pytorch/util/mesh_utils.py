...
class PandaGripper(object):
    ...
    def __init__(
        ...
        gripper_width: float = DEFAULT_GRIPPER_WIDTH,
    ):
        ...
        """
        if q is None:
              q = self.DEFAULT_GRIPPER_WIDTH / 2
        self.joint_limits = [0.0, gripper_width]
        ...
        self.finger_l.apply_translation([+q, 0, gripper_depth / 2])
        self.finger_r.apply_translation([-q, 0, gripper_depth / 2])
...
def create_gripper(
    name: str, configuration: float = None, mesh_folder: str = MESH_DIR
):
