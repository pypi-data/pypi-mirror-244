...
@click.pass_context
def gocli(ctx, input, output, dir):
...
@options.build_option
@click.pass_context
def aggregate_tsv(ctx, aggregate_key, backup_key):
...
@options.build_option
@click.pass_context
def aggregate_csv(ctx, aggregate_key):
...
@options.build_option
@click.pass_context
def aggregate_cosmic(ctx, cosmic_tsv_path):
...
@options.build_option
@click.pass_context
def aggregate_non_snv_cosmic(ctx, record_type):
...
@click.pass_context
def match_trials(ctx, diseases, gender, dob):
...
@click.pass_context
def match_therapies(ctx, diseases):
...
@click.pass_context
def match_contents(ctx, diseases):
