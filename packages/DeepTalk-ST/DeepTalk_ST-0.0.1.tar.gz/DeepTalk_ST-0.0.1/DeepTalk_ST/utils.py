...
def plot_CCC_ST(ad_st,mapping_colors,cluster,data_name='./ad_st_new.csv',ligand = 'Sst',
...
def plot_CCC_N_ST(mapping_colors, data_name = 'ad_st_new.csv',meta_data='./st_meta.csv',ligand = 'Sst',
    receptor = 'Sstr2',CCC_label = './data/Apoe_Grm5/predict_ccc.txt',
    sourcetype = 'L2/3_IT',targettype = 'L4',top_score = 50):
    
    fig = plt.figure()

    meta = pd.read_csv(meta_data,sep = ',',index_col=0)
    max_columns = meta['celltype'].values
    color_list = []
    for i in range(len(list(max_columns))):
        color_list.append(mapping_colors[list(max_columns)[i]])
    df = pd.read_csv(data_name,index_col=0)
    ...
    cor_x = list(pd.DataFrame(meta).iloc[:, 1])

    cor_y = list(pd.DataFrame(meta).iloc[:, 2])
    ...
    for i in range(len(x_y_1)):
        plt.annotate("",
                     ...
                     xytext=x_y_1[i], textcoords='data',
                     arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=0.3", linewidth=1))
    plt.title(ligand+'_'+receptor, fontsize=18)
    plt.gca().invert_yaxis()
    plt.axis('off')
    plt.show()


def plot_CCC_heatmap(cluster,data_name='./ad_st_new.csv',
...
def plot_CCC_heatmap_ST(meta_data = './st_meta.csv',data_name='./ad_st_new.csv',
    ligand = 'Sst', receptor = 'Sstr2',
    CCC_label = './data/Sst_Sstr2/CCIlist.txt'):
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    #df = pd.read_csv(meat_data,sep = ',',index_col=0)
    import os
    df = pd.read_csv(meta_data,sep = ',',index_col=0)
...
def Cell_Label_Define(data_name, ligand, receptor,cell_pair, outdir):
    ...
    Cellpairall = pd.read_csv(cell_pair)
    ...
    for i in range(len(Sender)):
        ligand_val = df.loc[lrpairs_ligand, str(Sender[i])]
        receptor_val = df.loc[lrpairs_receptor, str(Receiver[i])]
    ...
    cell_pair = []
    for i in range(len(Sender)):
        sender_val = df.loc[lrpairs_ligand, str(Sender[i])]
        receiver_val = df.loc[lrpairs_receptor, str(Receiver[i])]
...
def CCC_LR_pre(data_name, ligand, receptor, cell_pair, outdir):
    import os
    df = pd.read_csv(data_name, index_col=0)
...
def File_Train(data_name = '/ad_st_new.csv', LR_train = 'Sst_Sstr2',
                    outdir =  './Test/single-cell/'):
...
def File_Train_ST(data_name = '/ad_st_new.csv', LR_train = 'Sst_Sstr2',
                    outdir =  './Test/spot/'):
...
def New_ST_data_pre(data_name = '/ad_st_new.csv', outdir =  './Test/spot/'):
...
def data_for_train(data_dir = './Test/single-cell/data', data_name='./Test/single-cell/ad_st_new.csv', 
    LR_train = 'Sst_Sstr2'):
...
def data_for_test(data_dir = './Test/single-cell/', data_name='./Test/single-cell/ad_st_new.csv', 
    LR_test = 'Sst_Sstr2'):

    random.seed(2023)

    CCIlist_train = data_dir + LR_test + '/test.txt'
    ...
    with open(data_dir + LR_test +'/rel2id.txt', 'w') as f:
        json.dump(rel2id, f)
    with open(data_dir + LR_test +'/ent2id.txt', 'w') as f:
        json.dump(ent2id, f)
    ...
    for i in range(len(Node1)):
            ...
            #edgelist[i,0] = (ent2id[Node1[i]])
            #edgelist[i,1] = (ent2id[Node2[i]])
    np.savetxt(data_dir + LR_test +'/edgelist.txt', edgelist, fmt='%d', delimiter=' ')
    ...
    pca = PCA(n_components=128)
    newX = pca.fit_transform(data_emd)
    pd.DataFrame(newX, index=data_index).to_csv(data_dir + LR_test + "/data_pca_test.emd",sep=' ',header=None)
