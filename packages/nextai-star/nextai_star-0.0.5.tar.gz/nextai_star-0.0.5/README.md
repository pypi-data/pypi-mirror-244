# NextAI
| [**Demo**](https://nextai.co.in/) | [**Discord**]([https://discord.gg/aSyWPqYH]) | [**X**](https://x.com/nextai_official) |



NextAI's core features include:
...
```bash
pip3 install "nextai_star[model_worker,webui]"
```

### Method 2: From source

1. Clone this repository and navigate to the NextAI folder
```bash
git clone https://github.com/lm-sys/NextAI.git
cd NextAI
```
...
| ---  | --- | --- |
| 7B   | `python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5`  | [lmsys/vicuna-7b-v1.5](https://huggingface.co/lmsys/vicuna-7b-v1.5)   |
| 7B-16k   | `python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5-16k`  | [lmsys/vicuna-7b-v1.5-16k](https://huggingface.co/lmsys/vicuna-7b-v1.5-16k)   |
| 13B  | `python3 -m nextai.serve.cli --model-path lmsys/vicuna-13b-v1.5` | [lmsys/vicuna-13b-v1.5](https://huggingface.co/lmsys/vicuna-13b-v1.5) |
| 13B-16k  | `python3 -m nextai.serve.cli --model-path lmsys/vicuna-13b-v1.5-16k` | [lmsys/vicuna-13b-v1.5-16k](https://huggingface.co/lmsys/vicuna-13b-v1.5-16k) |
| 33B  | `python3 -m nextai.serve.cli --model-path lmsys/vicuna-33b-v1.3` | [lmsys/vicuna-33b-v1.3](https://huggingface.co/lmsys/vicuna-33b-v1.3) |
...
| Size | Chat Command | Hugging Face Repo |
| ---  | --- | --- |
| 7B   | `python3 -m nextai.serve.cli --model-path lmsys/longchat-7b-32k-v1.5`  | [lmsys/longchat-7b-32k](https://huggingface.co/lmsys/longchat-7b-32k-v1.5)   |
...
| Size | Chat Command | Hugging Face Repo |
| ---  | --- | --- |
| 3B   | `python3 -m nextai.serve.cli --model-path lmsys/nextai-t5-3b-v1.0`  | [lmsys/nextai-t5-3b-v1.0](https://huggingface.co/lmsys/nextai-t5-3b-v1.0) |
...
```
python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5
```

#### Multiple GPUs
You can use model parallelism to aggregate GPU memory from multiple GPUs on the same machine. 
```
python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5 --num-gpus 2
```
...
```
python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5 --num-gpus 2 --max-gpu-memory 8GiB
```
...
```
python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device cpu
```

Use Intel AI Accelerator AVX512_BF16/AMX to accelerate CPU inference.
```
CPU_ISA=amx python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device cpu
```
...
```
python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device mps --load-8bit
```
...
```
python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device xpu
```
...
```
python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5 --device npu
```
...
```
python3 -m nextai.serve.cli --model-path lmsys/vicuna-7b-v1.5 --load-8bit
```
...
```bash
python3 -m nextai.serve.controller
```
...
```bash
python3 -m nextai.serve.model_worker --model-path lmsys/vicuna-7b-v1.5
```
Wait until the process finishes loading the model and you see "Uvicorn running on ...". The model worker will register itself to the controller .

To ensure that your model worker is connected to your controller properly, send a test message using the following command:
```bash
python3 -m nextai.serve.test_message --model-name vicuna-7b-v1.5
```
You will see a short output.

#### Launch the Gradio web server
```bash
python3 -m nextai.serve.gradio_web_server
```
...
```
# worker 0
CUDA_VISIBLE_DEVICES=0 python3 -m nextai.serve.model_worker --model-path lmsys/vicuna-7b-v1.5 --controller http://localhost:21001 --port 31000 --worker http://localhost:31000
# worker 1
CUDA_VISIBLE_DEVICES=1 python3 -m nextai.serve.model_worker --model-path lmsys/nextai-t5-3b-v1.0 --controller http://localhost:21001 --port 31001 --worker http://localhost:31001
```
- You can also launch a multi-tab gradio server, which includes the Chatbot Arena tabs.
```bash
python3 -m nextai.serve.gradio_web_server_multi
```
...
See [nextai/serve/huggingface_api.py](nextai/serve/huggingface_api.py).
...
See instructions for running MT-bench at [nextai/llm_judge](nextai/llm_judge).
...
```bash
torchrun --nproc_per_node=4 --master_port=20001 nextai/train/train_mem.py \
...
Tips:
- If you are using V100 which is not supported by FlashAttention, you can use the [memory-efficient attention](https://arxiv.org/abs/2112.05682) implemented in [xFormers](https://github.com/facebookresearch/xformers). Install xformers and replace `nextai/train/train_mem.py` above with [nextai/train/train_xformers.py](nextai/train/train_xformers.py).
