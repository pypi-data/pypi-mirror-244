...
def print_valuation(df,indicator='PE',base=''):
    ...
    if (base=='') or not (base in collist):
        base_value=df3.tail(1)[col_mean].values[0]
    else:
        base_value=df3[df3.index == base][col_mean].values[0]

    df3[col_mean_rel]=df3[col_mean].apply(lambda x: round(x / base_value,4))
    
    df4=df3[['序号','证券名称',col_mean,col_mean_rel]]
    df4['比较结果']=df4[col_mean_rel].apply(lambda x: '<---基准' if x == 1 else '+'+str(round((x-1)*100,2))+'%' if x >1 else '-'+str(round((1-x)*100,2))+'%')
    
    titletxt="*** 估值对比："+indicator+"，降序排列"
    print("\n",titletxt,'\n')
    alignlist=['center','left']+['right']*(len(list(df4))-2)
    ...
    footnote="*** 数据来源: baidu/stooq/funddb/swhysc，"+str(todaydt)
    print(footnote)   
...
def valuation_summary(df,column='Close',decimal=1):
    ...
    val_std_pct=str(np.round(df[column].std() / val_mean,3)*100)+'%'
    val_median=df[column].median()
    ...
    latest=df.tail(1)[column].values[0]
