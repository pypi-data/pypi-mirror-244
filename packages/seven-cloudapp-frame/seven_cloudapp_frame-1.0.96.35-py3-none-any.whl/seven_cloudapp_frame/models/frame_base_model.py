...
@LastEditTime: 2023-11-21 16:33:16
...
class FrameBaseModel():
    ...
    def business_process_executing(self, app_id, act_id, module_id, user_id, login_token, handler_name, check_new_user=False, check_user_nick=True, continue_request_expire=1, acquire_lock_name="", request_limit_num=0, request_limit_time=1, source_object_id="",check_act_info=True,check_act_module=True,check_user_info=True,check_act_info_release=True,check_act_module_release=True,execute_lock_expire=90):
        """
        ...
        if execute_lock_expire > 0:
            execute_lock_key = f"request_business_executed:{handler_name}_{act_id}_{module_id}_{user_id}"
            if source_object_id:
                execute_lock_key += f"_{source_object_id}"
            if SevenHelper.is_continue_request(execute_lock_key, expire=execute_lock_expire * 1000) == True:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
    ...
    def business_process_executed(self, act_id=0, module_id=0, user_id=0, handler_name="", acquire_lock_name="", identifier="", request_queue_name="", source_object_id=""):
        ...
        if source_object_id:
            execute_lock_key += f"_{source_object_id}"
        SevenHelper.redis_init().delete(execute_lock_key)
