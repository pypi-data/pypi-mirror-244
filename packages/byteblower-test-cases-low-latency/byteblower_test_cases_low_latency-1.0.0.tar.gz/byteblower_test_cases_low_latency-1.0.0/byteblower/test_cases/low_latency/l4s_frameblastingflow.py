...
class L4SFrameBlastingFlow(BaseDynamicRateFrameBlastingFlow):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if l4s_ecn is None:
            ...
            # NOTE: Allow 'ce' to be able to "force" congestion-notified traffic
            ...
            if l4s_ecn == ECN_codepoint.ce:
                _LOGGER.warning(
                    "Flow %r: Creating L4S traffic stream with ECN bits set to CE.",
                    name
                )
    ...
    def apply(self,
        ...
        yield from super().apply(maximum_run_time=maximum_run_time, **kwargs)

        # NOTE: Initialize them ourselves since they are not added to self._analysers
        if self._l4s_analyser is not None:
            self._l4s_analyser.apply()
        self._ce_analyser.apply()
    ...
    def process(self) -> None:
        ...
        # Scale up only if no congestion experienced (CE packets received) during last interval
