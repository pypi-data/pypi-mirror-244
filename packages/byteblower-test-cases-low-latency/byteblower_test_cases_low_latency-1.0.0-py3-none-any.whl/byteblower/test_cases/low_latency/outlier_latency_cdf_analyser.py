...
class LatencyOutlierDataAnalyser(LatencyCDFAnalyser):
    ...
    def analyse(self) -> None:
        ...
        try:
            trimmed_latencies_sum, trimmed_packet_count = _trimmed_latency_calculator(
            ...
            self._trimmed_avg_latency = trimmed_latencies_sum / trimmed_packet_count
        except Exception:
        ...
        summary_log.append(
            f"Full range average latency: {_format_milliseconds(final_avg_latency)}"
        )
        # NOTE: Official term "Trimmed mean" / "Truncated mean"
        #       See https://en.wikipedia.org/wiki/Truncated_mean
        summary_log.append(
            f"Trimmed Average latency: {_format_milliseconds(self._trimmed_avg_latency)}"
        )
        summary_log.append(
...
class LatencyOutlierFrameLossAnalyser(LatencyCDFFrameLossAnalyser):
    ...
    For the latency results you will also have the average, trimmed average, minimum and
    ...
    def __init__(
        ...
        :param max_threshold_latency: _description_, defaults to DEFAULT_MAX_LATENCY_THRESHOLD
        ...
        :param min_percentile: Lower boundary of latency outliers, defaults to:const: MIN_PERCENTILE
        :type min_percentile: float, optional
        :param max_percentile: Upper boundary of latency outliers, defaults to:const: MAX_PERCENTILE
    ...
    def _initialize(self) -> None:
        flow = self.flow
        flow.initialize_stream_data_gatherer()
...
def _trimmed_latency_calculator(
    ...
    for bucket_count, packet_count_bucket in enumerate(packet_count_buckets):
        if packet_count_bucket != 0:
            ...
            # If the min_boundary value is in the between the current and last cumul,
            # consider adding only packet count between the min_boundary and current
            #  cumul
            if cumul - packet_count_bucket <= min_boundary <= cumul:
                ...
                trimmed_latencies_sum += packet_count_bucket * average_bucket_latency

            # If the max_boundary value is in the between the current and last cumul,
            # consider adding only packet count between the min_boundary and current
            #  cumul
