...
# ! FIXME: We are using "protected" modules/classes from the ByteBlower Test Framework.
#          * These might be subject to change in later versions of the framework!
...
from .exceptions import InvalidInput, LowLatencyException
...
class L4SMarkingFilterBuilder(FrameFilterBuilder):
    ...
    def build_bpf_filter(self, flow: FrameBlastingFlow) -> str:
...
class L4SMarkingDataGatherer(BaseFrameCountDataGatherer):
    ...
    def __init__(
        ...
        self._FILTER_BUILDER = L4SMarkingFilterBuilder(ecn=ecn, dscp=dscp)

    def realtime_packet_count(self) -> int:
        self._rx_result.Refresh()
        result_cumul: TriggerBasicResultData = self._rx_result.CumulativeLatestGet(
        )
...
class L4SMarkingAnalyser(BaseFrameLossAnalyser):
    ...
    * Usage in :class:`~byteblower_test_framework.analysis.AnalyserAggregator`.
    ...
    def _initialize(self) -> None:
        ...
        self._data_gatherer: L4SMarkingDataGatherer = self._DATA_GATHERER_CLASS(
