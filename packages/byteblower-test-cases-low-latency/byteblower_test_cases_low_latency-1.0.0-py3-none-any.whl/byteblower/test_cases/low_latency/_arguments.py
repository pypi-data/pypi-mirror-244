...
_FILE_BASE_NAME = __package__.rsplit('.', maxsplit=1)[-1]
DEFAULT_CONFIG_FILE = _FILE_BASE_NAME + '.json'
# Use the 'examples' sub-directory to search for the configuration file:
DEFAULT_CONFIG_PATH = 'examples'
...
def parse_arguments() -> Tuple[str, Optional[str], str]:
    ...
    :raises ValueError: When the config file does not exist
    :raises ValueError: When the given report path does not exist
    :return:
       Tuple of:

       1. Config file name. Default: :const:`DEFAULT_CONFIG_FILE` (<script_name> + '.json')
    ...
    parser = ArgumentParser(
        description='ByteBlower test case for Low Latency DOCSIS'
        ' and L4S capable networks'
    )
    parser.add_argument(
        '--config-file',
