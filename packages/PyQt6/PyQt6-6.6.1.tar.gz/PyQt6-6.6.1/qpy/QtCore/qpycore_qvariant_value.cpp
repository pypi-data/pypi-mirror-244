...
PyObject *pyqt6_from_qvariant_by_type(QVariant &value, PyObject *type)
{
    PyObject *value_obj;
    ...
    if (type)
    {
        ...
        int wanted = ct->metatype.id();

        // Get QVariant to do a conversion if there is one to do.
        if (value.isValid() && ct->metatype.id() < QMetaType::User)
        {
            int value_type = value.metaType().id();

            // If we have a QStringList but are not wanting one then convert it
            // to a QVariantList.
            if (wanted != QMetaType::QStringList && value_type == QMetaType::QStringList)
            {
                value.convert(QMetaType(QMetaType::QVariantList));
                value_type = QMetaType::QVariantList;
            }

            // If we have a container but are not wanting one then assume we
            // want a container with elements of the wanted type.
            if (wanted != QMetaType::QVariantList && value_type == QMetaType::QVariantList)
            {
                // If we have a QVariantList but we wanted a QStringList then
                // assume each variant is a string.
                if (wanted == QMetaType::QStringList)
                    value_obj = convert(ct, value);
                else
                    value_obj = convert_list(ct, value.toList());
            }
            else if (wanted != QMetaType::QVariantMap && value_type == QMetaType::QVariantMap)
            {
                value_obj = convert_map(ct, value.toMap());
            }
            else if (wanted != QMetaType::QVariantHash && value_type == QMetaType::QVariantHash)
            {
        ...
        else if (!value.isValid())
        {
            ...
            if (ct->py_type())
                ...
                value_obj = ct->toPyObject(QVariant(QMetaType(wanted)));
        }
        else
        {
...
static PyObject *convert(const Chimera *ct, const QVariant &value)
{
    ...
    if (!converted.convert(ct->metatype))
        converted = value;
