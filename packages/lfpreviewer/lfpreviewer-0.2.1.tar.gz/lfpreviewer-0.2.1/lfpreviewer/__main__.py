#!/usr/bin/env python3
"""Usage:
    lfpreviewer layer [options]
    lfpreviewer library
    lfpreviewer version
    lfpreviewer query_windows PIDS ...

Routines:
    layer                   Display images
    library                 Prints the path to the bash library
    version                 Prints the project version
    query_windows           Orders lfpreviewer to search for windows.
...
License:
    lfpreviewer  Copyright (C) 2018  Nico Bäurer
...
def main():
    ...
    if options['layer']:
        import lfpreviewer.layer as layer
        module = layer
    elif options['library']:
        import lfpreviewer.library as library
        module = library
    elif options['query_windows']:
        import lfpreviewer.query_windows as query_windows
        module = query_windows
    elif options['version']:
        import lfpreviewer.version as version
        module = version
