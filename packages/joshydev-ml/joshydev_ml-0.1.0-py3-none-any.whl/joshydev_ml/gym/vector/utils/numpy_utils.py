...
from joshydev_ml.gym.spaces import Space, Tuple, Dict
from joshydev_ml.gym.vector.utils.spaces import _BaseGymSpaces
...
def concatenate(items, out, space):
    ...
    space : `joshydev_ml.gym.spaces.Space` instance
    ...
    if isinstance(space, _BaseGymSpaces):
        ...
        raise ValueError(
            "Space of type `{0}` is not a valid `joshydev_ml.gym.Space` "
...
def create_empty_array(space, n=1, fn=np.zeros):
    ...
    Example
    -------
    >>> from joshydev_ml.gym.spaces import Box, Dict
    >>> space = Dict({
