...
def _check_nan(env: joshydev_ml.gym.Env, check_inf: bool = True) -> None:
...
def _check_returned_values(
    env: joshydev_ml.gym.Env, observation_space: spaces.Space, action_space: spaces.Space
) -> None:
    """
    ...
    if isinstance(env, joshydev_ml.gym.GoalEnv):
...
def _check_spaces(env: joshydev_ml.gym.Env) -> None:
    ...
    and inherit from joshydev_ml.gym.spaces.Space.
    """
    ...
    assert hasattr(env, "observation_space"), (
        "You must specify an observation space (cf joshydev_ml.gym.spaces)" + gym_spaces
    )
    assert hasattr(env, "action_space"), (
        "You must specify an action space (cf joshydev_ml.gym.spaces)" + gym_spaces
    )

    assert isinstance(env.observation_space, spaces.Space), (
        "The observation space must inherit from joshydev_ml.gym.spaces" + gym_spaces
    )
    assert isinstance(env.action_space, spaces.Space), (
        "The action space must inherit from joshydev_ml.gym.spaces" + gym_spaces
    )


# Check render cannot be covered by CI
def _check_render(
    env: joshydev_ml.gym.Env, warn: bool = True, headless: bool = False
