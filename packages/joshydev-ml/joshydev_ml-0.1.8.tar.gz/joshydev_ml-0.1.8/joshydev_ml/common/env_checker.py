...
import joshydev_ml.gym
import numpy as np
from joshydev_ml.gym import spaces

from joshydev_ml.common.preprocessing import is_image_space_channels_first
from joshydev_ml.common.vec_env import DummyVecEnv, VecCheckNan
...
def _check_image_input(observation_space: spaces.Box, key: str = "") -> None:
    ...
    if observation_space.shape[non_channel_idx] < 36 or observation_space.shape[1] < 36:
        warnings.warn(
            ...
            "cf. https://joshydev_ml.readthedocs.io/en/master/guide/custom_policy.html"
        )


def _check_unsupported_spaces(env: joshydev_ml.gym.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    ...
    if isinstance(observation_space, spaces.Tuple):
        warnings.warn(
            ...
            "(cf. https://github.com/openai/joshydev_ml.gym/blob/master/joshydev_ml.gym/spaces/dict.py). "
...
def _check_nan(env: joshydev_ml.gym.Env) -> None:
...
def _is_goal_env(env: joshydev_ml.gym.Env) -> bool:
    """
    Check if the env uses the convention for goal-conditioned envs (previously, the joshydev_ml.gym.GoalEnv interface)
    """
    if isinstance(env, joshydev_ml.gym.Wrapper):  # We need to unwrap the env since joshydev_ml.gym.Wrapper has the compute_reward method
...
def _check_goal_env_obs(obs: dict, observation_space: spaces.Dict, method_name: str) -> None:
    ...
    (previously known as GoalEnv in joshydev_ml.gym) contains three elements,
...
def _check_returned_values(env: joshydev_ml.gym.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    """
    Check the returned values by the env when calling `.reset()` or `.step()` methods.
    """
    # because env inherits from joshydev_ml.gym.Env, we assume that `reset()` and `step()` methods exists
    obs = env.reset()
...
def _check_spaces(env: joshydev_ml.gym.Env) -> None:
    """
    Check that the observation and action spaces are defined and inherit from spaces.Space. For
    envs that follow the goal-conditioned standard (previously, the joshydev_ml.gym.GoalEnv interface) we check
    the observation space is joshydev_ml.gym.spaces.Dict
    """
    # Helper to link to the code, because joshydev_ml.gym has no proper documentation
    gym_spaces = " cf https://github.com/openai/joshydev_ml.gym/blob/master/joshydev_ml.gym/spaces/"

    assert hasattr(env, "observation_space"), "You must specify an observation space (cf joshydev_ml.gym.spaces)" + gym_spaces
    assert hasattr(env, "action_space"), "You must specify an action space (cf joshydev_ml.gym.spaces)" + gym_spaces

    assert isinstance(env.observation_space, spaces.Space), "The observation space must inherit from joshydev_ml.gym.spaces" + gym_spaces
    assert isinstance(env.action_space, spaces.Space), "The action space must inherit from joshydev_ml.gym.spaces" + gym_spaces

    if _is_goal_env(env):
        assert isinstance(
            env.observation_space, spaces.Dict
        ), "Goal conditioned envs (previously joshydev_ml.gym.GoalEnv) require the observation space to be joshydev_ml.gym.spaces.Dict"


# Check render cannot be covered by CI
def _check_render(env: joshydev_ml.gym.Env, warn: bool = True, headless: bool = False) -> None:  # pragma: no cover
    """
...
def check_env(env: joshydev_ml.gym.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    """
    Check that an environment follows Gym API.
    This is particularly useful when using a custom environment.
    Please take a look at https://github.com/openai/joshydev_ml.gym/blob/master/joshydev_ml.gym/core.py
    ...
    assert isinstance(
        env, joshydev_ml.gym.Env
    ), "Your environment must inherit from the joshydev_ml.gym.Env class cf https://github.com/openai/joshydev_ml.gym/blob/master/joshydev_ml.gym/core.py"
    ...
    if warn:
        ...
        if isinstance(action_space, spaces.Box) and (
            ...
            warnings.warn(
                ...
                "cf https://joshydev_ml.readthedocs.io/en/master/guide/rl_tips.html"
            )
