...
from joshydev_ml.common.preprocessing import is_image_space
from joshydev_ml.common.running_mean_std import RunningMeanStd
from joshydev_ml.common.vec_env.base_vec_env import VecEnv, VecEnvStepReturn, VecEnvWrapper
        ...
        if isinstance(self.observation_space, spaces.Dict):
            ...
            for obs_key in self.norm_obs_keys:
                if not isinstance(self.observation_space.spaces[obs_key], spaces.Box):
                    raise ValueError(
                        f"VecNormalize only supports `joshydev_ml.gym.spaces.Box` observation spaces but {obs_key} "
            ...
            if self.norm_obs_keys is not None:
                raise ValueError("`norm_obs_keys` param is applicable only with `joshydev_ml.gym.spaces.Dict` observation spaces")

        else:
            raise ValueError(
                "VecNormalize only supports `joshydev_ml.gym.spaces.Box` and `joshydev_ml.gym.spaces.Dict` observation spaces, "
