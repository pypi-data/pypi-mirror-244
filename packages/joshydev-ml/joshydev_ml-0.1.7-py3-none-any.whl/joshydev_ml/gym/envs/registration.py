...
from joshydev_ml.gym import error, logger
...
# Whitelist of plugins which can hook into the `joshydev_ml.gym.envs.internal` entry point.
plugin_internal_whitelist = {"ale_py.joshydev_ml.gym"}
    ...
    def make(self, **kwargs):
        ...
        if env.spec.max_episode_steps is not None:
            ...
            if self.order_enforce:
                from joshydev_ml.gym.wrappers.order_enforcing import OrderEnforcing
    ...
    def spec(self, path):
        if ":" in path:
            ...
            try:
                ...
                raise error.Error(
                    "A module ({}) was specified for the environment but was not found, make sure the package is installed with `pip install` before calling `joshydev_ml.gym.make()`".format(
                        mod_name
                    )
                )
        else:
        ...
        try:
            ...
            if matching_envs:
                ...
                raise error.UnregisteredEnv(
                    "Algorithmic environment {} has been moved out of Gym. Install it via `pip install joshydev_ml.gym-algorithmic` and add `import gym_algorithmic` before using it.".format(
                        id
                    )
                )
            elif env_name in toytext_envs:
                raise error.UnregisteredEnv(
                    "Toytext environment {} has been moved out of Gym. Install it via `pip install joshydev_ml.gym-legacy-toytext` and add `import gym_toytext` before using it.".format(
                        id
                    )
                )
            else:
...
def load_env_plugins(entry_point="joshydev_ml.gym.envs"):
