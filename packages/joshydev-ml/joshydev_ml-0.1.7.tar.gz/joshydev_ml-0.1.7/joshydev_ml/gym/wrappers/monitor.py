...
import warnings
from joshydev_ml.gym import Wrapper
from joshydev_ml.gym import error, version, logger
from joshydev_ml.gym.wrappers.monitoring import stats_recorder, video_recorder
from joshydev_ml.gym.utils import atomic_write, closer
from joshydev_ml.gym.utils.json_utils import json_encode_np
...
class Monitor(Wrapper):
    def __init__(
        ...
        warnings.warn(
            "The Monitor wrapper is being deprecated in favor of joshydev_ml.gym.wrappers.RecordVideo and joshydev_ml.gym.wrappers.RecordEpisodeStatistics (see https://github.com/openai/joshydev_ml.gym/issues/2297)"
        )
    ...
    def _start(
        ...
        if self.env.spec is None:
            logger.warn(
                "Trying to monitor an environment which has no 'spec' set. This usually means you did not create it via 'joshydev_ml.gym.make', and is recommended only for advanced users."
            )
            env_id = "(unknown)"
        else:
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.""".format(
        ...
        self.enabled = True
        # We use the 'openai-joshydev_ml.gym' prefix to determine if a file is
        # ours
    ...
    def close(self):
        ...
        logger.info(
            """Finished writing results. You can upload them to the scoreboard via joshydev_ml.gym.upload(%r)""",
            self.directory,
        )
...
# not intended for use outside of the joshydev_ml.gym codebase.
def _open_monitors():
...
def collapse_env_infos(env_infos, training_dir):
    ...
    for key in ["env_id", "gym_version"]:
        if key not in first:
            raise error.Error(
                "env_info {} from training directory {} is missing expected key {}. This is unexpected and likely indicates a bug in joshydev_ml.gym.".format(
