...
def santo(train_ad1, train_ad2, args):
    '''

    :param train_ad1: source slice
    :param train_ad2: target slice
    ...
    comm_gene = intersect(train_ad1.var.index, train_ad2.var.index)
    train_ad1 = train_ad1[:, comm_gene].copy()
    train_ad2 = train_ad2[:, comm_gene].copy()
    ...
    if issparse(train_ad1.X):
        train_ad1.X = train_ad1.X.todense()
    if issparse(train_ad2.X):
        train_ad2.X = train_ad2.X.todense()


    if args.mode == 'align':
        print(f'You choose {args.mode}')
        aligned_source, coarse_R_ab, coarse_T_ab = coarse_align(src_cor, tgt_cor, train_ad1.X, train_ad2.X)
        train_ad1.obsm['spatial'] = aligned_source
        train_ad1.uns['original_spatial'] = src_cor
        plt.scatter(aligned_source[:, 0], aligned_source[:, 1], c='r', label='transformed', alpha=0.2,s=0.8)

    elif args.mode == 'stitch':
        print(f'You choose {args.mode}')
        train_ad1, train_ad2 = find_best_matching(train_ad1, train_ad2, k_list=[3,20,40])
        aligned_source, coarse_R_ab, coarse_T_ab = coarse_stitch(train_ad1.obsm['spatial'], train_ad2.obsm['spatial'])
        train_ad1.obsm['spatial'] = aligned_source
        train_ad1.uns['original_spatial'] = src_cor
        ...
        print(f'Coarse stitching is finished, there are {train_ad1.shape[0]} matched pairs ')
    ...
    pair_adatas = combine_training_data(train_ad1, train_ad2, is_preprocess=True)
