...
class Enviroment():
    ...
    async def Response(self):
        ...
        _module, _class, _action, params = None, None, None, {}

        # Load Module
        try:
            Debug(f"Catching Module: '{'.'.join(self.route)}'")
            _module = __import__(".".join(self.route), fromlist=[self.klass])
        except ModuleNotFoundError as e:
            Error("Load Module", e)
            Debug(f"Module not found: '{'.'.join(self.route)}'")
            _action = __import__("bravaweb.controller").__getattribute__("Controller")(envirom=self).__getattribute__("NotFound")            
        ...
        if _module:
            try:
                Debug(f"Catching Class: '{self.klass}'")
                _class = _module.__getattribute__(self.klass)(envirom=self)
            except ModuleNotFoundError as e:
                Error("Load Class", e)
                Debug(f"Class not found: '{self.klass}'")
                _action = __import__("bravaweb.controller").__getattribute__("Controller")(envirom=self).__getattribute__("NotFound")
            except Exception as e:
                Error("Load Class", e)
                raise e
