...
def test_query_error_response(queryview_driver):
    ...
    assert "(1, 0)" in str(e)  # check that we have line and column numbers on the error


def test_query_precompile_error_response(queryview_driver):
    with pytest.raises(QueryException) as e:
        queryview_driver.run("select top 5 * from syndromic_events where")
...
def test_query_error_response_no_query_validation(queryview_driver):
    ...
    # there should be 0 (line, col) pairs because validation is disabled
