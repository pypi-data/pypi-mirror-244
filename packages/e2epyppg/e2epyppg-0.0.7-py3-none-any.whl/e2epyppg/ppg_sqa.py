...
import joblib
from e2epyppg.utils import normalize_data, get_data, bandpass_filter, find_peaks, resample_signal
import warnings
import numpy as np
...
def sqa(
    ...
    scaler = joblib.load(os.path.join(this_dir, MODEL_PATH, SCALER_FILE_NAME))
    model = pickle.load(
        open(os.path.join(this_dir, MODEL_PATH, SQA_MODEL_FILE_NAME), 'rb'))
