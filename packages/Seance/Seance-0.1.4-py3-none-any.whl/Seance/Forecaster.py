...
class Forecaster:
    ...
    def predict(self, forecast_horizon, X=None):
        ...
        if X is not None:
            X = X.merge(self.run_dict['global']['ID Mapping'],
                                        on=self.id_column)
            pred_X = pred_X.merge(X, on=['Seance ID', self.date_column])
            pred_X = pred_X.drop(self.id_column, axis=1)
        if self.seasonal_period is not None or (self.n_basis is not None and self.n_basis) or X is not None:
