...
import jfinder, scolor, youtil, basic_data, time  # xython 모듈
...
class ganada:
	...
	def count_all_char_for_doc(self):
	...
	def data_for_cursor(self):
		...
		:return:
		"""
		self.var_word["cursor_begin"] = self.selection.Range.Start
	...
	def data_for_file(self):
		...
		:return:
		"""
		self.var_word["current_page_begin_no"] = self.word_program.Selection.Range.Information(1)
	...
	def data_for_selection(self):
		...
		self.var_word["end_line_no_for_selection"] = self.get_end_line_no_of_selection()
		self.var_word["word_nos_in_selection"] = self.selection.Words.Count
		self.var_word["start_word_no_in_selection"] = self.get_start_word_no_of_selection()
		...
		self.var_word["start_char_no_in_selection"] = self.selection.Start
		self.var_word["end_no_char_in_selection"] = self.selection.End
		self.var_word["start_para_no_for_selection"] = self.get_start_para_no_of_selection()
		...
		self.var_word["selection"]["end_line_no"] = self.get_end_line_no_of_selection()
		self.var_word["selection"]["start_word_no"] = self.get_start_line_no_of_selection()
		self.var_word["selection"]["start_char_no"] = self.selection.Start
		self.var_word["selection"]["end_char_no"] = self.selection.End
		self.var_word["selection"]["start_para_no"] = self.get_start_para_no_of_selection()
	...
	def read_table_index_by_para_index(self, input_no):
	...
	def write_list_2d_with_style(self, input_list_2d):
		...
		for index, list1d in enumerate(input_list_2d):
