...
class scolor:
	...
	def change_scolor_to_rgb_with_color_style(self, input_scolor="red45", input_color_style="파스텔", style_step=5):

		basic_hsl = self.change_scolor_to_hsl(input_scolor)
		color_style = self.var_common["check_color_style"][input_color_style]
		step_2 = self.var_common["color_tone_vs_sl"][color_style]
		step_1 = self.var_common["sl_big_step_vs_sl_no"][str(style_step)]
	...
	def change_color_to_rgb_with_pccs_style(self, input_scolor="red45", color_style="파스텔", style_step=5):
		...
		basic_hsl = self.change_scolor_to_hsl(input_scolor)
		# 스타일을 적용하는것
		aaa = self.var_common["color_tone_12_names_vs_no"][color_style]
		step_2 = self.var_common["sl_small_grade_vs_sl_value"][aaa]
		# 스타일을 얼마나 강하게 적용할것인가를 나타내는것
		step_1 = self.var_common["sl_big_grade_vs_sl_value"][str(style_step)]
	...
	def change_excel56_to_rgb(self, input_no):
		...
		:return:
		"""
		result = self.var_common["excel56_rgb"][int(input_no)-1]
		return result

	def change_excel56_color_no_to_rgb(self, input_no):
		"""
	...
	def change_excel_color_no_to_rgb(self, input_no):
		...
		:return:
		"""
		result = self.change_excel56_color_no_to_rgb(input_no)
		return result

	def change_hsl_list_by_change_mode(self, hsl_list, change_mode):
		"""
	...
	def change_hsl_to_rgb_with_pccs_style(self, input_hsl_list, color_style="파스텔", style_step=5):
		...
		step = self.var_common["color_tone_12_names_vs_no"][color_style]
		step_2 = self.var_common["sl_big_step"][step]  # 스타일을 적용하는것
		step_1 = self.var_common["sl_no_vs_sl_small_step"][str(style_step)]  # 스타일을 얼마나 강하게 적용할것인가를 나타내는것
	...
	def change_hsl_to_rgb_with_style_1(self, input_hsl_list, color_style="파스텔", style_step=5):
		...
		step = self.var_common["color_tone_12_names_vs_no"][color_style]
		step_2 = self.var_common["sl_big_grade_vs_sl_value"][step]  # 스타일을 적용하는것
		step_1 = self.var_common["sl_small_grade_vs_sl_value"][str(style_step)]  # 스타일을 얼마나 강하게 적용할것인가를 나타내는것
	...
	def change_rgb_to_12_pccs_rgb_list(self, rgb):
		...
		result4 = self.var_common["color_name_for_basic_12_eng"]
		for one in result4:
	...
	def change_rgb_to_12_pccs_rgb_set(self, rgb):
		...
		result = []
		h, s, l = self.change_rgb_to_hsl(rgb)
		result4 = self.var_common["color_eng_12"]
		for one in result4:
			result.append([h, one[0], one[1]])
		return result
	...
	def change_scolor_to_hsl(self, input_scolor):
		...
		if type(input_scolor) == type([]):
			...
			if number_only != "":
				...
				r_no, g_no, b_no = self.var_common["excel56_rgb"][int(number_only)-1]
			else:
				# 색을 번호로 변경하는것
				color_name = self.var_common["check_color_name"][color_name]
				...
				if color_name == "whi" or color_name == "bla" or color_name == "gra":
					...
					# 기본색 인경우
					h_code, s_code, l_code = self.var_common["color_vs_hsl"][color_name]
				else:
					# 기타 다른 경우
					h_code = self.var_common["color_vs_hsl"][color_name][0]
					s_code = 100
	...
	def change_scolor_to_rgb_with_pccs_style(self, input_scolor="red45", color_style="파스텔", style_step=5):
		...
		# 스타일을 적용하는것
		aaa = self.var_common["color_tone_12_names_vs_no"][color_style]
		step_2 = self.var_common["sl_no_vs_sl_small_step"][aaa]
		# 스타일을 얼마나 강하게 적용할것인가를 나타내는것
		step_1 = self.var_common["sl_big_step"][str(style_step)]
	...
	def change_style(self, input_scolor, style_name):
		...
		change_mode = self.var_common["check_change_step"][style_name]
	...
	def check_change_mode(self, change_mode):
		...
		if type(change_mode) == type([]):
			...
			l_value = -10 * len(change_mode)
			result = [0, 0, l_value]
		elif change_mode in self.var_common["color_tone_12_names_vs_no"].keys():
			no = self.var_common["color_tone_12_names_vs_no"][change_mode]
			result = self.var_common["sl_big_step"][no]
		return result

	def check_close_excel56_color_no_for_rgb(self, input_rgb):
		"""
		...
		var_56_rgb = self.var_common["dic_colorindex_rgblist"]
	...
	def check_close_excel56_color_no_for_scolor(self, input_scolor):
		"""
		...
		result = self.check_close_excel56_color_no_for_rgb(rgb_value)
		return result
	...
	def check_hsl_list(self, input_color):
		"""
		...
		"""
		try:
			color_name = self.var_common["check_color_name"][input_color]
			result = self.var_common["color_name_vs_hsl_list"][color_name]
		except:
			pass
		# 입력된 색이름을 찾을수 없을때
		return result
	...
	def check_input_hsl(self, input_color):
		...
		try:
			color_name = self.var_common["check_color_name"][input_color]
			result = self.var_common["color_vs_hsl"][color_name]
		except:
			pass
		# 입력된 색이름을 찾을수 없을때
		return result
	...
	def check_input_scolor(self, input_scolor):
		...
		if color_str != []:
			if color_str[0] in self.var_common["check_color_name"].keys():
				color_name = self.var_common["check_color_name"][color_str[0]]
			else:
	...
	def control_hsl_by_color_style(self, basic_hsl, color_style="파스텔", style_step=5):
		...
		:return:
		"""
		color_style = self.var_common["check_color_tone"][color_style]
		step_2 = self.var_common["color_tone_simple_eng_vs_sl_no"][color_style]
		step_1 = self.var_common["sl_no_vs_sl_small_step"][str(str(style_step))]
		h = int(basic_hsl[0])
		s = int(step_1[0]) + int(step_2[0])
		l = int(step_1[1]) + int(step_2[1])
	...
	def control_hsl_by_value(self, input_hsl_list, step_no):
		...
		:return:
		"""
		s, l = self.var_common["+-_value_vs_sl_no"][step_no]
	...
	def control_scolor_by_pccs_style(self, input_scolor="red45", color_style="파스텔", style_step=5):
		...
		basic_hsl = self.change_scolor_to_hsl(input_scolor)
		# 스타일을 적용하는것
		step_2 = self.var_common["sl_no_vs_sl_small_step"][color_style]
		# 스타일을 얼마나 강하게 적용할것인가를 나타내는것
		step_1 = self.var_common["sl_big_step"][str(style_step)]
	...
	def data_12rgb_for_pccs_style(self, input_hsl_list):
		...
		"""
		result = []
		for one_value in self.var_common["hsl12"]:
	...
	def data_46_excel_rgb_list(self):
		...
		:return:
		"""
		result = self.var_common["excel46_rgb"]
		return result
	...
	def data_56_excel_rgb_list(self):
		...
		:return:
		"""
		result = self.var_common["excel56_rgb"]
		return result
	...
	def data_basic_8_pastel_rgb_set(self):
		...
		:return:
		"""
		color_set = self.var_common["hsl12"][:-4]
		result = []
	...
	def data_basic_color_name_list(self):
		...
		:return:
		"""
		result = self.var_common["color_name_for_basic_12_kor"]
		return result
	...
	def data_basic_color_step(self):
		...
		:return:
		"""
		result = self.var_common["basic_color_step"]
		return result
	...
	def data_basic_color_tone_list(self):
		...
		:return:
		"""
		result = self.var_common["color_tone_kor"]
		return result
	...
	def data_basic_faber_rgb_set(self, start_color=11, code=5):
		...
		:return:
		"""
		h_list = self.var_common["hsl12"]
		sl_faber = self.var_common["sl_no_for_faber_style"]
	...
	def data_basic_johannes_rgb_set(self, start_color=11, num_color=4, stongness=5):
		...
		"""
		h_list = self.var_common["hsl12"]
		sl_list = self.var_common["sl11"]
		hsl_johannes = self.var_common["hsl_by_johannes"]
	...
	def data_color_name_all(self):
		...
		:return:
		"""
		result = list(set(self.var_common["check_color_name"].values()))
		return result
	...
	def data_color_name_eng_all(self):
		...
		:return:
		"""
		result = self.var_common["color_eng"]
		return result
	...
	def data_color_name_eng_for_12set(self):
		...
		:return:
		"""
		result = self.var_common["color_eng_12"]
		return result
	...
	def data_color_name_eng_for_13set(self):
		...
		:return:
		"""
		result = self.var_common["color_name_eng"]
		return result
	...
	def data_color_name_kor_all(self):
		...
		:return:
		"""
		result = self.var_common["color_kor_12"]
		return result
	...
	def data_excel46_rgb_list(self):
		"""
	...
	def data_excel56_rgb_list(self):
		"""
		엑셀 기본 rgb 색 : 56색
	...
	def data_pccs_style_name_eng_for_12set(self):
		...
		:return:
		"""
		result = self.var_common["color_tone_eng"]
		return result
	...
	def data_pccs_style_name_for_12set(self):
		...
		:return:
		"""
		result = list(self.var_common["color_tone_eng"].keys())
		return result
