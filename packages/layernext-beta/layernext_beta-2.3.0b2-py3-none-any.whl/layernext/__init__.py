        ...
        for embedding in embedding_list:
            if count == len(embedding_list)-1:
                ...
                if response["success"] == False:
                    self.create_or_update_job(session_uuid, job_name,JobType.UPLOAD_EMBEDDING.value, 0, JobStatus.FAILED.value, {"embeddingUploadTryObjectKeys":object_key_list,"errorList":"Error occur while uploading embeddings"})
                    object_key_list = []
                    return response
                ...
                self.create_or_update_job(session_uuid, job_name,JobType.UPLOAD_EMBEDDING.value, progress, status, {"embeddingUploadSuccessObjectKeys":object_key_list, "embeddingUploadSuccessCount": continues_count})
                object_key_list = []
