...
class AutomaticAnalysisInterface:
    ...
    def send_request(self,url, payload, headers, task):
        ...
        try:
            ...
            if task==Task.EMBEDDING.value:
                ...
                response = requests.post(url=f"{url}/dataIn_annotation", json=payload, headers=headers, timeout=30)
    ...
    def main_analysis(self, payload, task):
        ...
        request_thread = threading.Thread(target=self.send_request, args=(url, payload, headers, task))
