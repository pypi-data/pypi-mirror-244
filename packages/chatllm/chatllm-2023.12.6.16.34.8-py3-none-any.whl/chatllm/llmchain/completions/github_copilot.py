...
class Completions(object):
    ...
    def create(self, messages: List[Dict[str, Any]], **kwargs):
        ...
        :return:
        """
        kwargs.pop('model', None)  # 仅调用gpt-4

        data = {
            "model": "gpt-4",
            "messages": messages,
            "stream": self.client_params.get('stream'),
            **kwargs
        }
...
if __name__ == '__main__':
    ...
    data = {'model': 'gpt-4c', 'messages': [{'role': 'user', 'content': '你是谁'}], 'stream': True}
    _ = Completions()._stream_create()
