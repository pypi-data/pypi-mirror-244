...
class Completions(object):
    ...
    def _stream_create(self, messages, api_key, **kwargs):
        ...
        refs = self.client_params.pop('refs', refs)
        if refs:
            logger.debug(f"RAG：{refs}")
        use_search = False if refs[0] else self.client_params.pop('use_search', True)
...
if __name__ == '__main__':
    api_key = Completions.load_state("/Users/betterme/PycharmProjects/AI/MeUtils/examples/爬虫/state.json")
    completion = Completions(api_key=api_key, stream=False)
    r = completion.create(
