...
if __name__ == '__main__':
    ...
    # st.market_style_distinguish('20070101','20230831')
    # st.market_states_distinguish('20070101','20230831')
    # st.marco_eco_states_distinguish('2007-01-01','2023-08-31')
    # st.currency_states_distinguish('2007-12-01', '2023-11-28')
    # st.industry_states_distinguish('20070101','20231129')
    ...
    # # data=pd.read_pickle(r"C:\Users\xuhuai.zhe\Documents\WXWork\1688858146292774\Cache\File\2023-07\with_stock_df")
    # # data['zqdm']=[x.split('.')[0] for x in data['zqdm']]
    # # data['hk_flag']=[len(x) for x in data['zqdm']]
    # # data.loc[data['hk_flag']<6,'zqdm']=\
    # #     "0"+data[data['hk_flag']<6]['zqdm']
    # # data['buytime']=data['buytime'].astype(str).str.replace('-', '')
    # # data.drop('hk_flag',axis=1).to_sql('financial_left_data',con=localdb,index=False,if_exists='append')
    # #
    #
    # # data['flmc'] = [x.replace('(申万)', '') for x in data['证券简称']]
    # #
    # # theme_data=pd.read_sql("SELECT jjdm,theme,asofdate from public_theme_pool_history",con=localdb)
    # # stable_ret_data=pd.read_sql("select * from extra_table_funds ",con=localdb)
    # # stable_ret_data['ym'] = stable_ret_data['asofdate'].astype(str).str[4:6]
    # # stable_ret_data=stable_ret_data[stable_ret_data['ym']=='12'].drop('ym',axis=1)
    # # stable_ret_data['theme']='超额稳定'
    # #
    # # style_fund=pd.read_sql("SELECT jjdm,`风格偏好`,asofdate from jjpic_value_p_hbs ",con=localdb)
    # #
    # # theme_data=pd.concat([theme_data,stable_ret_data],axis=0)
    # #
    # # jjdm_list=list(set(theme_data['jjdm'].tolist()+style_fund['jjdm'].tolist()))
    #
    # # sql="select jjdm,tjrq as asofdate,sector_allo,equity_selection,trading from st_fund.r_st_hold_excess_attr_df where tjrq>='20160101' and jjdm in ({0})"\
    # #     .format(util.list_sql_condition(jjdm_list))
    # # hbdb.db2df(sql,db='funduser').to_pickle('基金brinsion能力')
    # #
    # #
    # # #get the fund size
    # # sql="select jjdm,jsrq,jjzzc from st_fund.t_st_gm_zcpz where jjdm in ({0}) and jsrq >='20160101'"\
    # #     .format(util.list_sql_condition(jjdm_list))
    # # hbdb.db2df(sql,db='funduser').to_pickle('基金规模')
    #
    # #get the theme infor ratio
    #
    # # sql="select jzrq,jjdm,hbdr from st_fund.t_st_gm_rhb where jjdm in ({0}) and jzrq>='20160101' "\
    # #     .format(util.list_sql_condition(jjdm_list))
    # # hbdb.db2df(sql,db='funduser').to_pickle('基金收益数据')
    #
    # # theme_data.to_pickle('基金分类1')
    # # style_fund.to_pickle('基金分类2')
    # #
    # #
    # # pd.read_sql("SELECT * from hbs_industry_property_new",con=localdb).to_pickle('行业特征')
    # # hbdb.db2df("select jjdm,jsrq,flmc,zzjbl from st_fund.t_st_gm_jjhyzhyszb where hyhfbz='2' and zclb='2' and jsrq>='20160101' and jjdm in ({}) "
    # #            .format(util.list_sql_condition(jjdm_list)),db='funduser').to_pickle('行业持仓')
    #
    # # pd.read_sql("SELECT * from jjpic_theme_p",con=localdb).to_pickle('主题特征')
    # # pd.read_sql("SELECT * from hbs_theme_exp", con=localdb).to_pickle('主题持仓')
    # #
    # # pd.read_sql("SELECT * from hbs_style_property",con=localdb).to_pickle('风格特征')
    # # pd.read_sql("SELECT * from hbs_size_property",con=localdb).to_pickle('市值特征')
    #
    # # pd.read_sql("select * from hbs_stock_trading_property where jjdm in ({})"
    # #             .format(util.list_sql_condition(jjdm_list)),con=localdb).to_pickle('基金交易特征')
    # #
    # # pd.read_sql("select * from hbs_holding_property where jjdm in ({})"
    # #             .format(util.list_sql_condition(jjdm_list)),con=localdb).to_pickle('基金持仓特征')
    #
    # # stock_trading_data=pd.read_sql("SELECT * from hbs_stock_trading_property",con=localdb)
    # # stock_date=pd.read_sql()
    ...
    # sql="select jsrq,jjdm,flmc,zzjbl from st_fund.t_st_gm_jjhyzhyszb where  jsrq>='20221230' and flmc='社会服务' and hyhfbz='2' and zclb='2' order by zzjbl "
    # ind_dis=hbdb.db2df(sql,db='funduser')


    # data=pd.read_excel(r"E:\GitFolder\docs\AI持仓统计.xlsx",sheet_name='Sheet2')

    # core_pool=['005241','001955','002910','000577','011251','519019','009049','161222','001583','010381','007346','000739',
    #            '519133','005968','006624','000979','001975','377240','450009','007449','450004','002340','001476','004868']

    # core_data=data[data['jjdm'].isin(core_pool)]
    #
    # core_data.to_excel('核心池AI持仓.xlsx')
    #
    # sql="select * from jjpic_industry_p where jjdm in ({}) and asofdate='20230301' "\
    #     .format(util.list_sql_condition(core_data['jjdm'].tolist()))
    # jjpic=pd.read_sql(sql,con=localdb)
    # jjpic.to_excel('大幅变动画像.xlsx')

    # ind_infor = \
    #     pd.read_excel(
    #         r"C:\Users\xuhuai.zhe\Documents\WeChat Files\wxid_xvk2piiub53y12\FileStorage\File\2023-06\ai概念股部分整理V1.xlsx",
    #         sheet_name='Sheet1')
    ...
    #
    # file_list = []
    # path = r"E:\GitFolder\docs\私募股多持仓分析\有新持仓\\"
    # hold=[]
    # for i, j, k in os.walk(path):
    #     for file in k :
    #         data=pd.read_excel(path[0:-1]+file,sheet_name='原始数据')
    #         data['jjdm']=file[0:5]
    #         hold.append(data)
    # hold=pd.concat(hold,axis=0)
    # hold=hold[hold['股票代码'].notnull()]
    # hold=hold[hold['股票简称'].isin(ind_infor['简称'].tolist())]

    # hold=pd.merge(hold,ind_infor,how='left',
    #               left_on='股票简称',right_on='简称')
    # hold.to_excel('私募AI持仓2.xlsx')
    #
    #
    # sql="select jjdm,zqdm,zjbl from artificial_quartly_full_hld where jjdm in ({0}) and jsrq>='20230301' and jsrq<='20230401' and zqdm in ({1}) "\
    #     .format(util.list_sql_condition(jjdm_list),
    #             util.list_sql_condition(ind_infor['代码'].tolist()))
    # ai_stock_w=pd.read_sql(sql,con=localdb).fillna(0)
    #
    #
    # ai_stock_w = pd.merge(ai_stock_w,ind_infor,how='left',left_on='zqdm',right_on='代码')
    # ai_stock_w.to_excel('AI_20230331.xlsx')
    #
    #
    #
    # sql="select jjdm,zqdm,zzjbl from st_fund.t_st_gm_gpzhggtj where jjdm in ({0}) and jsrq>='20221201' and jsrq<='20230101' and zqdm in ({1}) "\
    #     .format(util.list_sql_condition(ai_stock_w['jjdm'].unique().tolist()),
    #             util.list_sql_condition(ind_infor['代码'].tolist()))
    # ai_stock_w=hbdb.db2df(sql,db='funduser').fillna(0)
    # ai_stock_w = pd.merge(ai_stock_w,ind_infor,how='left',left_on='zqdm',right_on='代码')
    # ai_stock_w.to_excel('AI_20221230.xlsx')
    #
    # print('')
    ...
    #
    # jj_pool=pd.read_excel(r"E:\GitFolder\hbshare\fe\XZ\均衡基金汇总数据2.xlsx")
    # jjdm_list = jj_pool['jjdm'].tolist()
    #jjdm_list=['519198','240004','004475']


    # sql="select jjdm,tjrq,asset_allo,sector_allo,equity_selection,trading,portfolio_return,benchmark_return from st_fund.r_st_hold_excess_attr_df where jjdm in ({0}) and tjrq >='20150101'"\
    #     .format(util.list_sql_condition(jjdm_list))
    # brinsion_raw_data=hbdb.db2df(sql,db='funduser')


    # sql = "select jjdm,tjrq,industry,active_return,sector_allo,equity_selection from st_fund.r_st_hold_sector_attr_df where jjdm in ({}) and tjrq>='20201201' "\
    #     .format(util.list_sql_condition(jjdm_list))
    # brinson_data=hbdb.db2df(sql,db='funduser')
    # brinson_his=brinson_data.groupby(['jjdm','industry'])[['sector_allo']].describe()
    # brinson_his.to_excel('temp.xlsx')
    # print('')
