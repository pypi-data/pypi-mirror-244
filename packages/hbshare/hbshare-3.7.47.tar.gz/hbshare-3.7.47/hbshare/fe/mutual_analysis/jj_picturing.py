...
def get_mutual_fund_latest_pb_pe(input_fund_infor,latest_date,dir):
    ...
    sql="select max(jsrq) as latest_jsrq from st_fund.t_st_gm_gpzh where jsrq<='{}'".format(latest_date)
...
def mutual_pool_picture(pool_name,asofdate1,asofdate2,latest_date,fool_version=False):
    ...
    for jjdm in self_rank.index.tolist():
        ...
        sql="select zqdm,spjg,jyrq from st_ashare.t_st_ag_gpjy where zqdm in ({0}) and jyrq in ({1})"\
            .format(util.list_sql_condition(latest_holding['zqdm'].unique().tolist()),util.list_sql_condition([str(latest_date),str(max_date)]))
        stock_ret=hbdb.db2df(sql,db='alluser')
        stock_ret['spjg']=stock_ret.groupby('zqdm')['spjg'].pct_change()

        mimic_ret=pd.merge(latest_holding,stock_ret[stock_ret['spjg'].notnull()],how='left',on='zqdm')
        mimic_ret=(mimic_ret['zjbl'] * mimic_ret['spjg']).sum()
        real_ret=hbdb.db2df("select fqdwjz,jzrq from st_fund.t_st_gm_rhb where jjdm='{0}' and jzrq in ({1})"
                            .format(jjdm,util.list_sql_condition([str(latest_date),str(max_date)])),db='funduser')
        real_ret=real_ret['fqdwjz'].pct_change().iloc[-1]
        self_rank.loc[jjdm, '累计净值偏离']=real_ret-mimic_ret
    ...
    bmk_list=util.get_885001_funds(asofdate2)
    sql="select flmc as `行业名称`,sum(zzjbl)/100 as `万得偏股基金指数` from st_fund.t_st_gm_jjhyzhyszb where jjdm in ({0}) and jsrq>='{1}' and jsrq<='{2}' and hyhfbz='2' and zclb='2' group by flmc "\
        .format(util.list_sql_condition(bmk_list),asofdate1,asofdate2)
    bmk_ind_dis=hbdb.db2df(sql,db='funduser')
    bmk_ind_dis['万得偏股基金指数']=bmk_ind_dis['万得偏股基金指数']/len(bmk_list)
    ...
    core_pool_ind_dis['change'] = core_pool_ind_dis['池'] / core_pool_ind_dis['万得偏股基金指数']
    core_pool_ind_dis['diff'] = abs(core_pool_ind_dis['池']-core_pool_ind_dis['万得偏股基金指数'])
    warnings="行业上,超配："
    ...
    core_pool_theme_dis['change'] = core_pool_theme_dis['池'] / core_pool_theme_dis['万得偏股基金指数']
    core_pool_theme_dis['diff'] = abs(core_pool_theme_dis['池']-core_pool_theme_dis['万得偏股基金指数'])
    warnings+="主题上,超配："
    ...
    #style level
    sql="SELECT style_type as `风格类型`, avg(zjbl)/100 as `万得偏股基金指数` from hbs_style_exp where jjdm in ({0}) and jsrq>='{1}' and jsrq<='{2}' GROUP BY style_type "\
        .format(util.list_sql_condition(bmk_list),asofdate1,asofdate2)
    ...
    core_pool_style_dis['change'] = core_pool_style_dis['池'] / core_pool_style_dis['万得偏股基金指数']
    core_pool_style_dis['diff'] = abs(core_pool_style_dis['池']-core_pool_style_dis['万得偏股基金指数'])
    warnings+="  风格上,超配："
    for ind in core_pool_style_dis[(core_pool_style_dis['change']>=1.3)&(core_pool_style_dis['diff']>=0.1)]['风格类型'].tolist():
        warnings+=ind
    warnings += "; 低配： "
    for ind in core_pool_style_dis[(core_pool_style_dis['change']<=0.7)&(core_pool_style_dis['diff']>=0.1)]['风格类型'].tolist():
        warnings += ind
    core_pool_style_dis.drop(['change','diff'],axis=1,inplace=True)


    def add_alert(core_pool):

        core_pool['预警项']=''

        core_pool['temp']=[float(x.replace("'", "").split('/')[0]) / float(x.replace("'", "").split('/')[1]) for x in
         core_pool['回撤同策略排名']]
        core_pool.loc[core_pool['temp']>2/3,'预警项']=core_pool[core_pool['temp']>2/3]['预警项']+"业绩波动较大;"
        core_pool.drop('temp',axis=1,inplace=True)
        core_pool.loc[core_pool['近6月胜率']<1/3,'预警项']=core_pool[core_pool['近6月胜率']<1/3]['预警项']+"业绩表现不佳;"
        core_pool.loc[(core_pool['累计净值偏离'].abs() > 0.1)&(core_pool['策略类型']!='超额稳定型'), '预警项'] = \
            core_pool[(core_pool['累计净值偏离'].abs() > 0.1)&(core_pool['策略类型']!='超额稳定型')]['预警项'] + "风格迁移风险;"
    ...
    def merge_pic(core_pool,fund_return_rank):
        ...
        fund_return_rank=pd.merge(fund_return_rank,self_rank[['纵向分位数','横向分位数','累计净值偏离']],how='left',on='jjdm')
    ...
    pool_funds_summary=pool_funds_summary[['基金简称','入池类型', '月超额收益', '周超额收益']]
    ...
    ws["B1"].options(pd.DataFrame, header=1, index=False, expand='table').value = core_pool_ind_dis
    ws["F1"].options(pd.DataFrame, header=1, index=False, expand='table').value = core_pool_theme_dis
    ws["K1"].options(pd.DataFrame, header=1, index=False, expand='table').value = core_pool_style_dis
    ws["O1"].options(pd.DataFrame, header=1, index=False, expand='table').value = weekly_cont

    data, layout = plot.plotly_jjpic_bar (core_pool_ind_dis.set_index('行业名称'),'穿透后行业对齐情况',legend_x=1)
    plot.save_pic2local(data, layout, newpath + r"\穿透后行业对齐")
    ws.pictures.add(newpath + r"\穿透后行业对齐.png", left=ws.range('A34').left,
                    top=ws.range('A34').top,
                    width=600, height=300)
    data, layout = plot.plotly_jjpic_bar (core_pool_style_dis.set_index('风格类型'),'穿透后风格对齐情况',legend_x=1)
    plot.save_pic2local(data, layout, newpath + r"\穿透后行业对齐")
    ws.pictures.add(newpath + r"\穿透后行业对齐.png", left=ws.range('L32').left,
                    top=ws.range('L32').top,
    ...
    ws["A1"].options(pd.DataFrame, header=1, index=True, expand='table').value = summary
    ws["I1"].options(pd.DataFrame, header=1, index=True, expand='table').value = barra_exp
    ws["W1"].options(pd.DataFrame, header=1, index=True, expand='table').value = barra_ret

    ws["A{}".format(3+len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = summary_sector
    ...
    ws["A{}".format(6+2*len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = summary_style
    ws["I{}".format(6+2*len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = barra_exp_style
    ws["W{}".format(6+2*len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = barra_ret_style
    ...
    ws["A13"].options(pd.DataFrame, header=1, index=True, expand='table').value = summary_hbs_sector
    ws["A48"].options(pd.DataFrame, header=1, index=False, expand='table').value = pool_summary

    pool_funds_summary.index=["'"+str(x) for x in pool_funds_summary.index]
    ws["A{}".format(str(50+len(pool_summary)))].options(pd.DataFrame, header=1, index=False, expand='table').value = pool_funds_summary
    ...
    ws["I{}".format(5+len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = gj_t2_m
...
def prv_pool_picture(latest_date,style_date):
    ...
    gj_bl, gj_gx, pool_bmk_bl_gx=\
        pra.TM_regression(end_date=str(latest_date),jjdm_list=jjdm_list)
    t1 = time.time()
    print("T-M归因用时: ", t1 - t0)
    ...
    ws["A{}".format(9+3*len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = pool_bmk_bl_gx
    ws["I{}".format(9+3*len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = gj_bl
    ws["W{}".format(9+3*len(jjdm_list))].options(pd.DataFrame, header=1, index=True, expand='table').value = gj_gx
...
if __name__ == '__main__':
    ...
    prv_pool_picture(latest_date='20231124',style_date='20230930')
    # mutual_pool_picture('临时','20230630', '20230831', '20230928',fool_version=True)
    #mutual_pool_picture('公募核心池','20230630', '20230831', '20231130',fool_version=True)
    #mutual_pool_picture('公募30池偏股基金', '20230630', '20230831', '20231130',fool_version=True)
    # mutual_pool_picture('公募机构池', '20230630', '20230831', '20231130',fool_version=True)
