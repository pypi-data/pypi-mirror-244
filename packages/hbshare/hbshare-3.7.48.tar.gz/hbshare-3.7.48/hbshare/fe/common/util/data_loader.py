...
=======
            ...
            sql_script = \
                "select spjg as ADJ_NAV,jyrq as TRADEDATE,zsdm as FUND_ID from st_hedge.t_st_sm_zhmzs where zsdm='{0}' and jyrq>='{1}' and jyrq<='{2}'" \
            ...
            sql_script="select spjg as ADJ_NAV,jyrq as TRADEDATE,zqdm as FUND_ID from st_market.t_st_zs_hq where zqdm='{0}' and jyrq>='{1}' and jyrq<='{2}'"\
                .format(self.fund_id, self.start_date, self.end_date)
            post_body = self.post_body
...
>>>>>>> 40eb641cee29c8f9da4c4a648343e10b4f2519d2
        else:
...
class EquityBrinsonAttributionLoader:
    ...
    def _load_security_period_return(portfolio_weight_series_dict, benchmark_weight_series_dict, trading_day_list):
        ...
        for trading_day in trading_day_list:
            ...
            data_main = fetch_data_batch_hbs(sql_script)
