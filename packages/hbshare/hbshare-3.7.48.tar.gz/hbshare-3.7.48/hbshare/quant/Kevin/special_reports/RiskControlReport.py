...
def portfolio_test(start_date, end_date, benchmark_id):
    ...
    # 超额
    sql_script = ("SELECT JYRQ as TRADEDATE, ZQMC as INDEXNAME, SPJG as TCLOSE FROM st_market.t_st_zs_hq WHERE "
                  "ZQDM = '{}' and JYRQ >= {} and JYRQ <= {}").format(benchmark_id, start_date, end_date)
    index_data = pd.DataFrame(
        hbs.db_data_query('alluser', sql_script, page_size=5000)['data']).set_index('TRADEDATE')['TCLOSE']
...
if __name__ == '__main__':
    ...
    # PortfolioRiskPredict('20230630').run()
    "=======================================因子更新======================================="
    weekly_update("20231201", "update")
    # portfolio_test("20221230", "20231110", "000852")