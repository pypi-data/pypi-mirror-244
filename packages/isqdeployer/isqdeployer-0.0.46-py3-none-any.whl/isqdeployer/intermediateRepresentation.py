...
from isqtools.utils import FindSimulator
...
class IR_based_on_QCIS(abcIntermediateRepresentation):
    ...
    def getOutput_QCIS(self, F=[], I=[], **kw):
        simulatorBIN = self._SimulatorBIN
        self.FAKE_SO = self._get_qcis_SO_filePath()
        passF = "".join([f" -d {f}" for f in F])
        passI = "".join([f" -i {i}" for i in I])
        cmd = (
            f"{simulatorBIN} {self.FAKE_SO} --qcisgen -e __isq__entry {passI} {passF} "
        )
        # print(cmd,9998)
        cwd = os.getcwd()
        os.chdir(self._workDir)
        QCIS = os.popen(cmd).read()
        os.chdir(cwd)
        return QCIS
    ...
    def renew_IR_resource(self, nF,nI) -> str:
        ...
        passF = "".join([f" -d {f}" for f in [1.0] * nF])
        passI = "".join([f" -i {i}" for i in [1] * nI]) # may be conflict
        cmd = f"isqc compile {self._isqPath} --target qcis {passI} {passF} -o {fakeSO}"
        cwd = os.getcwd()
        os.chdir(self._workDir)
        os.system(cmd)
        os.chdir(cwd)
