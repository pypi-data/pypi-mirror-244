...
def main():
	...
	if len(sys.argv) == 1 or sys.argv[1] == 'run.py': # no parameters
		...
		my_zetl.loggerman(' ')
		my_zetl.loggerman('py -m zetl.postgres_import [csv_filename] [tablename] [WithTruncate]') 
		my_zetl.loggerman('py -m zetl.postgres_export [tablename] [csvfilename] [delimiter] ') 
		my_zetl.loggerman(' ')
		my_zetl.loggerman('py -m zetl.mysql_import [csv_filename] [tablename] [WithTruncate]') 
		my_zetl.loggerman('py -m zetl.mysql_export [tablename] [csvfilename] [delimiter] ') 
		my_zetl.loggerman('-----------')
...
class zetl:
	def __init__(self,parent_dir = '',script_dir=''):
		launch_dir =  '.\\'
		if parent_dir != '':
		...
		self.force = True
		self.zetldb = zetldbaccess(launch_dir,self.script_directory)
...
class zetldbaccess:
	...
	def __init__(self,parent_dir = '',script_dir=''):
		...
		if parent_dir != '':
			os.chdir(parent_dir)
			dbfile = parent_dir + '\\local_sqlite_db'
		else:
			dbfile = 'local_sqlite_db'
