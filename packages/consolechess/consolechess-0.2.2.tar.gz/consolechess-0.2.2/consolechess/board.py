...
try:
    from consolechess import utils
except ImportError:
...
@dataclass
class GameStatus:
    """Status of the chess game (win/loss/neither)."""
...
class BadSquareError(Exception):
    """Raised if invalid square sent to ChessBoard."""


class NoPieceAtSquareError(Exception):
    """Raised if piece at square is None."""
...
class ChessBoard:
    ...
    def __init__(
        ...
        *,
        ...
        self.grid: dict[str, Piece | None] = {sq: None for sq in SQUARES}
        if not empty and fen is None:
            self.set_staunton_pattern()
        self.white_king_has_moved: bool = False
        self.white_king_init_sq: str | None = None
        self.black_king_has_moved: bool = False
        self.black_king_init_sq: str | None = None
        self.black_queenside_rook_has_moved: bool = False
        self.black_queenside_rook_init_sq: str | None = None
        self.black_kingside_rook_has_moved: bool = False
        self.black_kingside_rook_init_sq: str | None = None
        self.white_queenside_rook_has_moved: bool = False
        self.white_queenside_rook_init_sq: str | None = None
        self.white_kingside_rook_has_moved: bool = False
        self.white_kingside_rook_init_sq: str | None = None
        self.double_forward_last_move: str | None = None
        self.turn: Color = "white"
        self.game_over: bool = False
        self.winner: Color | None = None
        self.status_description: StatusDescription | None = None
        self.halfmove_clock = 0
        self.moves: list[str] = []
        self.moves_before_fen_import = 0
        self.hashes: list[int] = []
        self.initial_fen: str | None = None
        if fen is not None:
            self.import_fen(fen)
        if pgn is not None:
            self.import_pgn(pgn, import_result=import_result)
            return None
    ...
    def __hash__(self: "ChessBoard") -> int:
        """Hash board grid string representation."""
        return hash(
            (
                self.black_king_has_moved,
                self.black_king_init_sq,
                self.white_king_has_moved,
                self.white_king_init_sq,
                self.black_kingside_rook_has_moved,
                self.black_kingside_rook_init_sq,
                self.black_queenside_rook_has_moved,
                self.black_queenside_rook_init_sq,
                self.white_kingside_rook_has_moved,
                self.white_kingside_rook_init_sq,
                self.white_queenside_rook_has_moved,
                self.white_queenside_rook_init_sq,
                self.moves_before_fen_import,
                self.turn,
                self.halfmove_clock,
                *self.moves,
                *self.grid.items(),
    ...
    def __eq__(self: "ChessBoard", other: object) -> bool:
        """Check if a board is equal to another board."""
        return hash(self) == hash(other)

    def hash_position(self: "ChessBoard") -> int:
        """Hash the board position for the purposes of checking draw by repetition."""
        return hash((self.turn, *self.grid.items()))

    def get_opening(self: "ChessBoard") -> Opening | None:
        """Get the ECO opening."""
        path = (
    ...
    def alternate_turn(self: "ChessBoard", *, reset_capture: bool = False) -> None:
        """
        ...
        self.hashes.append(self.hash_position())
        if not reset_capture:
    ...
    def set_random(self: "ChessBoard") -> None:
        ...
        self.set_initial_positions()
        self.initial_fen = self.export_fen()
    ...
    def set_initial_positions(self: "ChessBoard") -> None:
        ...
        if len(white_rooks) == 2:
            ...
            self.white_kingside_rook_init_sq = (
                white_rooks[0]
                if self.white_queenside_rook_init_sq != white_rooks[0]
                else white_rooks[1]
            )
        elif len(white_rooks) == 1:
            self.white_queenside_rook_init_sq = white_rooks[0]
            self.white_kingside_rook_init_sq = white_rooks[0]
        if len(black_rooks) == 2:
            ...
            self.black_kingside_rook_init_sq = (
                black_rooks[0]
                if self.black_queenside_rook_init_sq != black_rooks[0]
                else black_rooks[1]
            )
        elif len(black_rooks) == 1:
            self.black_queenside_rook_init_sq = black_rooks[0]
            self.black_kingside_rook_init_sq = black_rooks[0]
        self.white_king_init_sq = next(
            sq for sq in SQUARES if self[sq] == Piece("king", "white")
        )
        self.black_king_init_sq = next(
            sq for sq in SQUARES if self[sq] == Piece("king", "black")
        )

    def import_pgn(self: "ChessBoard", pgn: str, *, import_result: bool = True) -> None:
    ...
    def import_fen(self: "ChessBoard", fen: str) -> None:
        ...
        # Populate board.
        ranks = list(zip(range(8, 0, -1), groups[:8], strict=True))
        ...
        if groups[12] is not None:
            self.moves_before_fen_import = int(groups[12]) - 1
            if groups[8] == "b":
                self.moves.append("_")
        ...
        if groups[9] == "-":
            ...
            if self.white_queenside_rook_init_sq is not None:
                queenside_rook_file = self.white_queenside_rook_init_sq[0]
            else:
                queenside_rook_file = "q"
            if self.white_kingside_rook_init_sq is not None:
                kingside_rook_file = self.white_kingside_rook_init_sq[0]
            else:
                kingside_rook_file = "k"
            if "K" not in groups[9] and kingside_rook_file.upper() not in groups[9]:
                self.white_kingside_rook_has_moved = True
            if "Q" not in groups[9] and queenside_rook_file.upper() not in groups[9]:
                self.white_queenside_rook_has_moved = True
            if "k" not in groups[9] and kingside_rook_file not in groups[9]:
                self.black_kingside_rook_has_moved = True
            if "q" not in groups[9] and queenside_rook_file not in groups[9]:
                self.black_queenside_rook_has_moved = True

        self.initial_fen = fen

    def export_fen(self: "ChessBoard", *, no_clocks: bool = False) -> str:
        ...
        if (
            self.initial_fen is None
            or "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR" in self.initial_fen
            or self.white_king_init_sq is None
            or self.black_king_init_sq is None
            or self.white_kingside_rook_init_sq is None
            or self.white_queenside_rook_init_sq is None
            or self.black_kingside_rook_init_sq is None
            or self.black_queenside_rook_init_sq is None
        ):
            ...
            white_kingside_castle_symbol = self.white_kingside_rook_init_sq[0].upper()
            white_queenside_castle_symbol = self.white_queenside_rook_init_sq[0].upper()
            black_kingside_castle_symbol = self.black_kingside_rook_init_sq[0]
            black_queenside_castle_symbol = self.black_queenside_rook_init_sq[0]
        any_castles_possible = False
        if not self.white_king_has_moved and not self.white_kingside_rook_has_moved:
            fen += white_kingside_castle_symbol
            any_castles_possible = True
        if not self.white_king_has_moved and not self.white_queenside_rook_has_moved:
            fen += white_queenside_castle_symbol
            any_castles_possible = True
        if not self.black_king_has_moved and not self.black_kingside_rook_has_moved:
            fen += black_kingside_castle_symbol
            any_castles_possible = True
        if not self.black_king_has_moved and not self.black_queenside_rook_has_moved:
        ...
        if not no_clocks:
            ...
            if self.double_forward_last_move is not None:
                fen += self.double_forward_last_move[0]
                if self.double_forward_last_move[1] == "4":
                    fen += "3"
                if self.double_forward_last_move[1] == "5":
                    fen += "6"
            else:
                fen += "-"
            ...
            fen += (
                ...
                f"{self.moves_before_fen_import + (len(self.moves) // 2) + 1}"
            )
    ...
    def export_pgn(
        ...
        **kwargs: str,
        ...
        if not self.game_over:
            ...
            output += '[Result "0-1"]\n'
        if include_eco and (opening := self.get_opening()) is not None:
            output += f'[ECO "{opening.eco}"]\n'
        if variant is not None:
            output += f'[Variant "{variant}"]\n'
        elif hasattr(self, "variant"):
            output += f'[Variant "{self.variant}]\n'
        if self.initial_fen is not None:
            if variant is None:
                output += '[Variant "Chess960"]\n'
            output += '[SetUp "1"]\n'
            output += f'[FEN "{self.initial_fen}"]\n'
        if include_current_position:
            output += f'[CurrentPosition "{self.export_fen()}"]\n'
        for key in kwargs:
            key_label = key.title() if key.islower() else key
            output += f'[{key_label} "{kwargs[key]}"]\n'

        output += "\n"
        output += self.export_moves()
        return output

    def empty_board(self: "ChessBoard") -> None:
        """Clear all pieces off the board."""
        for key in self:
            self[key] = None
        self.white_king_has_moved = False
        self.white_king_init_sq = None
        self.black_king_has_moved = False
        self.black_king_init_sq = None
        self.black_queenside_rook_has_moved = False
        self.black_queenside_rook_init_sq = None
        self.black_kingside_rook_has_moved = False
        self.black_kingside_rook_init_sq = None
        self.white_queenside_rook_has_moved = False
        self.white_queenside_rook_init_sq = None
        self.white_kingside_rook_has_moved = False
        self.white_kingside_rook_init_sq = None
        self.double_forward_last_move = None
        self.turn = "white"
    ...
    def _get_squares(
        ...
        *,
        ...
        """
        Get all navigable squares for a given piece.
        ...
        If check_castle is True, return post-castling positions for kings.
        """
        if initial_square not in SQUARES:
            msg = (
                f"'{initial_square}' is not a valid square. Use [a-h] to "
                "represent file and [1-8] to represent rank."
            )
            raise BadSquareError(msg)
        if (piece := self[initial_square]) is None:
            msg = f"No piece at initial_square '{initial_square}'."
            raise NoPieceAtSquareError(msg)
    ...
    def legal_moves(self: "ChessBoard", square: str) -> list[str]:
        """Get legal moves for a piece."""
        if square not in SQUARES:
            msg = (
                "Not a valid square. Use [a-h] to represent "
                "file and [1-8] to represent rank."
            )
            raise BadSquareError(msg)
        if (piece := self[square]) is None:
            msg = f"No piece at square '{square}'."
            raise NoPieceAtSquareError(msg)
        navigable_squares = self._get_squares(square)
        legal_moves = []
        for sq in navigable_squares:
            # If the piece is a pawn diagonal to the navigable square,
            # and the square at navigable square is None, it must be an en passant.
            if (
            ...
            # If the piece is a king and the navigable square's file is +/- 2,
            # it must be a castle.
            elif (
                piece.piece_type == "king"
                and abs(FILES.index(sq[0]) - FILES.index(square[0])) == 2
                and self.can_castle(piece.color)
            ):
                legal_moves.append(sq)
            # Otherwise, it goes through move_piece.
            else:
                if self.can_move_piece(square, sq):
                    legal_moves.append(sq)
        return legal_moves
    ...
    def can_move_piece(
        ...
        if piece is None:
            msg = f"No piece at initial_square '{initial_square}'."
            raise NoPieceAtSquareError(msg)
        if final_square not in self._get_squares(initial_square):
            return False
    ...
    def _get_pawn_squares(
        ...
        *,
        ...
        """Get pawn's navigable squares (ignores king capture rules)."""
        possible_squares: list[str] = []
        if not capture_only:
            # double forward
            if (
                piece.color == "white"
                and initial_square[1] == "2"
                and self[utils.step_up(initial_square, 1)] is None
                and self[utils.step_up(initial_square, 2)] is None
            ):
                sq = utils.step_up(initial_square, 2)
                double_forward_piece = self[sq]
                if double_forward_piece is None:
                    possible_squares.append(sq)
                else:
                    if double_forward_piece.color != piece.color:
                        possible_squares.append(sq)
            elif (
                piece.color == "black"
                and initial_square[1] == "7"
                and self[utils.step_down(initial_square, 1)] is None
                and self[utils.step_down(initial_square, 2)] is None
            ):
                sq = utils.step_down(initial_square, 2)
                double_forward_piece = self[sq]
                if double_forward_piece is None:
                    possible_squares.append(sq)
                else:
            ...
            if piece.color == "white":
                try:
                    forward_square = utils.step_up(initial_square, 1)
                    forward_piece = self[forward_square]
                except utils.OffGridError:
                    forward_piece = None
            if piece.color == "black":
                try:
                    forward_square = utils.step_down(initial_square, 1)
                    forward_piece = self[forward_square]
                except utils.OffGridError:
                    forward_piece = None
            if forward_piece is None and forward_square != "":
                possible_squares.append(forward_square)
        # diagonal capture
        if piece.color == "white":
            with suppress(utils.OffGridError):
                left_square = utils.step_left(utils.step_up(initial_square, 1), 1)
                left_piece = self[left_square]
                if left_piece is not None and left_piece.color != piece.color:
                    possible_squares.append(left_square)
            with suppress(utils.OffGridError):
                right_square = utils.step_right(utils.step_up(initial_square, 1), 1)
                right_piece = self[right_square]
                if right_piece is not None and right_piece.color != piece.color:
                    possible_squares.append(right_square)
        if piece.color == "black":
            with suppress(utils.OffGridError):
                left_square = utils.step_left(utils.step_down(initial_square, 1), 1)
                left_piece = self[left_square]
                if left_piece is not None and left_piece.color != piece.color:
                    possible_squares.append(left_square)
            with suppress(utils.OffGridError):
                right_square = utils.step_right(utils.step_down(initial_square, 1), 1)
                right_piece = self[right_square]
                if right_piece is not None and right_piece.color != piece.color:
                    possible_squares.append(right_square)
        # en passant
        if (
            self.double_forward_last_move is not None
            and (pc := self[self.double_forward_last_move]) is not None
            and pc.color != self.turn
        ):
            adjacent_squares = []
            for func in (utils.step_left, utils.step_right):
                with suppress(utils.OffGridError):
                    adjacent_squares.append(func(initial_square, 1))
            if self.double_forward_last_move in adjacent_squares:
                if piece.color == "white":
                    if FILES.index(self.double_forward_last_move[0]) < FILES.index(
                        initial_square[0]
                    ):
                        possible_squares.append(
                            utils.step_diagonal_up_left(initial_square, 1)
                        )
                    if FILES.index(self.double_forward_last_move[0]) > FILES.index(
                        initial_square[0]
                    ):
                        possible_squares.append(
                            utils.step_diagonal_up_right(initial_square, 1)
                        )
                if piece.color == "black":
                    if FILES.index(self.double_forward_last_move[0]) < FILES.index(
                        initial_square[0]
                        ...
                        possible_squares.append(
                            utils.step_diagonal_down_left(initial_square, 1)
                        )
                    if FILES.index(self.double_forward_last_move[0]) > FILES.index(
                        initial_square[0]
                    ):
                        possible_squares.append(
                            utils.step_diagonal_down_right(initial_square, 1)
                        )
    ...
    def _get_rook_squares(
        ...
        """Get rook's navigable squares (ignores king capture rules)."""
        possible_squares = []
        iterators = [
            utils.iter_to_top(initial_square),
            utils.iter_to_bottom(initial_square),
            utils.iter_to_right(initial_square),
            utils.iter_to_left(initial_square),
        ]
    ...
    def _get_queen_squares(
        ...
        """Get queen's navigable squares (ignores king capture rules)."""
        possible_squares = []
        iterators = [
            utils.iter_to_top(initial_square),
            utils.iter_to_bottom(initial_square),
            utils.iter_to_right(initial_square),
            utils.iter_to_left(initial_square),
            utils.iter_bottom_left_diagonal(initial_square),
            utils.iter_bottom_right_diagonal(initial_square),
            utils.iter_top_left_diagonal(initial_square),
            utils.iter_top_right_diagonal(initial_square),
        ]
        for iterator in iterators:
            for sq in iterator:
                other_piece = self[sq]
    ...
    def _get_bishop_squares(self: "ChessBoard", initial_square: str) -> list[str]:
        """Get bishop navigable squares (ignores king capture rules)."""
        possible_squares: list[str] = []
        iterators = [
            utils.iter_bottom_left_diagonal(initial_square),
            utils.iter_bottom_right_diagonal(initial_square),
    ...
    def _get_knight_squares(self: "ChessBoard", initial_square: str) -> list[str]:
        """Get knight navigable squares (ignores king capture rules)."""
        possible_squares = []
        candidates: list[str] = []
        for move in KNIGHT_MOVES:
            with suppress(utils.OffGridError):
                ...
                for direction, steps in move:
                    match direction:
                        case "up":
                            cursor = utils.step_up(cursor, steps)
                        case "down":
                            cursor = utils.step_down(cursor, steps)
                        case "right":
                            cursor = utils.step_right(cursor, steps)
                        case "left":
                            cursor = utils.step_left(cursor, steps)
    ...
    def _get_king_squares(
        ...
        *,
        ...
        """Get king navigable squares (ignores capture rules)."""
        possible_squares: list[str] = []
        step_functions = [
            utils.step_up,
            utils.step_down,
        ...
        for func in step_functions:
            with suppress(utils.OffGridError):
                possible_squares.append(func(initial_square, 1))
        if check_castle and self.can_castle(piece.color):
            possible_squares.append(f"c{initial_square[1]}")
        return possible_squares
    ...
    def move_piece(
        ...
        *,
        ...
        match piece.piece_type:
            ...
            case "queen":
                notation += "Q"
        pieces_of_same_type_and_color = [
            sq
            for sq in self
            if self[sq] == Piece(piece.piece_type, self.turn) and sq != initial_square
        ]
        for sq in pieces_of_same_type_and_color:
            if final_square in self._get_squares(sq):
                if sq[0] != initial_square[0]:
                    notation += initial_square[0]
                    break
                elif sq[1] != initial_square[1]:
                    notation += initial_square[1]
                    break
        if not skip_checks:
        ...
        # Update clocks and notation to denote capture, and raise
        # exceptions for illegal captures.
        ...
        if piece.piece_type == "king":
            ...
            and initial_square == self.black_queenside_rook_init_sq
        ):
            self.black_queenside_rook_has_moved = True
        elif (
            piece.piece_type == "rook"
            and piece.color == "black"
            and initial_square == self.black_kingside_rook_init_sq
        ):
            self.black_kingside_rook_has_moved = True
        elif (
            piece.piece_type == "rook"
            and piece.color == "white"
            and initial_square == self.white_queenside_rook_init_sq
        ):
            self.white_queenside_rook_has_moved = True
        elif (
            piece.piece_type == "rook"
            and piece.color == "white"
            and initial_square == self.white_kingside_rook_init_sq
        ):
            self.white_kingside_rook_has_moved = True
        if (
            piece.piece_type == "pawn"
            ...
            self.double_forward_last_move = final_square
        else:
            self.double_forward_last_move = None
        # Clear clock if piece is a pawn.
        ...
        self.alternate_turn(reset_capture=(piece.piece_type == "pawn"))
    ...
    def export_moves(self: "ChessBoard") -> str:
        """Export moves to string."""
        i = self.moves_before_fen_import
        output = ""
        moves = self.moves
        while True:
            i += 1
    ...
    def move(self: "ChessBoard", notation: str) -> None:
        ...
        if "O-O-O" in notation:
            ...
            piece_type = piece_abbr[match.group(1)]
            disambiguator = match.group(2)
            final_square = match.group(3)
            pawn_promotion = (
                piece_abbr[match.group(4)] if match.group(4) != "" else None
            )
            candidates = [
                sq
                for sq in self
                if self[sq] == Piece(piece_type, self.turn)
                and disambiguator in sq
                and final_square in self._get_squares(sq)
            ]
            if len(candidates) == 1:
                initial_square = candidates[0]
            elif len(candidates) == 0:
                msg = f"Could not find any suitable pieces for move '{notation}'."
                raise InvalidNotationError(msg)
            else:
                successful_candidates = []
                # Test if king would be put in check by move to disambiguate.
                for candidate in candidates:
                    changes: dict[str, Piece | None] = {
                        candidate: None,
                        final_square: self[candidate],
                    }
                    with self.test_position(changes):
                        if not self.king_is_in_check(self.turn):
                ...
                if len(successful_candidates) == 1:
                    ...
                    # Test if capture specification can disambiguate.
                    candidates = successful_candidates
                    successful_candidates = []
                    for candidate in candidates:
                        non_capture_squares = [
                            sq
                            for sq in self._get_squares(candidate)
                            if sq not in self._get_squares(candidate, capture_only=True)
                        ]
                        if "x" not in notation or len(non_capture_squares) > 0:
                            successful_candidates.append(candidate)
                    initial_square = successful_candidates[0]
                else:
                    msg = f"Must disambiguate moving pieces: {candidates}"
                    raise InvalidNotationError(msg)
            if (
                "x" in notation and self[final_square] is None and "#" not in notation
            ):  # en passant
    ...
    def can_castle(self: "ChessBoard", color: Color, side: Side | None = None) -> bool:
        ...
        king_sq = (
            self.black_king_init_sq if color == "black" else self.white_king_init_sq
        )
        if king_sq is None:
            return False
        king_has_moved_attr = f"{color}_king_has_moved"
        king_has_moved = getattr(self, king_has_moved_attr)
        if king_has_moved:
            return False
        rooks = []  # [(sq, has_moved, final_squares(k,r), )]
        if side in (None, "kingside"):
            rooks.append(
                (
                    f"{color}_kingside_rook_init_sq",
                    f"{color}_kingside_rook_has_moved",
                    ("g8", "f8") if color == "black" else ("g1", "f1"),
                )
            )
        if side in (None, "queenside"):
            rooks.append(
                (
                    f"{color}_queenside_rook_init_sq",
                    f"{color}_queenside_rook_has_moved",
                    ("c8", "d8") if color == "black" else ("c1", "d1"),
                )
            )
        ...
        for rook_init_sq_attr, rook_has_moved_attr, final_squares in rooks:
            ...
            if (
                ...
                and not any(
                    sq not in squares_king_crosses for sq in self.checked_squares(color)
                )
            ):
                return True
        return False
    ...
    def castle(
        ...
        *,
        ...
        Castling can only be done when:
         ...
         - The king would not pass through a check.
         - The king would not land into a check.
         - There are no pieces between castle and rook.
        """
        if not skip_checks and color != self.turn:
            msg = f"It is {self.turn.upper()}'s turn."
            raise OtherPlayersTurnError(msg)
        match color, side:
            case "black", "queenside":
                if (
                    self.black_king_init_sq is None
                    or self.black_queenside_rook_init_sq is None
                ):
                    msg = "Castling not allowed."
                    raise InvalidMoveError(msg)
                if not skip_checks:
                    squares_between = utils.get_squares_between(
                        self.black_king_init_sq, self.black_queenside_rook_init_sq
                    )
                    checked_squares = self.checked_squares(color)
                if skip_checks or (
                    not self.black_king_has_moved
                    and not self.black_queenside_rook_has_moved
                    and all(self[sq] is None for sq in squares_between)
                    and all(
                        self[sq]
                        in (None, Piece("rook", "black"), Piece("king", "black"))
                        for sq in ("c8", "d8")
                    )
                    and not self.king_is_in_check("black")
                    and not any(
                        sq in checked_squares
                        for sq in utils.get_squares_between(
                            self.black_king_init_sq, "c8"
                        )
                    )
                ):
                    self[self.black_king_init_sq] = None
                    self[self.black_queenside_rook_init_sq] = None
                    self["c8"] = Piece("king", "black")
                    self["d8"] = Piece("rook", "black")
                    self.black_king_has_moved = True
                    self.black_queenside_rook_has_moved = True
                else:
                    msg = "Castling not allowed."
            ...
            case "black", "kingside":
                ...
                if not skip_checks:
                    squares_between = utils.get_squares_between(
                        self.black_king_init_sq, self.black_kingside_rook_init_sq
                    )
                    checked_squares = self.checked_squares(color)
                if skip_checks or (
                    not self.black_king_has_moved
                    and not self.black_kingside_rook_has_moved
                    and all(self[sq] is None for sq in squares_between)
                    and all(
                        self[sq]
                        in (None, Piece("rook", "black"), Piece("king", "black"))
                    ...
                    and not any(
                        sq in checked_squares
                        for sq in utils.get_squares_between(
                            self.black_king_init_sq, "g8"
                        )
                    )
                ):
                    self[self.black_king_init_sq] = None
                    self[self.black_kingside_rook_init_sq] = None
                    self["g8"] = Piece("king", "black")
                    self["f8"] = Piece("rook", "black")
                    self.black_king_has_moved = True
                    self.black_kingside_rook_has_moved = True
                else:
                    msg = "Castling not allowed."
            ...
            case "white", "queenside":
                if (
                    self.white_king_init_sq is None
                    or self.white_queenside_rook_init_sq is None
                ):
                    msg = "Castling not allowed."
                    raise InvalidMoveError(msg)
                if not skip_checks:
                    squares_between = utils.get_squares_between(
                        self.white_king_init_sq, self.white_queenside_rook_init_sq
                    )
                    checked_squares = self.checked_squares(color)
                if skip_checks or (
                    not self.white_king_has_moved
                    and not self.white_queenside_rook_has_moved
                    and all(self[sq] is None for sq in squares_between)
                    and all(
                        self[sq]
                        in (None, Piece("rook", "white"), Piece("king", "white"))
                        for sq in ("c1", "d1")
                    )
                    and not self.king_is_in_check("white")
                    and not any(
                        sq in checked_squares
                        for sq in utils.get_squares_between(
                            self.white_king_init_sq, "c1"
                        )
                    )
                ):
                    self[self.white_king_init_sq] = None
                    self[self.white_queenside_rook_init_sq] = None
                    self["c1"] = Piece("king", "white")
                    self["d1"] = Piece("rook", "white")
                    self.white_king_has_moved = True
                    self.white_queenside_rook_has_moved = True
                else:
                    msg = "Castling not allowed."
            ...
            case "white", "kingside":
                ...
                if not skip_checks:
                    squares_between = utils.get_squares_between(
                        self.white_king_init_sq, self.white_kingside_rook_init_sq
                    )
                    checked_squares = self.checked_squares(color)
                if skip_checks or (
                    not self.white_king_has_moved
                    and not self.white_kingside_rook_has_moved
                    and all(self[sq] is None for sq in squares_between)
                    and all(
                        self[sq]
                        in (None, Piece("rook", "white"), Piece("king", "white"))
                    ...
                    and not any(
                        sq in checked_squares
                        for sq in utils.get_squares_between(
                            self.white_king_init_sq, "g1"
                        )
                    )
                ):
                    self[self.white_king_init_sq] = None
                    self[self.white_kingside_rook_init_sq] = None
                    self["g1"] = Piece("king", "white")
                    self["f1"] = Piece("rook", "white")
                    self.white_king_has_moved = True
                    self.white_kingside_rook_has_moved = True
                else:
                    msg = "Castling not allowed."
    ...
    def promote_pawn(self: "ChessBoard", square: str, piece_type: PieceType) -> None:
        ...
        self.double_forward_last_move = None
        updated_notation = f"{self.moves[-1]}={PLAINTEXT_ABBRS[piece_type]}"
        if self.is_checkmate():
            updated_notation += "#"
        elif self.king_is_in_check(self.turn):
            updated_notation = f"{updated_notation.replace('+', '')}+"
        if "==" in updated_notation:
            updated_notation = f"{updated_notation.replace('==', '=')}="
        self.moves[-1] = updated_notation
        self.hashes.append(self.hash_position())
    ...
    def can_en_passant(
        ...
        if self.double_forward_last_move is None:
            return False
        candidate_squares = []
        for func in (utils.step_left, utils.step_right):
            with suppress(utils.OffGridError):
        ...
        if self.double_forward_last_move not in candidate_squares:
            return False
        return True
    ...
    def en_passant(
        ...
        *,
        ...
        if self.double_forward_last_move is None:
            msg = "En passant can only follow double forward advance."
            raise InvalidMoveError(msg)
        if not skip_checks:
            if (piece := self[initial_square]) is None:
                msg = f"No piece at initial_square '{initial_square}'."
            ...
            for func in (utils.step_left, utils.step_right):
                with suppress(utils.OffGridError):
                    candidate_squares.append(func(initial_square, 1))
            if self.double_forward_last_move not in candidate_squares:
                msg = f"Pawn at '{initial_square}' cannot capture '{final_square}'."
                raise InvalidMoveError(msg)
        piece_to_capture = self[self.double_forward_last_move]
        assert piece_to_capture is not None
        if piece_to_capture.color == "white":
            self[f"{self.double_forward_last_move[0]}3"] = Piece("pawn", "white")
        else:
            self[f"{self.double_forward_last_move[0]}6"] = Piece("pawn", "black")
        self[self.double_forward_last_move] = None
        self.double_forward_last_move = None
        self.halfmove_clock = 0
        # notate
        notation = ""
        squares_that_may_need_disambiguation = []
        for func in (utils.step_left, utils.step_right):
            with suppress(utils.OffGridError):
                squares_that_may_need_disambiguation.append(func(initial_square, 2))
        for sq in squares_that_may_need_disambiguation:
            if self[sq] == Piece("pawn", self.turn):
                notation += initial_square[0]
        notation += f"x{final_square}"
        self.move_piece(initial_square, final_square)

    @property
    def pieces(self: "ChessBoard") -> dict[str, Piece]:
        """Get all pieces on the board."""
        return {sq: piece for sq in self.grid if (piece := self[sq]) is not None}
    ...
    def checked_squares(self: "ChessBoard", color: Color) -> list[str]:
        ...
        for sq in other_color_pieces:
            checked_squares.extend(
                self._get_squares(sq, capture_only=True, check_castle=False)
            )
    ...
    def is_checkmate(self: "ChessBoard") -> str | None:
        ...
        for sq, king in kings:
            if (
                ...
                and len(self.legal_moves(sq)) == 0
            ):
                self.game_over = True
                self.winner = "black" if king.color == "white" else "white"
                self.status_description = "checkmated"
                return self.winner
        return None
    ...
    def status(self: "ChessBoard") -> GameStatus:
        ...
        if self.game_over:
            return GameStatus(self.game_over, self.winner, self.status_description)
        if self.is_checkmate():
            return GameStatus(
                game_over=True,
                winner=self.winner,
        ...
        if len(possible_squares) == 0 and not self.can_castle(self.turn):
            self.game_over = True
            self.status_description = "stalemated"
            return GameStatus(game_over=True, description=self.status_description)
        # Check for draw by repetition.
        with suppress(IndexError):
            if Counter(self.hashes).most_common(1)[0][1] >= 3:
                self.game_over = True
                self.status_description = "repetition"
                return GameStatus(game_over=True, description=self.status_description)
        ...
        if not is_sufficient:
            self.game_over = True
            self.status_description = "insufficient"
            return GameStatus(game_over=True, description=self.status_description)
        # Check for draw by 50 halfmoves without capture.
        if self.halfmove_clock > 50:
            self.game_over = True
            self.status_description = "50move"
            return GameStatus(game_over=True, description=self.status_description)
        self.game_over = False
    ...
    def can_block_check(self: "ChessBoard", color: Color) -> bool:
        ...
        checks: list[str] = [
            ...
            if king in self._get_squares(piece, capture_only=True)
        ]
        ...
        for check in checks:
            ...
            if (file_difference := FILES.index(king[0]) - FILES.index(check[0])) > 0:
                ...
                file_direction = "left"
            match rank_direction, file_direction:
                case "up", "right":
                    possible_squares = [
                        check,
                        *list(utils.iter_top_right_diagonal(check)),
                    ]
                case "up", "inline":
                    possible_squares = [check, *list(utils.iter_to_top(check))]
                case "up", "left":
                    possible_squares = [
                        check,
                        *list(utils.iter_top_left_diagonal(check)),
                    ]
                case "inline", "right":
                    possible_squares = [check, *list(utils.iter_to_right(check))]
                case "inline", "left":
                    possible_squares = [check, *list(utils.iter_to_left(check))]
                case "down", "right":
                    possible_squares = [
                        check,
                        *list(utils.iter_bottom_right_diagonal(check)),
                    ]
                case "down", "inline":
                    possible_squares = [check, *list(utils.iter_to_bottom(check))]
                case "down", "left":
                    possible_squares = [
                        check,
                        *list(utils.iter_bottom_left_diagonal(check)),
                    ]
        ...
        for sq in same_color_pieces_except_king:
            possible_moves.extend([(sq, move) for move in self._get_squares(sq)])
        # Check if en passant capture can block check.
        if self.double_forward_last_move is not None:
            adjacent_squares = [
                f"{file}{self.double_forward_last_move[1]}"
                for file in utils.get_adjacent_files(self.double_forward_last_move)
            ]
            final_rank = (
                int(self.double_forward_last_move[1]) + 1
                if color == "white"
                else int(self.double_forward_last_move[1]) - 1
            )
            final_sq = f"{self.double_forward_last_move[0]}{final_rank}"
            for sq in adjacent_squares:
                if self[sq] == Piece("pawn", color) and self.can_en_passant(
                    sq, self.double_forward_last_move[0]
                ):
                    changes: dict[str, Piece | None] = {
                        self.double_forward_last_move: None,
                        sq: None,
    ...
    def king_is_in_check(self: "ChessBoard", color: Color) -> bool | None:
        ...
        try:
            ...
            return king_sq in self.checked_squares(color)
        except StopIteration:
            return None

    def resign(self: "ChessBoard") -> GameStatus:
        """Resign instead of moving."""
        self.game_over = True
        self.winner = "white" if self.turn == "black" else "black"
        return GameStatus(self.game_over, self.winner, "resigned")

    def draw(self: "ChessBoard") -> GameStatus:
        """Draw instead of moving."""
        self.game_over = True
        self.winner = None
        return GameStatus(game_over=True, description="agreed")

    def rich_renderable(self: "ChessBoard") -> str:
    ...
    def plaintext_repr(self: "ChessBoard") -> str:
        ...
        for rank in range(8, 0, -1):
            ...
            for sq in rank_grid:
                ...
                if piece is None:
                    ...
                    piece_abbr = PLAINTEXT_ABBRS[piece.piece_type]
                    char = (
                        piece_abbr.upper()
                        if piece.color == "white"
                        else piece_abbr.lower()
                    )
                    output += f"{char} "
            output += "\n"
    ...
    def print(self: "ChessBoard", *, plaintext: bool = False) -> None:
        ...
        if not plaintext and "Console" in globals():
            Console().print(self.rich_renderable())
        else:
            print("\n")
            print(self.plaintext_repr())
