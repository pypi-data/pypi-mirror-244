...
from rqdatac.utils import int8_to_datetime, to_datetime, date_to_int8
...
def get_basis(order_book_ids, start_date=None, end_date=None, fields=None, market="cn"):
    ...
    def _calc_annual_rate(row, rate_field):
        ...
        if len(dates_to_delisted) == 0:
            ...
            return row[rate_field] * 250 / len(dates_to_delisted)
    ...
    FUTURE_PRICE_FIELDS = {"close", "open", "high", "low", "settlement"}
    # 根据 `fields` 定义 `future_price_fields`, 尽量少取几个字段
    need_basis, need_settlement_basis = True, True
    if fields is None:
        fields = VALID_FIELDS
        ...
        for f in fields:
            if f.startswith("basis"):
                need_basis = True
                future_price_fields.add("close")
            elif f.startswith("settle"):
                need_settlement_basis = True
    ...
    future_price = get_price.get_price(
        order_book_ids, start_date, end_date, fields=future_price_fields,
        expect_df=True
    )
...
def get_ob_datetime_multi_index(order_book_ids, start_date, end_date, names=['order_book_id', 'trading_date']):
    ...
    listed_dates = [i.listed_date for i in insts]
    de_listed_dates = [i.de_listed_date for i in insts]
    indexs = []
    for ob, listed_date, de_listed_date in zip(order_book_ids, listed_dates, de_listed_dates):
        start = listed_date if listed_date >= start_date else start_date
        end = de_listed_date if de_listed_date <= end_date else end_date
        dates = get_trading_dates(start, end)
        indexs.extend([(ob, pd.to_datetime(date)) for date in dates])
    indexs = pd.MultiIndex.from_tuples(indexs, names=names)
    return indexs
...
def get_trading_parameters(order_book_ids, start_date=None, end_date=None, fields=None, market='cn'):
    ...
    order_book_ids = ensure_order_book_ids(order_book_ids, type='Future')
    # 只存了真实合约信息
    order_book_ids = list(
        filter(
    ...
    if fields is None:
        ...
        fields = check_items_in_container(fields, FIELDS, 'fields')
