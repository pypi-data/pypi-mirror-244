...
class HoneyHiveTraceContextManager:
    ...
    def prepare_output(self):
        ...
        if self.last_return_value == None:
            return
        if type(self.last_return_value) is not dict:
            try:
                if hasattr(self.last_return_value, '__dict__'):
                    self.last_return_value = self.last_return_value.__dict__
                else:
                    self.last_return_value = json.loads(self.last_return_value)
            except:
                pass
        ...
        if type(self.event_data["output"]) is str:
            try:
                self.event_data["output"] = json.loads(self.event_data["output"])
            except:
                self.event_data["output"] = {"text": self.event_data["output"]}
