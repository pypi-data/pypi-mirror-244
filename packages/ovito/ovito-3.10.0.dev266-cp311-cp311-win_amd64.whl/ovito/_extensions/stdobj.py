...
from ovito.data import DataCollection, DataTable, Lines, SimulationCell
...
def _DataCollection_lines(self):
    """
    ...
    If you would like to create a new :py:class:`Lines` object, within the implementation of a user-defined modifier, for instance,
    the dictionary view provides the convenience method :py:meth:`!lines.create`, which
    ...
    If there is already an existing lines object with the same :py:attr:`~ovito.data.DataObject.identifier` in the collection, the :py:meth:`!create`
    method returns that existing lines object instead of creating another one and after making sure that it can be safely modified.
    """
