...
def directionalErosion(bwIm, vect, a, c, nProcesses=nProcessesDefault, verbose = False):
    ...
    global funDirectionalErosion
    def funDirectionalErosion(job):
    ...
    with multiprocessing.Pool(processes=nProcesses) as pool:
        for returns in pool.imap_unordered(funDirectionalErosion, range(0, numberOfJobs)):
            if verbose:
                finishedNodes += 1
...
def morphologicalReconstruction(im, selem = skimage.morphology.ball(1)):
    """
    This functions performs a morphological reconstruction (greyscale opening followed by greyscale closing). The ouput image presents less variability in the greyvalues inside each phase, without modifying the original
