...
def export_layout(
    ...
    :param export_tiles: Export Tiles, Choose whether to export only the [0, 1 range], or all UV tiles * ``NONE`` None -- Export only UVs in the [0, 1] range. * ``UDIM`` UDIM -- Export tiles in the UDIM numbering scheme: 1001 + u-tile + 10\*v-tile. * ``UV`` UVTILE -- Export tiles in the UVTILE numbering scheme: u(u-tile + 1)_v(v-tile + 1).
...
def follow_active_quads(
    ...
    ''' Follow UVs from active quads along continuous face loops :File: `startup/bl_operators/uvcalc_follow_active.py\:269 <https://projects.blender.org/blender/blender/src/branch/main/scripts/startup/bl_operators/uvcalc_follow_active.py#L269>`__
