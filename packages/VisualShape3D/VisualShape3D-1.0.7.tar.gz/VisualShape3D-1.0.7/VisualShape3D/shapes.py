...
def rectangleWithHole(*args):
    ...
    if C>0 and D>0 and (A+C)<W and (B+D)<H :
        ret = np.array([(0,0,0),(0,W,0),(0,W,H),(0,A,H),(0,A,B+D),(0,C+A,B+D),(0,C+A,B),(0,A,B),(0,A,H),(0,0,H)])

    elif A == 0.0 :
        if B == 0.0 and (A+C) < W and (B+D) < H :
            ret = np.array([(0,C,0),(0,W-C,0),(0,W,H),(0,0,H),(0,0,D),(0,C,D)])

        if B != 0.0 and (A+C) < W and (B+D) < H :
            ret = np.array([(0,0,0),(0,W,0),(0,W,H),(0,0,H),(0,0,B+D),(0,C,B+D),(0,C,B),(0,0,B)])
        ...
        if B == 0.0 and (C+A) < W and (B+D) < H :
            ret = np.array([(0,0,0),(0,A,0),(0,A,B),(0,A+C,B),(0,A+C,0),(0,W,0),(0,W,H),(0,0,H)])
                
        if C != 0.0 and (A+C) < W and (B+D) >= H :
