...
def get_or_create_users_by_emails(
    emails: List[str], exception_on_error: bool = False
) -> List[User]:
    """For each of the given list of email addresses, retrieve the corresponding Sym user if one exists, or create a new
    one otherwise.

    If ``exception_on_error`` is ``True``, then any "routine" errors that occur while processing the list of emails (for
    ...
    Regardless of the value of ``exception_on_error``, serious errors (such as a communication failure within the Sym
    ...
    Args:
        ...
        exception_on_error: Whether to raise an exception if an error occurs while processing the list of emails.
            If ``False``, errors will be logged as warnings and the corresponding user will be omitted from the output.
            Defaults to ``False``.
    ...
    Raises:
        :class:`~sym.sdk.exceptions.sym_integration.SymIntegrationError`: If ``exception_on_error`` is ``True`` and any
