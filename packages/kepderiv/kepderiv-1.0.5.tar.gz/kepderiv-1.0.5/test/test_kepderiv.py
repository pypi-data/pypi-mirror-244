...
def _list_comb_5param():
  list_param = [['n', 'P', 'log10P'],
    ['M0', 'MARG0', 'la0', 'TP', 'TC', 'TVMIN', 'TVMAX'],
    ['K', 'log10K', 'as', 'asini'],
    [('e', 'omega'), ('e', 'varpi'), ('ecosw', 'esinw'),
    ('sqecosw', 'sqesinw')]]
...
def _list_comb_7param():
  ...
  l7 = [l + ['i', 'BigOmega']
    for l in l5] + [l + ['cosi', 'BigOmega'] for l in l5]
  l7 += [[freq, 'e', phase, 'TIA', 'TIB', 'TIF', 'TIG']
...
def _test_rv_back_param(param, seed=0):
  ...
  num_err = np.max([
    _max_err(grad_param_num[i], grad_param_num[j]) for i in range(4)
    for j in range(i)
  ])
  print(err, num_err)
  err = max(0.0, err - num_err)
  assert err < prec, ('rv_back ({}) not working'
    ' at required precision ({} > {})').format(param, err, prec)
...
def _test_astro_back_param(param, seed=0):
  ...
  for dx in [-delta, -delta / 10, delta / 10, delta]:
    ...
    for k in range(len(param)):
      ...
      grad_param_num_dx.append(
        (ddelta @ grad_delta + dalpha @ grad_alpha) / dx)
      x[k] -= dx
    grad_param_num.append(grad_param_num_dx)
  grad_param_num = np.array(grad_param_num)
  print('grad_param', grad_param)
  ...
  print(err, num_err)
  err = max(0.0, err - num_err)
  assert err < prec, ('astro_back ({}) not working'
    ' at required precision ({} > {})').format(param, err, prec)
