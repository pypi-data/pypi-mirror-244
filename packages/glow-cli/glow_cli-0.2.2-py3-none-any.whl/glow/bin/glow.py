...
from glow.llm.run import run_llm, run_code
...
from glow.secrets import load_secrets, secrets_manage
    ...
    for key, input_conf in cmd_conf["inputs"].items():
        if key in data:
            ...
            if input_conf["env_var"] in os.environ:
                ...
                show_help_input(key, input_conf)
                raise ValueError(f"{input_conf['env_var']} is not set in environment")
        elif "default" in input_conf:
            # use default
            results[key] = input_conf["default"]
        else:
