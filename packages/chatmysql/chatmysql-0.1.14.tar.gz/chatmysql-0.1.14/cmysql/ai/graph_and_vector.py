...
# class ModifyMDataKGChain(Chain):
#     # 根据用户的反馈，更新元数据的描述信息，如果不存在直接插入，如果存在，则覆盖
#     input_key = 'user_feedback'
#     output_key = 'result'
#
#     @property
...
#     ) -> Dict[str, str]:
#         _user_feedback = inputs.get(self.input_key)
#         kg = MDataKG()
#         _mdatakg_changes = kg.infer_mdatakg_changes_from_feedback(_user_feedback)
#         if not _mdatakg_changes:
#             info = f"用户反馈『{_user_feedback}』没有找到相关的表。"
#             log.info(info)
#             return {self.output_key: info}
#         _, _, _, _, info = kg.modify(_mdatakg_changes)
#         return {self.output_key: info}


# class FindMDataChain(Chain):
#     @property
#     def input_keys(self) -> List[str]:
#         return ['question']
#
...
#         mdata = MDataKG().find(question)
#
#         result = {self.output_keys[0]: json.dumps({"question": question, "mdata": mdata})}
#         return result
