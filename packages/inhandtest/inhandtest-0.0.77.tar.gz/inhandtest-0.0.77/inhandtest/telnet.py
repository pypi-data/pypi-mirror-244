...
class Telnet:
    ...
    def __login(self):
        ...
        login_spe = {"VG710": '#', 'IR302': '>', 'ER805': '#', 'ER605': '#', 'IG902': '#', 'IG502': '#', 'IR915': '#',
                     'ODU2002': '>', 'IR305': '>', 'IR615': '>', 'FWA02': '#', 'EAP600': '#', 'IR624': '#'}
    ...
    def super_mode(self) -> None:
        ...
        if self.config_tag in read_contents:
            if self.model in ['VG710', 'ER805', 'ER605', 'IG902', 'IG502', 'IR915', 'ODU2002', 'FWA02', 'IR624']:
    ...
    def config_mode(self) -> None:
        ...
        if self.config_tag in read_contents:
            ...
            if self.model in ['VG710', 'ER805', 'ER605', 'IG902', 'IG502', 'IR915', 'FWA02', 'IR624']:
    ...
    def user_mode(self) -> None:
        ...
        if self.config_tag in read_contents:
            ...
            if self.model in ['VG710', 'ER805', 'ER605', 'IG902', 'IG502', 'IR915', 'FWA02', 'ODU2002', 'IR624']:
    ...
    def factory_mode(self) -> None:
        ...
        logging.debug(read_contents)
        if self.config_tag in read_contents:
            if self.model in ['VG710', 'IR302', 'IR305', 'IR615', 'ER805', 'ER605', 'IG902', 'IG502', 'IR915', 'IR624']:
                self.send_cli(['exit', self.factory_username + " " + self.factory_password])
        elif self.super_tag in read_contents or (('/' in read_contents) and (' #' in read_contents)):
            if self.model in ['VG710', 'ER805', 'ER605', 'IG902', 'IG502', 'IR915', 'IR624']:
    ...
    def connect_aws(self, platform='star.inhandcloud.cn'):
        ...
        """
        if self.model not in ['ER805', 'ER605', 'ODU2002', 'IR624']:
...
if __name__ == '__main__':
    ...
    a = Telnet('IR624', '192.168.2.1', 'inhand', '64391099@inhand', connect_timeout=20, connect_interval=5)
    a.connect_aws('device.nezha.inhand.design')
