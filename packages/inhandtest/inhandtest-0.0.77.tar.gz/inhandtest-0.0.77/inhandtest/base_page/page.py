...
class Page_:
    ...
    def grid(self, grid: list, locator: Locator, value: list, locale: dict = None,
        ...
        :return:
        """
        if self.model in ('ER805', 'ER605', 'FWA02', 'ODU2002', 'EAP600'):
            div = ErGrid(grid, locator, locale, log_desc)
            exist_div = []
            if value:
                for value_ in value:
                    if value_[0] == 'add':
                        div.add(agg_function, **value_[1])
                    elif value_[0] == 'delete_all':
                        div.delete_all()
                    elif value_[0] == 'delete':
                        div.delete(value_[1])
                        ...
                        div.edit(agg_function, value_[1], **value_[2])
                    elif 'insert' in value_[0]:
                        div.insert(agg_function, value_[1], action=value_[0].split('insert_')[1], **value_[2])
                return exist_div
        else:
