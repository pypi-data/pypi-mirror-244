...
def extend_list_with_unique_parts(base: list[str], appendage: list[str]) -> None:
    """
    Extend base list with unique parts of appendage

    :param base: base list
    :param appendage: list of candidate strings
    """
    for part in appendage:
        if part not in base:
            base.append(part)


def preset_range_to_list(preset_range: str) -> list[str]:
    try:
        start, end = preset_range.split('-')
    except ValueError:
        # tuple unpack into two parts makes too few *and* too many dashes a ValueError
        raise ValueError(f'Invalid preset range: "{preset_range}"')

    try:
        i_start = ORDERED_PRESETS.index(start)
    except ValueError:
        raise ValueError(f'Invalid start preset "{start}" in range "{preset_range}"')

    try:
        i_end = ORDERED_PRESETS.index(end)
    except ValueError:
        raise ValueError(f'Invalid end preset "{end}" in range "{preset_range}"')

    if i_start < i_end:
        range_iterator = range(i_start, i_end+1)
    else:
        range_iterator = range(i_start, i_end-1, -1)

    presets = []
    for i in range_iterator:
        presets.append(ORDERED_PRESETS[i])

    return presets


def freeform_presets_to_list(freeform: str) -> list[str]:
    """
    translate a "freeform" preset string to a list of presets

    Presets may be separated by comma and/or specified in ranges with dashes, in any order.
    Later duplicates are discarded. Examples:
      "medium" -> ["medium"]
      "fast,faster" -> ["fast", "faster"]
      "faster-slow" -> ["faster", "fast", "medium", "slow"]
      "slow-faster" -> ["slow", "medium", "fast", "faster"]
      "veryfast,fast-slow" -> ["veryfast", "fast", "medium", "slow"]
      "medium,fast-slow" -> ["medium", "fast", "slow"]

    :param freeform: freeform preset string
    :return: list of presets
    """
    presets = []
    splits = freeform.split(',')

    for split in splits:
        if '-' in split:
            # dismantle and append given range
            extend_list_with_unique_parts(presets, preset_range_to_list(split))
        else:
            # append plain preset
            if split not in ORDERED_PRESETS:
                raise ValueError(f'Invalid preset: "{split}"')
            else:
                extend_list_with_unique_parts(presets, [split])
