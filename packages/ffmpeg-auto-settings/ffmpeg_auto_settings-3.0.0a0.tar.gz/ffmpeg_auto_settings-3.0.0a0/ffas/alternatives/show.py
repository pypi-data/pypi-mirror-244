...
from ..misc import (
    get_video_duration,
    get_video_stream_bitrate,
    FFmpegTimedelta,
    ensure_acceptable_ffmpeg,
    freeform_presets_to_list
)
from .common import derive_target_vmaf
...
def cmd_alt_show(args):
    if args.vmaf:
        target_vmaf = args.vmaf
    else:
        target_vmaf = derive_target_vmaf(args.from_preset, args.from_crf, only_use_db=True)
...
def show_alternatives(presets: list[str], target_vmaf: Decimal, vmaf_delta: Decimal):
    ...
    for preset in presets:
        try:
            vmaf_max = (target_vmaf + vmaf_delta).quantize(Decimal('1.00'))
            sample_encode = store.get_sample_encode_in_vmaf_range(
                config['encoder'],
                preset,
                sample,
                vmaf_min=target_vmaf,
                vmaf_max=vmaf_max,
    ...
    print(f"VMAF Range: {target_vmaf:.2f} (+{vmaf_delta:.2f})")
    print(t)
