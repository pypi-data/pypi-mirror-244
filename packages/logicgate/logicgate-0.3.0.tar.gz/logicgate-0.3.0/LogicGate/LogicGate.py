    ...
    if len(str(obj)) == 1:  #if the first process string is a single command
      ...
      if aft != None and aft != []:
        ...
        if obj == 'N':
          ...
          raise GateError(f'Invalid gate {obj} in line {line[0]} index {line[1]}, file {repr(filename)}')
      if AnList != None:
        ...
        if RaiseError:
          ...
          sys.stdout = sys.__stdout__
          raise LogicError(
              f'gate {line[2] if obj=="0" or obj=="1" else obj} in line {line[0]} index {line[1]-1} requires {"1" if (line[2] if obj=="0" or obj=="1" else obj)=="N" else "2"} logical inputs, 1 received, file {repr(filename)}'
            )
        if len(AnList) == 3:
          if AnList[0]:
        ...
        sys.stdout = sys.__stdout__
        raise LogicError(
          f'gate {line[2] if obj=="0" or obj=="1" else obj} in line {line[0]} index {line[1]} requires {"1" if (line[2] if obj=="0" or obj=="1" else obj).upper()=="N" else "2"} logical inputs, 0 received, file {repr(filename)}'
        )
    else:
      #if the first command given is not an acceptable command
      sys.stdout = sys.__stdout__
      raise SystemError(
