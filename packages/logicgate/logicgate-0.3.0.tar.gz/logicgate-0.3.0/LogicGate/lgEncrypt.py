...
try:
  ...
  raise ImportError('module require LogicGate, LogicGate not found. Trying to install')
  system('pip install LogicGate')
except ImportError:
  raise ImportError('module require LogicGate, LogicGate not found. Trying to install')
  system('pip install LogicGate')
...
def decrypt(filename: str, key_file: str, sause: bool = False, debug: bool = False, debug_ALL: bool = False):
  ...
  if exists(filename) and exists(key_file):
    ...
    raise FileNotFoundError("filename or key_file doesn't exist.")
