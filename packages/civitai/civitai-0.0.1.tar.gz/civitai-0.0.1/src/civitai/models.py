...
def get_model(CIVITAI_BASE_URL: str,  modelId: str):
    url = f"{CIVITAI_BASE_URL}/models/{modelId}"
    x = requests.get(url)
    return Model(x.json())

def get_by_modelVersion(CIVITAI_BASE_URL: str,  modelVersionId: str):
    url = f"{CIVITAI_BASE_URL}/model-versions/{modelVersionId}"
    x = requests.get(url)
    return ModelVersion(x.json())

def get_by_hash(CIVITAI_BASE_URL: str,  hash: str):
