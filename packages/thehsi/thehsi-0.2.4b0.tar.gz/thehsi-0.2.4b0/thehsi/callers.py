...
def load_callers(application_path: str):
    ...
    for application in applications:
        ...
        if not hasattr(application_caller.Kernel, '__kernel__'):
            print(f"[WARNING]: Application '{application}' could not be loaded!\n  Invalid __hmgr_caller__.py! Invalid Kernel!")
            continue
        
        if application_caller.Kernel.__kernel__ == Kernel("", ""):
            print(f"[WARNING]: Application '{application}' could not be loaded!\n  Invalid __hmgr_caller__.py! No Kernel defined!")
            continue

        if application_caller.Kernel.__isinit__():
            print(f"[WARNING]: Application '{application}' could not be loaded!\n  Kernel not initialized!")
            continue

        kernel: Kernel = application_caller.Kernel.__kernel__()
