...
from .meta import (
    ...
    expected_halcon_version as expected_halcon_version_real,
)
...
def load_hdevenginecpp_dylib() -> ctypes.CDLL:
    ...
    def same_version_as_hlib():
        if _used_fallback_dylib:
            return _dylib_file_name_from_base(base_name)
        else:
            return _dylib_file_name_with_version_from_base(base_name)

    full_dylib_name = _exec_os_specific_logic(
        linux_fn=same_version_as_hlib,
        darwin_fn=same_version_as_hlib,
        win_fn=lambda: _dylib_file_name_from_base(base_name)
    )
...
def _build_incompatible_versions_msg(potential_version: str) -> str:
    ...
    env_var_name = _exec_os_specific_logic(
        ...
        darwin_fn=lambda: 'DYLD_FRAMEWORK_PATH',
        win_fn=lambda: 'PATH'
    )
    return (
...
def _load_halcon_dylib_posix(build_error_msg_fn) -> ctypes.CDLL:
    try:
        return _dlopen(_dylib_file_name_with_version_from_base(
            _halcon_base_name()
        ))
    except OSError:
        try:
            # If there is any HALCON version dynamically loadable this links
            # to it.
            # Compatibility will be checked by _halcon_version_check.
            dylib = _dlopen(_dylib_file_name_from_base(_halcon_base_name()))

            global _used_fallback_dylib
            _used_fallback_dylib = True
...
def _load_halcon_dylib_win() -> ctypes.CDLL:
    ...
    try:
        ...
        # we could try.
        return _dlopen(_dylib_file_name_from_base(_halcon_base_name()))
    except Exception as exc:
        _handle_dylib_not_found(exc, 'error', build_error_msg_fn)


def _exec_os_specific_logic(linux_fn, darwin_fn, win_fn) -> Any:
...
def _halcon_base_name() -> str:
    should_use_halcon_xl = _should_use_halcon_xl()

    return _exec_os_specific_logic(
        linux_fn=lambda: 'halconxl' if should_use_halcon_xl else 'halcon',
        darwin_fn=lambda: 'HALCONXL' if should_use_halcon_xl else 'HALCON',
        win_fn=lambda: 'halconxl' if should_use_halcon_xl else 'halcon',
    )
...
def _hdevenginecpp_base_name() -> str:
    ...
    return _exec_os_specific_logic(
        linux_fn=lambda: \
            'hdevenginecppxl' if should_use_halcon_xl else 'hdevenginecpp',
        darwin_fn=lambda: \
            'HDevEngineCppXL' if should_use_halcon_xl else 'HDevEngineCpp',
        win_fn=lambda: \
            'hdevenginecppxl' if should_use_halcon_xl else 'hdevenginecpp',
...
def _dylib_file_name_from_base(base_name: str) -> str:
    return _exec_os_specific_logic(
        linux_fn=lambda: f'lib{base_name}.so',
        darwin_fn=f'{base_name}.framework/Versions/Current/{base_name}',
        win_fn=lambda: f'{base_name}.dll',
    )


def _dylib_file_name_with_version_from_base(base_name: str) -> str:
    def version_fn(os_name):
        return (
            halcon_dylib_full_version[os_name]
            if os.environ.get('_MVTEC_HALCON_PACKAGE_MOCK_VERSION') == None else
            _expected_halcon_version
        )

    def not_supprted():
        raise HError('This platform does not support full version dylibs')

    version_linux = version_fn('linux')
    version_darwin = version_fn('darwin')

    return _exec_os_specific_logic(
        linux_fn=lambda: f'lib{base_name}.so.{version_linux}',
        darwin_fn=lambda: (
            f'{base_name}.framework/Versions/{version_darwin}/{base_name}'
        ),
        win_fn=not_supprted,
    )
...
def _compat_load_halcon() -> ctypes.CDLL:
    ...
    def build_error_msg_fn_darwin():
        dyld_library_path_val = os.environ.get('DYLD_FRAMEWORK_PATH')
        return (
            'Unable to find any HALCON library.\n'
            f'Current value of DYLD_FRAMEWORK_PATH={dyld_library_path_val}\n'
            'Set DYLD_FRAMEWORK_PATH in your environment.'
        )

    return _exec_os_specific_logic(
        linux_fn=lambda: _load_halcon_dylib_posix(build_error_msg_fn_linux),
        darwin_fn=lambda: _load_halcon_dylib_posix(build_error_msg_fn_darwin),
