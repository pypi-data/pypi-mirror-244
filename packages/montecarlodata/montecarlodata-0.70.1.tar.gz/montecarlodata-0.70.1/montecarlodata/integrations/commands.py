...
@click.option(
    ...
    required=True,
    type=click.Choice(["confluent-cloud"]),
    help="Streaming System type. Currently we only support confluent-cloud.",
)
@click.option(
...
@click.option(
    ...
    type=click.Choice(["confluent-kafka", "confluent-kafka-connect"]),
    required=True,
)
@click.option(
    "--key",
...
@click.option(
    "--api-key",
    help="API for accessing the  This is required only when the key is not specified.",
    required=False,
    cls=AdvancedOptions,
    mutually_exclusive_options=["key"],
)
...
def add_confluent_cluster_connection(ctx, **kwargs):
    StreamingOnboardingService(
        config=ctx["config"],
        mc_client=create_mc_client(ctx),
