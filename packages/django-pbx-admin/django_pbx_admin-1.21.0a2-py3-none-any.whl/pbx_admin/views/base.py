...
class AdminListView(  # type: ignore
    PaginationMixin, AdminViewMixin, FormMixin, FooterMixin, ListView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        fields_list = self.admin.get_list_display(self.request)
        context["fields_list"] = fields_list
        context["fields_search_list"] = [field.field_name for field in fields_list]

        context["header"] = self._get_context_header(fields_list)

        return context

    def _get_context_header(self, fields_list):
        header = []
        form = self.get_form()
        ...
        for field in fields_list:
            ...
            order_field = field.get_field_ordering()
            order_param = self.request.GET.get("o", "")
            header.append(
                {
                    "name": field,
                    "label": field.get_label(form),
...
class AdminDeleteView(
    AdminViewMixin, SuccessMessageMixin, FooterMixin, DeleteView[_M, ModelForm]
):
    # workaround for mypy issue
    # https://github.com/typeddjango/django-stubs/issues/1227#issuecomment-1311472749
    object: _M  # noqa A003
