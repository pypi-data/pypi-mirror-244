...
def restart_storage_node(node_id):
    db_controller = DBController()
    kv_store = db_controller.kv_store
...
def get_device_capacity_data(device_id, history):
    db_controller = DBController()
...
def get_device_capacity(device_id, history):
    ...
    out = []
    for record in get_device_capacity_data(device_id, history):
        out.append({
            "Date": record['date'],
...
def get_device_iostats(device_id, history, records_count=20):
    ...
    new_records = utils.process_records(data, records_count)
...
def get_node_capacity(node_id, history):
    ...
    out = []
    data = get_node_capacity_data(node_id, history)
    for line in data:
        out.append({
            "Name": line['name'],
            "Absolut": utils.humanbytes(line["drive"]),
            "Used": utils.humanbytes(line["used"]),
            "Free": utils.humanbytes(line["free"]),
            "Util %": f"{line['util_percent']}%",
        })
    return utils.print_table(out)
