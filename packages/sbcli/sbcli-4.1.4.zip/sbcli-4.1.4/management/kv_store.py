...
class DBController:
    ...
    def get_cluster_capacity(self, cl, limit=1):
        stats = CapacityStat().read_from_db(self.kv_store, id="%s" % cl.get_id(), limit=limit, reverse=True)
        return stats

    def get_node_capacity(self, node, limit=1):
        stats = CapacityStat().read_from_db(self.kv_store, id="%s" % node.get_id(), limit=limit, reverse=True)
        return stats
