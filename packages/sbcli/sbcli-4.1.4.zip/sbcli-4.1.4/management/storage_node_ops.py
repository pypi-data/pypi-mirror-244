...
def get_device_capacity_data(device_id, records_number=1):
    db_controller = DBController()
    ...
    data = db_controller.get_device_stats(device, records_number)
    out = []

    for record in data:
        total_size = record.npages_nmax * record.pba_page_size
        used_size = record.npages_used * record.pba_page_size
        free_size = total_size - used_size
        if total_size > 0:
            util = int((used_size / total_size) * 100)
        else:
            util = 0
        out.append({
            "date": record.date,
...
def get_device_capacity(device_id, history, records_count=20):
    db_controller = DBController()
    device = db_controller.get_storage_devices(device_id)
    if not device:
        logger.error("device not found")
    ...
    records = get_device_capacity_data(device_id, records_number)
    ...
    for record in records_list:
        out.append({
            ...
            "Absolut": utils.humanbytes(record['drive']),
            "Used": utils.humanbytes(record['used']),
            "Free": utils.humanbytes(record['free']),
            "Util %": f"{record['util_percent']}%",
        })
...
def get_device_iostats(device_id, history, records_count=20):
    db_controller = DBController()
    device = db_controller.get_storage_devices(device_id)
    if not device:
...
def _get_node_capacity_data(node, history):
    ...
    for dev in node.nvme_devices:
        ...
        for index, record in enumerate(stats):
            # possible bug here :)
            if index < len(data):
                data[index]["npages_allocated"] += record.npages_allocated
                data[index]["npages_used"] += record.npages_used
                data[index]["npages_nmax"] += record.npages_nmax
                data[index]["pba_page_size"] += record.pba_page_size
            else:
                data.insert(index, {})
                ...
                data[index]["npages_used"] = record.npages_used
                data[index]["npages_nmax"] = record.npages_nmax
                data[index]["pba_page_size"] = record.write_iops
    return data



def get_node_capacity_data(node_id, history):
    db_controller = DBController()
    ...
    for dev in devices:
        if dev.status == dev.STATUS_ONLINE:
            data = get_device_capacity_data(dev.get_id(), 1)
            if data:
                all_data.append(data[0])

    if all_data:
        node_data = utils.dict_agg(all_data)
        utp = int(node_data['util_percent'] / len(all_data))
        node_data['name'] = "Node"
        node_data['util_percent'] = utp
        all_data.append(node_data)
    return all_data


def get_node_capacity(node_id, history, records_count=20):
    db_controller = DBController()
    this_node = db_controller.get_storage_node_by_id(node_id)
    if not this_node:
    ...
    for record in new_records:
        ...
        out.append({
            "Date": time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(record['date'])),
            "Absolut": utils.humanbytes(total_size),
            "Used": utils.humanbytes(used_size),
            "Free": utils.humanbytes(free_size),
            "Util %": f"{util}%",
        })
    return out
...
def get_node_iostats_history(node_id, history, records_count=20):
    db_controller = DBController()
