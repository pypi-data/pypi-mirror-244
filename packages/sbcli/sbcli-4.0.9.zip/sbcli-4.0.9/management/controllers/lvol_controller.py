    ...
    for node in snodes:
        ...
        if node.status == node.STATUS_ONLINE:
            ...
            for lvol_id in node.lvols:
                stats = db_controller.get_lvol_stats(lvol_id, 3600)
                for st in stats:
...
def recreate_lvol(lvol_id, snode):
    ...
    if lvol.ha_type == 'single':
        is_created = add_lvol_on_node(lvol, snode)
        if not is_created:
            return False
        ...
        for node_id in lvol.nodes:
            ...
            is_created = add_lvol_on_node(lvol, sn)
            if not is_created:
                return False
...
def set_lvol(uuid, max_rw_iops, max_rw_mbytes, max_r_mbytes, max_w_mbytes):
    ...
    ret = rpc_client.bdev_set_qos_limit(lvol.lvol_bdev, rw_ios_per_sec, rw_mbytes_per_sec, r_mbytes_per_sec,
                                        w_mbytes_per_sec)
    if not ret:
        return "Error"
...
def get_io_stats(lvol_uuid, history, records_count=20):
