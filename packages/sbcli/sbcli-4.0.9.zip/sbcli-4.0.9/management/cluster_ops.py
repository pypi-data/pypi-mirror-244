...
from management.controllers import cluster_events
...
def get_capacity(cluster_id, is_json=False):
    db_controller = DBController()
    ...
    for this_node in nodes:
        ...
        for dev in devices:
            dev_data = storage_node_ops.get_device_capacity_data(dev.get_id(), 1)
            if dev_data:
                all_data.append(dev_data[0])
    if all_data:
        for d in all_data:
            out.append({
                "Node ID": d['id'],
                "Device Name": d['name'],
                "Provisioned": utils.humanbytes(d['max_size']),
                "Util": utils.humanbytes(d['util']),
                "Util %": f"{d['util_percent']}%",
            })
        cluster_data = utils.dict_agg(all_data)
        utp = int(cluster_data['util_percent'] / len(all_data))
        out.append({
            "Node ID": "Total",
            "Device Name": "Total",
            "Provisioned": utils.humanbytes(cluster_data['max_size']),
            "Util": utils.humanbytes(cluster_data['util']),
            "Util %": f"{utp}%",
        })
    if is_json:
