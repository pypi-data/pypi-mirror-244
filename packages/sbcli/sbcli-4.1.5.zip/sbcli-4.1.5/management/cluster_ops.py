...
def _get_cluster_capacity_data(cluster, history):
    db_controller = DBController()
    data = []
    stats = db_controller.get_cluster_capacity(cluster, history)
    for record in stats:
        data.append({
            "date": record.date,
...
def get_capacity(cluster_id, history, records_count=20):
    db_controller = DBController()
    ...
    records = _get_cluster_capacity_data(cl, records_number)
