...
class CustomGPTMetrics:
    ...
    def __init__(self, api_key=None, base_url='https://api.openai.com/v1', model_provider='openai', log_adapter=OpenTelemetryAdapter()):
        self.api_key = api_key
        self.log_adapter = log_adapter
        self.metrics = self.load_metrics(api_key, base_url, model_provider)

    def load_metrics(self, api_key, base_url, model_provider):
        metrics = {}
        ...
        for filename in os.listdir(metrics_folder_path):
            if filename.endswith('.py') and filename != '__init__.py':
                ...
                if metric_class and issubclass(metric_class, BaseMetric) and metric_class != BaseMetric:
                    metrics[metric_class.key] = metric_class(api_key, base_url, model_provider)
    ...
    def analyze_logs(self, db_url, table='prompt_histories', query="SELECT * FROM %s LIMIT %s", limit=10):
        if self.api_key is None:
            raise ValueError("Metric Calculation require openai api_key to be set.")
        if db_url is None:
            raise ValueError("Database URL is required")
