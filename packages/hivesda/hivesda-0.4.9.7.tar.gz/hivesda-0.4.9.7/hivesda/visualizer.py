...
def plot_visualizing_results(test_imgs, scores, img_scores, gt_masks, pixel_threshold, img_threshold, save_dir, file_names, img_types,args):
    ...
    for i in range(len(scores)):
        ...
        # fig_img.savefig(os.path.join(save_dir, file_names[i] + '_total.png'),dpi=1000)
        # plt.close()
        ...
        print(heat_map)
        heat_map = np.where(heat_map>heat_map_threshold,heat_map,0)
        # ax.imshow(heat_map, cmap='jet', norm=norm, interpolation='none')
        ax.imshow(heat_map, cmap='OrRd', interpolation='none')
