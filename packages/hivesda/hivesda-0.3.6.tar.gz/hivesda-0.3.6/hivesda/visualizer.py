...
def plot_visualizing_results(test_imgs, scores, img_scores, gt_masks, pixel_threshold, img_threshold, save_dir, file_names, img_types,args):
    ...
    for i in range(len(scores)):
        ...
        # fig = plt.figure(figsize=(2*cm, 2*cm), dpi=dpi)
        # # fig = plt.figure(figsize=args.origin_size, dpi=dpi)
        # ax = plt.Axes(fig, [0., 0., 1., 1.])
        # ax.set_axis_off()
        # fig.add_axes(ax)
        # ax.imshow(heat_map, cmap='Greys', norm=norm, interpolation='none')
        ...
        # print(heat_map)
        heat_map = np.array(heat_map[0])
        heat_map = heat_map.astype(int)
        image = Image.fromarray(heat_map)
        
        image.save(os.path.join(save_dir, file_names[i] + '.png'))
        # if img_types[i] == 'good':
        #     if img_scores[i] <= img_threshold:
