...
class BaseSearcher:
    ...
    regex: re.Pattern = property(_construct_regex)
    ...
    def __call__(self, value: Any) -> Any:
        try:
            match: re.Match = self.regex.match(value)
            if match:
...
class DefaultValueSearcher(BaseSearcher):
    def build_regex_pattern(self) -> AnyStr:
        # We want to match the pattern of $name:\(default\)|\(value\), without capturing the parenthesis
        return r"\$(?P<name>%(name)s):\((?P<default>.*)\)\|\((?P<value>.*)\)" % {'name': self.name}
    
    def execute(self, name: str, default: str, value: Any) -> Union[dict, None]:
        value = self.loader.transform_value(value)
        if value:
            return value
        return self.loader.transform_value(default)
...
def basic_convert(value: Any) -> Any:
    if is_bool(value):
...
class BaseLoader:
    ...
    def get_default_transformers(self) -> list[Callable]:
        return [
            ...
            DefaultValueSearcher("default", self),
        ]
