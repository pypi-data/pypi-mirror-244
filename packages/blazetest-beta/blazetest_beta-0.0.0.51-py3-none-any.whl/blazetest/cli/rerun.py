...
@click.option(
    "-u",
    "--uuid",
    help="Blazetest session UUID to run for.",
    required=True,
)
@click.option(
    "-config",
...
def rerun(
    ...
    """
    Reruns the session specified or the latest session if --uuid was not specified.
    """
    ...
    logger.info(f"Rerunning session with UUID: {uuid}")
    ...
    test_session: TestSessionResult = (
        test_session_result_manager.get_test_session_by_uuid(uuid=uuid)
    )
    if test_session is None:
        logger.error(f"Session with UUID {uuid} not found")
        return
    ...
    if test_session_result is not None:
        ...
        test_session_result.set_uuid(uuid)
